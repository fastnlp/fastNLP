r"""
该文件中主要包含的是 character 的 Embedding ，包括基于 CNN 与 LSTM 的 character
Embedding。与其它 Embedding 一样，这里的 Embedding 输入也是词的 index 而不需要使用
词语中的 char 的 index 来获取表达。
"""
...
from .utils import _construct_char_vocab_from_vocab, get_embeddings

__all__ = ['CNNCharEmbedding', 'LSTMCharEmbedding']


class CNNCharEmbedding(TokenEmbedding):
    r"""
    使用 ``CNN`` 生成 ``character embedding``。``CNN`` 的结构为：char_embed(x)
    -> Dropout(x) -> CNN(x) -> activation(x) -> pool -> fc -> Dropout。

    不同的 ``kernel`` 大小的 ``fitler`` 结果是拼起来然后通过一层 **全连接层** 然后
    输出 ``word`` 的表示。
    ...
    :param char_emb_size: character 的 embed 的维度。character 是从 ``vocab`` 中
        生成的。
    ...
    :param pool_method: character 的表示在合成一个表示时所使用的 pool 池化方法，支
        持 ``['avg', 'max']``。
    :param activation: CNN 之后使用的激活方法，支持 ``['relu', 'sigmoid',
        'tanh']`` 或者自定义函数。
    ...
    def __init__(self,
                 vocab: Vocabulary,
                 embed_size: int = 50,
                 char_emb_size: int = 50,
                 word_dropout: float = 0,
                 dropout: float = 0,
                 filter_nums: Sequence[int] = (40, 30, 20),
                 kernel_sizes: Sequence[int] = (5, 3, 1),
                 pool_method: str = 'max',
                 activation='relu',
                 min_char_freq: int = 2,
        ...
        if isinstance(activation, str):
            ...
            raise Exception('Undefined activation function: choose from: '
                            '[relu, tanh, sigmoid, or a callable function]')
        ...
        for word, index in vocab:
            ...
            # # 如果是pad的话，直接就为pad_value了。修改为不区分pad,
            # 这样所有的<pad>也是同一个embed
            if include_word_start_end:
        ...
        if pre_train_char_embed:
            self.char_embedding = StaticEmbedding(
                self.char_vocab, model_dir_or_name=pre_train_char_embed)
        else:
        ...
        self.convs = nn.ModuleList([
            nn.Conv1d(
                self.char_embedding.embedding_dim,
                filter_nums[i],
    ...
    def forward(self, words):
        ...
        # batch_size x max_len x max_word_len x embed_size
        chars = self.char_embedding(chars)
        chars = self.dropout(chars)
        reshaped_chars = chars.reshape(batch_size * max_len, max_word_len, -1)
        reshaped_chars = reshaped_chars.transpose(1, 2)  # B' x E x M
        conv_chars = [
            conv(reshaped_chars).transpose(1,
                                           2).reshape(batch_size, max_len,
                                                      max_word_len, -1)
        ...
        if self.pool_method == 'max':
            ...
            chars = torch.sum(conv_chars, dim=-2) / \
...
class LSTMCharEmbedding(TokenEmbedding):
    r"""
    使用 ``LSTM`` 的方式对 ``character`` 进行 ``encode``。结构为：embed(x) ->
    Dropout(x) -> LSTM(x) -> activation(x) -> pool -> Dropout 。
    ...
    :param word_dropout: 按照一定概率随机将 word 设置为 ``unk_index``，这样可以使
        ...
        用。
    :param dropout: 以多大的概率 drop 分布式表示与 char embedding 的输出。
    :param hidden_size: ``LSTM`` 的中间 hidden 的大小，如果为 ``bidirectional``
        的，hidden 会除二。
    :param pool_method: character 的表示在合成一个表示时所使用的 pool 池化方法，支
        持 ``['avg', 'max']``。
    :param activation: LSTM 之后使用的激活方法，支持 ``['relu', 'sigmoid',
        'tanh']`` 或者自定义函数。
    :param min_char_freq: character 的最少出现次数。
    ...
    def __init__(self,
        ...
        for word, index in vocab:
            ...
            self.word_lengths[index] = len(word)
        if pre_train_char_embed:
            self.char_embedding = StaticEmbedding(self.char_vocab,
    ...
    def forward(self, words):
        ...
        chars = self.dropout(chars)
        reshaped_chars = chars.reshape(batch_size * max_len, max_word_len, -1)
        lstm_chars = self.lstm(reshaped_chars,
                               None)[0].reshape(batch_size, max_len,
                                                max_word_len, -1)
        # B x M x M x H

        lstm_chars = self.activation(lstm_chars)
        if self.pool_method == 'max':
            lstm_chars = lstm_chars.masked_fill(
                chars_masks.unsqueeze(-1), float('-inf'))
            # batch_size x max_len x H
            chars, _ = torch.max(lstm_chars, dim=-2)
        else:
            lstm_chars = lstm_chars.masked_fill(chars_masks.unsqueeze(-1), 0)
            chars = torch.sum(
                lstm_chars, dim=-2) / chars_masks.eq(False).sum(
