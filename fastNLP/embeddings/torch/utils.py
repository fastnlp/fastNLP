...
__all__ = ['get_embeddings', 'get_sinusoid_encoding_table']


def _construct_char_vocab_from_vocab(vocab: Vocabulary,
                                     min_freq: int = 1,
...
def get_embeddings(init_embed, padding_idx=None):
    ...
    if isinstance(init_embed, tuple):
        res = nn.Embedding(
            num_embeddings=init_embed[0],
            embedding_dim=init_embed[1],
            padding_idx=padding_idx)
        nn.init.uniform_(
            res.weight.data,
            a=-np.sqrt(3 / res.weight.data.size(1)),
...
def get_sinusoid_encoding_table(n_position: int,
                                d_hid: int,
                                padding_idx=None) -> 'torch.FloatTensor':
    r"""sinusoid 的 embedding，其中 ``position`` 的表示中，偶数维
    ``(0,2,4,...)`` 是 sin，奇数 ``(1,3,5...)`` 是 cos。
...
def _check_vocab_has_same_index(vocab, other_vocab):
    ...
    if other_vocab != vocab:
        for word, word_ix in vocab:
            ...
            assert other_word_idx == word_ix, \
                f'Word {word} has different index in vocabs, ' \
                f'{word_ix} Vs. {other_word_idx}.'
