r"""
该模块中的 :class:`Embedding` 主要用于随机初始化的 embedding （更推荐使用
:class:`fastNLP.embeddings.torch.StaticEmbedding` ），或按照预训练权重初始化
Embedding。
...
class Embedding(Module):
    ...
    def __init__(self,
                 init_embed: Union[Tuple[int, int], 'torch.FloatTensor',
        ...
        if not isinstance(self.embed, TokenEmbedding):
            ...
            if word_dropout > 0 and not isinstance(unk_index, int):
                raise ValueError(
                    'When drop word is set, you need to pass in the unk_index.'
                )
        else:
            self._embed_size = self.embed.embed_size
...
class TokenEmbedding(Module):
    ...
    def __init__(self, vocab, word_dropout=0.0, dropout=0.0):
        ...
        if vocab.rebuild:
            vocab.build_vocab()
        assert vocab.padding is not None, 'Vocabulary must have a padding ' \
                                          'entry.'
        self._word_vocab = vocab
        self._word_pad_index = vocab.padding_idx
        if word_dropout > 0:
            assert vocab.unknown is not None, \
                'Vocabulary must have unknown entry when you want to ' \
                'drop a word.'
        self.word_dropout = word_dropout
    ...
    def drop_word(self, words):
        r"""
        按照设定随机将 words 设置为 unknown_index。
    ...
    def dropout(self, words):
        r"""
        对 embedding 后的 word 表示进行 drop。

        :param words: batch_size x max_len x embed_size
        :return:
        """
