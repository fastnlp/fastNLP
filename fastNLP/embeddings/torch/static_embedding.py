...
from ...io.file_utils import (PRETRAIN_STATIC_FILES, _get_embedding_url,
                              _get_file_name_base_on_postfix, cached_path)
...
class StaticEmbedding(TokenEmbedding):
    ...
    :param init_method: 如何初始化没有找到的值。可以使用 :mod:`torch.nn.init` 中的
        各种方法，传入的方法应该接受一个 tensor，并 inplace 地修改其值。
    :param lower: 是否将 ``vocab`` 中的词语小写后再和预训练的词表进行匹配。如果您的
        词表中包含大写的词语，或者就是需要单独为大写的词语开辟一个 vector 表示，则将
        ``lower`` 设置为 ``False``。
    ...
    :param word_dropout: 按照一定概率随机将 word 设置为 ``unk_index``，这样可以使
        ...
        用。
    :param normalize: 是否对 vector 进行 ``normalize``，使得每个 vector 的 norm
        为 1。
    :param min_freq: Vocabulary 词频数小于这个数量的 word 将被指向 ``<UNK>``。
    :kwargs:
        * *only_train_min_freq* (``bool``) -- 仅对 train 中的词语使用
          ``min_freq`` 筛选
        * *only_norm_found_vector* (``bool``) -- 默认为 ``False``，是否仅对在预
          训练中找到的词语使用 ``normalize``
        * *only_use_pretrain_word* (``bool``) -- 默认为 ``False``，仅使用出现在
          pretrain 词表中的词，如果该词没有在预训练的词表中出现则为 ``<UNK>``。如
          果 embedding 不需要更新建议设置为 ``True``。
    ...
    def __init__(self,
        ...
        if embedding_dim > 0:
            if model_dir_or_name:
                logger.info('StaticEmbedding will ignore `model_dir_or_name`, '
                            'and randomly initialize embedding with dimension '
                            f'{embedding_dim}. If you want to use pre-trained '
                            'embedding, set `embedding_dim` to 0.')
            model_dir_or_name = None

        # 得到cache_path
        if model_dir_or_name is None:
            assert embedding_dim >= 1, 'The dimension of embedding should be '\
                                       'larger than 1.'
            ...
            model_path = os.path.abspath(  # type: ignore[assignment]
                os.path.expanduser(model_dir_or_name))
        elif os.path.isdir(
                os.path.abspath(os.path.expanduser(model_dir_or_name))):
        ...
        if truncate_vocab:
            ...
            if lower:  # 如果有lower，将大小写的的freq需要同时考虑到
                lowered_word_count: dict = defaultdict(int)
            ...
            for word, index in vocab:
                truncated_words_to_words[index] = truncated_vocab.to_index(
                    word)
            logger.info(
                f'{len(vocab) - len(truncated_vocab)} words have frequency '
                f'less than {min_freq}.')
        ...
        # 读取embedding
        if lower:
            lowered_vocab = Vocabulary(
                padding=vocab.padding, unknown=vocab.unknown)
            for word, index in vocab:
            ...
            logger.info(f'All word in the vocab have been lowered. '
                        f'There are {len(vocab)} words, {len(lowered_vocab)} '
                        f'unique lowered words.')
            if model_path:
                embedding = self._load_with_vocab(
                    model_path, vocab=lowered_vocab, init_method=init_method)
            else:
                embedding = self._randomly_init_embed(
                    len(lowered_vocab), embedding_dim, init_method)
            ...
            for word, index in vocab:
                if word not in lowered_vocab:
                    word = word.lower()
                    if word not in lowered_vocab and \
                            lowered_vocab._is_word_no_create_entry(word):
            ...
            if model_path:
                embedding = self._load_with_vocab(
                    model_path, vocab=vocab, init_method=init_method)
            else:
                embedding = self._randomly_init_embed(
                    len(vocab), embedding_dim, init_method)
                self.register_buffer('words_to_words',
                                     torch.arange(len(vocab)).long())
    ...
    def _load_with_vocab(self,
                         ...
                         init_method=None):
        r"""
        从 embed_filepath 这个预训练的词向量中抽取出 vocab 这个词表的词的
        embedding。EmbedLoader 将自动判断 embed_filepath 是 word2vec（第一行只有
        两个元素）还是glove格式的数据。
        ...
        :param str error: `ignore` , `strict` ; 如果 `ignore`，错误将自动跳过；
            如果 `strict` , 错误将抛出。
            这里主要可能出错的地方在于词表有空行或者词表出现了维度不一致。
        :param init_method: 如何初始化没有找到的值。可以使用 torch.nn.init.* 中各
            种方法。默认使用torch.nn.init.zeros_
        :return torch.tensor:  shape为 [len(vocab), dimension], dimension由
            pretrain的embedding决定。
        """
        ...
        with open(embed_filepath, 'r', encoding='utf-8') as f:
            ...
            if len(parts) == 2:
                ...
                dim = len(parts) - 1
                f.seek(0)
            # index是word在vocab中的index，value是vector或
            # None(如果在pretrain中没有找到该word)
            matrix = {}
            if vocab.padding:
            ...
            for idx, line in enumerate(f, start_idx):
                try:
                    ...
                    if word in vocab:
                        ...
                        if index in matrix:
                            logger.warning(
                                'Word has more than one vector in embedding '
                                'file. Set logger level to DEBUG for detail.')
                            logger.debug(
                                f'Word:{word} occurs again in line:{idx} '
                                '(starts from 0)')
            ...
            if not self.only_use_pretrain_word:
                # 如果只用pretrain中的值就不要为未找到的词创建entry了
                for word, index in vocab:
                    if index not in matrix and \
                            not vocab._is_word_no_create_entry(word):
    ...
    def save(self, folder: str):
        ...
        with open(
                os.path.join(folder, STATIC_HYPER_FILENAME), 'w',
                encoding='utf-8') as f:
            json.dump(kwargs, f, indent=2)

        with open(
                os.path.join(folder, STATIC_EMBED_FILENAME), 'w',
                encoding='utf-8') as f:
            f.write('{}\n'.format(' ' * 30))  # 留白之后再来填写
            word_count = 0
            saved_word = {}
    ...
    def load(cls, folder: str):
        ...
        :param folder: 该 ``folder`` 下应该有以下三个文件 ``vocab.txt``,
            ``static_embed.txt``, ``static_hyper.json``
        ...
        with open(
                os.path.join(folder, STATIC_HYPER_FILENAME), 'r',
                encoding='utf-8') as f:
            hyper = json.load(f)
        ...
        embed = cls(
            vocab=vocab,
            model_dir_or_name=os.path.join(folder, STATIC_EMBED_FILENAME),
            **hyper)
        return embed
