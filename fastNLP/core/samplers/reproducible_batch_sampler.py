...
__all__ = [
    'BucketedBatchSampler', 'ReproduceBatchSampler', 'RandomBatchSampler'
]
...
class ReproducibleBatchSampler:
    ...
    **可复现** 的 BatchSampler 对象。
    ...
    意，所有 :meth:`__init__` 中初始化的变量，都不能含有 ``_`` 下横线作为开头；所有
    不在 :meth:`__init__` 中设置的变量都必须以下横线开头。
    """
    ...
    def set_distributed(self, num_replicas, rank, pad=True):
        raise NotImplementedError(
            'Each specific batch_sampler should implement its own '
            '`set_distributed` method.')
...
class ReproduceBatchSampler(ReproducibleBatchSampler):
    ...
    def __init__(self, batch_sampler, batch_size: int, drop_last: bool,
                 **kwargs):
        super().__init__()
    ...
    def _iterate_sampler(self):
        ...
        for idx in self.batch_sampler:
            ...
            # 说明是在初始化时传入的是一个 sampler，理论上对应于 dataloader 在初始
            # 化时没有 batch_size，也没有 batch_sampler 的情况；
            else:
    ...
    def __iter__(self):
        ...
        # 暂时弃用。记住每个 batch 对应的 consumed_samples, 需要这个原因是由于现在
        # 的 dataloader 都存在预取数据的设计，需要再结合 Trainer 中
        # batch_idx_in_epoch 才能最终确定实际消耗的数据。这个变量需要记录每次 yield
        # 出去时的真实 num_consumed_samples 的数值。
        # self.num_consumed_samples_array = NumConsumedSamplesArray(
        #     buffer_size=os.environ.get(ASTNLP_DEQUE_SIZE, 30),
        ...
        for idx in index_list:
            ...
            if len(batch) == self.batch_size:
                # [16, 32, 48, 64,..., ]
                self.num_consumed_samples += self.batch_size
                yield batch
                batch = []
    ...
    def load_state_dict(self, states: Dict):
        assert states['sampler_type'] == self.__class__.__name__, \
            f"The sampler type in checkpoint is {states['sampler_type']}," \
            f'we cannot use {self.__class__.__name__} to load it.'

        _index_list = states['index_list']
        assert len(_index_list) == len(self.index_list), \
            'The number of samples is different between the checkpoint ' \
    ...
    def set_distributed(self, num_replicas, rank, pad=True):
        raise RuntimeError('ReproduceBatchSampler does not support to change '
                           'to distributed training.')

    def set_epoch(self, epoch):
        if hasattr(self.batch_sampler, 'sampler') and hasattr(
                self.batch_sampler.sampler, 'set_epoch') and callable(
    ...
    def batch_idx_in_epoch(self):
        if self.drop_last:
            ...
            return (len(self.index_list) + self.batch_size - 1) // self.batch_size - \
                   (len(self.index_list) - self.num_consumed_samples +
...
class RandomBatchSampler(ReproducibleBatchSampler):
    ...
    def __init__(self,
        ...
        # 是否处于iteration之间，为True不允许调用 set_distributed()和
        # load_state_dict()
    ...
    def set_distributed(self, num_replicas, rank, pad=True):
        ...
        :param pad: 如果 sample 数量不整除 ``num_replicas`` 的时候，要不要 pad 一
            下，使得最终使得每个进程上的 sample 数量是完全一致的
        :return: 自身
        """
        assert self.during_iter is False, 'Cannot set the sampler to be ' \
                                          'distributed when it is during an ' \
    ...
    def __iter__(self):
        if self.during_iter:
            # 如果发现_during_iter为True，说明之前的还没结束，只有强制重新初始化了
            self.num_consumed_samples = 0
        ...
        if self.shuffle:
            ...
            batches = self.batchify(
                indices, self.batch_size, seed=self.seed + self.epoch)
        ...
        if self.drop_last and len(batches) >= 1 and len(
    ...
    @property
    def total_size(self):
        r"""当前 BatchSampler 会最终产生出的 index 数量（包括了其它 rank 的），因
        为 ``replica`` 和 ``pad`` 的原因，这个值可能等于、大于或者小于 ``len
        (dataset)``。"""
        return self.num_consumed_samples + \
            self.num_replicas * self.num_left_samples
    ...
    def state_dict(self) -> Dict:
        if self.old_batch_size != self.batch_size:
            raise RuntimeError(
                'BucketedBatchSampler does not support saving before last '
                'checkpoint states have been consumed. ')
        states = {
            'seed': self.seed,
            'epoch': self.epoch,
            'num_consumed_samples': self.num_consumed_samples,
    ...
    def load_state_dict(self, states: Dict):
        ...
        assert self.during_iter is False, 'Cannot call load_state_dict() ' \
                                          'when it is during an unfinished ' \
                                          'iteration.'
        ...
        assert length == self.num_samples, \
            'The number of samples is different between the checkpoint ' \
            f'record({length}) and current dataset({self.num_samples}).'
        self.seed = states['seed']
        self.epoch = states['epoch']
        self.num_consumed_samples = states['num_consumed_samples']
        if self.num_consumed_samples >= length:
            # 如果保存的时候已经到达了最后一个sample了，则直接将结果重置为0
            self.num_consumed_samples = 0
        if self.shuffle != states['shuffle']:
            logger.info(
                f"The shuffle from the checkpoint is {states['shuffle']}, "
                f'while set as {self.shuffle}, we use shuffle='
                f"{states['shuffle']}")
...
class BucketedBatchSampler(ReproducibleBatchSampler):
    ...
    :param shuffle: 如果为 ``True``，将不进行 shuffle，实际上数据会以从长到短的方式
        输出。
    :param drop_last: 如果最后一个 batch 的 ``sample`` 数无法凑齐 ``batch_size``
        这么多，是否需要丢掉。
    :param seed: 设置的随机数种子
    ...
    def __init__(self,
                 dataset,
                 length: Union[List[int], str],
                 batch_size: int = 32,
                 num_batch_per_bucket: int = 10,
                 shuffle: bool = True,
                 drop_last: bool = False,
                 seed: int = 0,
                 **kwargs):
        ...
        if isinstance(dataset, DataSet) and isinstance(length, str):
            ...
            if not isinstance(length[0], int):
                length = list(map(len, length))  # type: ignore[arg-type]
            self.length = np.array(length, dtype=int)
            # 按长度从高到低排序的
            self.sorted_indices = np.argsort(self.length)[::-1]
        else:
            try:
                self.length = np.array(length, dtype=int)
                ...
                logger.error(
                    f'Cannot use {self.__class__.__name__} as length, '
                    'since it is not sortable.')

        assert len(length) == len(dataset), \
            f'The length of `dataset`({len(dataset)}) and ' \
            f'`length`({len(length)}) should be equal.'
        assert len(self.sorted_indices) == len(dataset), \
            'The indices and dataset should have equal length.'
    ...
    def set_distributed(self, num_replicas, rank, pad=True):
        ...
        # num_samples = (len(self.dataset) + self.num_replicas - 1) // self.num_replicas * \
        #     self.num_replicas if pad else len(self.dataset)

        # if self.drop_last:
        #     assert self.num_replicas * self.batch_size <= num_samples, \
        #         "The number of samples should be greater than the number " \
        #         "of replicates multiplied with batch_size when drop_last=True."
    ...
    def __iter__(self):
        ...
        # 按长度从高到低排序的
        sorted_indices = deepcopy(self.sorted_indices).tolist()
            ...
            if self.num_consumed_samples > 0:  # 需要先按照原来的排序，删掉多余的
                ...
                for _i in range(self.old_num_replicas):
                    _sorted_indices = sorted_indices[
                        _i:len(sorted_indices):self.old_num_replicas]
                    __batches = self.bucketerize(
                        _sorted_indices,
                        self.old_batch_size,
                        self.old_num_batch_per_bucket,
                        seed=self.seed + self.epoch)
                    _batches.append(__batches)
                batches = list(chain(*[_ for _ in zip(*_batches)]))
                ...
                sorted_indices = np.array(sorted_indices)[np.argsort(
                    sub_length)[::-1]]  # 按长度从高到低排序的
            # 取出这个 rank ，
            sorted_indices = sorted_indices[self.rank:len(sorted_indices):self.
                                            num_replicas]
            batches = self.bucketerize(
                sorted_indices,
                self.batch_size,
                self.num_batch_per_bucket,
                seed=self.seed + self.epoch)
            ...
            if _num_batches == 0:
                ...
                batches = list(
                    map(
                        list,
                        np.array_split(
                            sorted_indices[:_num_batches * self.batch_size],
                            _num_batches)))
                if len(sorted_indices) % self.batch_size != 0:
                    batches.append(sorted_indices[_num_batches *
                                                  self.batch_size:])
        ...
        # 暂时弃用
        # self.num_consumed_samples_array = NumConsumedSamplesArray(
        #     buffer_size=os.environ.get(FASTNLP_DEQUE_SIZE, 30),
        #     num_consumed_samples=self.num_consumed_samples
        # )
        for batch in batches:
            self.num_consumed_samples += self.num_replicas * len(batch)
    ...
    def bucketerize(self, sorted_indices, batch_size, num_batch_per_bucket,
        ...
        # 实际的 bucket 大小
        bucket_size = min(
            len(sorted_indices), batch_size * num_batch_per_bucket)
        rng = np.random.default_rng(abs(seed))
        ...
        for i in range(num_buckets):
            ...
            if _num_batches == 0:
                ...
                _batches = np.array_split(bucket[:_num_batches * batch_size],
                                          _num_batches)
                if len(bucket) % batch_size != 0:
                    _batches.append(bucket[_num_batches * batch_size:])
            batch_indices.extend(
                list(range(len(batches),
                           len(batches) + len(_batches))))
            batches.extend(_batches)
        last_batches = []
        # 最后一个 batch 统一不参与 shuffle，因为有的 rank 最后一个 batch 可能不足
        # 一个 batch_size （不足的时候一定要放在末尾，所以就干脆所有的rank都不对最后
        # 一个 batch 进行 shuffle）。
        if len(batches) >= 1:
            last_batches = [list(batches[-1])]
        batch_indices = list(batch_indices[:-1])
        # 这里防止由于bucket长度不同，对随机数状态有影响
        rng = np.random.default_rng(abs(seed))
        # 不同的 batch 也 shuffle ，当前这种可以保证每张卡上每个 batch
        # 长度都接近的。
    ...
    def state_dict(self) -> Dict:
        if self.old_batch_size != self.batch_size or \
                self.old_num_batch_per_bucket != self.num_batch_per_bucket:
            raise RuntimeError(
                'BucketedBatchSampler does not support saving before last '
                'checkpoint states have been consumed. ')
