...
from fastNLP.core.samplers.unrepeated_sampler import (
    UnrepeatedRandomSampler, UnrepeatedSampler, UnrepeatedSequentialSampler,
...
def conversion_between_reproducible_and_unrepeated_sampler(sampler):
    ...
    :return:
    """
    assert isinstance(sampler, UnrepeatedSampler) or \
        isinstance(sampler, ReproducibleSampler), \
    ...
    if isinstance(sampler, UnrepeatedSampler):
        if isinstance(sampler, UnrepeatedRandomSampler):
            return re_instantiate_sampler(
                sampler, new_sampler_class=RandomSampler)
        elif isinstance(sampler, UnrepeatedSequentialSampler):
            return re_instantiate_sampler(
                sampler, new_sampler_class=SequentialSampler)
        elif isinstance(sampler, UnrepeatedSortedSampler):
            return re_instantiate_sampler(
                sampler, new_sampler_class=SortedSampler)
        raise TypeError(f'{sampler.__class__} has no unrepeated version.')
    else:
        if isinstance(sampler, RandomSampler):
            return re_instantiate_sampler(
                sampler, new_sampler_class=UnrepeatedRandomSampler)
        elif isinstance(sampler, SequentialSampler):
            return re_instantiate_sampler(
                sampler, new_sampler_class=UnrepeatedSequentialSampler)
        elif isinstance(sampler, SortedSampler):
            return re_instantiate_sampler(
                sampler, new_sampler_class=UnrepeatedSortedSampler)
