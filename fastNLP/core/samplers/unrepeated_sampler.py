...
__all__ = [
    'UnrepeatedSampler', 'UnrepeatedSortedSampler', 'UnrepeatedRandomSampler',
...
class UnrepeatedRandomSampler(UnrepeatedSampler):
    ...
    def __len__(self):
        ...
        num_samples = num_common + int(
            self.rank < (self.num_samples - num_common * self.num_replicas))
        return num_samples
    ...
    def generate_indices(self) -> List[int]:
        ...
        if self.shuffle:
            ...
            rng.shuffle(indices)
            if self.epoch < 0:
                # 防止用户忘记调用 set_epoch，至少这样可以保证每次epoch出来的index
                # 顺序不同。
                self.epoch -= 1
        else:
            indices = list(range(self.num_samples))
    ...
    def set_distributed(self, num_replicas, rank):
        r"""该方法本质上等同于 ddp 情形下的没有完成的初始化，应当在初始化该 Sampler
        本身后立即被调用。
        ...
        :return: 自身
        """
        assert num_replicas <= self.num_samples, \
            f'The number of replicas({num_replicas}) should be lesser than ' \
            f'the number of samples({self.num_samples}).'
    ...
    def __init__(self, dataset, length: Union[str, List], **kwargs):
        ...
        if isinstance(dataset, DataSet) and isinstance(length, str):
            ...
            assert len(length) == len(dataset), \
                'When the dataset is not fastNLP.DataSet, the length ' \
                'parameter can only be List[int]'

        assert len(length) == len(dataset), \
            'The length of `data` and `length` should be equal.'

        length = np.array(length, dtype=int)  # 按照长到短排列的序号。
        # 按长度从高到低排序的
        self.sorted_indices = np.argsort(length)[::-1].tolist()
...
class UnrepeatedSequentialSampler(UnrepeatedRandomSampler):
    ...
    :param chunk_dist: 如果为 ``True``，当多卡时将不间隔索取数据；为 ``False`` 时
        ...
        ``False``，则卡 **0** 拿 [0, 2, 4, 8, 8], 卡 **1** 拿 [1, 3, 5, 7, 9] 。
    :param kwargs:
    """
