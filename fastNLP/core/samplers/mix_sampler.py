...
class MixSampler:
    ...
    :param ds_ratio: ``ds_ratio`` 是控制 datasets 怎么组成一个混合大数据集的重要参
        ...
        * ds_ratio 为 ``Dict[str, float]`` 时，datasets 类型也必须为 ``Dict
          ...
          的浮点数，代表着 datasets 的 value 数据进行扩充或者缩减的倍数；
    ...
    def __init__(self,
                 ...
                 ds_ratio: Union[str, Dict[str, float], None] = None,
        ...
        if not isinstance(sampler, str) and (rank >= 0 or word_size >= 0):
            raise ValueError(
                'if rank>=0 and word_size>=0, sampler must be str')

        if sampler is None and (word_size < 0 or rank < 0):
            self.sampler = {
                name: SequentialSampler(ds)
            ...
            if isinstance(sampler, str):
                if sampler not in ['seq', 'rand']:
                    raise ValueError(
                        f'sampler is {sampler}, but seq or rand is required')
        ...
        if isinstance(self.sampler, Dict):
            if ds_ratio is None:
                ...
                sampler_lens = [
                    max(len(spl) for _, spl in self.sampler.items())
                ] * sampler_len

            elif ds_ratio == 'truncate_to_least':
                sampler_len = sum([1 for _ in self.sampler.keys()])
                sampler_lens = [
                    min(len(spl) for _, spl in self.sampler.items())
                ] * sampler_len
                ...
                sampler_lens = [
                    int(len(spl) * ds_ratio[name])
                    for name, spl in self.sampler.items()
                ]
            else:
                raise ValueError(
                    f'{ds_ratio} must be pad_to_least or truncate_to_least '
                    'or None or Dict')
...
class DopedSampler(MixSampler):
    ...
    def __init__(self,
                 ...
                 rank: int = -1,
                 word_size: int = -1) -> None:
        super(DopedSampler, self).__init__(
            dataset=dataset,
    ...
    def __iter__(self) -> Generator[List[Any], None, None]:
        ...
        if isinstance(self.sampler, str):
            if self.sampler == 'seq':
                ...
                for name, per_ds in self.datasets.items():
                    if self.word_size >= 0 and self.rank >= 0:
                        self.sampler[name] = InnerSampler(
                            list(range(
                                len(per_ds)))[self.rank::self.word_size])
                    else:
                        self.sampler[name] = InnerSampler(
                            list(range(len(per_ds))))
                ...
                for name, per_ds in self.datasets.items():
                    ...
                    if self.word_size >= 0 and self.rank >= 0:
                        self.sampler[name] = InnerSampler(
                            indices[self.rank::self.word_size])
                    else:
            ...
            if isinstance(self.sampler, Dict):
                if self.ds_ratio is None:
                    sampler_lens = [
                        len(spl) for _, spl in self.sampler.items()
                    ]
                    ...
                    if not all(item >= 0  # type: ignore
                               for item in self.ds_ratio):
                        raise ValueError(
                            'batch_size should be a positive integer value, '
                            'but got ds_ratio={}'.format(self.ds_ratio))
                    sampler_lens = [
                        int(len(spl) * self.ds_ratio[name])
                        for name, spl in self.sampler.items()
                    ]
                else:
                    raise ValueError(
                        f'{self.ds_ratio} must be pad_to_least or '
                        'truncate_to_least or None or List')
                total_lens = sum(sampler_lens)
            else:
        ...
        if self.rank < 0 or self.word_size < 0:
            ...
            if (self.len_samplers // self.word_size) > 42e8:
                # 整分给每个卡的数据
                self.len_samplers = self.len_samplers - \
                    self.len_samplers % self.word_size
                total_index = array.array(
                    'L',
                    list(range(self.len_samplers))[self.rank::self.word_size])
            else:
                total_index = array.array(
                    'I',
                    list(range(self.len_samplers))[self.rank::self.word_size])
        ...
        for idx in total_index:
            ...
            try:
                ...
                # 重新初始化一个新的sampler，因为不可能为空，
                # 故一定不会出现stopIteration
    ...
    def __len__(self) -> int:
        ...
        if self.rank >= 0 and self.word_size >= 0:
            # 整分给每个卡的数据
            self.len_samplers = (self.len_samplers - self.len_samplers %
...
class MixSequentialSampler(MixSampler):
    ...
    def __init__(self,
                 ...
                 rank: int = -1,
                 word_size: int = -1) -> None:
        super(MixSequentialSampler, self).__init__(
            dataset=dataset,
            batch_size=batch_size,
            sampler=sampler,
            ds_ratio=ds_ratio,
    ...
    def __iter__(self) -> Iterable[List[int]]:
        ...
        batch_idx, total_index, samplers = [], list(range(
            self.len_samplers)), []
        start_idx = 0
        ...
        # if self.word_size > 0 and self.rank >= 0:
        #     sampler_base_index = [
        #         0] + [len(spl) * self.word_size for _, spl in self.sampler.items()][:-1]
        # else:
        #     sampler_base_index = [0] + [len(spl)
        #                                 for _, spl in self.sampler.items()][:-1]
        # samplers = [(iter(spl), name, sampler_base_index[idx])
...
class PollingSampler(MixSampler):
    ...
    def __init__(self,
                 ...
                 sampler: Union[Dict[str, 'Sampler'], None, str] = None,
                 drop_last: bool = False,
                 ds_ratio='pad_to_most',
                 rank: int = -1,
                 word_size: int = -1) -> None:
        super(PollingSampler, self).__init__(
            dataset=dataset,
            batch_size=batch_size,
            sampler=sampler,
            ds_ratio=ds_ratio,
    ...
    def __iter__(self) -> Generator[List[Any], None, None]:
        ...
        for idx, (name, spl) in enumerate(self.sampler.items()):
            ...
            samplers.append(
                (iter(range(true_start_idx,
                            true_end_idx)), iter(spl), start_idx, name))
            start_idx += end_idx
        ...
        while True:
            ...
            ds_total_iter, ds_sampler, ds_base_idx, sampler_idx = samplers.pop(
                0)
            ...
            if flag is False:
                samplers.append(
                    (ds_total_iter, ds_sampler, ds_base_idx, sampler_idx))
            if len(batch_idx) == self.batch_size:
                yield batch_idx
