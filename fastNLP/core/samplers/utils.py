...
__all__ = ['re_instantiate_sampler']
...
class NumConsumedSamplesArray:
    ...
    def __init__(self, buffer_size=2000, num_consumed_samples=0):
        r"""保留 buffer_size 个 num_consumed_samples 数据，可以索引得到某个 index
        下的 num_consumed_samples 多少.
        ...
        Example::
            ...
            array[6]  # 报错，因为只保留了3个最近的数据，6超过了最大buffer的记录，即 [7, 8, 9]
    ...
    def __getitem__(self, item):
        if len(self.deque) == 0:
            # 如果没有任何缓存的内容，说明还没有写入，直接返回0
            return 0
        assert isinstance(item, int), 'Only int index allowed.'
        assert self.count - len(
            self.deque
        ) <= item < self.count, f'Only keep {len(self.deque)} history index.'
