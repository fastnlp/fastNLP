...
from .packer_unpacker import (MappingPackerUnpacker,
                              NestedMappingPackerUnpacker,
                              SequencePackerUnpacker, SinglePackerUnpacker)
...
SUPPORTED_BACKENDS = [
    'torch', 'jittor', 'paddle', 'oneflow', 'numpy', 'raw', 'auto', None
]
# 由于 jittor DataLoader 存在自动的 to_jittor 的转换，
# 所以只需要 collate 成为 numpy 就行
AUTO_BACKEND_MAPPING = {'jittor': 'numpy'}


def _get_backend() -> str:
    r"""当 Collator 的 backend 为 None 的时候，通过这个函数自动判定其 backend 。
    判断方法主要为以下两个：

    1. 尝试通过向上寻找当前 collator 的 callee 对象，根据 callee 对象寻找。然后使
       用 '/site-packages/{backend}' 来寻找是否是某个 backend 的 dataloader 。
    ...
    catch_backend = []
    for i in range(100):
        currentframe = currentframe.f_back  # type: ignore
    ...
    if len(catch_backend):
        logger.debug(
            'Find a file named:{catch_backend[1]} from stack contains '
            f'backend:{catch_backend[0]}.')
    ...
    for key, module in sys.modules.items():
        ...
        if catch_backend:
            break
    if len(catch_backend):
        logger.debug(f'Find a module file named:{catch_backend[1]} from '
                     f'sys.modules contains backend:{catch_backend[0]}.')
...
class Collator:
    ...
    .. note::
        ...
        ``Collator`` 的原理是使用第一个 ``batch`` 的数据尝试推断每个 ``field`` 应
        该使用哪种类型的 ``Padder``，如果第一个 ``batch`` 的数据刚好比较特殊，可能
        导致在之后的 pad 中遭遇失败，这种情况请通过 :meth:`set_pad` 函数手动设置一
        下。
    ...
    def __init__(self, backend='auto'):
        ...
        self.padders = {}
        self.input_fields = {}
        # 只能是 d ，s ，l 三种，分别对应输入的 batch 的每个 sample 为
        # dict, single，list。
        self.batch_data_type = None
    ...
    def __call__(self, batch) -> Union[List, Dict]:
        ...
        第一次调用会根据当前 batch 数据决定使用哪个 unpack_batch_func ，这个函数的
        作用是把不同 sample 的同一个 field 的放入到一个 list 中；同时也会决定
        pack_batch_func，这个函数的作用是在返回 pad 好的 batch 之前，将 batch 恢复
        为输入时一个 sample 的类别。
        ...
        if self.unpack_batch_func is None:
            ...
            if self.batch_data_type is None:
                ...
                logger.debug(
                    f'Since batch[0] has type:{type(batch[0])}, so the '
                    f'batch_data_type is `{self.batch_data_type}`.')
            if self.batch_data_type == 's':
                # 不需要做任何调整
                self.packer_unpacker: Any = SinglePackerUnpacker()
            elif self.batch_data_type == 'l':
                self.packer_unpacker = SequencePackerUnpacker()
            elif self.batch_data_type == 'd':
                if any([isinstance(v, Mapping) for v in batch[0].values()]):
                    # 可能存在嵌套的 dict。{'a': {'b': xx}}->{('a', 'b'): value}
                    self.packer_unpacker = NestedMappingPackerUnpacker()
                else:
                    self.packer_unpacker = MappingPackerUnpacker()

        # 将 batch 中各个 field 组成自己的 batch；
        # 同时忽略处于 ignore_fields 中的数据。
        unpack_batch = self.packer_unpacker.unpack_batch(
            batch, self.ignore_fields, self.input_fields)

        pad_batch = {}
        if len(self.padders) == 0:  # 第一次运行，准备 padder
            if self.backend == 'auto':
                # 如果 backend 为 auto ，则尝试通过调用栈等自动获取 backend 。
                self.backend: str = _get_backend()

            for field_name, batch_field in unpack_batch.items():
                setting = self.input_fields.get(
                    field_name, {
                ...
                if callable(pad_fn):
                    ...
                    backend = self.backend if setting[
                        'backend'] == 'auto' else setting['backend']
                    padder = get_padder(
                        batch_field=batch_field,
                        pad_val=setting['pad_val'],
                        dtype=setting['dtype'],
                        backend=backend,
            ...
            if self.batch_data_type == 'l':
                # sort, 这样 _0, _1 能够保持顺序
                self.padders = dict(
                    sorted(self.padders.items(), key=lambda x: int(x[0][1:])))
        try:
            ...
            try:
                logger.error('The following exception happens when try to pad '
                             f'the `{key}` field with padder:{padder}:')
            except Exception:
                pass
            raise e

        # 根据情况恢复成与输入一致的类型
        return self.packer_unpacker.pack_batch(pad_batch)
    ...
    def set_pad(self,
        ...
        :param pad_val: 这个 field 的默认 pad 值。如果设置为 ``None``，则表示该
            ...
            如果对应 field 本身就不是可以 pad 的形式，可以不主动设置为 ``None``。
        ...
        if self.batch_data_type == 's':
            ...
            if isinstance(field_name, str):
                assert sequence_idx_str.match(field_name) is None, \
                    f'Field name:{field_name} will be recognized as list ' \
                    'index, but other field is set as dict mode.'
        elif self.batch_data_type == 'l':
            if isinstance(field_name, str):
                assert sequence_idx_str.match(field_name) is not None, \
                    'Other field is set as list mode. But the new field ' \
                    f'name is {field_name}.'

        if field_name == '_single':
            self.batch_data_type = 's'
        elif isinstance(field_name,
                        str) and sequence_idx_str.match(field_name):
            self.batch_data_type = 'l'
        else:
            self.batch_data_type = 'd'

        # 检测是否已经设置了，主要需要考虑它的父亲节点的情况
        ignore_fields = [(field, field) if isinstance(field, tuple) else
                         ((field, ), field) for field in self.ignore_fields]
        ...
        for field, o_field in ignore_fields:
            ...
            if d == 0:
                logger.rank_zero_warning(
                    f'Field:`{field_name}` has been set as ignored before. '
                    'It will not be ignored afterwards.')
                self.ignore_fields.remove(o_field)
            if d > 0:
                raise KeyError(
                    f'Cannot set `{field_name}` as input, since its children '
                    f'`{o_field}` has been set as ignore field.')
            if d < 0:
                raise KeyError(
                    f'Cannot set `{field_name}` as input, since its parent '
                    f'`{o_field}` has been set as ignore field.')
    ...
    def set_backend(self, backend: str):
        ...
        :param backend: 对于可以 pad 的 field，使用哪种 tensor，支持 ``['torch',
            'jittor','paddle','oneflow','numpy','raw', 'auto', None]``，若为
            ``'auto'``，则在进行 pad 的时候会根据调用的环境决定其 ``backend``。
        :return:
        """
        assert backend in SUPPORTED_BACKENDS
        self._renew()
        ...
        for field in field_names:
            ...
            for _field_name, o_field_name in input_field_names:
                ...
                if d == 0:
                    ...
                    logger.rank_zero_warning(
                        f'Field:{o_field_name} has been set as pad before. '
                ...
                if d > 0:
                    raise KeyError(
                        f'Cannot ignore {field} since its parent key '
                        f'{o_field_name} has been set as pad.')
...
def _compare_tuple(t1, t2):
    r"""检测 t1 和 t2 的关系。例如 (1, ) 和 (1, ) 关系为 0，表示两者完全没有差异
