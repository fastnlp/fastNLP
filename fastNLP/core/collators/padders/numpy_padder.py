...
__all__ = ['NumpyNumberPadder', 'NumpySequencePadder', 'NumpyTensorPadder']
    ...
    def pad(batch_field, pad_val=0, dtype=None):
        ...
        try:
            if not isinstance(batch_field[0], np.ndarray):
                batch_field = [
                    np.array(field.tolist(), dtype=dtype)
                    for field in batch_field
                ]
        except AttributeError:
            raise RuntimeError('If the field is not a np.ndarray '
