...
if _NEED_IMPORT_ONEFLOW:
    ...
    number_to_oneflow_dtype_dict = {
        # 因为 oneflow.tensor([1], dtype=float) 是 oneflow.float64
        float: oneflow.float32,
        int: oneflow.int64,
        bool: oneflow.bool
    }
...
from .utils import (get_shape, is_number, is_number_or_numpy_number,
                    is_numpy_generic_class, is_numpy_number_dtype)

__all__ = [
    'OneflowNumberPadder', 'OneflowSequencePadder', 'OneflowTensorPadder'
]
...
def _get_dtype(ele_dtype, dtype, class_name):
    ...
    """
    if not (ele_dtype is None or (is_number_or_numpy_number(ele_dtype)
                                  or is_oneflow_tensor(ele_dtype))):
    ...
    if dtype is not None:
        ...
        if ele_dtype is not None:
            if (is_number(ele_dtype) or is_oneflow_tensor(ele_dtype)):
                ele_dtype = number_to_oneflow_dtype_dict.get(
                    ele_dtype, ele_dtype)
                dtype = ele_dtype
...
class OneflowNumberPadder(Padder):
    ...
    def pad(batch_field, pad_val=0, dtype=None):
        r"""将 ``batch_field`` 数据转为 :class:`oneflow.Tensor` 并 pad 到相同长
        度。
...
class OneflowSequencePadder(Padder):
    ...
    @staticmethod
    def pad(batch_field, pad_val=0, dtype=None):
        r"""将 ``batch_field`` 数据 转为 :class:`oneflow.Tensor` 并 pad 到相同长
        度。
...
class OneflowTensorPadder(Padder):
    ...
    @staticmethod
    def pad(batch_field, pad_val=0, dtype=None):
        """将 ``batch_field`` 数据 转为 :class:`oneflow.Tensor` 并 pad 到相同长 度。
        ...
        try:
            if not isinstance(batch_field[0], oneflow.Tensor):
                batch_field = [
                    oneflow.tensor(field.tolist(), dtype=dtype)
            ...
            raise RuntimeError('If the field is not a oneflow.Tensor '
...
def fill_tensor(batch_field, padded_batch, dtype):
    ...
    if padded_batch.ndim == 2:
        for i, content_i in enumerate(batch_field):
            padded_batch[i, :len(content_i)] = oneflow.tensor(
                content_i, dtype=dtype)
    elif padded_batch.ndim == 3:
        for i, content_i in enumerate(batch_field):
            for j, content_ii in enumerate(content_i):
                padded_batch[i, j, :len(content_ii)] = oneflow.tensor(
            ...
            for i, content_i in enumerate(batch_field):
                for j, content_ii in enumerate(content_i):
                    for k, content_iii in enumerate(content_ii):
                        padded_batch[i, j,
                                     k, :len(content_iii)] = oneflow.tensor(
