...
if _NEED_IMPORT_PADDLE:
    ...
    number_to_paddle_dtype_dict = {
        # 因为 paddle.tensor([1], dtype=float)是paddle.float64
        float: 'float32',
        int: 'int64',
        bool: 'bool'
    }
...
from .utils import (get_padded_numpy_array, is_number,
                    is_number_or_numpy_number, is_numpy_generic_class,
                    is_numpy_number_dtype)

__all__ = ['PaddleNumberPadder', 'PaddleTensorPadder', 'PaddleSequencePadder']
...
def _get_dtype(ele_dtype, dtype, class_name):
    ...
    if not (ele_dtype is None or is_number_or_numpy_number(ele_dtype)
            or is_paddle_tensor(ele_dtype) or is_paddle_dtype_str(ele_dtype)):
    ...
    if dtype is not None:
        if not (is_paddle_tensor(dtype) or is_number(dtype)
                or is_paddle_dtype_str(dtype)):
...
class PaddleNumberPadder(Padder):
    ...
    def pad(batch_field, pad_val=0, dtype=None):
        r"""将 ``batch_field`` 数据 转为 :class:`paddle.Tensor` 并 pad 到相同长
        度。
    ...
    def pad(batch_field, pad_val=0, dtype=None):
        ...
        try:
            if not isinstance(batch_field[0], paddle.Tensor):
                batch_field = [
                    np.array(field.tolist()) for field in batch_field
                ]
            else:
                if dtype is None:
                    dtype = batch_field[0].dtype
        except AttributeError:
            raise RuntimeError('If the field is not a paddle.Tensor '
...
def get_padded_paddle_tensor(batch_field, dtype=None, pad_val=0):
    ...
    array = get_padded_numpy_array(
        batch_field=batch_field, dtype=None, pad_val=pad_val)
    tensor = paddle.to_tensor(array, dtype=dtype)
    return tensor
