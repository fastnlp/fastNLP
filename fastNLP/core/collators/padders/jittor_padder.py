...
if _NEED_IMPORT_JITTOR:
    ...
    numpy_to_jittor_dtype_dict = {
        ...
        np.float16: 'float16',
        np.float32: 'float32',
        # 这里都统一为到 float32 吧，这是由于 numpy 大部分时候都默认 float64 了
        np.float64: 'float32',
    }
    # number_to_jittor_dtype_dict = {
    #     # 因为 paddle.tensor([1], dtype=float) 是 paddle.float64
    #     float: 'float32',
    #     int: 'int64',
    #     bool: 'bool'
    # }
...
from .utils import (get_shape, is_number, is_number_or_numpy_number,
                    is_numpy_generic_class)

__all__ = ['JittorNumberPadder', 'JittorSequencePadder', 'JittorTensorPadder']
...
def is_jittor_dtype_str(dtype):
    ...
    try:
        if isinstance(dtype, str) and dtype in {
                ...
                u'bool', u'float16', u'uint16', u'float32', u'float64',
                u'int8', u'int16', u'int32', u'int64', u'uint8'
        }:
            return True
    except Exception:
        pass
...
def _get_dtype(ele_dtype, dtype, class_name):
    ...
    if not (ele_dtype is None or
            (is_number_or_numpy_number(ele_dtype)
             or is_jittor_tensor(ele_dtype) or is_jittor_dtype_str(dtype))):
        raise EleDtypeUnsupportedError(
            f'`{class_name}` only supports padding python numbers '
            f'or numpy numbers or jittor.Var but get `{ele_dtype}`.')

    if dtype is not None:
        if not (is_jittor_tensor(dtype) or is_number(dtype)
                or is_jittor_dtype_str(dtype)):
            raise DtypeUnsupportedError(
                f'The dtype of `{class_name}` only supports python numbers '
    ...
    def pad(batch_field, pad_val=0, dtype=None):
        ...
        try:
            if not isinstance(batch_field[0], jittor.Var):
                batch_field = [
                    jittor.Var(np.array(field.tolist(), dtype=dtype))
                    for field in batch_field
                ]
        except AttributeError:
            raise RuntimeError('If the field is not a jittor.Var '
                               f'(it is {type(batch_field[0])}), '
                               'it must have tolist() method.')
...
def fill_tensor(batch_field, padded_batch, dtype):
    ...
    if padded_batch.ndim == 2:
        for i, content_i in enumerate(batch_field):
            padded_batch[i, :len(content_i)] = jittor.Var(
                np.array(content_i, dtype=dtype))
    elif padded_batch.ndim == 3:
        for i, content_i in enumerate(batch_field):
            for j, content_ii in enumerate(content_i):
                padded_batch[i, j, :len(content_ii)] = jittor.Var(
                    np.array(content_ii, dtype=dtype))
            ...
            for i, content_i in enumerate(batch_field):
                for j, content_ii in enumerate(content_i):
                    for k, content_iii in enumerate(content_ii):
                        padded_batch[i, j, k, :len(content_iii)] = jittor.Var(
                            np.array(content_iii, dtype=dtype))
