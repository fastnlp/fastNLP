...
from fastNLP.core.metrics.backend import AutoBackend, Backend
...
class Metric:
    r"""
    **fastNLP** 中 :class:`Metric` 的基类，自定义 :class:`Metric` 时，请继承该对
    象。使用该对象，将有助于减少在分布式状态下的 Metric 计算。

    .. note::

        在多卡情况下，所有 **fastNLP** 提供的 :class:`Metric` 默认情况下都会最终将
        所有设备上的评估结果集中到同一张卡上，并以此为基础输出最终的评测分数。如果您不
        需要这一功能，请将 ``aggregate_when_get_metric`` 置为 ``False``。
    ...
    def __init__(self,
        ...
        self._updated = False
        self.get_metric = self._sync_get_metric(  # type: ignore
            self.get_metric)
        self.update = self._wrap_update(self.update)  # type: ignore
        self.reset = self._wrap_auto_reset_elements(self.reset)  # type: ignore
        self.aggregate_when_get_metric = aggregate_when_get_metric
        self._cannot_change_element = False
        # 用于检查用户是否在 get_metric 中调用了 all_gather_object
    ...
    def register_element(self,
                         ...
                         backend='auto') -> Element:
        r"""注册一个 element 对象，注册之后便可以通过在 Metric 中直接通过 ``self.
        {name}`` 进行调用，可以认为该对象即为对应 backend 的 tensor 直接进行加减乘
        除计算即可。

        .. warning::

            如果想使得该 metric 可自动扩展到多卡的情况，请一定申明
            ``aggregate_method``。

        :param name: 当前 element 的名字，注册后，在 Metric 中可以通过 ``self.
            {name}`` 访问该变量。
        :param value: 初始化的值。在调用 :meth:`Metric.reset` 方法时也将自动设置为
            该值
        :param aggregate_method: 如何聚合多卡上的结果，如果为单卡执行，该值无意义。
            如果设置为 None 则表示该 element 不进行聚合。
        :param backend: 使用的 backend 。Element 的类型会根据 ``backend`` 进行实
            际的初始化。例如 ``backend`` 为 ``'torch'`` 则该对象为 :class:`torch.
            ...
            **fastNLP** 会根据实际调用 :meth`Metric.update`  函数时传入的参数进行
            ...
            非 tensor 类型的输入）则认为 ``backend`` 为 ``'jittor'``。如果没有检测
            到任何一种 tensor ，就默认使用 :class:`float` 类型作为 element 。
    ...
    def reset(self):
        r"""在对每个 ``evaluate_dataloaders`` 遍历进行验证之前，:meth:`reset` 函
        数会被调用来重置每个非 element 对象；如果有非 element 的对象需要重置的时候，
        在本方法中写下非 element 的重置方式。注册的 element 对象则会自动 reset 为初
        始值。
        """
        pass
    ...
    def _sync_get_metric(self, get_metric):
        ...
        def _wrap_get_metric(*args, **kwargs):
            assert self._updated, "You have to call `{}'s update() " \
                                  'function before calling get_metric().' \
                                  .format(self.__class__.__name__)
            with self.sync(
                    recover=True, aggregate=self.aggregate_when_get_metric):
                self._call_gather_object = False
                results = get_metric(*args, **kwargs)
                # 如果直接返回了Element，帮他转录一下，不然的话会在reset()之后
                # 就被重置为0了
                results = apply_to_collection(
                    results,
                    dtype=Element,
                    function=lambda x: x.get_scalar()
                    if isinstance(x.init_value, Number) else x.to_list(),
                    include_none=False)
                # elements 为空、没有 call 则准备报错
                if len(self._elements) == 0 and not self._call_gather_object:
                    # 需要 aggregate 并且在多卡环境下
                    if self.aggregate_when_get_metric and \
                            is_cur_env_distributed():
                        logger.rank_zero_warning(
                            "There is no `<class 'Element'>` registered in "
                            "metric `{}` and you didn't call "
                            '`Metric.all_gather_object()` in method '
                            '`get_metric()` either. Therefore your results '
                            'may not be aggregated in distributed training.'.
    ...
    def __setattr__(self, key, value):
        if getattr(self, '_cannot_change_element', False):
            if key in self.elements and isinstance(value,
                                                   (float, int, bool, List)):
                self.elements[key].fill_value(value)
                return
            elif key in self.elements:
                raise TypeError(
                    f'self.{key} is an Element, only float/int/bool type '
                    f'value can be assigned to it, instead of {type(value)}.')
        if isinstance(value, Element) and key not in self.elements:
            raise RuntimeError(
                'Please use register_element() function to add Element.')
        attrs = self.__dict__
        if key in attrs and isinstance(value, Element):
            raise RuntimeError(
                f'`{key}` has been registered as an attribute, cannot be '
    ...
    def _wrap_check_get_metric(self, get_metric):
        ...
        def _wrapper(*args, **kwargs):
            ...
            if self.aggregate_when_get_metric and not self._call_gather_object:
                # warning
                logger.warning(
                    "There is no `<class 'Element'>` registered in metric "
                    "`{}` and you didn't call `Metric.all_gather_object()` "
                    'in method `get_metric()` either. This may cause some '
                    'problems in distributed training since the results are '
                    'not aggregated.'.format(self.__class__))
    ...
    def sync(self, recover=True, aggregate=False):
        r"""在这个上下文下，:meth:`Metric` 会自动先同步需要同步操作的 element 。当
        ``recover`` 为 ``True`` 时，在退出环境的时候，会重新将 element 的 值恢复到
        计算前的值。"""
        keep_value = {}
        if aggregate:
    ...
    def all_gather_object(self, obj, group=None) -> List:
        """给定 ``obj`` 将各个 rank 上的 ``obj`` 汇总到每个 ``obj`` 上。返回一个 list 对象，里面依次为各个
        rank 对应的 ``obj``。

        :param obj: 需要汇总的对象，必须是个 pickable 的对象。
        :param group:
        :return: -> List[obj0, obj1, ...] 其中 obj0 是rank 0 上的 obj；obj1 是
            rank 1 上的 obj...
        """
