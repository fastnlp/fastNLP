...
__all__ = ['SpanFPreRecMetric']


def _check_tag_vocab_and_encoding_type(tag_vocab: Union[Vocabulary, dict],
                                       encoding_type: str):
    r"""
    检查 vocab 中的 tag 是否与 encoding_type 是匹配的

    :param tag_vocab: 支持传入tag Vocabulary; 或者传入形如{0:"O", 1:"B-tag1"}，即
        index在前，tag在后的dict。
    ...
    for idx, tag in tag_vocab.items():  # type: ignore
    ...
    tags = encoding_type
    for tag in tag_set:
        assert tag in tags, f'{tag} is not a valid tag in ' \
                            f'encoding type:{encoding_type}. ' \
                            f'Please check your encoding_type.'
        tags = tags.replace(tag, '')  # 删除该值
    if tags:  # 如果不为空，说明出现了未使用的tag
        logger.warning(
            f'Tag:{tags} in encoding type:{encoding_type} is not presented '
            f'in your Vocabulary. Check your encoding_type.')


def _get_encoding_type_from_tag_vocab(
        tag_vocab: Union[Vocabulary, dict]) -> str:
    r"""
    给定 Vocabular y自动判断是哪种类型的 encoding, 支持判断 bmes, bioes, bmeso,
    bio。

    :param tag_vocab: 支持传入 tag Vocabulary; 或者传入形如 {0:"O",
        1:"B-tag1"}，即 index 在前，tag 在后的 dict。
    :return:
    """
    tag_set = set()
    unk_token: Optional[str] = '<unk>'
    ...
    if tag_set == bioes_tag_set:
        return 'bioes'
    raise RuntimeError(
        'encoding_type cannot be inferred automatically. Only support '
        "'bio', 'bmes', 'bmeso', 'bioes' type.")


def _bmes_tag_to_spans(tags, ignore_labels=None):
    r"""
    给定一个 tags 的 lis，比如 ['S-song', 'B-singer', 'M-singer', 'E-singer',
    'S-moive', 'S-actor']。返回 [('song', (0, 1)), ('singer', (1, 4)),
    ('moive', (4, 5)), ('actor', (5, 6))] (左闭右开区间)，也可以是单纯的
    ['S', 'B', 'M', 'E', 'B', 'M', 'M', ...] 序列
...
def _bmeso_tag_to_spans(tags, ignore_labels=None):
    r"""
    给定一个 tag s的 lis，比如 ['O', 'B-singer', 'M-singer', 'E-singer', 'O',
    'O']。返回 [('singer', (1, 4))] (左闭右开区间)
...
def _bioes_tag_to_spans(tags, ignore_labels=None):
    r"""
    给定一个 tags 的 lis，比如 ['O', 'B-singer', 'I-singer', 'E-singer', 'O',
    'O']。返回 [('singer', (1, 4))] (左闭右开区间)
    ...
    for idx, tag in enumerate(tags):
        ...
        if bioes_tag in ('b', 's'):
            spans.append((label, [idx, idx]))
        elif bioes_tag in ('i', 'e') and prev_bioes_tag in (
                'b', 'i') and label == spans[-1][0]:
            spans[-1][1][1] = idx
        elif bioes_tag == 'o':
            pass
        else:
...
def _bio_tag_to_spans(tags, ignore_labels=None):
    r"""
    给定一个 tags 的 lis，比如 ['O', 'B-singer', 'I-singer', 'I-singer', 'O',
    'O']。返回 [('singer', (1, 4))] (左闭右开区间)
    ...
    for idx, tag in enumerate(tags):
        ...
        if bio_tag == 'b':
            spans.append((label, [idx, idx]))
        elif bio_tag == 'i' and prev_bio_tag in (
                'b', 'i') and label == spans[-1][0]:
            spans[-1][1][1] = idx
...
class SpanFPreRecMetric(Metric):
    ...
    :param f_type: ``'micro'`` 或 ``'macro'``。
        ...
        - *micro* -- 通过先计算总体的 ``TP``，``FN`` 和 ``FP`` 的数量，再计算
          ``f``, ``precision``, ``recall``；
        - *macro* -- 分别计算每个类别的 ``f`` , ``precision`` , ``recall``，然后
          做平均（各类别 ``f`` 的权重相同）；
    ...
    :param beta: **f_beta** 分数中的 ``beta`` 值。常用为 ``beta=0.5, 1, 2`` 若
        ...
        **召回率** 权重高于 **精确率**。**f_beta** 分数的计算公式为：
        ...
        .. math::
            ...
            f_{beta} = \\frac{(1 + {beta}^{2})*(pre*rec)}{({beta}^{2}*pre
             + rec)}
    ...
    def update(self, pred, target, seq_len) -> None:  # type: ignore[overridr]
        r"""
        :meth:`update` 函数将针对一个批次的预测结果做评价指标的累计。

        :param pred: 预测的结果，大小为 ``[batch, seq_len]`` 或者
            ``[batch, seq_len, len(tag_vocab)]``
        ...
        if pred.ndim == target.ndim and target.ndim == 2:
            ...
            raise RuntimeError(
                f'when pred have size:{pred.ndim}, target should have size: '
                f'{pred.ndim} or {pred.shape[:-1]}, got {target.ndim}.')
