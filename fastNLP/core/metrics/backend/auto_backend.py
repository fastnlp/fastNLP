...
class AutoBackend(Backend):
    r"""不需要初始化 ``backend`` 的 :class:`AutoBackend`，能够根据
    :meth:`get_metric` 时候判断输入数据类型来选择 ``backend``。

    :param backend: 目前支持三种值，为 ``[str, Backend, None]``。

        * 当 backend 为 :class:`str` 时，其只能为 ``'auto'``；
        * 当 backend 为 ``Backend`` 对象时，其直接使用该对象方法覆盖
          :class:`AutoBackend`；
        * 当 backend 为 ``None`` 时，根据 :meth:`get_metric` 时候判断输入数据类型
          来选择 ``backend``；
    """
    ...
    def _convert_backend(self, backend):
        ...
        :param backend: 传入的 backend 值。
            ...
            * 当 backend 为 ``'torch'`` 时，
              选择 :class:`~fastNLP.core.metric.TorchBackend`
            * 当 backend 为 ``'paddle'` 时，
              选择 :class:`~fastNLP.core.metric.PaddleBackend`
            * 当 backend 为 ``'jittor'`` 时，
              选择 :class:`~fastNLP.core.metric.JittorBackend`
            * 当 backend 为 ``'oneflow'`` 时，
              选择 :class:`~fastNLP.core.metric.OneflowBackend`
        ...
        # 如果是str，直接选择就好了
            ...
            raise RuntimeError(f'We did not support `{backend}` to be used'
                               f' as backend for now.')
        self._specified = True

    def choose_real_backend(self, args):
        """根据 args 参数类型来选择需要真正初始化的 backend.

        :param args: args 参数，可能为 ``'jittor'``, ``'torch'``, ``'paddle'``,
            ``'oneflow'``, ``'numpy'`` 类型，能够检测并选择真正的 backend。
        """
        assert not self.is_specified(), 'This method should not be called ' \
                                        'after backend has been specified. ' \
        ...
        # 根据 https://stackoverflow.com/a/3464154 ，可以通过这种方法实现切换成
        # 真实的 backend 上
        if len(torch_types) > 0 and len(jittor_types) == 0 and len(
                paddle_types) == 0:
            backend = 'torch'
        elif len(torch_types) == 0 and len(jittor_types) > 0 and len(
                paddle_types) == 0:
            backend = 'jittor'
        elif len(torch_types) == 0 and len(
                jittor_types) == 0 and len(paddle_types) > 0:
            backend = 'paddle'
        elif len(torch_types) == 0 and len(jittor_types) == 0 and len(
                paddle_types) == 0:
            backend = None
            ...
            if len(find_backends) == 1:
                ...
                logger.debug(f'Cannot find backend through sys.modules, '
                             f'since find:{find_backends}.')
        else:
            types = list(set(torch_types + jittor_types + paddle_types))
            raise RuntimeError(
                f'Mixture of tensor type:{types} have been accept, please '
                f'manually set backend instead of using backend=auto.')
