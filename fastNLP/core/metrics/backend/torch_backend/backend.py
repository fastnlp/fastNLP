...
class TorchBackend(Backend):
    ...
    def aggregate(self, tensor, method: str):
        ...
        :param method: 聚合的方法，目前支持 ``['sum', 'mean', 'max', 'min']``:
            ...
            * method 为 ``'sum'`` 时，会将多张卡上聚合结果在维度 `0` 上累加起来。
        ...
        if isinstance(tensor, torch.Tensor):
            ...
            if dist.is_available() and dist.is_initialized():
                if method is None:
                    raise AggregateMethodError(
                        should_have_aggregate_method=True)
                ...
                if method == 'sum':
                    ...
                    raise AggregateMethodError(
                        should_have_aggregate_method=False)
    ...
    def all_gather_object(self, obj, group=None) -> List:
        r"""给定 ``obj`` 将各个 rank 上的 ``obj`` 汇总到每个 ``obj`` 上。返回一个
        :class:`list` 对象，里面依次为各个 rank 对应的 ``obj``。
