...
class Backend:
    r"""执行评测时使用的 backend，是所有 backend 的父类。Backend 及其子类的所有方法
    都必须是无状态的。"""
    ...
    def aggregate(self, tensor, method: str):
        ...
        if method is not None:
            return AggregateMethodError(
                should_have_aggregate_method=False, only_warn=True)
    ...
    def all_gather_object(self, obj, group=None):
        ...
        if self.__class__.__name__ == 'AutoBackend':
            raise RuntimeError(
                'fastNLP cannot determine the backend automatically, '
                'please pass in the backend through initialization.')

        raise NotImplementedError(
            'all_gather_object() function is not implemented for '
