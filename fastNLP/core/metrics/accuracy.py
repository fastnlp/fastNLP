...
__all__ = ['Accuracy', 'TransformersAccuracy']
...
class Accuracy(Metric):
    ...
    def __init__(self,
        ...
        self.register_element(
            name='correct', value=0, aggregate_method='sum', backend=backend)
        self.register_element(
    ...
    def get_metric(self) -> dict:
        ...
        :return: 包含以下内容的字典：``{"acc": float, 'total': float,
            'correct': float}``；
        """
        evaluate_result = {
            'acc':
            round(
                self.correct.get_scalar() / (self.total.get_scalar() + 1e-12),
                6)
        }
        ...
        if pred.ndim == target.ndim:
            if np.prod(pred.shape) != np.prod(target.shape):
                raise RuntimeError(
                    ...
                    'have same element numbers. while target have shape:'
                    f'{target.shape}, pred have shape: {pred.shape}')

        elif pred.ndim == target.ndim + 1:
            pred = pred.argmax(axis=-1)
            if seq_len is None and target.ndim > 1:
                logger.warning('You are not passing `seq_len` to exclude pad '
                               'when calculate accuracy.')

        else:
            raise RuntimeError(
                f'when pred have size:{pred.shape}, target should have size: '
                f'{pred.shape} or {pred.shape[:-1]}, got {target.shape}.')
...
class TransformersAccuracy(Accuracy):
    ...
    def update(self, logits, labels, attention_mask=None):
        ...
        :param logits: 形状为 ``[B, n_classes]`` 或
            ``[B, max_len,n_classes]``。
