...
from .backend import AutoBackend, Backend
...
class Element:
    ...
    :param name: 名称
    :param value: 元素的值
    :param aggregate_method: 聚合的方法，目前支持 ``['sum', 'mean', 'max',
        'min']``:

        * method 为 ``'sum'`` 时，会将多张卡上聚合结果在维度 `0` 上 累加起来。
        * method 为 ``'mean'`` 时，会将多张卡上聚合结果在维度 `0` 上取平均值。
        * method 为 ``'max'`` 时，会将多张卡上聚合结果在维度 `0` 上取最大值。
    ...
    def __init__(self, name, value: Union[float, List], aggregate_method,
                 backend: Backend):
        self.name = name
        ...
        if backend == 'auto':
            raise RuntimeError(
                f'You have to specify the backend for Element:{self.name}.')
        ...
        if self.backend.is_specified():
            _value: Any = self.backend.create_tensor(self.init_value)
        else:
            _value = None
        self._value = _value
        self.device = None
    ...
    def aggregate(self):
        ...
        if self.aggregate_method is None:  # 如果没有 aggregate 则不进行聚合。
            return
        try:
            self._value = self.backend.aggregate(self._value,
            ...
            if self.name and e.should_have_aggregate_method:
                msg = f'Element:{self.name} has no ' \
                      'specified `aggregate_method`.'
            elif self.name and not e.should_have_aggregate_method:
                msg = "Element:{}'s backend:{} does not support " \
                      'aggregate_method:{}.'.format(
                          self.name, self.backend.__class__.__name__,
                          self.aggregate_method)
            if e.only_warn:
    ...
    def fill_value(self, value):
        r"""对元素进行 :meth:`fill_value`，会执行对应 backend 的
        :meth:`fill_value` 方法。"""
        self._check_value_initialized()
    ...
    def to(self, device):
        ...
        if self._value is not None:
            self._value = self.backend.move_tensor_to_device(
                self._value, device)
        self.device = device
    ...
    def _check_value_when_call(self):
        if self.value is None:
            ...
            raise RuntimeError(
                prefix +
                ' is not initialized. Please either specify backend when '
                'creating this element, or use it after it being used by '
                'the `Metric.update()` method.')
    ...
    def __getitem__(self, item):
        ...
        if isinstance(self.init_value, Number):
            raise TypeError(f"'{type(self.init_value).__name__}' object "
    ...
    def __setitem__(self, key, value):
        if isinstance(self.init_value, Number):
            raise TypeError(
                f"'{type(self.init_value).__name__}' object does not "
