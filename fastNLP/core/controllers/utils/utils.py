...
class _TruncatedDataLoader:
    r"""
    ``_TruncatedDataLoader`` 用于实现 ``Trainer`` 和 ``Evaluator`` 中的 '预跑'
    和 '假跑' 功能：

        1. 预跑 是针对 trainer 的验证而言的，即我们在正式的训练前会先使用 trainer
           内置的 evaluator（如果不为 None）评测数量非常少的数据，来检验用户的
           metric 和 evaluate_dataloader 以及模型是否能够合作完成正确的评测过程；
        2. 假跑 的意思是改变每一个 epoch 中训练或者评测的实际的 batch 的数量，例如改
           成 10，来让模型快速地迭代整体的训练或者评测流程，来查看整体的过程的正确性；

    ``_TruncatedDataLoader`` 的实现也非常简单，我们在该类中内置一个计数器，当迭代器
    的迭代数量达到指定数值后 ``raise StopIteration``；
    ...
    def __iter__(self):
        # 将初试的 `dataloader` 转换成一个 `Iterator` 的逻辑应该放在这里，即只有当
        # 外界真正的调用 iter(dataloader) 的时候才需要返回一个 Iterator；
        # TODO 测试一下
    ...
    def __setattr__(self, key, value):
        # 添加该函数使得在进行实验性训练或者评测时，用户对于 trainer.dataloader 的
        # 感觉和正常训练完全一样；
        # 注意这里不能直接 ``setattr(self.dataloader, key, value)``，会导致死循
        # 环；
...
def check_evaluate_every(evaluate_every):
    ...
    主要在于当参数 ``evaluate_every`` 是一个 Callable 的函数时，需要保证其参数的正
    确性；
    """
    if not callable(evaluate_every) and (not isinstance(evaluate_every, int)
                                         or evaluate_every == 0):
        raise ValueError(
            "Parameter 'evaluate_every' should be set to 'int' type and "
            'either < 0 or > 0.')
    if callable(evaluate_every):
        _check_valid_parameters_number(
            evaluate_every, expected_params=['trainer'])
