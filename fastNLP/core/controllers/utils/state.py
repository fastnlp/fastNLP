...
class State(dict):
    r"""
    提供给用户使用的 ``state``，用来记载您的 ``callback`` 实时数据，该 ``state``
    实际上是一个字典，我们通过复用 :meth:`__getattr__` 方法来实现类似类属性的字典
    调用方式；
    ...
    推荐的使用方式::
        ...
        >>> print(state.best_accuracy)
    """
    # 用户不应当使用 state.name = "name" 来使用此类，因此我们限制用户不可自己对该类
    # 设置属性，但是可以通过属性访问字典；
    __slots__ = ()
...
class TrainerState:
    ...
    :param n_batches: 完整训练过程会 forward 的 step 数量，注意 ``n_batches =
        num_batches_per_epoch * n_epochs``；
    """
    ...
    def state_dict(self) -> Dict:
        ...
        return {
            ...
            'global_forward_batches': self.global_forward_batches,
    ...
    def load_state_dict(self, state_dict: Dict):
        ...
        for key in state_dict:
            assert key in {
                'cur_epoch_idx', 'global_forward_batches', 'batch_idx_in_epoch'
            }, 'Wrong state_dict for `TrainerState`.'
