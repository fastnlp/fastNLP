...
from fastNLP.core.metrics.utils import (_is_allennlp_metric, _is_paddle_metric,
                                        _is_torcheval_metric,
                                        _is_torchmetrics_metric)
from fastNLP.core.utils import (auto_param_call, dataclass_to_dict,
                                f_rich_progress, f_tqdm_progress,
                                flat_nest_dict, match_and_substitute_params)
from fastNLP.core.utils.utils import _check_valid_parameters_number
from fastNLP.envs import FASTNLP_MODEL_FILENAME
from ..drivers.choose_driver import choose_driver
from .loops import EvaluateBatchLoop, Loop
...
class Evaluator:
    ...
    :param dataloaders: 用于评测的数据集。如果为多个，您需要使用 ``dict`` 传入，即
        ...
        数的名称。
    :param metrics: 评测时使用的指标。注意该参数必须为 ``dict`` 类型，其中 ``key``
    ...
    :param evaluate_fn: 用来控制 ``Evaluator`` 在评测的前向传播过程中调用的是哪一
        ...
        1. 如果该值是 ``None``，那么我们会默认使用 ``evaluate_step`` 当做前向传播
           的函数，如果在模型中没有找到该方法，则使用训练框架默认的前向传播函数；
        2. 如果为 ``str`` 类型，例如为 ``'my_evaluate_step_fn'``，则尝试寻找
           :meth:`model.my_evaluate_step_fn`，如果找不到则直接报错；

    :param input_mapping: 等价于 ``Trainer`` 中的 ``input_mapping`` 参数；对具体
        的用于评测一个 batch 的数据使用 ``input_mapping`` 处理之后再输入到
        ``model`` 以及 ``metric`` 中。如果针对 ``model`` 和 ``metric`` 需要不同
        的 ``mapping``，请考虑使用 ``evaluate_batch_step_fn`` 参数定制；也可以使
        用 evaluate_input_mapping 参数名传入。

        .. todo::

            之后链接上 参数匹配 的文档；

    :param output_mapping: 等价于 ``Trainer`` 中的 ``output_mapping`` 参数；对
        ``model`` 输出的内容，将通过 ``output_mapping`` 处理之后再输入到
        ``metric`` 中；也可以使用 evaluate_output_mapping 参数名传入。
    :param model_wo_auto_param_call: 等价于 ``Trainer`` 中的
        ``model_wo_auto_param_call`` 参数；

        .. note::

            一个十分需要注意的问题在于 ``model_wo_auto_param_call`` 只会关闭部分的
            参数匹配，即指挥关闭前向传播时的参数匹配，但是由于 ``Evaluator`` 中
            ``metric`` 的计算都是自动化的，因此其一定需要参数匹配：根据 ``metric.
            update`` 的函数签名直接从字典数据中抽取其需要的参数传入进去；
    ...
    :kwargs:
        ...
        * *model_use_eval_mode* (``bool``) --
          ...
          认为 ``True``。如果为 ``False``，``fastNLP`` 不会对 ``model`` 的
          ``evaluate`` 状态做任何设置。无论该值是什么，``fastNLP`` 都会在评测后将
          ``model`` 的状态设置为 ``train``；
        * *use_dist_sampler* --
          表示在 ``Evaluator`` 中在使用分布式的时候是否将保证 dataloader 的
          ``sampler`` 替换为分布式的 ``sampler``，其特点是每个卡上的数据之间不重
          叠，所有卡上数据的加起来是整个数据集。若传入的 dataloader 的 sampler 为：
    ...
    def __init__(self,
        ...
        dataloaders = dataloaders if dataloaders is not None else kwargs.get(
        ...
        self.evaluate_batch_loop = EvaluateBatchLoop(
            batch_step_fn=evaluate_batch_step_fn)

        if evaluate_batch_step_fn is not None:
            _check_valid_parameters_number(
                evaluate_batch_step_fn, ['evaluator', 'batch'],
                fn_name='evaluate_batch_step_fn')
        self.evaluate_batch_step_fn = evaluate_batch_step_fn

        self.input_mapping = input_mapping if input_mapping is not None else kwargs.get(
            'evaluate_input_mapping')
        self.output_mapping = output_mapping if output_mapping is not None else kwargs.get(
        ...
        if use_dist_sampler:
            self._dist_sampler = 'unrepeatdist'  # type: ignore
        else:
            self._dist_sampler = None  # type: ignore
        self._metric_wrapper = None
        _ = self.metrics_wrapper  # 触发检查

        if evaluate_fn is not None and not isinstance(evaluate_fn, str):
            raise TypeError(
                'Parameter `evaluate_fn` can only be `str` type when '
                'it is not None.')
        self._evaluate_step, self._evaluate_step_signature_fn = \
            self.driver.get_model_call_fn(
                'evaluate_step' if evaluate_fn is None else evaluate_fn)
        self.evaluate_fn = evaluate_fn

        self.dataloaders = {}
        for name, dl in dataloaders.items():  # 替换为正确的 sampler
            dl = self.driver.set_dist_repro_dataloader(
                dataloader=dl, dist=self._dist_sampler, reproducible=False)
    ...
    def load_model(self,
        ...
        :param only_state_dict: 要读取的文件中是否仅包含模型权重。当
        ...
        .. note::
            ...
            注意您需要在初始化 ``Evaluator`` 后再通过 ``evaluator`` 实例来调用该函
            数；这意味着您需要保证在保存和加载时使用的 ``driver`` 是属于同一个训练框
            架的，例如都是 **pytorch** 或者 **PaddlePaddle**；
        """
        self.driver.barrier()
    ...
    def run(self, num_eval_batch_per_dl: int = -1) -> Dict:
        ...
        """
        assert isinstance(num_eval_batch_per_dl, int), \
            'num_eval_batch_per_dl must be of int type.'
        assert num_eval_batch_per_dl > 0 or num_eval_batch_per_dl == -1, \
            'num_eval_batch_per_dl must be -1 or larger than 0.'

        metric_results = {}
        self.reset()
        evaluate_context = self.driver.get_evaluate_context()
        self.driver.set_model_mode(
            mode='eval' if self.model_use_eval_mode else 'train')
        with evaluate_context():
            try:
                for dataloader_name, dataloader in self.dataloaders.items():
                    self.driver.barrier()
                    if num_eval_batch_per_dl != -1:
                        dataloader = _TruncatedDataLoader(
                            dataloader, num_eval_batch_per_dl)
                    self.driver.set_sampler_epoch(dataloader, -1)
                    self.start_progress_bar(
                        total=len(dataloader), dataloader_name=dataloader_name)
        ...
        metric_results = flat_nest_dict(
            metric_results,
            separator=self.separator,
            compress_none_key=True,
            top_down=False)
        if len(metric_results) > 0:  # 如果 metric 不为 None 需要 print 。
            # metric_results = flat_nest_dict(
            #     metric_results, separator=self.separator,
            #     compress_none_key=True, top_down=False)
            if self.verbose:
    ...
    def start_progress_bar(self, total: int, dataloader_name):
        if self.progress_bar in ('rich', 'tqdm'):
            ...
            if self.progress_bar == 'rich':
                self._task_id = f_rich_progress.add_task(
                    description=desc, total=total)
            else:
                self._task_id = f_tqdm_progress.add_task(
                    description=desc, total=total)
    ...
    def update_progress_bar(self, batch_idx, dataloader_name, **kwargs):
        ...
        if self.progress_bar == 'rich':
            assert hasattr(self, '_task_id'), \
                'You must first call `start_progress_bar()` before calling ' \
                'update_progress_bar()'
            f_rich_progress.update(
                self._task_id,
                description=desc,
                post_desc=kwargs.get('post_desc', ''),
    ...
    def evaluate_batch_loop(self, loop: Loop):
        if getattr(self, 'evaluate_step_fn', None) is not None:
            logger.rank_zero_warning(
                '`evaluate_batch_step_fn` was customized in the '
                'Evaluator initialization, it will be ignored when '
                'the `evaluate_batch_loop` is also customized.')
    ...
    def update(self, batch, outputs):
        ...
        :param batch: 一般是来自于 DataLoader 的输出，如果不为 dict 类型的话，该值
            将被忽略。
        :param outputs: 一般是来自于模型的输出。类别应为 dict 或者 dataclass 类
            型。
        :return:
        """
    ...
    def metrics_wrapper(self):
        r"""由于需要保持 Evaluator 中 ``metrics`` 对象与用户传入的 ``metrics`` 保
        持完全一致（方便在 ``evaluate_batch_step_fn`` ）中使用，同时也为了支持不同
        形式的 metric（ fastNLP 的 metric/torchmetrics 等），所以 Evaluator 在进
        行metric 操作的时候都调用 metrics_wrapper 进行操作。"""
        if self._metric_wrapper is None:
            self._metric_wrapper = _MetricsWrapper(
                self.metrics, evaluator=self)
    ...
    def evaluate_step(self, batch):
        ...
        :return: ``evaluate_fn`` 函数的输出结果，如果有设置 ``output_mapping``，
            将是 ``output_mapping`` 之后的结果。
        """
        outputs = self.driver.model_call(batch, self._evaluate_step,
                                         self._evaluate_step_signature_fn)
...
class _MetricsWrapper:
    ...
    def __init__(self, metrics, evaluator):
        ...
        if metrics is not None:
            ...
            for metric_name, metric in metrics.items():
                # 因为 torchmetrics 是一个 nn.Module，因此
                # 我们需要先将其移到对应的机器上；
                if _is_torchmetrics_metric(metric) and isinstance(
                        evaluator.driver, TorchDriver):
                    # torchmetrics 是默认自动开启了多卡的
                    evaluator.driver.move_model_to_device(
                        metric, evaluator.driver.data_device)
                if _is_torcheval_metric(metric):
                    # torcheval也是默认开启多卡，需要在这一步将数据移动到
                    # 对应的机器上；
                    metric.to(evaluator.driver.data_device)
                elif isinstance(metric, Metric):
                    # 如果数据是分布式的，但是不aggregate的话可能有问题
                    if evaluator._dist_sampler is not None and \
                            metric.aggregate_when_get_metric is False:
                        logger.rank_zero_warning(
                            'You have replaced the sampler as distributed '
                            'sampler when evaluation, but your metric '
                            f"{metric_name}:{metric.__class__.__name__}'s "
                            '`aggregate_when_get_metric` is False.',
                            once=True)
                    if metric.aggregate_when_get_metric is None:
                        metric.aggregate_when_get_metric = \
                            evaluator._dist_sampler is not None
    ...
    def update(self, batch, outputs):
        ...
        for metric in self._metrics:
            ...
            if not isinstance(batch, dict):
                logger.rank_zero_warning(
                    'The output of the DataLoader is of type:`{}`, fastNLP '
                    'will only depend on the output of model to update '
                    'metric.'.format(type(batch)),
                    once=True)
            else:
                args.append(batch)
            if not isinstance(outputs, dict):
                raise RuntimeError(
                    ...
                    'either directly return a dict from your model or '
                    'use `output_mapping` to convert it into dict type.'.
                    format(type(batch)))
            if isinstance(metric, Metric):
                ...
                auto_param_call(
                    ...
                    outputs,
                    *args,
                    signature_fn=metric.update.__wrapped__)
    ...
    def reset(self):
        ...
        for metric in self._metrics:
            ...
            elif _is_torchmetrics_metric(
                    metric) or _is_paddle_metric(metric) or isinstance(
                        metric, Metric) or _is_torcheval_metric(metric):
                metric.reset()
