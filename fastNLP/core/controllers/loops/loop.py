r"""
``TrainBatchLoop`` 和 ``EvaluateBatchLoop`` 的父类，为了在实现 **fastNLP** 主要功
能的同时保证 **fastNLP** 的易用性和代码的易读性，我们只对训练中的循环做了非常简单的抽
...
class Loop:
    r"""
    ``TrainBatchLoop`` 和 ``EvaluateBatchLoop`` 的父类，您可以继承此类来定制自己的
    训练或者评测 ``loop``
    """
    ...
    def run(self, controller, dataloader):
        ...
        .. note::
            ...
            ``Trainer`` 和 ``Evaluator`` 中都提供了方便您进行定制 ``Loop`` 的接口
