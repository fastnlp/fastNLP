...
def _convert_data_device(device: Union[str, int]) -> str:
    r"""用于转换 ``driver`` 的 ``data_device`` 的函数。如果用户设置了
    ``FASTNLP_BACKEND=paddle``，那么 **fastNLP** 会将 可见的设备保存在
    ``USER_CUDA_VISIBLE_DEVICES`` 中，并且将 ``CUDA_VISIBLE_DEVICES``
    ...
    ``rank1`` 的进程中有::
        ...
        # 为了向用户正确地反映他们设置的设备减少歧义，因此这里没有设置为 "gpu:5"
        driver.data_device = "gpu:2"

    此时我们便需要通过这个函数将 ``data_device`` 转换为 ``gpu:0``。具体过程便是通过
    索引 **2** 在 ``USER_CUDA_VISIBLE_DEVICES`` 中找到设备 **5**，然后在
    ``CUDA_VISIBLE_DEVICES`` 中找到设备 **5** 的索引 **0** 返回。

    .. note::

        在分布式单进程仅支持单卡的情况下中，这个函数实际等同于直接转换为 ``gpu:0``
        返回。
    ...
    try:
        ...
        idx = user_visible_devices.split(',')[device_id]
        # 此时 CUDA_VISIBLE_DEVICES 一定不是 None
        cuda_visible_devices_list = os.getenv('CUDA_VISIBLE_DEVICES',
                                              '').split(',')
        return f'gpu:{cuda_visible_devices_list.index(idx)}'
    except Exception:
        raise ValueError(
            f'Cannot convert device {device} when USER_CUDA_VISIBLE_DEVICES= '
            f'{user_visible_devices} and CUDA_VISIBLE_DEVICES= '
            f'{cuda_visible_devices_list}. If this situation happens, please '
            'report this bug to us.')
...
def paddle_move_data_to_device(batch: Any,
                               device: Optional[Union[str, int]]) -> Any:
    r"""
    将 **paddle** 的数据集合传输到给定设备。只有 :class:`paddle.Tensor` 对象会被传
    输到设备中，其余保持不变。

    :param batch: 需要进行迁移的数据集合；
    :param device: 目标设备。可以是显卡设备的编号，或是``cpu``, ``gpu`` 或
        ``gpu:x`` 格式的字符串；当这个参数为 `None`` 时，不会执行任何操作。
    :return: 迁移到新设备上的数据集合；
    """
    if device is None:
        return batch
...
def is_in_paddle_dist() -> bool:
    """判断是否处于 **paddle** 分布式的进程下，使用 ``PADDLE_RANK_IN_NODE`` 和
    ``FLAGS_selected_gpus`` 判断。"""
    return ('PADDLE_RANK_IN_NODE' in os.environ
            and 'FLAGS_selected_gpus' in os.environ)
...
def is_in_paddle_launch_dist() -> bool:
    r"""判断是否处于 ``python -m paddle.distributed.launch`` 方法启动的
    **paddle** 分布式进程中。"""
