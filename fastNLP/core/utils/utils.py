...
def auto_param_call(fn: Callable,
                    *args,
    ...
    对应的值，并将这个值传递给形参中名为 ``value`` 的参数。

    1. 该函数用来提供给用户根据字符串匹配从而实现自动调用；
    2. 注意 ``mapping`` 默认为 ``None``，如果您希望指定输入和运行函数的参数的对应方
       式，那么您应当让 ``mapping`` 作为一个字典传入进来；如果 ``mapping`` 不为
       ``None``，那么我们一定会先使用 ``mapping`` 将输入的字典的 ``keys`` 修改过
       来，因此请务必亲自检查 ``mapping`` 的正确性；
    3. 如果输入的函数的参数有默认值，那么如果之后的输入中没有该参数对应的值，我们就会
       使用该参数对应的默认值，否则也会使用之后的输入的值；
    ...
    :param args: 一系列的位置参数，应当为一系列的字典，我们需要从这些输入中提取
        ``fn`` 计算所需要的实际参数；
    :param signature_fn: 函数，用来替换 ``fn`` 的函数签名，如果不为 ``None``，那么
        我们首先会从该函数中提取函数签名，然后通过该函数签名提取参数值后，再传给
        ``fn`` 进行实际的运算；
    ...
    if signature_fn is not None:
        if not callable(signature_fn):
            raise ValueError('Parameter `signature_fn` should be `Callable`.')
    ...
    for _name, _param in _need_params.items():
        if _param.kind == Parameter.VAR_POSITIONAL:
            ...
            raise ValueError(
                'It is not allowed to have parameter `*args` in your '
                f'function:{fn_msg}.')
    ...
    if mapping is not None:
        ...
        assert isinstance(mapping, Dict), \
            f'Exception happens when calling {fn_msg}. Parameter `mapping` ' \
            f"should be of 'Dict' type, instead of {type(mapping)}."
    ...
    for arg in args:
        if not isinstance(arg, Dict):
            fn_msg = _get_fun_msg(fn if signature_fn is None else signature_fn)
            raise TypeError(
                f'Exception happens when calling {fn_msg}. The input part of '
                'function `auto_param_call` must be `Dict` type, instead of '
                f'{type(arg)}.')
        for _name, _value in arg.items():
            if mapping is not None and _name in mapping:
                _name = mapping[_name]  # type: ignore
    ...
    if duplicate_names:
        fn_msg = _get_fun_msg(fn if signature_fn is None else signature_fn)
        raise ValueError(
            f'The following key present in several inputs:{duplicate_names} '
            f'when calling {fn_msg}.')
    ...
    if len(_has_params) < len(_need_params):
        ...
        fn_msg = _get_fun_msg(fn if signature_fn is None else signature_fn)
        _provided_keys = _get_keys(args)  # type: ignore
        raise ValueError(
            f'The parameters:`{miss_params}` needed by function:{fn_msg} '
...
def check_user_specific_params(user_params: Dict, fn: Callable, fn_name=None):
    r"""该函数使用用户的输入来对指定函数的参数进行赋值，主要用于一些用户无法直接调用函
    数的情况；主要作用在于帮助检查用户对使用函数 ``fn`` 的参数输入是否有误；

    :param user_params: 用户指定的参数的值，应当是一个字典，其中 ``key`` 表示每一个
        参数的名字，``value`` 为每一个参数的值；
    :param fn: 将要被调用的函数；
    ...
    for arg_name, arg_value in user_params.items():
        if arg_name not in fn_arg_names:
            logger.rank_zero_warning(
                f'Notice parameter `{arg_name}` may not be used '
                f'by `{fn_name}`.')
    return user_params
...
def dataclass_to_dict(data) -> Dict:
    ...
    if not is_dataclass(data):
        raise TypeError('Parameter `data` can only be `dataclass` type '
...
def match_and_substitute_params(mapping: Optional[Union[Callable,
                                                        Dict]] = None,
    ...
    * 对于 ``output_mapping``，该函数会在 :class:`~fastNLP.core.Trainer` 的
    ...
    2. 如果 ``mapping`` 是一个 **Dict**，那么 ``data`` 的类型只能为以下三种：
       ``[Dict, dataclass, Sequence]``；
        ...
        * 如果 ``data`` 是 **Sequence**，那么该函数会先将其转换成一个对应的字典::
          ...
          然后使用 ``mapping`` 对这个字典进行转换，如果没有匹配上 ``mapping`` 中的
          ``key`` 则保持 ``'_number'`` 这个形式。

    :param mapping: 用于转换的字典或者函数；当 ``mapping`` 是函数时，返回值必须为字
        典类型；
    :param data: 需要被转换的对象；
    :return: 转换后的结果；
    """
    if mapping is None:
        return data
    if callable(mapping):
        # 注意我们在 `Trainer.extract_loss_from_outputs` 函数里会检查
        # outputs 的输出，outputs 的类型目前只支持 `Dict` 和 `dataclass`；
        return mapping(data)

    if not isinstance(mapping, Dict):
        raise ValueError(
            'Parameter `mapping` should be of type `Dict` or `Callable`, '
            f'not `{type(mapping)}`. This is caused by your `input_mapping` '
            'or `output_mapping` parameter in your `Trainer` or `Evaluator`.')
    if not isinstance(data,
                      Dict) and not is_dataclass(data) and not isinstance(
                          data, Sequence):
        raise ValueError(
            'Parameter `data` should be type `Dict` or `dataclass` when '
            'the other parameter `mapping` is type `Dict`.')

    # 如果 `data` 是一个 dataclass，那么先将其转换为一个 `Dict`；
    if is_dataclass(data):
        data = dataclass_to_dict(data)
    # 如果 `data` 是一个 List，那么我们同样先将其转换为一个 `Dict`，
    # 为 {"_0": list[0], "_1": list[1], ...}；
    elif isinstance(data, Sequence):
        data = {'_' + str(i): data[i] for i in range(len(data))}

    _new_data = {}
    for _name, _value in data.items():  # type: ignore
        if _name in mapping:
...
def _is_namedtuple(obj: object) -> bool:
    ...
    return isinstance(obj, tuple) and hasattr(obj, '_asdict') and hasattr(
        obj, '_fields')
...
def apply_to_collection(
    ...
    **kwargs: Any,
    ...
    r"""递归地对 ``data`` 中的元素执行函数 ``function``，且仅在满足元素为
    ``dtype`` 时执行。

    该函数参考了 `pytorch-lightning <https://github.com/PyTorchLightning/
    pytorch-lightning>`_ 的实现

    :param data: 需要进行处理的数据集合或数据；
    :param dtype: 数据的类型，函数 ``function`` 只会被应用于 ``data`` 中类型为
        ``dtype`` 的数据；
    ...
    """
    # Breaking condition
    if isinstance(data, dtype) and (wrong_dtype is None
                                    or not isinstance(data, wrong_dtype)):
    ...
    if _is_dataclass_instance(data):
        ...
        for field_name, (field_value, field_init) in fields.items():
            ...
            if not field_init or (not include_none
                                  and v is None):  # retain old value
...
def deprecated(help_message: Optional[str] = None):
    ...
    def decorator(deprecated_function: Callable):
        ...
        warning_msg = (
            (f'{deprecated_function.__name__} is deprecated and will be '
             'removed in the next major version of datasets.') +
        ...
        def wrapper(*args, **kwargs):
            ...
            if func_hash not in _emitted_deprecation_warnings:
                logger.warning(
                    warning_msg, category=FutureWarning, stacklevel=2)
                _emitted_deprecation_warnings.add(func_hash)
            return deprecated_function(*args, **kwargs)

        wrapper._decorator_name_ = 'deprecated'  # type: ignore
        return wrapper
...
def wait_filepath(path, exist=True):
    ...
    while True:
        ...
        if count % 1000 == 0:
            ...
            logger.warning(
                f'Waiting path:{path} to {msg} for {count*0.01} seconds...')
...
def get_class_that_defined_method(method):
    ...
    if inspect.ismethod(method) or (inspect.isbuiltin(method) and getattr(
            method, '__self__', None) is not None and getattr(
                method.__self__, '__class__', None)):
        for cls in inspect.getmro(method.__self__.__class__):
            if method.__name__ in cls.__dict__:
                return cls
        method = getattr(method, '__func__',
                         method)  # fallback to __qualname__ parsing
...
def flat_nest_dict(d: Dict,
                   separator: str = '#',
                   compress_none_key: bool = True,
                   top_down: bool = False) -> Dict:
    """将一个 nested 的 dict 转成 flat 的 dict，例如：

        >>> d = {'test': {'f1': {'f': 0.2, 'rec': 0.1}}}
        >>> flat_nest_dict(d)
        {'f#f1#test':0.2, 'rec#f1#test':0.1}
        ...
        if isinstance(value, Mapping):
            flat_d.update(
                _flat_nest_dict(
                    ...
                    compress_none_key=compress_none_key))
        else:
            flat_d[key] = value
...
def _flat_nest_dict(d: Mapping, parent_key: Tuple, compress_none_key: bool):
    ...
    for k, v in d.items():
        ...
        if isinstance(v, Mapping):
            _d = _flat_nest_dict(
                v, parent_key=_key, compress_none_key=compress_none_key)
            flat_d.update(_d)
        else:
            flat_d[_key] = v
