...
from fastNLP.core.samplers import (DopedSampler, MixSampler,
                                   MixSequentialSampler, PollingSampler)
...
__all__ = ['MixDataLoader']


class _MixDataset:
    r"""将所有数据集当成一个混合大数据集来对待，在 __getitem__() 能根据输入的 idx 来
    判断属于哪个小数据并返回其 ds_index。
    ...
    def __getitem__(
        ...
        if isinstance(idx, int):
            ...
            for i in idx:
                ...
                dataset.append(instance)  # type: ignore
...
class _MixCollateFn:
    r"""存在多个 auto_collate 和多个 collate_fn 时候，对一个批次数据集应用哪个
    auto_collate 和 collate_fn 的问题。"""

    def __init__(self, collate_fns: Union[List[Callable], Callable,
                                          Any]) -> None:

        if isinstance(collate_fns, Sequence):
            self.collate_fns = lambda idx, lst: collate_fns[idx](lst)
        elif callable(collate_fns):
            self.collate_fns = lambda idx, lst: collate_fns(  # type: ignore
                lst)
        else:
...
class MixDataLoader(DataLoader):
    ...
    :param mode: ``mode`` 控制 ``MixDataLoader`` 运行模式。``mode`` 的取值范围为
        ...
        * 当 mode 为 ``'sequential'`` 时，``MixDataLoader``  将 ``datasets`` 的
          ...
          个接一个的 sample 完所有数据。
        * 当 mode 为 ``'mix'`` 时，``MixDataLoader``  将 ``datasets`` 的序列或者
          字典视为一个混合大数据集，然后根据用户输入的 idx 序列随机 sample 混合数据
          集 datasets 的数据组成一个 batch 序列返回。
        * 当 mode 为 ``'polling'`` 时，``MixDataLoader`` 按照 ``datasets`` 数据
          集的顺序，先从第一个数据集采样一个 batch 的数据返回，再从第二数据集采样一
          个 batch 数据返回，直至最后一个数据集采样一个 batch 数据返回后再从第一个数
          据采样第二个 batch 数据返回，直至所有的数据集都被轮询地的采样完。
        * 当 mode 为 ``'Sampler'`` 时，该 Sampler 是实现 __iter__() 的实例化对
          象，其功能是每次 iter 时返回一个 batch 序列，其类型为 List[int]；且
    ...
    :param collate_fn: 用于从 dataset 取到的一个 batch 数据进行打包处理的
        ...
        * collate_fn 为  :class:`Callable` 时，该 collate_fn 会被 datasets 序列
          ...
          该 Callable 函数还应当返回一个对象。
        * collate_fn 为 ``Dict[str, Callable]`` 时，datasets 的 key 必须和
          callable_fn 的 key 一致。``MixDataLoader`` 会将 ``collate_fn[key]`` 用
    ...
    :param sampler: 取样器，应该是实现了 :meth:`__len__` 和 :meth:`__iter__`
        ...
        * sampler 为 ``str`` 时，sampler 选择范围为 ``[rand, seq]``。当 sampler
          为 ``rand`` 时，``MixDataLoader`` 默认初始化 **pytorch** 的
          ``RandomSampler`` 作为默认值，其功能时随机采样 dataset 的下标并返回。当
          sampler 为 ``seq`` 时，``MixDataLoader`` 默认初始化 **pytorch** 的
          ``SequentialSampler`` 作为默认值。其功能时顺序返回 dataset 的下标。
    ...
    :param batch_size: 批次大小，默认为 ``16`` 且当 batch_sampler 为 ``None`` 有
        效。且 datasets 上所有 dataset 的 batch_size 一致。
    :param drop_last: 当 ``drop_last=True`` 时，``MixDataLoader`` 会扔掉
        datasets 中 每个 dataset 最后一个长度小于 ``batch_size`` 的 batch 数据；
        若 ``drop_last=False``，则会返回该 batch 数据。默认为 ``False``。
    :param ds_ratio: ``ds_ratio`` 是控制 datasets 怎么组成一个混合大数据集的重要参
        数，其取值为 ``[None, 'truncate_to_least', 'pad_to_most', List[float],
        Dict[str, float]]``:

        * ds_ratio 为 ``None``, datasets 数据集序列或字典不进行数据扩充处理。
        * ds_ratio 为 ``'truncate_to_least'``, datasets 数据集序列或字典会计算得
          到 dataset s序列中 dataset 最断长度 ``mix_len``，其他数据集会被切断到最
          短长度 ``mix_len``。这种切断不是物理上切断，``MixDataLoader`` 会根据
        ...
        * ds_ratio 为 ``Dict[str, float]`` 时，datasets 类型也必须为 ``Dict
          [str,DataSet]``，其 key 一一对应。ds_ratio 的 value 是任意大于 0 的浮点
          数，代表着 datasets 的 value 数据进行扩充或者缩减的倍数。
    """

    def __init__(self,
                 datasets: Dict,
                 mode: str = 'sequential',
                 collate_fn: Union[str, Callable, Dict[str,
                                                       Callable]] = 'auto',
                 sampler: Union[str, None, Dict[str, Any]] = None,
                 num_workers: int = 0,
                 batch_size: int = 16,
                 drop_last=False,
        ...
        if isinstance(sampler, Dict):
            for key in datasets.keys():
                if not sampler[key]:
                    raise ValueError(
                        f'the key:{key} of datasets is not in sampler, '
                        'where sampler is a dict!')
        ...
        if isinstance(collate_fn, Dict):
            ...
            for key in datasets.keys():
                if key not in collate_fn:
                    raise ValueError(
                        f'the key:{key} of datasets is not in collate_fn, '
                        'where collate_fn is a dict!')
        ...
        # Dict 类型的 collate_fn 转化为 List，以便于 _MixCollateFn 里面根据 idx
        # 定位 dataset
        if isinstance(collate_fn, Dict):
            collate_fn = [fn for _, fn in collate_fn.items()]  # type: ignore
        ...
        if mode == 'sequential':
            batch_sampler: MixSampler = MixSequentialSampler(
                datasets,
            ...
            raise ValueError(
                f'{mode} must be sequential, polling, mix or batch_sampler')

        if parse_version(torchversion) >= parse_version('1.7'):
            super(MixDataLoader, self).__init__(
                _MixDataset(datasets=dataset),
                batch_size=1,
                shuffle=False,
                sampler=None,
