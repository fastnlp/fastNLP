...
from fastNLP.core.samplers import RandomBatchSampler, ReproducibleBatchSampler
from ..utils import HasLenGetitemType, _match_param

__all__ = ['PaddleDataLoader', 'prepare_paddle_dataloader']


class _PaddleDataset(Dataset):
    r"""对用户传的dataset进行封装，以便 :class:`PaddleDataLoader` 能够支持使用自定
    义的 dataset。"""
...
class PaddleDataLoader(DataLoader):
    r"""提供给 ``paddle`` 框架使用的 ``DataLoader`` 类。

    ``PaddleDataLoader`` 是专门提供给 ``paddle`` 框架的 ``DataLoader``，其集成了
    **fastNLP** 的 ``Collator`` （具体详见 :class:`.Collator`），并对 paddle 的
    ``DataLoader`` 进行了封装，使得其具备以下功能：

    1. ``PaddleDataLoader`` 支持输入的 dataset 是无框架的，只要实现了 __getitem__
       () 和 __len__() 即可，当不使用  :class:`~fastNLP.core.DataSet` 时也不需要
       传入 collate_fn，只需要将 ``collate_fn='auto'`` 就能够自动探测数据的类型并
       判断能否 pad 。此时可以调用 :meth:`set_pad` 和 :meth:`set_ignore` 方法来设
       置 field 的 pad_val 或者忽略某个 field 的 pad 操作。
    ...
    2. 当 collate_fn 为 ``None`` 时，``PaddleDataLoader`` 默认使用 ``paddle`` 自
       带的 ``default_collate_fn`` 作为 collate_fn 的值。

    .. note::
        当传入的 dataset 为 fastNLP 的 DataSet 时，collate_fn 不能为 None。默认可
        以是 "auto"或者自定义 callable 函数。

    3. 当 collate_fn 为 :class:`Callable` 时，该 Callable 函数应当接受一个 batch
    ...
    :param dataset: 需要遍历的数据集，可以是 :class:`~fastNLP.core.DataSet`、
        paddle 的 :class:`Dataset`、hugginface 的数据集对象，以及所有实现了
    ...
    :param return_list: 每个设备上的返回值是否为以列表形式显示。如果
        :attr:`return_list=False`，每个设备上的返回值值为 str -> Tensor 的
        dict，其中 dict 的 key 为每个 fed Tensors 的名字。如果
    ...
    :param shuffle: 是否打乱数据集，默认为 ``None``, 如果传入的 ``ds_or_db`` 可以
        判断出哪个是 ``'train'`` 则设置其 shuffle 为 ``True``，它的为 False 。
    :param drop_last: 当 ``drop_last=True`` 时，``PaddleDataLoader`` 会扔掉最后
    ...
    :param collate_fn: 用于从 dataset 取到的一个 batch 数据进行打包处理的
        ...
        * collate_fn 为 ``None`` 时，需要注意的是此时传进来的 dataset 类型不能为
          ...
          时，``PaddleDataLoader`` 调用默认的 Paddle 框架的 ``DataLoader`` 自带
          的 `default_collate_fn` 作为 collate_fn 的默认值，其无法处理
          :class:`~fastNLP.core.dataset.DataSet` 的dataset对象。
        * collate_fn 为 ``'auto'`` 时，``PaddleDataLoader`` 使用
          :class:`~fastNLP.core.collators.Collator` 作为 collate_fn 的默认值。
          此时可以配套使用 :meth:`set_pad` 和 :meth:`set_ignore` 方法来设置
          pad_val 或 忽略某个 field 的检测。
    ...
    :param use_buffer_reader: 是否开启 buffer_reader 。如果
        ``use_buffer_reader=True``，那么 ``PaddleDataLoader`` 会异步地预取下一
        个 batch 的 数据，因此它将会加快数据传输的速度，但是将会占用更多的内存或者显
        存。默认值是 ``True``。
    :param use_shared_memory: 是否使用共享内存。当 ``use_shared_memory=True``
        时，将采用共享内存来加快将数据放进进程队列。建议仅当计算机上的共享空间足够大
        时。（例如 Linux 上的 /dev/shm/ 空间足够大）共享内存仅在多进程模式（
        ``num_workers>0`` ）下生效。
    :param timeout: 从子进程的输出队列获取数据的超时值。
    :param worker_init_fn: init 函数，如果不设置为 None ,则将会在每个子进程初始化
        时调用该函数。
    :param persistent_workers: 如果其为 ``True``, ``PaddleDataLoader`` 在迭代完
        一次 dataset 后不会关闭所有进程。默认为 ``False``。
    """
    ...
    def __init__(self,
                 ...
                 collate_fn: Union[str, Callable, None] = 'auto',
                 num_workers: int = 0,
        ...
        # FastNLP Dataset, collate_fn not None
        if isinstance(dataset, FDataSet) and collate_fn is None:
            raise ValueError(
                'When use FastNLP DataSet, collate_fn must be not None')
        ...
        if isinstance(collate_fn, str):
            if collate_fn == 'auto':
                if isinstance(dataset.dataset, FDataSet):
                    ...
                    collate_fn.set_backend(backend='paddle')  # type: ignore
                else:
        ...
        dl_kwargs = _match_param(PaddleDataLoader.__init__,
                                 DataLoader.__init__, DataLoader.__name__)
        if dl_kwargs is None:
            super(PaddleDataLoader, self).__init__(
                dataset=dataset,
                feed_list=feed_list,
                places=places,
    ...
    def __iter__(self):
        # 如果没有auto_collator 也没有自定义collate_fn，那么此时采用dataloader自
        # 带的collate_fn，将数据打包即可。
    ...
    def set_pad(self,
                field_name: Union[str, tuple],
                pad_val: Union[int, float, None] = 0,
                dtype=None,
                backend=None,
        ...
        :param field_name: 需要调整的 field 的名称。如果 :meth:`Dataset.\
            ...
            **0** 个和第 **1** 个元素。如果该 field 在数据中没有找到，则报错；如果
            :meth:`Dataset.__getitem__` 返回的是就是整体内容，请使用 "_single" 。
        :param pad_val: 这个 field 的默认 pad 值。如果设置为 ``None``，则表示该
            field 不需要 pad , fastNLP 默认只会对可以 pad 的 field 进行 pad，所以
            如果对应 field 本身就不是可以 pad 的形式，可以不需要主动设置为
        ...
        if isinstance(collator, Collator):
            ...
            raise ValueError('Only when the collate_fn is a fastNLP Collator, '
                             'set_pad() is allowed.')
    ...
    def _get_collator(self):
        ...
        """
        collator = None
        if hasattr(self.collate_fn, '__wrapped__') and isinstance(
                self.collate_fn.__wrapped__, Collator):
    ...
    def get_batch_indices(self) -> List[int]:
        ...
        """
        return self.cur_batch_indices  # type: ignore[return-value]


def prepare_paddle_dataloader(ds_or_db, feed_list=None, places=None,
                              return_list: bool = True,
                              batch_sampler: Union[None, 'Sampler[Sequence[int]]', ReproducibleBatchSampler] = None,
    ...
    r"""将输入的单个或多个 dataset 同时转为 :class:`PaddleDataLoader` 对象，根据
    ``ds_or_db`` 的类型不同而有不同返回结果。

    :param ds_or_db: 可以有以下三种取值：

        * 当 ds_or_db 为 :class:`.DataSet` 等数据集类型时，
          ``prepare_paddle_dataloader`` 会将使用的除了 non_train_batch_size 和
          non_train_sampler 以外的参数来实例化一个 :class:`PaddleDataLoader` 对象
          并返回。详见 :class:`PaddleDataLoader`。
        * 当 ds_or_db 为 :class:`.DataBundle` 时，
          ``prepare_paddle_dataloader`` 会遍历 :class:`.DataBundle` 的数据集的
          key-value 来创建不同的 :class:`PaddleDataLoader` 对象；当 key 中包含
          ``'train'`` 字符串时，``prepare_Paddle_dataloader`` 默认该 value 为训
          练数据集，会将 ``batch_size`` 和 ``sampler`` 作为参数，其他 key 不包含
          ``'train'`` 字符串的数据集则使用 non_train_size 和 non_train_sampler 作
          为参数。最终根据 ``key: PaddleDataLoader`` 组成 ``Dict[key,
          PaddleDataLoader]`` 的字典返回。
        * 当 ds_or_db 为 ``Dict[str, DataSet]`` 字典类型时，
          ``prepare_paddle_dataloader`` 会遍历 该 dict 的的 key-value 来创建不同
          的 :class:`PaddleDataLoader` 对象；当 key 中包含 ``'train'`` 字符串时，
          ``prepare_paddle_dataloader`` 默认该 value 为训练数据集，会将
          ``batch_size`` 和 ``sampler`` 作为参数，其他 key 不包含 ``'train'`` 字
          符串的数据集则使用 ``non_train_size`` 和 ``non_train_sampler`` 作为参
          数。最终根据  ``key: PaddleDataLoader`` 组成 ``Dict[key,
          PaddleDataLoader]`` 的字典返回。
    ...
    :param return_list: 每个设备上的返回值是否为以列表形式显示。如果
        ...
        ``True``。
    :param batch_sampler: 批量取出数据的采样器，如 :class:`~fastNLP.core.\
        ReproducibleBatchSampler`、paddle 的 :class:`BatchSampler` 对象，以及所
        有实现了 :meth:`__len__` 和 :meth:`__iter__` 的对象。其 :meth:`__iter__`
        方法每次都会返回一个 List 对象，List 中的值为 dataset 的下标 index ；默认
        为 ``None``，当其不为 ``None`` 时，参数 ``bacth_size``、``sampler``、
        ``shuffle`` 均失效。
    ...
    :param drop_last: 当 ``drop_last=True`` 时，:class:`PaddleDataLoader` 会扔掉
    ...
    :param collate_fn: 用于从 dataset 取到的一个 batch 数据进行打包处理的
        ...
        * collate_fn 为 ``None`` 时，需要注意的是此时传进来的 dataset 类型不能为
          :class:`.DataSet`，此时 ``PaddleDataLoader`` 会调用默认的 paddle 框
          架的 ``DataLoader`` 自带的 ``collate_batch`` 作为 collate_fn 的默认值，
          不过需要注意的是其无法处理 :class:`.DataSet` 的 dataset 对象。
        * collate_fn 为 ``'auto'`` 时，``PaddleDataLoader`` 将会使用 fastNLP 的
          :class:`~fastNLP.core.Collator` 作为 collate_fn 的默认值。此时可以配套
          使用 :meth:`PaddleDataLoader.set_pad` 和 :meth:`PaddleDataLoader.\
    ...
    :param use_buffer_reader: 是否开启 buffer_reader 。如果为 ``True``，那么
        :class:`PaddleDataLoader` 会异步地预取下一个 batch 的数据，因此它将会加快
        数据传输的速度，但是将会占用更多的内存或者显存。默认值是 ``True``。
    :param use_shared_memory: 是否使用共享内存。当 ``use_shared_memory=True``
        时，将采用共享内存来加快将数据放进进程队列。建议仅当计算机上的共享空间足够大
        时。（例如 Linux 上的 /dev/shm/ 空间足够大）共享内存仅在多进程模式（
        ``num_workers>0`` ）下生效。
    ...
    :param worker_init_fn: init 函数，如果不设置为 None ,则将会在每个子进程初始化
        时调用该函数。
    :param persistent_workers: 如果其为 ``True``, :class:`PaddleDataLoader` 在迭
        代完一次 dataset 后不会关闭所有进程。默认为 ``False``
    """
    from fastNLP.io.data_bundle import DataBundle

    if isinstance(ds_or_db, DataBundle):
        assert batch_sampler is None, 'batch_sampler can only be None when ' \
                                      'multiple datasets are presented.'
        dl_bundle = {}
        for name, ds in ds_or_db.iter_datasets():
            if 'train' in name:
                dl_bundle[name] = PaddleDataLoader(
                    ds,
                    feed_list=feed_list,
                    places=places,
                    return_list=return_list,
                ...
                dl_bundle[name] = PaddleDataLoader(
                    ...
                    return_list=return_list,
                    batch_sampler=batch_sampler,
                    batch_size=non_train_batch_size
                    if non_train_batch_size else batch_size,
        ...
        raise ValueError(f'ds_or_db: {ds_or_db} must be fastnlp dataset or '
                         'data_bundle or mapping!')
