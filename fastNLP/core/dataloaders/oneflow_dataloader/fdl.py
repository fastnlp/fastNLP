...
if _NEED_IMPORT_ONEFLOW:
    from oneflow.utils.data import DataLoader, Sampler
else:
    from fastNLP.core.utils.dummy_class import DummyClass as DataLoader

__all__ = ['OneflowDataLoader', 'prepare_oneflow_dataloader']


class _FDataSet:
    r"""提供给 :class:`OneflowDataLoader` 使用的 warp 类，其功能是对 dataset 进行
...
class OneflowDataLoader(DataLoader):
    ...
    :param persistent_workers: 如果其为 ``True``, ``OneflowDataLoader`` 在迭代完
        ...
        if isinstance(collate_fn, str):
            if collate_fn == 'auto':
                if isinstance(dataset.dataset, DataSet):
                    ...
                    collate_fn.set_backend(backend='oneflow')  # type: ignore
                else:
    ...
    def set_pad(self,
        ...
        :param field_name: 需要调整的 field 的名称。如果 :meth:`Dataset.
            ...
            示序列中第 **0** 个和第 **1** 个元素。如果该 field 在数据中没有找到，则
            报错；如果 :meth:`Dataset.__getitem__` 返回的是整体内容，请使用
            "_single" 。
...
def prepare_oneflow_dataloader(ds_or_db, batch_size: int = 16,
                               shuffle: Optional[bool] = None,
                               sampler: Union['Sampler[int]', None, ReproducibleSampler, UnrepeatedSampler] = None,
                               batch_sampler: Union['Sampler[Sequence[int]]', None, ReproducibleBatchSampler] = None,
                               num_workers: int = 0,
                               collate_fn: Union[Callable, str, None] = 'auto',
                               pin_memory: bool = False,
    ...
    返回结果，具体如下:
        ...
        * 当 ds_or_db 为 :class:`~fastNLP.io.DataBundle` 时，
          ...
          ``non_train_sampler`` 作为参数。最终根据 ``key: OneflowDataLoader`` 组
          成 ``Dict[key, OneflowDataLoader]`` 的字典返回。
        * 当 ds_or_db 为 ``Dict[str, DataSet]`` 字典类型时，
          ``prepare_oneflow_dataloader`` 会遍历 该 dict 的的 key-value 来创建不
          同的 :class:`OneflowDataLoader` 对象；当 key 中包含 ``'train'`` 字符串
          时，``prepare_oneflow_dataloader`` 默认该 value 为训练数据集，会将
    ...
    :param generator: 如果其不为 ``None``, 将会使用 RandomSampler 去生成随机的
        index 且会为每个子进程生成一个 ``base_seed``
    :param prefetch_factor: 每个 worker 提前装载的 samples 数量。``2`` 意味着在
        所有的进程中会有 2*num_workers 的数据被预取。默认值为 ``2``。
    :param persistent_workers: 如果其为 ``True``，:class:`OneflowDataLoader` 在
        迭代完一次 dataset 后不会关闭所有进程。默认为 ``False``
    """
    ...
    if isinstance(ds_or_db, DataBundle):
        ...
        assert batch_sampler is None, 'batch_sampler can only be None ' \
                                      'when multiple datasets are presented.'
        dl_bundle = {}
        for name, ds in ds_or_db.iter_datasets():
            if 'train' in name:
                dl_bundle[name] = OneflowDataLoader(
                    dataset=ds,
                    batch_size=batch_size,
