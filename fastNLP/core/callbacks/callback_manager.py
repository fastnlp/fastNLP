...
from .progress_callback import (ExtraInfoStatistics, ProgressCallback,
...
def _transfer(func):
    ...
    def wrapper(manager, *arg, **kwargs):
        manager.callback_counter[
            func.__name__] += 1  # 给实际被调用的 callback_fn 的计数加 1；
        for callback_fn in manager.callback_fns[func.__name__]:
            try:
                ...
                logger.error('The following callback_fn raise exception: '
                             f'{_get_fun_msg(callback_fn)}.')
                raise e
...
def prepare_callbacks(callbacks, progress_bar: str,
                      extra_show_keys: Union[str, Sequence[str],
                                             ExtraInfoStatistics, None]):
    """
    :param callbacks: 对用户传入的类 ``callback`` 进行检查，查看是否是否继承了我们
        的 ``Callback`` 类；
    ...
    if callbacks is not None:
        ...
        if not isinstance(callbacks, Sequence):
            raise ValueError(
                "Parameter `callbacks` should be type 'List' or 'Tuple'.")
        callbacks = list(callbacks)
        for _callback in callbacks:
            if not isinstance(_callback, Callback):
                raise TypeError('callbacks must be of Callback type, '
                                f'instead of `{type(_callback)}`')
    ...
    if has_no_progress and progress_bar is not None:
        ...
        if callback is not None:
            # 放在最前面，方便分割不同 epoch
            _callbacks = [callback] + _callbacks
            has_no_progress = False
    elif has_no_progress is False and progress_bar not in ('auto', None):
        logger.rank_zero_warning(
            'Since you have passed in ProgressCallback, '
            f'progress_bar={progress_bar} will be ignored.')

    if has_no_progress:
        logger.rank_zero_warning(
            'No progress bar is provided, there will have no progress output '
            'during training.')
...
class CallbackManager:
    ...
    class_callbacks: List[Callback]  # 用来保留原始的类callback；
    callback_fns: dict
    ...
    def __init__(self, callbacks: List[Callback]):
        ...
        :param callbacks: 初始化时可以传入的一系列 :class:`~fastNLP.Callback`
            类，通常为用户在初始化 ``Trainer`` 时直接传入的 callback 列表；
        """
        self._need_reproducible_sampler = False

        self.callback_fns = defaultdict(list)
        # 因为理论上用户最多只能通过 'trainer.on_train_begin'
        # 或者 'trainer.callback_manager.on_train_begin' 来调用，即其无法直接调用
        # 具体的某一个 callback 函数，而不调用其余的同名的 callback 函数
        # 因此我们只需要记录具体 Event 的时机即可；
        self.callback_counter: dict = defaultdict(lambda: 0)
        if len(callbacks):
            # 这一对象是为了保存原始的类 callback 对象来帮助用户进行 debug
            # 理论上在正常的使用中你并不会需要它；
            self.class_callbacks = callbacks
        else:
            self.class_callbacks: List[Callback] = []

        # 预跑需要拿到每一个被 `Filter` 修饰的函数的 `Filter` 实例，
        # 从而在预跑结束后重置它们的内部状态；
        self._callback_filters: List[Tuple[str, str, Filter]] = [
        ]  # [(callback_name, fn_name, filter 实例), ]

        # 保留所有 callback 的引用，用于断点重训；包括全部的三种callback：
        # 函数修饰器 callback；类 callback；纯函数 callback；
        # 因为所有的 callback 都是通过函数 `self.add_one_callback` 添加，因此我们
        # 选择在其下进行添加；
        # 一个比较重要的概念在于在训练过程运行的时候，两个 callback 的
        # callback_name 可以是一样的，并且理论上不会造成任何影响；但是当
        # `on_load_checkpoint` 时，我们需要处理两个 callback_name 一样的情况
        # 因此这里的 `all_callbacks` 为了避免正常训练过程的运行，只能是一个 List，
        # 而不能是一个 dict，`_callback_filters` 也是一样；
        self.all_callbacks = []

    def initialize_class_callbacks(self):
        r"""
        在实际的运行过程中，我们会将具体的一个 callback 实例拆分为单独的一个个
        callback 函数，然后将它们加在一个字典里，该字典的键值就是一个个 callback 时
        机，也就是 `Event` 的类别；
        如果一个 callback 类的 callback 函数并不具备任何作用，我们实际并不会将其加在
        字典当中；
        """
        for each_callback in self.class_callbacks:
            self._need_reproducible_sampler |= \
                each_callback.need_reproducible_sampler
    ...
    def dissect_one_callback(self, callback: Callback):
        ...
        for name, member in Event.__dict__.items():
            if isinstance(member, staticmethod):
                ...
                if inspect.getsource(_fn) != inspect.getsource(
                        getattr(Callback, name)):
                    self.callback_fns[name].append(_fn)
                    self.extract_callback_filter_state(callback.callback_name,
                                                       _fn)
    ...
    def extract_callback_filter_state(self, callback_name, callback_fn):
        ...
        if hasattr(callback_fn, '__fastNLP_filter__'):
            # 注意我们的 `Filter` 使用了 `@wraps` 来保证被修饰的函数的 `__name__`
            # 属性仍旧是其真实的名字；
            self._callback_filters.append((callback_name, callback_fn.__name__,
                                           callback_fn.__fastNLP_filter__))
    ...
    def on_save_checkpoint(self, trainer) -> Dict:
        ...
        .. code-block::

            {
                "callback_name_1": {
                    "states": {...},
                    "filter_states": {
                        "on_train_begin": filter1.state_dict(),
                        ...
                    }
                }
            }
        ...
        for each_callback in self.all_callbacks:
            if each_callback.callback_name in states:
                ...
                states[each_callback.callback_name][
                    'states'] = each_callback.on_save_checkpoint(trainer)
        ...
        for each_callback_filters in self._callback_filters:
            if each_callback_filters[
                    0] not in _record_duplicated_callback_names:
                ...
                states[each_callback_filters[0]]['filter_states'][
                    each_callback_filters[1]] = each_callback_filters[
                        2].state_dict()

        # 3. 保存 callback_counter；
        # callback_counter 不应当被保存，因为其在断点重训时会由新的
        # callback_manager 重新初始化；
        # 对于断点重训，我们不会保存 Trainer 的所有参数，例如 batch_step_fn；如果在
        # 断点重训时重新初始化 Trainer 发现 batch_step_fn
        # 不为 None，那么 Trainer 就会调用实际的 check_batch_step_fn 函数，从而需
        # 要 callback_counter 为全新的状态；
    ...
    def on_load_checkpoint(self, trainer, states: Dict):
        ...
        # self._callback_filters 是当前的 Trainer 的 callback 的 filter 状态
        # 是我们要去维护的对象；
        ...
        for each_callback_filters in self._callback_filters:
            if each_callback_filters[0] in states:
                if each_callback_filters[
                        0] not in _already_loaded_callback_names:
                    _already_loaded_callback_names.add(
                        each_callback_filters[0])
                    if 'filter_states' in states[each_callback_filters[0]] \
                            and each_callback_filters[1] in states[each_callback_filters[0]]['filter_states']:
                        each_callback_filters[2].load_state_dict(
                            states[each_callback_filters[0]]['filter_states'][
                                each_callback_filters[1]])
                else:
                    _duplicated_callback_names.add(each_callback_filters[0])

        if len(_duplicated_callback_names) > 0:
            logger.rank_zero_warning(
                f'Notice these callback_name: {_duplicated_callback_names} '
                'are duplicated, fastNLP will only load the first callback\'s '
                'state.')
        ...
        _already_loaded_callback_names = set()
        for each_callback in self.all_callbacks:
            if each_callback.callback_name in states and \
                    each_callback.callback_name not in _already_loaded_callback_names:
                ...
                each_callback.on_load_checkpoint(
                    trainer, states[each_callback.callback_name]['states'])
