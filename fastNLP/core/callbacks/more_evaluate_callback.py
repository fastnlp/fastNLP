...
class MoreEvaluateCallback(HasMonitorCallback):
    ...
    :param evaluate_every: 用来控制 ``Trainer`` 内部的 ``Evaluator`` 验证的频率，
        ...
        3. 为函数时表示用户自己传入的用于控制 evaluate 的频率的函数，该函数的应该接
           受当前 trainer 对象作为参数，并返回一个 bool 值，返回为 ``True`` 说明需
           要进行 evaluate ；将在每个 ``batch`` 结束后调用该函数判断是否需要
           evaluate；
    ...
    :param watch_monitor: 这个值用来表示监控的 Trainer 中 evaluate 的结果，当该值
        ...
        1. ``str`` 类型，含义为监控的 metric 值。如果在 evaluation 结果中没有找到
    ...
    :param num_eval_sanity_batch: 在初始化 Evaluator 后运行多少个 sanity check
        的 batch ，检测一下。
    :param topk: 如果需要根据当前 callback 中的 evaluate 结果保存模型或 Trainer ，
        可以通过设置 topk 实现：

        1. 为 ``-1`` 表示每次 evaluate 后都保存；
        2. 为 ``0`` （默认），表示不保存；
        3. 为整数，表示保存性能最好的 ``topk`` 个。
    :param topk_monitor: 如果需要根据当前 callback 中的 evaluate 结果保存。这个参
        数是指在当前 callback 中的 evaluate 结果寻找
    :param topk_larger_better: ``topk_monitor`` 的值是否是越大越好。
    :param folder: 保存的文件夹，fastNLP 将在该文件下以时间戳创建子文件夹，并在里面
        保存。因此不同次运行可以将被保存到不同的时间戳文件夹中。如果为 ``None``，默
        认使用当前文件夹。
    :param only_state_dict: 保存模型是否只保存 state_dict 。当``model_save_fn``
        不为 ``None`` 时，该参数无效。
    ...
    def __init__(self,
                 ...
                 watch_monitor: Union[str, Callable, None] = None,
                 watch_monitor_larger_better: bool = True,
                 evaluate_fn=None,
                 num_eval_sanity_batch=2,
                 topk=0,
                 topk_monitor=None,
                 topk_larger_better=True,
                 folder=None,
                 ...
                 save_kwargs=None,
                 **kwargs):
        super(MoreEvaluateCallback, self).__init__(
            watch_monitor,
            watch_monitor_larger_better,
            must_have_monitor=False)
        if watch_monitor is not None and evaluate_every == -1:
            # 将evaluate_every 弄掉。
            evaluate_every = None
        if watch_monitor is None and evaluate_every is None:
            raise RuntimeError(
                '`evaluate_every` and `watch_monitor` cannot be None at the '
                'same time.')
        if watch_monitor is not None and evaluate_every is not None:
            raise RuntimeError(
                f'`evaluate_every`({evaluate_every}) and `watch_monitor` '
                f'({watch_monitor}) cannot be set at the same time.')
        ...
        self.topk_saver = TopkSaver(
            topk=topk,
            monitor=topk_monitor,
            larger_better=topk_larger_better,
            folder=folder,
            only_state_dict=only_state_dict,
            model_save_fn=model_save_fn,
            save_evaluate_results=save_evaluate_results,
    ...
    def on_after_trainer_initialized(self, trainer, driver):
        ...
        if self.monitor is not None:
            assert trainer.evaluator is not None, \
                f'You set `watch_monitor={self.monitor}`, but no ' \
                'evaluate_dataloaders is provided in Trainer.'

        # 初始化 evaluator , 同时避免调用 super 对 monitor 赋值
        kwargs = {
            'model':
            self.kwargs.get('model', trainer.model),
            'dataloaders':
            self.dataloaders,
            'metrics':
            self.metrics,
            'driver':
            self.kwargs.get('driver', trainer.driver),
            'device':
            self.kwargs.get('device', trainer.device),
            'evaluate_batch_step_fn':
            self.kwargs.get('evaluate_batch_step_fn',
                            trainer.evaluate_batch_step_fn),
            'evaluate_fn':
            self.evaluate_fn,
            'input_mapping':
            self.kwargs.get('input_mapping', trainer.input_mapping),
            'output_mapping':
            self.kwargs.get('output_mapping', trainer.output_mapping),
            'fp16':
            self.kwargs.get('fp16', trainer.fp16),
            'use_dist_sampler':
            self.kwargs.get('use_dist_sampler',
                            trainer.kwargs.get('eval_use_dist_sampler', None)),
            'progress_bar':
            self.kwargs.get('progress_bar',
                            trainer.kwargs.get('progress_bar', 'auto')),
            'verbose':
            self.kwargs.get('verbose', 0)
        }
        ...
        if self.num_eval_sanity_batch > 0:
            ...
            results = self.evaluator.run(
                num_eval_batch_per_dl=self.num_eval_sanity_batch)
    ...
    def on_train_batch_end(self, trainer):
        ...
        if callable(self.evaluate_every):
            if self.evaluate_every(trainer):
                ...
                trainer.global_forward_batches % self.evaluate_every == 0:
            self.run_evaluation(trainer, self.evaluator, self.topk_saver)
    ...
    def on_load_checkpoint(  # type: ignore[override]
            self, trainer, states: Dict):
    ...
    def callback_name(self):
        ...
        return f'more_evaluate_callback#metric_name-{metric_names}' \
               f'#monitor-{self.monitor_name}#topk_saver:{self.topk_saver}'
