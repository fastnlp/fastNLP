...
from fastNLP.core.controllers import Trainer
...
class CheckpointCallback(Callback):
    ...
    如果 ``model_save_fn`` 为 ``None``，则 fastNLP 会在以上每个文件夹中，都创建一
    个 fastnlp_model.pkl.tar 文件。若 ``model_save_fn`` 不为 ``None``，则
    fastNLP 将 folder 的绝对路径传递给该函数，fastNLP 在该 folder 下不进行模型保
    存。默认情况下，本 checkpoint 只保存了 model 的状态；如还需保存 Trainer 的状态
    以断点重训的话，请使用 ``save_object='trainer'``。
    ...
    :param folder: 保存的文件夹，fastNLP 将在该文件下以时间戳创建子文件夹，并在里面
        保存。因此不同次运行可以将被保存到不同的时间戳文件夹中。如果为 None ，默认使
        用当前文件夹。
    ...
    def __init__(
            ...
            last: bool = False,
            topk: int = 0,
            on_exceptions: Optional[Union[
                BaseException, Sequence[BaseException]]] = (
                    EarlyStopException  # type: ignore
                ),
        ...
        if on_exceptions is not None:
            ...
            for exception in on_exceptions:
                if not issubclass(exception, BaseException):  # type: ignore
                    raise TypeError(
                        'Each exception in parameter `on_exception` can only '
                        'be `BaseException` type.')
        else:
            on_exceptions = []
    ...
    def on_after_trainer_initialized(self, trainer, driver):
        if self.topk_saver.topk_queue:  # 需要设置 monitor
            if self.topk_saver.monitor is None:
                self.topk_saver.set_monitor(
                    monitor=trainer.monitor,
                    larger_better=trainer.larger_better)
        if self.topk_saver.topk_queue and trainer.evaluator is None:
            logger.warning(
                f'You set `topk={self.topk}`, but `evaluate_dataloaders` is '
                'not set in Trainer.')
