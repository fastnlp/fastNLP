...
from fastNLP.envs.imports import _NEED_IMPORT_FAIRSCALE, _NEED_IMPORT_TORCH
...
class TorchGradClipCallback(Callback):
    ...
    :param clip_value: 将梯度限制到 [-clip_value, clip_value] 之间。
        ``clip_value`` 应该为正数；
    ...
    def __init__(self,
        ...
        if parameters is not None:
            ...
            self.parameters = None  # type: ignore
        self.clip_value = clip_value
        self.clip_type = clip_type

    def on_after_trainer_initialized(self, trainer, driver):
        assert isinstance(driver, TorchDriver), \
            f'Callback:{self.__class__.__name__} only supports torch ' \
            'related drivers for now.'
        parameters = []
        ...
        if isinstance(trainer.driver, FairScaleDriver):
            if isinstance(
                    trainer.driver.model,
                    FullyShardedDataParallel) and self.clip_type == 'norm':
                self.clip_fun = trainer.driver.model.clip_grad_norm_

        assert len(
            self.parameters), 'There is no parameters need to be clipped.'
