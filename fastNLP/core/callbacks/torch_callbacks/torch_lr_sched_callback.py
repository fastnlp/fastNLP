...
class TorchWarmupCallback(Callback):
    ...
    :param schedule: 对学习率进行调整的策略：
        ...
        1. *linear* -- 前 ``warmup`` 的 step 上升到指定的学习率（从 Trainer 中
           optimizer 处获取）, 在剩下的 step 中下降到 0；
        2. *constant* -- 前 ``warmup`` 的 step 上升到指定的学习率，余下的 step
           保持不变。
    """

    def __init__(self,
                 warmup: Union[int, float] = 0.1,
                 schedule: str = 'linear'):
        super().__init__()
        self.warmup = max(warmup, 0.)

        self.initial_lrs: List[float] = []  # 存放param_group的learning rate
    ...
    def on_train_begin(self, trainer):
        ...
        self.t_steps = math.ceil(
            self.t_steps /
            trainer.accumulation_steps) * trainer.accumulation_steps
    ...
    def on_before_optimizers_step(self, trainer, optimizers):
        ...
        progress = (trainer.global_forward_batches +
                    trainer.accumulation_steps) / self.t_steps
        for optimizer in trainer.driver.optimizers:
