...
def check_legality(fn):
    ...
    def wrap(every=None, once=None, filter_fn=None):
        ...
        if not ((every is not None) ^ (once is not None) ^
...
class Event:
    ...
    :param once: 只在第多少次触发才真正运行一次。即第 ``once`` 次调用时才运行相应的
        函数，无论之前还是之后均不会运行；
    :param filter_fn: 输入参数的应该为 ``(filter, trainer)``，其中 ``filter`` 对
        象中包含了 `filter.num_called` 和 `filter.num_executed` 两个变量来分别获
        取当前被调用了多少次，真正执行了多少次；``trainer`` 对象即为当前正在运行的
        Trainer；
    """
    every: Optional[int]
    once: Optional[int]
    ...
    def __str__(self):
        return '<event={0}, every={1}, once={2}, filter fn is:{3}>'.format(
            self.value, self.every, self.once, self.filter_fn)
    ...
    def on_after_trainer_initialized(every=None, once=None, filter_fn=None):
        ...
        """
        return Event(
            value='on_after_trainer_initialized',
            every=every,
            once=once,
            filter_fn=filter_fn)
    ...
    def on_train_end(every=None, once=None, filter_fn=None):
        ...
        """
        return Event(
            value='on_train_end', every=every, once=once, filter_fn=filter_fn)
    ...
    def on_exception(every=None, once=None, filter_fn=None):
        ...
        """
        return Event(
            value='on_exception', every=every, once=once, filter_fn=filter_fn)
    ...
    def on_save_model(every=None, once=None, filter_fn=None):
        ...
        """
        return Event(
            value='on_save_model', every=every, once=once, filter_fn=filter_fn)
    ...
    def on_load_model(every=None, once=None, filter_fn=None):
        ...
        """
        return Event(
            value='on_load_model', every=every, once=once, filter_fn=filter_fn)
    ...
    def on_before_optimizers_step(every=None, once=None, filter_fn=None):
        ...
        """
        return Event(
            value='on_before_optimizers_step',
            every=every,
            once=once,
            filter_fn=filter_fn)
    ...
    def on_after_optimizers_step(every=None, once=None, filter_fn=None):
        ...
        """
        return Event(
            value='on_after_optimizers_step',
            every=every,
            once=once,
            filter_fn=filter_fn)
...
class Filter:
    ...
    :param once: 只在第多少次触发才真正运行一次。即第 ``once`` 次调用时才运行相应的
        函数，无论之前还是之后均不会运行；
    :param filter_fn: 用户定制的频率控制函数；注意该函数内部的频率判断应当是无状态
        的，除了参数 `self.num_called` 和 `self.num_executed` 外，因为我们会在预跑
        后重置这两个参数的状态；
    """
    ...
    def __init__(self,
        ...
        if every is not None:
            ...
            self._filter = filter_fn  # type: ignore
    ...
    def __call__(self, fn: Callable):
        ...
        def wrapper(*args, **kwargs) -> Optional[Callable]:
            ...
            # 因为我们的 callback 函数的输入是固定的，而且我们能够保证第一个参数一定
            # 是 trainer；
            trainer = args[0]
        ...
        wrapper.__fastNLP_filter__ = self  # type: ignore
        return wrapper
