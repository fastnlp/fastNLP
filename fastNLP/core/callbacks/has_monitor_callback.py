...
__all__ = ['HasMonitorCallback', 'ExecuteOnceBetterMonitor', 'ResultsMonitor']
...
class HasMonitorCallback(ResultsMonitor, Callback):
    ...
    def on_after_trainer_initialized(self, trainer, driver):
        ...
        if self.monitor is None and trainer.monitor is not None:
            self.set_monitor(
                monitor=trainer.monitor, larger_better=trainer.larger_better)
        ...
        # 用户可能会在自定义 Callback 中自行 evaluate 结果并且不使用 Evaluator，
        # 此时该限制会变得不合理，暂时注释掉
        if self.must_have_monitor and self.monitor is not None and \
                trainer.evaluator is None:
            raise RuntimeError(
                '"No `evaluate_dataloaders` is set for Trainer. '
                f'But Callback: {self.log_name} need to watch the '
...
class ExecuteOnceBetterMonitor(HasMonitorCallback):
    ...
    :param execute_fn: 一个可执行的函数，不接受任何参数，没有返回值。在 monitor 取
        得更好结果的时候会调用；
    """
    ...
    def __init__(self, monitor, larger_better, execute_fn):
        ...
        _check_valid_parameters_number(
            execute_fn, expected_params=[], fn_name='execute_fn')
