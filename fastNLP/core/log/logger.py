r"""
:class:`Logger` 是 **fastNLP** 中记录日志的模块，**logger** 封装了 logging 模块
的 Logger，具体使用方式与直接使用 :class:`logging.Logger` 相同，同时也新增一些简单
好用的API
...
import sys
import warnings
from logging import CRITICAL, DEBUG, ERROR, INFO, WARNING, raiseExceptions
...
from fastNLP.envs.env import (FASTNLP_BACKEND_LAUNCH, FASTNLP_GLOBAL_RANK,
                              FASTNLP_LAUNCH_TIME, FASTNLP_LOG_LEVEL)
...
class LoggerSingleton(type):
    ...
    def __call__(cls, *args, **kwargs):
        if cls not in cls._instances:
            cls._instances[cls] = super(LoggerSingleton,
...
class FastNLPLogger(logging.Logger, metaclass=LoggerSingleton):
    ...
    def add_file(self,
        ...
        :param path: 若 path 为文件路径（通过 path 是否包含后缀判定 path 是否表示
            文件名，例如 output.log 会被认为是文件，而output 则认为是文件夹）则直接
            写入到给定文件中；如果判定为文件夹，则是在该文件夹下以 时间戳 创建一个日
            志文件。
        :param level: 可选 ['INFO', 'WARNING', 'DEBUG', 'ERROR', 'AUTO'], 其中
            AUTO表示根据环境变量"FASTNLP_LOG_LEVEL'进行设置。
        :param remove_other_handlers: 是否移除其它 handler ，如果移除，则terminal
            中将不会有 log 输出。
        :param mode: 可选为['w', 'a']，如果传入的 path 是存在的文件，'w' 会覆盖原
            有内容 'a' 则会在文件结尾处继续添加。
        :return:
        """
        r"""添加日志输出文件和输出级别"""
        if level == 'AUTO':
            level = parse_level()
        return _add_file_handler(self, path, level, remove_other_handlers,
                                 mode)
    ...
    def set_stdout(self, stdout: str = 'raw', level: str = 'AUTO', **kwargs):
        ...
        :param kwargs: 支持以下的参数
            * rich_show_path (*bool*) -- 当 stdout 为 rich 时，是否在打印信息的
              时候附上当前打印来自于代码哪个位置。
        :return:
        """
        r"""设置标准输出格式和输出级别"""
    ...
    def log(self, level, msg, *args, **kwargs):
        """Delegate a log call to the underlying log, after adding contextual
        information from this adapter instance."""
        ...
        for handler in self.handlers:
            if isinstance(handler, logging.FileHandler):
                formatter = logging.Formatter(
                    fmt='Rank: %(rank)s - %(asctime)s - %(module)s - '
...
def _add_file_handler(_logger: logging.Logger,
                      path: Optional[Union[str, Path]] = None,
                      level: str = 'INFO',
                      remove_other_handlers: bool = False,
                      mode: str = 'w'):
    ...
    if not isinstance(path, Path):
        raise TypeError(
            'Parameter `path` can only be `str` or `pathlib.Path` type.')
    if not path.exists():
        head, tail = os.path.splitext(path)
        if tail == '':  # 说明没有后缀，理解为是一个folder
            path.mkdir(parents=True, exist_ok=True)
        else:
            # 主进程会帮助我们创建文件夹，但是由于主从进程几乎是同步的，因此到这里时
            # 子进程也会尝试创建文件夹，即使主进程会做这件事情；
            dirname = os.path.dirname(path)
            if dirname:
                os.makedirs(dirname, exist_ok=True)
    if path.is_dir():
        if os.environ.get(FASTNLP_BACKEND_LAUNCH, '0') == '1':
            # 如果是通过 python -m xxx.launch 等启动的话，FASTNLP_LAUNCH_TIME这个
            # 名称可能是不一致的。
            path = path.joinpath(
                f"RANK-{os.environ.get(FASTNLP_GLOBAL_RANK, '0')}-" +
                os.environ[FASTNLP_LAUNCH_TIME] + '.log')
        else:
            path = path.joinpath(os.environ[FASTNLP_LAUNCH_TIME] + '.log')

    if not isinstance(remove_other_handlers, bool):
        raise TypeError(
            'Parameter `remove_other_handlers` can only be `bool` type.')

    if not isinstance(mode, str):
        raise TypeError("Parameter 'evaluate_fn' can only be `str` type.")
    if mode not in {'w', 'a'}:
        raise ValueError('Parameter `evaluate_fn` can only be one of '
                         "these values: ('w', 'a').")
    ...
    # 这里只要检测到是分布式训练，我们就将 evaluate_fn 改为 "a"；这样会导致的一个问
    # 题在于，如果第二次训练也是分布式训练，logger记录的log不会重新覆盖掉原文件，而是
    # 会接着上一次的 log 继续添加；
    # 这样做主要是为了解决这样的情形所导致的问题：在分布式训练中，进程 1 比 进程 0 先
    # 运行到这里，然后使得进程 0 将进程 1 的 log 覆盖掉；
    if is_cur_env_distributed(
    ):  # and int(os.environ.get(FASTNLP_GLOBAL_RANK, 0)) != 0:
        mode = 'a'
    ...
    if is_cur_env_distributed():
        file_formatter = logging.Formatter(
            fmt='Rank: %(rank)s - %(asctime)s - %(module)s - [%(levelname)s] '
            '- %(message)s',
            datefmt='%Y/%m/%d %H:%M:%S')
    else:
        file_formatter = logging.Formatter(
...
def _set_stdout_handler(_logger, stdout='raw', level='INFO', **kwargs):
    ...
    stream_handler = None
    _handlers = (logging.StreamHandler, TqdmLoggingHandler,
                 StdoutStreamHandler, RichHandler)
    for i, h in enumerate(_logger.handlers):
    ...
    if stdout == 'raw':
        ...
        stream_handler = RichHandler(
            level=level,
            log_time_format='[%X]',
            show_path=kwargs.get('rich_show_path', True))
    elif stdout == 'naive':
...
def parse_level():
    if os.environ[FASTNLP_LOG_LEVEL] == 'AUTO':
        level = 'WARNING' if int(os.environ.get(FASTNLP_GLOBAL_RANK,
                                                0)) != 0 else 'INFO'
    else:
