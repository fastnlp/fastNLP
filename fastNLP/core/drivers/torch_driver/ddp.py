...
1. 用户自己不进行 ddp 的任何操作，直接使用我们的 Trainer，这时是由我们自己使用
   `open_subprocesses` 拉起多个进程，然后 `TorchDDPDriver` 自己通过调用 `dist.
   init_process_group` 来初始化 ddp 的通信组；（情况 A）
2. 用户同样不在 Trainer 之外初始化 ddp，但是用户自己使用 python -m torch.
   distributed.launch 拉起来创建多个进程，这时我们仍旧会通过调用 `dist.
   init_process_group` 来初始化 ddp 的通信组；（情况 B）
3. 用户自己在外面初始化 DDP，并且通过 python -m torch.distributed.launch 拉起，这
   时无论是多个进程的拉起和 ddp 的通信组的建立都由用户自己操作，我们只会在 driver.
   setup 的时候对 `TorchDDPDriver` 设置一些必要的属性值；（情况 C）

注意多机的启动强制要求用户在每一台机器上使用 python -m torch.distributed.launch 启
动；因此我们不会在 `TorchDDPDriver` 中保存任何当前有多少台机器的信息（num_nodes，不
是 gpu 的数量）；

Part 1：三种启动方式的具体分析：
（1）对于用户运行的脚本中，如果 `driver.setup` 只会被调用一次（意味着用户的启动脚本中
只初始化了一个 trainer/evaluator）时，`TorchDDPDriver` 在初始化以及 `setup` 函数中
会做的事情分别如下所示：

    -> 情况 A：这种情况下用户传入的 model 在一定是普通的 model（没有经
     `DistributedDataParallel` 包裹的model），因为 `DistributedDataParallel` 的
     使用一定要求 init_process_group 已经被调用用来建立当前的 ddp 通信组；但是这意
     味着如果用户需要使用 2 张以上的显卡，那么其必然需要使用 torch.distributed.
     launch 来启动，意味着就不是情况 A 了；

     这时我们首先会调用 `TorchDDPDriver.open_subprocess` 函数来拉起多个进程，其中
     进程的数量等于用户传入给 trainer 的使用的 gpu 的数量（例如 `Trainer` 中的参数
     是 device=[0, 1, 6, 7]，那么我们就会使用第 0、1、6、7 张 gpu 来拉起 4 个进
     程）；

     接着我们会调用 `dist.init_process_group` 来初始化各个进程之间的通信组；
     这里需要注意拉起的新的进程会从前到后完整地运行一遍用户的启动脚本（例如 main.
     py），因此也都会运行这两个函数，但是需要注意只有进程 0 才会去真正地运行
     `TorchDDPDriver.open_subprocess`；进程 0 运行到 `dist.init_process_group`，
     pytorch 会阻塞进程 0 继续向前运行，直到其它进程也运行到这里；

     最后我们会设置这个进程对应的 device，然后将模型迁移到对应的机器上，再使用
     `DistributedDataParallel` 将模型包裹；至此，ddp 的环境配置过程全部完成；

    -> 情况 B：注意这种情况我们直接限定了用户是通过 torch.distributed.launch 拉起，
     并且没有自己建立 ddp 的通信组。这时在`TorchDDPDriver` 的初始化和 setup 函数的
     调用过程中，与情况 A 首要的不同就在于用户在 trainer 中输入的参数 device 不再有
     效，这时每个进程所使用的 gpu 是我们直接通过 torch.device("cuda:{local_rank}")
     来配置的；因此，如果用户想要实现使用特定 gpu 设备的目的，可以通过自己设置环境变
     量实现（例如 os.environ["CUDA_VISIBLE_DEVICE"] 来实现）；剩下的操作和情况 A
     类似；

    -> 情况 C：注意这种情况我们限定了用户是通过 torch.distributed.launch 拉起，并
      且 ddp 的通信组也是由自己建立。这时基本上所有的与操作相关的操作都应当由用户自己
      完成，包括迁移模型到对应 gpu 上以及将模型用 `DistributedDataParallel` 包裹
      等。

（2）如果 `driver.setup` 函数在脚本中会被调用两次及以上（意味着用户的启动脚本初始化了
两个及以上的 trainer/evaluator）时：

注意这种情况下我们是会保证前后两个 trainer/evaluator 使用的 `TorchDDPDriver` 以及其
初始化方式的一致性，换句话说，如果 trainer1 检测到的启动方式是 '情况 A'，那么我们会保
证 trainer2 检测到的启动方式同样是 '情况A'（即使这需要一些额外的处理）；因此这里我们
主要讨论我们是通过怎样的操作来保证 trainer2/3/... 检测到的启动方式是和 trainer1 一致
的；简单来说，我们是通过使用环境变量来标记每一种不同的启动方式来实现这一点的：

我们会用 `FASTNLP_DISTRIBUTED_CHECK` 来标记 '情况 A'，使用
`fastnlp_torch_launch_not_ddp` 来标记 '情况 B'，意味着我们在使用 '情况 A' 来启动
`TorchDDPDriver` 时，我们会将 `FASTNLP_DISTRIBUTED_CHECK` 这一字符串注入到环境变量
中，而 '情况 B' 时则会将 `fastnlp_torch_launch_not_ddp` 这一字符串注入到环境变量
中。因此在 trainer2 的 `TorchDDPDriver` 的初始化和 setup 过程中，如果检测到这些特殊
的环境变量，我们就会将启动方式变更为其对应的启动方式，即使其它的参数特征属于另外的启动
方式。

Part 2：对应的代码细节：
    1. 如何判断当前的各进程之间的通信组已经被建立（ddp 已经被初始化）；
        dist.is_initialized()；
    2. 如何判断不同的进程是否是由 `python -m torch.distributed.launch` 拉起还是由
       我们的 `TorchDDPDriver.open_subprocess` 函数拉起；

       我们会在用户脚本 `import fastNLP` 的时候检测当前的环境变量中是否有
       'LOCAL_RANK'、'WORLD_SIZE' 以及没有 `FASTNLP_DISTRIBUTED_CHECK`，如果满足
       条件，则我们会向环境变量中注入特殊的值 'FASTNLP_BACKEND_LAUNCH' 来标记用户是
       否使用了 `python -m torch.distributed.launch` 来拉起多个进程；
    3. 整体的处理判断流程：
   ...
   |  拉起，或者由我们自己定义的  |  --------------------->
   |  open_subprocess 函数拉起   |                      |
    —————————————————————————————                      |
                ↓ 由 torch.distributed.launch 拉起      | 我们自己的
                                                       | open_subprocess
                                                       | 函数拉起多个进程
             _________________                         |
     ←←←←←  |   检测用户是否   |                        |
...
Part 3：其它的处理细节：
    1. 环境变量；
       fastNLP 的 `TorchDDPDriver` 运行时所需要的环境变量分为两种，一种是 torch
       的 ddp 运行所需要的环境变量；另一种是 fastNLP 自己的环境变量。前者的配置情况
       如上表所示；而后者中的大多数环境变量则是在用户 import fastNLP 时就设置好了；
    2. parallel_device, model_device 和 data_device 的关系；
       parallel_device 为 `TorchDDPDriver` 的参数，model_device 和 data_device
       都为 driver 的属性；
    其中 data_device 仅当情况 C 时由用户自己指定；如果其不为 None，那么在模型
    forward 的时候，我们就会将数据迁移到 data_device 上；model_device 永远都为单独
    的一个 torch.device；
  ...
  ————————————————————————————————————————————————————————————————————————————————————————————————————————
    ...
    3. _DDPWrappingModel 的作用；
       因为我们即需要调用模型的 `train_step`、`evaluate_step`、`test_step` 方法，
       又需要通过 `DistributedDataParallel` 的 forward 函数来帮助我们同步各个设备
       上的梯度，因此我们需要先将模型单独包裹一层，然后在 forward 的时候，其先经过
       `DistributedDataParallel` 的 forward 方法，然后再经过
       `_DDPWrappingModel` 的 forward 方法，我们会在该 forward 函数中进行判断，确
       定调用的是模型自己的 forward 函数，还是 `train_step`、`evaluate_step`、
       `test_step` 方法。

    4. 当某一个进程出现 exception 后，`TorchDDPDriver` 的处理；
       不管是什么情况，`TorchDDPDriver` 在 `setup` 函数的最后，都会将所有进程的
       pid 主动记录下来，这样当一个进程出现 exception 后，driver 的 on_exception
       函数就会被 trainer 调用，其会调用 os.kill 指令将其它进程 kill 掉；
"""
...
from fastNLP.core.drivers.torch_driver.dist_utils import (
    convert_sync_batchnorm, fastnlp_torch_all_gather,
    fastnlp_torch_broadcast_object)
from fastNLP.core.drivers.torch_driver.utils import (_DDPWrappingModel,
                                                     replace_batch_sampler,
                                                     replace_sampler)
from fastNLP.core.drivers.utils import distributed_open_proc
from fastNLP.core.log import logger
from fastNLP.core.samplers import (
    RandomSampler, ReproducibleBatchSampler, ReproducibleSampler,
    UnrepeatedSampler, UnrepeatedSequentialSampler,
    conversion_between_reproducible_and_unrepeated_sampler,
    re_instantiate_sampler)
from fastNLP.core.utils import auto_param_call, check_user_specific_params
from fastNLP.envs import (FASTNLP_DISTRIBUTED_CHECK, FASTNLP_GLOBAL_RANK,
                          FASTNLP_GLOBAL_SEED, FASTNLP_NO_SYNC)
from .torch_driver import TorchDriver
from .utils import _check_dataloader_args_for_distributed

__all__ = ['TorchDDPDriver']
...
class TorchDDPDriver(TorchDriver):
    ...
    .. note::
        ...
        3. 用户自己在外面初始化 ``DDP``，并且通过 ``python -m torch.distributed.
           ...
           .. code-block::
                ...
                # 注意此时仍旧不需要您主动地将 datalaoder 的 sampler 替换为
                # DistributedSampler；
                trainer = Trainer(
                    ...
                    driver='torch',
    ...
    :param torch_kwargs:
        * *ddp_kwargs* -- 用于在使用 ``TorchDDPDriver`` 时指定
          ``DistributedDataParallel`` 初始化时的参数；例如传入
          ``{'find_unused_parameters': True}`` 来解决有参数不参与前向运算导致的报
          错等
    ...
    def __init__(self,
        ...
        if isinstance(model, torch.nn.DataParallel):
            raise ValueError(
                ...
                '`TorchDDPDriver`, it should be `torch.nn.Module` or '
        ...
        # 注意我们在 initialize_torch_driver 中的逻辑是：
        # 如果是 is_pull_by_torch_run，那么我们就直接把 parallel_device
        # 置为当前进程的gpu；
        ...
        if dist.is_initialized() and \
            ...
            if not isinstance(model, DistributedDataParallel):
                raise RuntimeError(
                    ...
                    '`DistributedDataParallel` when you initialize the ddp '
                    'process out of our control.')

            self.outside_ddp = True
            # 用户只有将模型上传到对应机器上后才能用 DistributedDataParallel
            # 包裹，因此如果用户在外面初始化了 DDP，那么在 TorchDDPDriver 中
            # 我们就直接将 model_device 置为 None；
            self.model_device = None

        # 当用户自己在外面初始化 DDP 时我们会将 model_device 置为 None，
        # 这时用户可以通过 `data_device` 将对应的数据移到指定的机器上;
        ...
        check_user_specific_params(self._fsdp_kwargs,
                                   ...
                                   DistributedDataParallel.__name__)
        if len(self.model._buffers) != 0 and self._fsdp_kwargs.get(
                'broadcast_buffers', None) is None:
            logger.info(
                'Notice your model has buffers and you are using '
                "`TorchDDPDriver`, but you do not set 'broadcast_buffers' "
                'in your trainer. Cause in most situations, this parameter '
                "can be set to 'False' to avoid redundant data communication "
        ...
        # sync_bn 表示是否要将模型中可能存在的 BatchNorm 层转换为
        # 可同步所有卡数据计算均值和方差的 SyncBatchNorm 层；
        ...
        # self.parallel_device 一定是 List[torch.device]；
            ...
            if not dist.is_initialized():
                ...
                dist.init_process_group(
                    ...
                    rank=self.global_rank,
                    world_size=self.world_size)
            # 用户在这个 trainer 前面又初始化了一个 trainer，
            # 并且使用的是 TorchDDPDriver；
            else:
    ...
    def open_subprocess(self):
        if self.local_rank == 0:
            ...
            os.environ[
                FASTNLP_DISTRIBUTED_CHECK] = f'{len(self.parallel_device)}'
            ...
            for rank in range(1, len(self.parallel_device)):
                ...
                # 如果是多机，一定需要用户自己拉起，因此我们自己使用
                # open_subprocesses 开启的进程的 FASTNLP_GLOBAL_RANK 一定是
                # LOCAL_RANK；
                env_copy[FASTNLP_GLOBAL_RANK] = str(rank)

                proc = distributed_open_proc(self.output_from_new_proc,
                                             command, env_copy,
                                             self.global_rank)
    ...
    def model_call(self, batch, fn: Callable,
                   signature_fn: Optional[Callable]) -> Dict:
        if self._has_ddpwrapped:
            return self.model(
                batch,
                fastnlp_fn=fn,
                fastnlp_signature_fn=signature_fn,
    ...
    def get_model_call_fn(self, fn: str) -> Tuple:
        ...
        if self._has_ddpwrapped:
            if hasattr(model, fn):
                ...
                if not callable(fn):
                    raise RuntimeError(
                        f'The `{fn}` attribute of model is not `Callable`.')
                return fn, None
            ...
            if hasattr(model, fn):
                logger.warning(
                    'Notice your model is a `DistributedDataParallel` model. '
                    f'And your model also implements the `{fn}` method, which '
                    'we can not call actually, we will call `forward` '
                    'function instead of `train_step` and you should '
                    'note that.')
            elif fn not in {'train_step', 'evaluate_step'}:
                raise RuntimeError(
                    f'There is no `{fn}` method in your model. And also '
                    'notice that your model is a `DistributedDataParallel` '
                    'model, which means that we will only call model.forward '
                    'function when we are in forward propagation.')

            return self.model, model.forward

    def set_dist_repro_dataloader(
            self,
            dataloader,
            dist: Optional[Union[str, ReproducibleSampler,
                                 ReproducibleBatchSampler]] = None,
            reproducible: bool = False):
        # 如果 dist 为 ReproducibleBatchSampler, ReproducibleSampler 说明是在
        # 断点重训时 driver.load_checkpoint 函数调用；
        # 注意这里不需要调用 dist_sampler.set_distributed；因为如果用户使用的是
        # TorchDDPDriver，那么其在 Trainer 初始化的时候就已经调用了该函数；
        if isinstance(dist, ReproducibleBatchSampler):
            dist.set_distributed(
                num_replicas=self.world_size, rank=self.global_rank, pad=True)
        ...
        if dist is None:
            if reproducible:
                raise RuntimeError(
                    'It is not allowed to save checkpoint if the sampler '
                    'is not allowed to be replaced.')
            else:
                args = self.get_dataloader_args(dataloader)
                if isinstance(args.batch_sampler, ReproducibleBatchSampler):
                    return replace_batch_sampler(
                        dataloader, re_instantiate_sampler(args.batch_sampler))
                if isinstance(args.sampler, ReproducibleSampler):
                    return replace_sampler(
                        dataloader, re_instantiate_sampler(args.sampler))
                return dataloader
        # trainer
        elif dist == 'dist':
            args = self.get_dataloader_args(dataloader)
            # 如果用户的 trainer.use_dist_sampler 为 True，那么此时其是否进行
            # 断点重训，不影响这里的行为；
            if isinstance(args.batch_sampler, ReproducibleBatchSampler):
        ...
        # evaluator
            ...
            if type(args.batch_sampler) != BatchSampler:
                ...
                logger.warning('Note that you are using customized '
                               '``batch_sampler`` in evaluate dataloader '
                               'which may cause the data for distributed '
                               'evaluation is not unrepeated. If you get this '
                               'warning when testing ``overfit_batches``, you '
                               'can ignore it.')
            if isinstance(args.sampler, ReproducibleSampler):
                sampler = conversion_between_reproducible_and_unrepeated_sampler(
                    args.sampler)
            elif not isinstance(args.sampler, UnrepeatedSampler):
                _check_dataloader_args_for_distributed(
                    args, controller='Evaluator')
                sampler = UnrepeatedSequentialSampler(dataset=args.dataset)
            else:
                sampler = re_instantiate_sampler(args.sampler)
            sampler.set_distributed(
            ...
            raise ValueError(
                'Parameter `dist_sampler` can only be one of three values: '
                "('dist', 'unrepeatdist', None).")
    ...
    def barrier(self):
        ...
        """
        if int(os.environ.get(FASTNLP_NO_SYNC, 0)) < 1 and \
                dist.is_initialized():
            # 当 FASTNLP_NO_SYNC 小于 1 时实际执行
            torch.distributed.barrier(async_op=False)

    def is_distributed(self):
        r"""
        :return: 当前使用的 driver 是否是分布式的 driver，对于
            ``TorchDDPDriver`` 来说，该函数一定返回 ``True``。
        """
        return True

    def broadcast_object(self, obj, src: int = 0, group=None, **kwargs):
        r"""
        从 ``src`` 端将 ``obj`` 对象（可能是 tensor ，可能是 object ）广播到其它进
        程。如果是非 tensor 的对象会尝试使用 pickle 进行打包进行传输，然后在接收处处
        再加载回来。仅在分布式的 driver 中有实际意义。

        :param obj: obj，可能是 Tensor 或 嵌套类型的数据
        :param src: 发送方的 ``global_rank``
        :param group: 进程所在的通信组
        :return: 如果当前 rank 是接收端，则返回接收到的参数；如果是 source 端则返回
            发送的内容。如果环境变量 ``FASTNLP_NO_SYNC`` 为 **2** 则返回 ``None``
        """
        return fastnlp_torch_broadcast_object(
            obj, src, device=self.data_device, group=group)

    def all_gather(self, obj, group) -> List:
        r"""
        将 ``obj`` 互相传送到其它所有的 rank 上，其中 ``obj`` 可能是 Tensor，也可
        能是嵌套结构的 object 。如果不是基础类型的数据，将会尝试通过 pickle 进行序列
        化，接收到之后再反序列化。
        ...
        :param group: 进程所在的通信组。
        :return: 所有 rank 发送的 ``obj`` 聚合在一起的内容；如果环境变量
            ``FASTNLP_NO_SYNC`` 为 **2** 则不会执行，直接返回 ``[obj]``。
        """
...
def find_free_network_port() -> str:
    r"""在 localhost 上找到一个空闲端口；
    当我们不想连接到真正的主节点但必须设置“MASTER_PORT”环境变量时在单节点训练中很有
    用。"""
