...
__all__ = ['torch_seed_everything', 'optimizer_state_to_device']


def torch_seed_everything(seed: Optional[int] = None,
    ...
    torch.manual_seed(fnlp_seed)
    torch.cuda.manual_seed_all(fnlp_seed)
    return fnlp_seed
...
class _DDPWrappingModel(Module):
    r"""
    该函数用于 DDP 训练时处理用户自己定制的 train_step 等函数；之所以要使用这一额外
    的包裹模型，是因为在使用 DDP 时，必须使用 DistributedDataParallel 的 forward
    函数才能实现正常的运行；
...
class _DeepSpeedWrappingModel(_DDPWrappingModel):
    r"""继承 ``_DDPWrappingModel``，区别在于进行 forward 之前先将 float 数据转换
    为 float16."""
    ...
    def _move_float_tensors_to_half(self, batch: Any):
        batch = apply_to_collection(
            batch, (torch.FloatTensor, torch.cuda.FloatTensor),
            function=self.batch_to)
        return batch
...
def _build_fp16_env(dummy=False):
    if dummy:
        ...
        if not torch.cuda.is_available():
            raise RuntimeError('Pytorch is not installed in gpu version, '
...
def replace_sampler(dataloader: 'DataLoader', sampler):
    ...
    # 防止用户的 DataLoader 是继承了 pytorch 的 DataLoader，
    ...
    # 如果初始化dataloader所使用的参数不是默认值，
    # 那么我们需要将其记录下来用于重新初始化时设置；
    non_default_params = {
        name
...
def _check_dataloader_args_for_distributed(args, controller='Trainer'):
    ...
    止在分布式训练中出现错误会报错。"""
    error_flag = (
        type(args.sampler) not in {TorchRandomSampler, TorchSequentialSampler})
    if controller == 'Trainer':
        mode = 'training'
        substitution = 'fastNLP.RandomSampler'
        error_flag = (type(args.batch_sampler) !=
                      TorchBatchSampler) or error_flag
    else:  # Evaluator
        mode = 'evaluation'
...
def _create_default_config(
    ...
    if zero_optimization:
        zero_config: Dict[str, Any] = zero_kwargs
    ...
    if logging_batch_size_per_gpu != 'auto':
        cfg = {
            'train_micro_batch_size_per_gpu': logging_batch_size_per_gpu,
            **cfg
        }
    return cfg
