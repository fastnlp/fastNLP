...
from fastNLP.core.samplers import (RandomSampler, ReproduceBatchSampler,
                                   ...
                                   ReproducibleSampler, re_instantiate_sampler)
from fastNLP.core.utils import auto_param_call
from fastNLP.core.utils.utils import _get_fun_msg
from .torch_driver import TorchDriver

__all__ = ['TorchSingleDriver']
...
class TorchSingleDriver(TorchDriver):
    ...
    def __init__(self,
        ...
        if isinstance(model, DistributedDataParallel):
            raise ValueError('`DistributedDataParallel` is not supported in '
                             '`TorchSingleDriver`')

        cuda_visible_devices = os.environ.get('CUDA_VISIBLE_DEVICES', None)
        if cuda_visible_devices == '':
            device = torch.device('cpu')
            logger.info("You have set `CUDA_VISIBLE_DEVICES` to '' in system "
                        'environment variable, and we are gonna to use `cpu` '
                        'instead of `gpu` device.')

        super(TorchSingleDriver, self).__init__(
            model, fp16=fp16, torch_kwargs=torch_kwargs, **kwargs)

        if device is None:
            logger.debug(
                'device is not set, fastNLP will try to automatically get it.')
            try:
                device = next(model.parameters()).device
                assert isinstance(device, torch.device)
            except Exception:
                raise ValueError(
                    'fastNLP cannot get device automatically, please set '
                    'device explicitly.')
        ...
        if isinstance(self.model, DataParallel):
            ...
            if hasattr(model, fn):
                ...
                raise RuntimeError(
                    ...
                    'notice that your model is a `DataParallel` model, '
                    'which means that we will only call model.forward '
                    'function when we are in forward propagation.')

            return self.model, model.forward
        else:
            # TODO 这种直接调用模型某个接口的方法无法触发hook，也许需要
            # 做一个warning，如果用户有钩子，提醒他train_step无法触发。
            if hasattr(self.model, fn):
                fn = getattr(self.model, fn)
                if not callable(fn):
                    raise RuntimeError(
                        f'The `{fn}` attribute is not `Callable`.')
    ...
    def set_dist_repro_dataloader(self,
                                  dataloader,
                                  dist: Union[str, ReproducibleBatchSampler,
                                              ReproducibleSampler,
                                              None] = None,
                                  reproducible: bool = False):

        # 如果 dist 为 ReproducibleBatchSampler, ReproducibleIterator 说明是在
                ...
                if type(args.sampler) is TorchRandomSampler:
                    if getattr(args.sampler, '_num_samples', None) is None \
                        ...
                        sampler = RandomSampler(
                            args.sampler.data_source, shuffle=True)
                        logger.debug(
                            'Replace torch RandomSampler into fastNLP '
                            'RandomSampler.')
                        return replace_sampler(dataloader, sampler)
                elif type(args.sampler) is TorchSequentialSampler:
                    # 需要替换为不要 shuffle 的。
                    sampler = RandomSampler(
                        args.sampler.data_source, shuffle=False)
                    logger.debug(
                        'Replace torch SequentialSampler into fastNLP '
                        'RandomSampler.')
                    return replace_sampler(dataloader, sampler)
    ...
    def is_distributed(self):
        ...
        :return: 当前使用的 driver 是否是分布式的 driver，对于
            ``TorchSingleDriver`` 来说直接返回 ``False``
        """
        return False
