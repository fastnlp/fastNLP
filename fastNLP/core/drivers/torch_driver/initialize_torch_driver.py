...
def initialize_torch_driver(driver: str,
                            device: Optional[Union[str, 'torch.device', int,
                                                   List[int]]],
                            model: 'torch.nn.Module', **kwargs) -> TorchDriver:
    r"""
    用来根据参数 ``driver` 和 ``device`` 来确定并且初始化一个具体的 ``Driver`` 实
    例然后返回回去；
    ...
    if parse_version(torch.__version__) < parse_version('1.6'):
        raise RuntimeError(
            f'Pytorch(current version:{torch.__version__}) need to be '
            'older than 1.6.')
    # world_size 和 rank
    if FASTNLP_BACKEND_LAUNCH in os.environ:
        if device is not None:
            logger.rank_zero_warning(
                ...
                '`torch.distributed.run` to pull up your script. And we '
                'will directly get the local device via '
                "`os.environ['LOCAL_RANK']`.",
                once=True)
        if driver == 'fairscale':
            return FairScaleDriver(
                model,
                torch.device(f"cuda:{os.environ['LOCAL_RANK']}"),
                is_pull_by_torch_run=True,
                **kwargs)
    ...
    if driver not in {'torch', 'fairscale', 'deepspeed', 'torch_fsdp'}:
        raise ValueError('Parameter `driver` can only be one of these values: '
                         "['torch', 'fairscale', 'deepspeed, 'torch_fsdp'].")
    ...
    if isinstance(device, str):
        ...
        if device < 0:
            if device != -1:
                raise ValueError(
                    "Parameter `device` can only be '-1' when it is "
                    'smaller than 0.')
            device = [
                torch.device(f'cuda:{w}') for w in range(_could_use_device_num)
            ]
    ...
    if driver == 'torch':  # single, ddp, 直接启动。
        ...
        if not isinstance(device, List):
            if device.type == 'cpu':
                raise ValueError(
                    'You are using `fairscale` driver, but your chosen '
                    "`device` is 'cpu'.")
            logger.warning_once(
                'Notice you are using `fairscale`, but the `device` is '
                'only one gpu.')
        ...
        if not isinstance(device, List):
            if device.type == 'cpu':
                raise ValueError(
                    'You are using `deepspeed` driver, but your chosen '
                    "`device` is 'cpu'.")
            logger.warning_once(
                'Notice you are using `deepspeed`, but the `device` is '
                'only one gpu.')
        ...
        if not isinstance(device, List):
            if device.type == 'cpu':
                raise ValueError(
                    'You are using `torch_fsdp` driver, but your chosen '
                    "`device` is 'cpu'.")
            logger.warning_once(
                'Notice you are using `torch_fsdp`, but the `device` is '
                'only one gpu.')
