...
class TorchDriver(Driver):
    ...
    def __init__(self,
        ...
        # 因为 ddp 和 single_device 的混合精度训练的设置是一样的，因此可以统一
        # 抽象到这里；
        self.fp16 = fp16
        self.auto_cast, _grad_scaler = _build_fp16_env(dummy=not self.fp16)
        self.grad_scaler = _grad_scaler(
            **self._torch_kwargs.get('gradscaler_kwargs', {}))
    ...
    def tensor_to_numeric(tensor, reduce: Optional[str] = None):
        ...
        return apply_to_collection(
            data=tensor, dtype=torch.Tensor, function=_translate)
    ...
    def load_model(  # type: ignore[override]
        ...
        """
        model = self.unwrap_model()
        # todo torch.load 在加载时会使得卡 0 多出一个（甚至多个）model 的显存；
        # 因此在多卡断点重训时可能会出现错误；
    ...
    def get_sampler_state(self, dataloader, num_consumed_batches):
        ...
        # 首先 pytorch 的 DataLoader 一定会有 sampler；另一方面，我们在断点重训的
        # 时候一定会在 `set_` 中将 dataloader 的 sampler 替换为
        # `ReproducibleSampler`；否则就是在单卡情况下将 batch_sampler 替换为
        # `ReproducibleBatchSampler`；
    ...
    def load_sampler_state(self, dataloader, sampler_states):
        ...
        if isinstance(dataloader_args.batch_sampler, ReproducibleBatchSampler):
            ...
            logger.debug(
                'Replace torch RandomSampler into fastNLP RandomSampler.')
        elif self.is_distributed():
            raise RuntimeError(
                'It is not allowed to use checkpoint retraining when you do '
                'not use our `ReproducibleSampler`.')
        else:
    ...
    def load_checkpoint(  # type: ignore[override]
        ...
        if 'grad_scaler_state_dict' in states:
            ...
            logger.rank_zero_warning(
                ...
                'resume to a fp16=True training, the training process may be '
                'unstable.')
    ...
    def move_data_to_device(self, batch):
        ...
        """
        return torch_move_data_to_device(batch, self.data_device,
                                         self.non_blocking)
    ...
    def worker_init_function(
        ...
        """"""
        """The worker_init_fn that Lightning automatically adds to your
        dataloader if you previously set the seed with ``seed_everything(seed,
        workers=True)``.
    ...
    def get_dataloader_args(dataloader: 'DataLoader'):
        ...
        # dataloader 使用的是 batch_sampler；
            ...
            # ReproduceBatchSampler 的情况
                ...
                # 使用的是自己的 batch_sampler，且没有 "sampler" 属性；这种情况下
                # DataLoader 会自己初始化一个 sampler；我们因此将这个默认初始化的
                # sampler 挂载到 res 上；
                res.sampler = dataloader.sampler
                res.shuffle = False
