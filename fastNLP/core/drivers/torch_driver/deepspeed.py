...
class DeepSpeedDriver(TorchDDPDriver):
    ...
    def __init__(self,
                 ...
                 fp16: bool = False,
                 deepspeed_kwargs: Optional[Dict] = None,
                 **kwargs):
        assert _NEED_IMPORT_DEEPSPEED, 'Deepspeed is not imported.'
        kwargs.pop('torch_kwargs', None)
        self._ds_kwargs = deepspeed_kwargs if deepspeed_kwargs is not None else {}
        ...
        # 如果用户自己在外面初始化 DDP，那么其一定是
        # 通过命令行 deepspeed xxx.py 拉起的；
        ...
        if not is_pull_by_torch_run and parallel_device is None:
            raise ValueError(
                ...
                '`DeepSpeedDriver`. This error is caused when your value '
                'of parameter `device` is `None` in your `Trainer` instance.')

        # 注意我们在 initialize_torch_driver 中的逻辑就是如果是
        # is_pull_by_torch_run，那么我们就直接把 parallel_device 置为
        # 当前进程的gpu；
        ...
        if dist.is_initialized() and \
            ...
            # 如果用户自己在外面初始化了 deepspeed，那么我们要求用户传入的模型一定是
            # 已经由 DeepSpeedEngine 包裹后的模型；
            if not isinstance(model, DeepSpeedEngine):
                raise RuntimeError(
                    'It is not allowed to input a normal model instead of '
                    '`DeepSpeedEngine` when you initialize the ddp process '
                    'out of our control.')
        ...
        deepspeed_logging_level = self._ds_kwargs.get('logging_level',
                                                      logging.ERROR)
    ...
    def setup(self):
        ...
        if len(self.optimizers) != 1:
            raise ValueError(
                'Multi optimizers is not supported for `DeepSpeedDriver` '
                'right now.')
        if self._has_setup:
            return
            ...
            if self.outside_ddp:
                ...
                if not dist.is_initialized():
                    deepspeed.init_distributed(
                        'nccl', distributed_port=self.master_port)
        ...
        # self.parallel_device 一定是 List[torch.device]；
            ...
            # 使用的是 DeepSpeedDriver；
            else:
                # 如果 `dist.is_initialized() is True`，那么说明
                # DeepSpeedDriver 在之前已经初始化并且已经 setup 过一次，那么我们
                # 需要保证现在使用的（即之后的）DeepSpeedDriver 的设置和第一个
                # DeepSpeedDriver 是完全一样的；
                pre_num_processes = int(os.environ[FASTNLP_DISTRIBUTED_CHECK])
                if pre_num_processes != len(self.parallel_device):
                    raise RuntimeError(
                        'Notice you are using `DeepSpeedDriver` after one '
                        'instantiated `DeepSpeedDriver`, it is not allowed '
                        'that your second `DeepSpeedDriver` has a new setting '
        ...
        if not self.outside_ddp:
            ...
            # 不加 dist.broadcast_object_list 会发生设备在 4,5 但是模型会
            # 同步到 0,1 的情况原因未知
        ...
        if not isinstance(self.model, DeepSpeedEngine):
            ...
            if self.config.get('activation_checkpointing'):
                ...
                deepspeed.checkpointing.configure(
                    ...
                    checkpoint_in_cpu=checkpoint_config.get(
                        'cpu_checkpointing'),
    ...
    def zero_grad(self):
        r"""进行梯度置零操作；由于 :meth:`DeepSpeedEngine.step` 包含了
        :meth:`zero_step` 的功能，因此该接口实际无意义。"""
    ...
    def get_model_no_sync_context(self):
        r"""
        :return: 一个 ``context`` 上下文环境，用于关闭各个进程之间的同步；在
            ``deepspeed`` 中，返回一个空的上下文
        """
    ...
    def save_model(  # type: ignore[override]
        ...
        :param filepath: 文件路径
        :param only_state_dict: 是否只保存权重；在 ``DeepSpeedDriver`` 中该参数无
            效。
        :param kwargs: 需要传入 **deepspeed** 模型 :meth:`save_checkpoint` 的其
            它参数。
        :return:
        """
        # deepspeed engine 要求在每个 rank 都调用 save_checkpoint，
        # 故去掉了 rank_zero_call 装饰器
        if self.stage_3:
            logger.rank_zero_warning(
                'When saving the DeepSpeed Stage 3 checkpoint, each worker '
                'will save a shard of the checkpoint within a directory. '
                # TODO check一下
    ...
    def load_model(  # type: ignore[override]
        ...
        :param load_state_dict: 保存的内容是否只是权重；在 ``DeepSpeedDriver`` 中
            该参数无效。
        :param kwargs: 需要传入 **deepspeed** 模型 :meth:`load_checkpoint` 的其
            它参数。
        :return:
        """
        if not only_state_dict:
    ...
    def save_checkpoint(self,
        ...
        **模型** （若 ``should_save_model`` 为 ``True``）
        ...
        :param only_state_dict: 是否只保存模型的参数。在 ``DeepSpeedDriver`` 中该
            参数无效。
        :param should_save_model: 是否应该保存模型。在 ``DeepSpeedDriver`` 中该参、 数只能为 ``True``
        """
        # deepspeed engine 要求在每个 rank 都调用 save_checkpoint，
        # 故去掉了 rank_zero_call 装饰器
        # 1. 保存 sampler 的状态
        ...
        if not should_save_model:
            logger.rank_zero_warning(
                'Saving checkpoint without model is not allowed for '
    ...
    def load_checkpoint(  # type: ignore[override]
        ...
        :param only_state_dict: 是否仅读取模型的 state_dict。 在
            ``DeepSpeedDriver`` 中该参数无效。
        :param should_load_model: 是否应该加载模型。在 ``DeepSpeedDriver`` 中该参
            数只能为 ``True``。
        ...
        if not should_load_model:
            logger.rank_zero_warning(
                'Loading checkpoint without model is not allowed for '
                '`DeepSpeedDriver`, so we will still load the model '
                'for you.')
        load_path, states = self.model.load_checkpoint(
            folder.joinpath(FASTNLP_CHECKPOINT_FILENAME))
        if load_path is None:
            raise RuntimeError(
                f'Failed to load checkpoint from path: {str(folder)}')
