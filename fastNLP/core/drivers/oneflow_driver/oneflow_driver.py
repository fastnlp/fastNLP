...
if _NEED_IMPORT_ONEFLOW:
    ...
    from oneflow.utils.data import BatchSampler, DataLoader, Dataset
...
from fastNLP.core.samplers import (RandomSampler, ReproduceBatchSampler,
                                   ...
                                   ReproducibleSampler)
from fastNLP.core.utils import (apply_to_collection, auto_param_call,
...
from .utils import DummyGradScaler, optimizer_state_to_device

__all__ = ['OneflowDriver']
...
class OneflowDriver(Driver):
    ...
    def __init__(self,
                 ...
                 oneflow_kwargs: Optional[Dict] = None,
                 **kwargs):
        ...
        if fp16:
            logger.warn(
                'OneflowDriver of eager mode dose not support fp16 now.``')
        # self.auto_cast, _grad_scaler = _build_fp16_env(dummy=not self.fp16)
        # self.grad_scaler = _grad_scaler(
        #     **self._oneflow_kwargs.get("gradscaler_kwargs", {}))
        ...
        for each_optimizer in optimizers:
            if not isinstance(each_optimizer, Optimizer):
                raise TypeError(
                    ...
                    f"'Optimizer' type, not {type(each_optimizer)}.")
    ...
    def tensor_to_numeric(tensor, reduce: Optional[str] = None):
        ...
        return apply_to_collection(
            data=tensor, dtype=oneflow.Tensor, function=_translate)
    ...
    def save_model(self,
        ...
        :param only_state_dict: 是否只保存权重；如果使用
            ``DistributedDataParallel`` 启动分布式训练的话，该参数只能为 ``True``
        :return:
        """
        model = self.unwrap_model()
        if not only_state_dict and self.is_distributed():
            logger.warn('`Cannot save ddp model directly, we will save its '
                        'state_dict for you.')
    ...
    def load_model(  # type: ignore[override]
        ...
        :param filepath: 保存文件的文件位置
        :param load_state_dict: 保存的内容是否只是权重；如果使用
            ``DistributedDataParallel`` 启动分布式训练的话，该参数只能为 ``True``
        :param kwargs:
            * *strict* (``bool``) -- 加载 state_dict 时是否严格要求 state_dict
              的内容与模型的参数完全一致。
        """
        model = self.unwrap_model()
    ...
    def save_checkpoint(self,
        ...
        **模型** （若 ``should_save_model`` 为 ``True``）
        ...
        # 所有 dataloader 我们是不会去改变它的，而是通过改变 trainer.dataloader
        # 来改变 dataloader 的状态，从而适配训练或者评测环境；
    ...
    def get_sampler_state(self, dataloader, num_consumed_batches):
        ...
        if hasattr(sampler, 'state_dict') and callable(sampler.state_dict):
            ...
            if dataloader_args.batch_size is not None:
                ...
                logger.rank_zero_warning(
                    'fastNLP cannot get batch_size, we have to save based on '
                    "sampler's `num_consumed_samples`, it may cause missing "
                    'some samples when reload.')
        else:
            raise RuntimeError(
                'The sampler has no `state_dict()` method, fastNLP cannot '
    ...
    def load_sampler_state(self, dataloader, sampler_states):
        ...
        if isinstance(dataloader_args.batch_sampler, ReproducibleBatchSampler):
            ...
            logger.debug(
                'Replace oneflow RandomSampler into fastNLP RandomSampler.')
        elif self.is_distributed():
            raise RuntimeError(
                'It is not allowed to use checkpoint retraining when you '
                'do not use our `ReproducibleSampler`.')
        else:
        ...
        if not isinstance(sampler, ReproducibleBatchSampler):
            if dataloader_args.drop_last:
                batch_idx_in_epoch = len(sampler) // dataloader_args.batch_size - \
    ...
    def get_optimizer_state(self):
        ...
        for i in range(len(self.optimizers)):
            ...
            optimizer_state['state'] = optimizer_state_to_device(
                optimizer_state['state'], oneflow.device('cpu'))
    ...
    def load_checkpoint(  # type: ignore[override]
        ...
        #     logger.rank_zero_warning(
        #         f"Checkpoint {folder} is not trained with fp16=True, while "
        #         "resume to a fp16=True training, the training process may "
        #         "be unstable.")
    ...
    def get_model_call_fn(self, fn: str) -> Tuple:
        if hasattr(self.model, fn):
            ...
            logger.debug(
                f'Use {_get_fun_msg(self.model.forward, with_fp=False)}...')
            return self.model, self.model.forward
        else:
            raise RuntimeError(
                f'There is no `{fn}` method in your {type(self.model)}.')

    @staticmethod
    def move_model_to_device(model: 'oneflow.nn.Module',
    ...
    def move_data_to_device(self, batch):
        ...
        :param batch: 包含 :class:`oneflow.Tensor` 的数据集合，可以是 **List**、
            **Dict** 等嵌套类型
    ...
    def worker_init_function(
        ...
        # 取余数避免 Overflow when unpacking long
        flow_seed = oneflow_ss.generate_state(1, dtype=np.uint64)[0] % 2**63
        oneflow.manual_seed(flow_seed)
    ...
    def set_sampler_epoch(self, dataloader: 'DataLoader', cur_epoch_idx: int):
        ...
        """
        # 保证 ddp 训练时的 shuffle=True 时的正确性，因为需要保证每一个进程上的
        # sampler 的shuffle 的随机数种子是一样的；
    ...
    def get_dataloader_args(dataloader: 'DataLoader'):
        ...
        if dataloader.batch_sampler is None:
            ...
            res.batch_size = 1
            res.shuffle = True if isinstance(dataloader.sampler,
                                             RandomSampler) else False
            res.drop_last = False
        # dataloader 使用的是 batch_sampler；
            ...
            # ReproduceBatchSampler 的情况
                ...
                # 如果 dataloader.batch_sampler 没有 sampler 这个属性，那么说明其
                # 使用的是自己的 batch_sampler，且没有 "sampler" 属性；
                # 这种情况下 DataLoader 会自己初始化一个 sampler；我们因此将这个
                # 默认初始化的 sampler 挂载到 res 上；
                res.sampler = dataloader.sampler
                res.shuffle = False
