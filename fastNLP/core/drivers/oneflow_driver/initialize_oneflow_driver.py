...
def initialize_oneflow_driver(driver: str,
                              device: Optional[Union[str, 'oneflow.device',
                                                     int, List[int]]],
                              model: 'oneflow.nn.Module',
                              **kwargs) -> OneflowDriver:
    r"""
    ...
    if FASTNLP_BACKEND_LAUNCH in os.environ:
        if device is not None:
            logger.rank_zero_warning(
                'Parameter `device` would be ignored when you are '
                'using `oneflow.distributed.launch` to pull up your '
                'script. ',
                once=True)
    ...
    if driver not in {'oneflow'}:
        raise ValueError(
            "Parameter `driver` can only be one of these values: ['oneflow'].")
        ...
        if device < 0:
            if device != -1:
                raise ValueError(
                    ...
                    'smaller than 0.')
            device = [
                oneflow.device(f'cuda:{w}')
                for w in range(_could_use_device_num)
            ]
    ...
    if driver == 'oneflow':  # single, ddp, 直接启动。
        if not isinstance(device, List):
            ...
            raise RuntimeError(
                'If you want to run distributed training, please use '
