    ...
    def __init__(self,
        ...
        if cuda_visible_devices == '':
            ...
            logger.info("You have set `CUDA_VISIBLE_DEVICES` to '' in system "
                        'environment variable, and we are gonna to'
                        'use `cpu` instead of `gpu` device.')

        super(OneflowSingleDriver, self).__init__(
        ...
        if reproducible:
            if type(args.batch_sampler) is OneflowBatchSampler:
                if type(args.sampler) is OneflowSequentialSampler:
                    ...
                    logger.debug(
                        'Replace oneflow SequentialSampler into fastNLP '
                        'RandomSampler.')
                    return replace_sampler(dataloader, sampler)
