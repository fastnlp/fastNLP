...
from fastNLP.core.drivers.oneflow_driver.dist_utils import (
    fastnlp_oneflow_all_gather, fastnlp_oneflow_broadcast_object)
...
class OneflowDDPDriver(OneflowDriver):
    ...
    :param oneflow_kwargs:
        * *ddp_kwargs* -- 用于 ``DistributedDataParallel`` 的其它参数，详情可查
          阅 **oneflow** 的官方文档
    :kwargs:
    ...
    def __init__(self,
        ...
        super(OneflowDDPDriver, self).__init__(
            model, fp16=fp16, oneflow_kwargs=oneflow_kwargs, **kwargs)

        # oneflow 会自己初始化通信组，因此 parallel_device 实际上不起作用，
        # 可以通过 current_device 获取设备
        self.model_device = oneflow.device('cuda',
        ...
        check_user_specific_params(self._ddp_kwargs,
        ...
        if len(self.model._buffers) != 0 and self._ddp_kwargs.get(
            ...
            logger.info(
                ...
                "`OneflowDDPDriver`, but you do not set 'broadcast_buffers' "
    ...
    def setup(self):
        ...
        # self._pids = [oneflow.tensor(0, dtype=oneflow.int).to(
        #     self.data_device) for _ in range(dist_env.get_world_size())]
        # comm.all_gather(self._pids, oneflow.tensor(
        #     os.getpid(), dtype=oneflow.int).to(self.data_device))
        # local_world_size = int(os.environ.get(
        #     "LOCAL_WORLD_SIZE")) if "LOCAL_WORLD_SIZE" in os.environ else None
        # if local_world_size is None:
        #     local_world_size = oneflow.tensor(
        #         int(os.environ.get("LOCAL_RANK")), dtype=oneflow.int).to(self.data_device)
        ...
        # self._pids = self._pids[node_rank *
        #                         local_world_size: (node_rank + 1) * local_world_size]
    ...
    def configure_ddp(self):
        if not hasattr(self.model, '_ddp_state_for_reversed_params'):
            ...
            self.model = DistributedDataParallel(
                # 注意这里的 self.model_device 是 `oneflow.device` type，
    ...
    def set_dist_repro_dataloader(
        ...
        # 注意这里不需要调用 dist_sampler.set_distributed；
        # 因为如果用户使用的是 OneflowDDPDriver，那么其在 Trainer 初始化的时候
        # 就已经调用了该函数；
        ...
        # trainer
            ...
            args = self.get_dataloader_args(dataloader)
            # 如果用户的 trainer.use_dist_sampler 为 True，
            # 那么此时其是否进行断点重训，不影响这里的行为；
            if isinstance(args.batch_sampler, ReproducibleBatchSampler):
        ...
        # evaluator
            ...
            raise ValueError('Parameter `dist_sampler` can only be one of '
                             "three values: ('dist', 'unrepeatdist', None).")
    ...
    def get_model_no_sync_context(self):
        r"""
        :return: 一个 ``context`` 上下文环境，用于关闭各个进程之间的同步；该功能暂
            时无效，返回一个空的上下文环境
        """
    ...
    def barrier(self):
        ...
        同步各个进程之间的操作
        """
        if int(os.environ.get(FASTNLP_NO_SYNC, 0)) < 1:
    ...
    def is_distributed(self):
        ...
        :return: 当前使用的 driver 是否是分布式的 driver，对于
            ``OneflowDDPDriver`` 来说，该函数一定返回 ``True``
        """
        return True
    ...
    def broadcast_object(self, obj, src: int = 0, group=None, **kwargs):
        ...
        if int(os.environ.get(FASTNLP_NO_SYNC, 0)) == 2:
            # 如果 FASTNLP_NO_SYNC == 2 直接返回。
            return
        return fastnlp_oneflow_broadcast_object(
            obj, src, device=self.data_device)
