...
class JittorMPIDriver(JittorDriver):
    ...
    def __init__(self,
                 model,
                 parallel_device: Union[str, int, List[int], None] = None,
                 is_pull_by_jittor_run: bool = False,
                 fp16: bool = False,
                 jittor_kwargs: Optional[Dict] = None,
                 **kwargs):

        super(JittorMPIDriver, self).__init__(
            model, fp16=fp16, jittor_kwargs=jittor_kwargs, **kwargs)
    ...
    def __fork_with_mpi__(self):
        ...
        if jt.in_mpi:
            ...
            if self.parallel_device == -1:
                # device 为 -1，那么默认使用全部的显卡
                raise NotImplementedError(f'Device={self.parallel_device}')
            elif type(self.parallel_device) is int:
                # device 为 *int*: 将使用 ``device_id`` 为该值的 ``gpu`` 进行训练
                num_procs = 1
                devices = self.parallel_device
            elif type(self.parallel_device) is list:
                # device 为 *list(int)*: 多于 1 个device，应当通过该种方式进行设定
            ...
            print(sys.argv)
            cmd = ' '.join([
                "CUDA_VISIBLE_DEVICES='%s'" % devices, 'mpirun', '-np',
                str(num_procs), sys.executable
            ] + sys.argv)
            print('[RUN CMD]:', cmd)
            os.system(cmd)
            exit(0)
    ...
    def set_dist_repro_dataloader(
            self,
            dataloader,
            dist: Optional[Union[str, ReproducibleSampler]] = None,
            reproducible: bool = False,
            sampler_or_batch_sampler=None):
        return dataloader
    ...
    def is_distributed(self):
        r"""判断是否为分布式的 **Driver**，在 ``JittorMPIDriver`` 中，返回
        ``True``。"""
        return True
