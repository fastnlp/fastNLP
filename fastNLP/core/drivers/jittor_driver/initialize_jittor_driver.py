...
def initialize_jittor_driver(driver: str, device: Optional[Union[str, int,
                                                                 List[int]]],
                             model: 'jittor.Module', **kwargs) -> JittorDriver:
    r"""
    ...
    if driver not in {'jittor'}:
        raise ValueError(
            "Parameter `driver` can only be one of these values: ['jittor'].")
