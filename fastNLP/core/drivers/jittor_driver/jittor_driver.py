...
from fastNLP.core.dataloaders import JittorDataLoader, OverfitDataLoader
from fastNLP.core.drivers.driver import Driver
from fastNLP.core.log import logger
from fastNLP.core.samplers import RandomSampler, ReproducibleSampler
...
if _NEED_IMPORT_JITTOR:
    ...
    from jittor.dataset import BatchSampler as JittorBatchSampler
    from jittor.dataset import Dataset
    from jittor.dataset import RandomSampler as JittorRandomSampler
    from jittor.dataset import Sampler as JittorSampler
    from jittor.dataset import SequentialSampler as JittorSequentialSampler
    from jittor.optim import Optimizer

    _reduces = {'max': jt.max, 'min': jt.min, 'mean': jt.mean, 'sum': jt.sum}
...
class JittorDriver(Driver):
    ...
    def __init__(self,
                 model,
                 fp16: bool = False,
                 jittor_kwargs: Optional[Dict] = None,
                 **kwargs):
        if not isinstance(model, Module):
            raise ValueError(
                f'Parameter `model` can not be `{type(model)}` in '
                '`JittorDriver`, it should be exactly `jittor.Module` type.')
    ...
    def check_dataloader_legality(self, dataloader):
        ...
        :param dataloder:
        """
        if not isinstance(dataloader,
                          (Dataset, JittorDataLoader, OverfitDataLoader)):
            raise TypeError(
                f'{Dataset} or {JittorDataLoader} is expected, instead of '
                f'`{type(dataloader)}`')
        if len(dataloader) == 0:
    ...
    def _check_optimizer_legality(optimizers):
        for each_optimizer in optimizers:
            if not isinstance(each_optimizer, Optimizer):
                raise TypeError(f'Each optimizer of parameter `optimizers` '
                                "should be 'jittor.optim.Optimizer' type, "
    ...
    def save_model(  # type: ignore[override]
        ...
        :param only_state_dict: 在 **Jittor** 中，该参数只能为 ``True``，因为
            **Jittor** 仅支持保存模型的 ``state_dict``。
        """
        if not only_state_dict:
            logger.rank_zero_warning(
                'Jittor only supports saving state_dict, and we will also '
                'save state_dict for you.',
                once=True)
    ...
    def load_model(  # type: ignore[override]
        ...
        if not only_state_dict:
            logger.rank_zero_warning(
                'Jittor only supports loading state_dict, and we will also '
                'load state_dict for you.',
                once=True)
        if isinstance(filepath, Path):
            filepath = str(filepath)
        model = self.unwrap_model()
        ...
        **模型** （若 ``should_save_model`` 为 ``True``）
        ...
        if hasattr(sampler, 'state_dict') and callable(sampler.state_dict):
            ...
            if dataloader_args.batch_size is not None:
                sampler_states[
                    'num_consumed_samples'] = sampler.num_replicas * dataloader_args.batch_size * num_consumed_batches
            else:
                logger.rank_zero_warning(
            ...
            raise RuntimeError('The sampler has no `state_dict()` method, '
                               'fastNLP cannot save the training state.')
    ...
    def get_optimizer_state(self):
        ...
        for i in range(len(self.optimizers)):
            ...
            optimizers_state_dict[f'optimizer{i}'] = optimizer.state_dict(
    ...
    def load_checkpoint(  # type: ignore[override]
        ...
        if dataloader_args.sampler is None:
            sampler: Any = RandomSampler(
                dataloader_args.dataset, shuffle=dataloader_args.shuffle)
            ...
            logger.debug(
                'Replace jittor RandomSampler into fastNLP RandomSampler.')
        elif isinstance(dataloader_args.sampler, JittorSequentialSampler):
            sampler = RandomSampler(
                dataloader_args.sampler.dataset, shuffle=False)
            logger.debug('Replace jittor Sampler into fastNLP RandomSampler '
                         'without shuffle.')
        elif self.is_distributed():
            raise RuntimeError(
                'It is not allowed to use checkpoint retraining when '
                'you do not use our `ReproducibleSampler`.')
        else:
            raise RuntimeError(
                f'Jittor sampler {type(dataloader_args.sampler)} is not '
                'supported now.')
    ...
    def move_model_to_device(model: 'jt.Module', device):
        r"""
        将模型转移到指定的设备上。由于 **Jittor** 会自动为数据分配设备，因此该函数实
        际上无效。
        """
        ...

    def move_data_to_device(self, batch: 'jt.Var'):
        r"""将数据 ``batch`` 转移到指定的设备上。由于 **Jittor** 会自动为数据分配设
        备，因此该函数实际上无效。"""
        return batch
    ...
    def tensor_to_numeric(tensor, reduce=None):
        ...
        return apply_to_collection(
            data=tensor, dtype=jt.Var, function=_translate)
    ...
    def set_deterministic_dataloader(self,
                                     dataloader: Union['JittorDataLoader',
                                                       'Dataset']):
        r"""
        为了确定性训练要对 ``dataloader`` 进行修改，保证在确定随机数种子后，每次重新
        训练得到的结果是一样的。**jittor** 暂时不提供该功能。
        """
        ...

    def set_sampler_epoch(self, dataloader: Union['JittorDataLoader',
                                                  'Dataset'],
                          cur_epoch_idx: int):
        r"""
    ...
    def get_dataloader_args(dataloader: Union['JittorDataLoader', 'Dataset']):
        ...
        res = Res()
        from fastNLP.core.dataloaders.jittor_dataloader.fdl import \
            _JittorDataset
        ...
        if dataloader.sampler is None:
            ...
            raise NotImplementedError(
                'Jittor does not support using batch_sampler in '
                '`Dataset` now, please check if you have set '
                '`Dataset.sampler` as `BatchSampler`.')
        else:
            # sampler 不为 None
