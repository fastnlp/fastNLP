...
def jittor_seed_everything(seed: Optional[int] = None,
    ...
    if not (min_seed_value <= seed <= max_seed_value):
        logger.rank_zero_warning(
            'Your seed value is too big or too small for numpy, '
            'we will choose a random seed for you.')
        seed %= max_seed_value

    os.environ[FASTNLP_GLOBAL_SEED] = f'{seed}'
    if add_global_rank_to_seed:
        seed += get_global_rank()  # type: ignore
...
def replace_batch_sampler(dataloader, batch_sampler):
    raise NotImplementedError(
        'Jittor does not support using batch_sampler in `Dataset` now, '
...
def replace_sampler(dataloader: Union['Dataset', 'JittorDataLoader'], sampler):
    ...
    if isinstance(dataloader, JittorDataLoader):
        init_params = dict(inspect.signature(
            dataloader.__init__).parameters)  # type: ignore
        reconstruct_args = {
            name: getattr(dataloader, name, p.default)
            for name, p in init_params.items()
        }
        reconstruct_args['dataset'] = replace_sampler(
            reconstruct_args['dataset'].dataset,
            reconstruct_args['dataset'].sampler)
