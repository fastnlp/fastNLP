...
class JittorSingleDriver(JittorDriver):
    ...
    def __init__(self,
        ...
        if device not in [None, 'cpu', 'gpu', 'cuda']:
            raise RuntimeError('Parameter `device` should be one of '
                               "[None, 'cpu', 'gpu', 'cuda'] .")
        super(JittorSingleDriver, self).__init__(
            model, fp16, jittor_kwargs=jittor_kwargs)
    ...
    def setup(self):
        r"""
        初始化训练环境；根据传入的 ``device`` 值设置模型的训练场景为 ``cpu`` 或
        ``gpu``。
        """
    ...
    def get_model_call_fn(self, fn: str) -> Tuple:
        if hasattr(self.model, fn):
            ...
            logger.debug(
                f'Use {_get_fun_msg(self.model.execute, with_fp=False)}...')
    ...
    def is_distributed(self):
        r"""判断是否为分布式的 **Driver**，在 ``JittorSingleDriver`` 中，返回
        ``False``。"""
        return False
        ...
        if reproducible:
            if args.sampler is None:
                ...
                if getattr(args.sampler, '_num_samples', None) is None \
                    ...
                    logger.debug('Replace jittor RandomSampler into '
                ...
                logger.debug('Replace jittor SequentialSampler into '
                             'fastNLP RandomSampler.')
                return replace_sampler(dataloader, sampler)
