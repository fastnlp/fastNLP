r"""
用于实现 **PaddlePaddle** 框架下使用 ``fleet`` 分布式训练 API 进行集群式
...
不过在使用该 Driver 之前，我们需要向您说明 **fastNLP** 实现 ``PaddleFleetDriver``
的思路，以便于您理解代码编写过程中可能出现的问题。

在 **fastNLP** 中，为了尽可能减少单卡向分布式训练转换过程中的代码变动，我们需要在
``PaddleFleetDriver`` 中进行 **分布式环境初始化** 和 **将模型转换为并行模式** 等操
作，同时实现多卡训练的方法是从主进程（``rank=0``）中创建其它的所有子进程（``rank=1,
2,...``）。
...
设置 ``FASTNLP_BACKEND=paddle`` 后，**fastNLP** 会在 ``import paddle`` 之前通过
``CUDA_VISIBLE_DEVICES`` 将设备限制在所有可见设备的第 **0** 张卡上，以此绕开通信和
同步上的种种限制。我们会将用户希望可见的设备（用户设置了 ``CUDA_VISIBLE_DEVICES``
的情况）保存在另一个环境变量 ``USER_CUDA_VISIBLE_DEVICES`` 中来确保 **fastNLP** 能
够知道用户的设置。假设用户希望在 ``[0,2,3]`` 三张显卡上进行分布式训练，那么在三个训练
进程中，``CUDA_VISIBLE_DEVICES`` 就分别为 0、2 和 3 。

.. note::

    我们会事先将设备限制在所有可见设备的第 **0** 张卡上，因此多卡训练的参数
    ``device`` 一定要以 **0** 开始，否则会无法正常地启动。如果您希望调整使用的第一张
    显卡，请使用 ``CUDA_VISIBLE_DEVICES`` 进行限制。

.. note::

    根据 **PaddlePaddle** 的说明，设置 ``CUDA_VISIBLE_DEVICES`` 之后启动分布式训练
...
.. note::
    ...
    launch`` 启动；因此我们不会在 ``PaddleFleetDriver`` 中保存任何当前有多少台机器
    的信息。
...
from fastNLP.core.samplers import (
    RandomSampler, ReproduceBatchSampler, ReproducibleBatchSampler,
    ReproducibleSampler, UnrepeatedSampler, UnrepeatedSequentialSampler,
...
from fastNLP.envs.env import (FASTNLP_DISTRIBUTED_CHECK, FASTNLP_GLOBAL_SEED,
                              FASTNLP_NO_SYNC, USER_CUDA_VISIBLE_DEVICES)
from fastNLP.envs.imports import _NEED_IMPORT_PADDLE
from .dist_utils import (fastnlp_paddle_all_gather,
...
from .utils import (_check_dataloader_args_for_distributed,
                    _FleetWrappingModel, replace_batch_sampler,
                    replace_sampler)
    ...
    **fastNLP** 支持三种启动分布式训练的方式（假设执行训练的文件名为
    ...
    def __init__(self,
        ...
        if USER_CUDA_VISIBLE_DEVICES not in os.environ:
            raise RuntimeError(
                'To run paddle distributed training, please set '
                "`FASTNLP_BACKEND` to 'paddle' before using fastNLP.")
        super(PaddleFleetDriver, self).__init__(
            model, fp16=fp16, paddle_kwargs=paddle_kwargs, **kwargs)
        ...
        if not is_pull_by_paddle_run:
            if parallel_device is None:
                raise ValueError(
                    ...
                    '`PaddleFleetDriver`. This error is caused when your '
                    'value of parameter `device` is `None` in your `Trainer` '
                    'instance.')
            if not isinstance(parallel_device, List):
                raise ValueError(
                    "Parameter `parallel_device`'s type must be List when "
                    f'using `PaddleFleetDriver`, not {type(parallel_device)}.')
            if get_paddle_device_id(parallel_device[0]) != 0:
                raise ValueError(
                    "The first device of `parallel_device` must be 'gpu:0' "
                    'in fastNLP.')
        ...
        # 在初始化时，如果发现 is_pull_by_paddle_run ，则将 parallel_device
        # 设置成当前进程的gpu
        ...
        # 判断传入的模型是否经过 _has_fleetwrapped 包裹；
        self._has_fleetwrapped = False

    def setup(self):
        """初始化分布式训练的环境。

        1. 如果是通过 ``paddle.distributed.launch`` 方法启动的，则根据已经设置好的
           环境获取分布式的属性。
        2. 否则启动子进程。
        """
        if self._has_setup:
        ...
        if self.is_pull_by_paddle_run:
            ...
            if self.outside_fleet:
                ...
                logger.info(f'World size: {self.world_size}, Global rank: '
                            f'{self.global_rank}')
                if not parallel_helper._is_parallel_ctx_initialized():
                    fleet.init(self.role_maker, self.is_collective,
                               self.strategy)
            ...
            # 并且使用的是 PaddleFleetDriver；
            else:
                # 已经设置过一次，保证参数必须是一样的
                pre_gpus = os.environ[FASTNLP_DISTRIBUTED_CHECK]
                pre_gpus = [int(x) for x in pre_gpus.split(',')]
                # cur_gpus = [
                #     get_paddle_device_id(g) for g in self.parallel_device
                # ]
                if sorted(pre_gpus) != sorted(self.parallel_device):
                    raise RuntimeError(
                        'Notice you are using `PaddleFleetDriver` after one '
                        'instantiated `PaddleFleetDriver`, it is not allowed '
                        'that your second `PaddleFleetDriver` has a new '
                        'setting of parameters `parallel_device`.')
        ...
        # TODO 不用.to会怎么样？
        self._pids = []
        paddledist.all_gather(self._pids,
                              paddle.to_tensor(os.getpid(), dtype='int32'))
    ...
    def _init_fleet_and_set(self):
        ...
        if self.local_rank == 0:
            ...
            # 是 rank0 的话，则拉起其它子进程
            launcher = FleetLauncher(self.parallel_device,
                                     self.output_from_new_proc)
            launcher.launch()
    ...
    def _set_from_fleet_environment(self):
        r"""当用户使用了 `python -m paddle.distributed.launch xxx.py` 启动时，我
        们需要根据 paddle 设置的环境变量来获得各种属性。"""
        self.world_size = paddledist.get_world_size()
        self.global_rank = paddledist.get_rank()
    ...
    def configure_fleet(self):
        ...
        if not self._has_fleetwrapped and \
                not isinstance(self.model, DataParallel):
            self.model = DataParallel(
                _FleetWrappingModel(self.model), **self._fleet_kwargs)
    ...
    def data_device(self):
        r"""数据所在的设备；由于 **PaddlePaddle** 可以通过环境变量获取当前进程的设
        备，因此该属性和 ``model_device`` 表现相同。"""
    ...
    def get_model_call_fn(self, fn: str) -> Tuple:
        ...
        if self._has_fleetwrapped:
            ...
            if hasattr(model, fn):
                logger.warning(
                    'Notice your model is a `DataParallel` model. And your '
                    f'model also implements the `{fn}` method, which we can '
                    'not call actually, we will call `forward` function '
                    'instead of `train_step` and you should note that.')
    ...
    # trainer, evaluator
        if dist is None:
            if reproducible:
                raise RuntimeError(
                    'It is not allowed to save checkpoint if the sampler is '
    ...
    def is_distributed(self) -> bool:
        """
        :return: 当前使用的 driver 是否是分布式的 driver，在
            ``PaddleFleetDriver`` 中，返回 ``True``。
        """
        return True

    @staticmethod
    def _check_optimizer_legality(optimizers):
        # TODO paddle 存在设置分布式 optimizers 的函数，
        # 返回值为 fleet.meta_optimizers.HybridParallelOptimizer
        DistribuedOptimizer = fleet.meta_optimizers.HybridParallelOptimizer
        for each_optimizer in optimizers:
            if not isinstance(each_optimizer,
                              (Optimizer, DistribuedOptimizer)):
                raise ValueError(
                    'Each optimizer of parameter `optimizers` '
                    "should be 'paddle.optimizer.Optimizer' type, "
    ...
    def broadcast_object(self, obj, src: int = 0, group=None, **kwargs):
        ...
        return fastnlp_paddle_broadcast_object(
            obj, src, device=device, group=group)
