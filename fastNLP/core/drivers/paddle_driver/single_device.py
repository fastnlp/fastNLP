...
from fastNLP.core.utils import (auto_param_call, get_paddle_device_id,
                                get_paddle_gpu_str)
...
if _NEED_IMPORT_PADDLE:
    ...
    from paddle.io import SequenceSampler as PaddleSequenialSampler
    ...
    **Driver**。
    ...
    def __init__(self,
        ...
        if isinstance(model, DataParallel):
            raise ValueError('`paddle.DataParallel` is not supported in '
                             '`PaddleSingleDriver`')
        ...
        super(PaddleSingleDriver, self).__init__(
            model, fp16=fp16, paddle_kwargs=paddle_kwargs, **kwargs)

        if device is None:
            raise ValueError(
                'Parameter `device` can not be None in `PaddleSingleDriver`.')
                ...
                if type(args.sampler) is PaddleRandomSampler:
                    if isinstance(args.sampler, PaddleRandomSampler):
                        if getattr(args.sampler, '_num_samples', None) is None \
                            ...
                            logger.debug(
                                'Replace paddle RandomSampler into fastNLP '
                                'RandomSampler.')
                            return replace_sampler(dataloader, sampler)
                    ...
                    logger.debug(
                        'Replace paddle SequentialSampler into fastNLP '
                        'RandomSampler.')
                    return replace_sampler(dataloader, sampler)
