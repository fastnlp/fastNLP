...
from fastNLP.envs.env import (FASTNLP_DISTRIBUTED_CHECK, FASTNLP_GLOBAL_RANK,
                              FASTNLP_GLOBAL_SEED, FASTNLP_LOG_LEVEL)
...
class FleetLauncher:
    r"""复原了 paddle 的 launch_collective 函数，将其简化后集成到一个类里。仅支持每
    个机器单卡的情况。"""
    ...
    def open_subprocess(self):
        r"""从 sub_trainers 中获取各个 rank 的信息，并且使用 subprocess.Popen 建
        立新的子进程。"""
        ...
        for idx, t in enumerate(self.sub_trainers):
            ...
            if os.environ.get(FASTNLP_GLOBAL_SEED
                              ) is None and FASTNLP_GLOBAL_SEED in current_env:
            ...
            if idx != 0:
                ...
                distributed_open_proc(self.output_from_new_proc, command,
                                      current_env, t.rank)
            else:
                # 更新当前的环境变量
    ...
    def get_global_env(self):
        ...
        # 通过FNLP初始化的标志
        global_envs[FASTNLP_DISTRIBUTED_CHECK] = ','.join(
            [str(g) for g in self.devices])
        ...
        global_envs.update({
            # world_size
            'PADDLE_TRAINERS_NUM':
            f'{len(self.sub_trainers)}',
            'PADDLE_TRAINER_ENDPOINTS':
            ','.join(self.endpoints),
            'PADDLE_WORLD_DEVICE_IDS':
            ','.join(world_device_ids),
        })

        return global_envs

    def set_endpoints(self):
        r"""寻找用户设置的端口或是空闲端口用于分布式训练，参考了 PaddlePaddle 中的
        `get_cluster_from_args` 数。"""
        ...
        self.endpoints = [
            '%s:%d' % (self.node_ip, port) for port in free_ports
        ]

    def get_process_info(self):
        r"""获取各个训练进程的设备、rank 和端口信息，参考 PaddlePaddle 的
        `get_cluster` 函数。"""
        sub_trainers = []
        assert len(self.endpoints) >= len(self.devices), \
