...
from fastNLP.envs import (FASTNLP_CHECKPOINT_FILENAME, FASTNLP_GLOBAL_RANK,
                          FASTNLP_MODEL_FILENAME, rank_zero_call)
from fastNLP.envs.imports import _NEED_IMPORT_PADDLE
from .utils import DummyGradScaler, _build_fp16_env, optimizer_state_to_device

if _NEED_IMPORT_PADDLE:
    import paddle
    from paddle.io import BatchSampler, DataLoader, Dataset
    from paddle.io import RandomSampler as PaddleRandomSampler
...
class PaddleDriver(Driver):
    ...
    def __init__(self,
        ...
        if not isinstance(model, paddle.nn.Layer):
            raise ValueError(
                f'Parameter `model` can not be `{type(model)}` in '
                '`PaddleDriver`, it should be exactly `paddle.nn.Layer` type.')
        ...
        self.grad_scaler = _grad_scaler(
            **self._paddle_kwargs.get('gradscaler_kwargs', {}))
    ...
    def check_dataloader_legality(self, dataloader):
        ...
        if not isinstance(dataloader, DataLoader) and not isinstance(
                dataloader, OverfitDataLoader):
            raise TypeError(
                f'{DataLoader} is expected, instead of `{type(dataloader)}`')
        if dataloader.batch_size is None and dataloader.batch_sampler is None:
            raise ValueError(
                "Please ensure at least one of your dataloader's batch_size "
                'and batch_sampleris not None')
        if len(dataloader) == 0:
            logger.rank_zero_warning(
                'Your dataloader is empty, which is not recommended because '
                'it may cause some unexpected exceptions.',
                once=True)
    ...
    def _check_optimizer_legality(optimizers):
        ...
        for each_optimizer in optimizers:
            if not isinstance(each_optimizer, Optimizer):
                raise TypeError(
                    f'Each optimizer of parameter `optimizers` should be '
                    "'paddle.optimizer.Optimizer' type, "
    ...
    def tensor_to_numeric(tensor, reduce=None):
        ...
        :param reduce: 当 tensor 是一个多数值的张量时，应当使用何种归一化操作来转换
            成单一数值，应当为以下类型之一：``['max', 'min', 'sum', 'mean']``。
        :return: 一个单一数值，其数值类型是 python 中的基本的数值类型，例如
            :class:`int`、:class:`float` 等
        """
        if tensor is None:
            return None
        ...
        return apply_to_collection(
            data=tensor, dtype=paddle.Tensor, function=_translate)

    def set_model_mode(self, mode: str):
        r"""
        设置模型为 ``train`` 或 ``eval`` 的模式；目的是为切换模型的训练和推理（会关
        闭 dropout 等）模式。
    ...
    def save_model(self,
        ...
        :param only_state_dict: 是否只保存模型的 ``state_dict``；若为 ``False``，
            则会调用 ``paddle.jit.save`` 函数保存整个模型的参数，此时需要传入
            ``input_spec`` 参数。
        :kwargs:
            * *input_spec* -- 描述存储模型 ``forward`` 方法的输入；
              当 ``only_state_dict`` 为 ``False`` 时必须传入，否则加载时会报错。您
              可以通过 ``InputSpec`` 或者示例 ``Tensor`` 进行描述。详细的使用方法
              可以参考 **PaddlePaddle** `关于 paddle.jit.save 函数的文档
              <https://www.paddlepaddle.org.cn/documentation/docs/zh/api/paddle/jit/save_cn.html#save>`_。
        """
        ...
        if only_state_dict:
            ...
            if input_spec is None:
                raise ValueError(
                    'To save the whole Paddle Layer, parameter `input_spec` '
                    'is needed.')
    ...
    def load_model(  # type: ignore[override]
        ...
        if isinstance(filepath, Path):
            filepath = str(filepath)
        # paddle 中，通过 paddle.jit.save 函数保存的模型也可以通过 paddle.load
        # 加载为相应的 state dict，但是此时对输入的 path 有要求， 必须是
        #  dir/filename 的形式，否则会报错。
    ...
    def save_checkpoint(self,
        ...
        **模型** （若 ``should_save_model`` 为 ``True``）
        ...
        :param should_save_model: 是否应该保存模型，如果为 ``False``，Driver 将不
            负责 model 的保存。
        :param kwargs: 同 :meth:`save_model` 。
        """
        # 传入的 dataloader 参数是 trainer 的 dataloader 属性，因为 driver 的
        # 所有 dataloader 我们是不会去改变它的，而是通过改变 trainer.dataloader 来
        # 改变 dataloader 的状态，从而适配训练或者评测环境；

        # 1. sampler 的状态，因为我们支持 resume training，即精确恢复到具体的一个
        # batch；
        # paddle 的 DataLoader 在初始化之后 batch_sampler 可能为 None，也可能为
        # 用户设置的 batch_sampler
        ...
        if isinstance(dataloader_args.batch_sampler, ReproducibleBatchSampler):
            ...
            raise RuntimeError('This condition is not supposed to appear. '
        ...
        if hasattr(sampler, 'state_dict') and callable(sampler.state_dict):
            ...
            if dataloader_args.batch_size is not None:
                sampler_states['num_consumed_samples'] = sampler.num_replicas * \
                    dataloader_args.batch_size * num_consumed_batches
            else:
                logger.rank_zero_warning(
                    'fastNLP cannot get batch_size, we have to save based '
                    'on `num_consumed_samples`, it may cause missing some '
                    'samples when reload.')
        else:
            raise RuntimeError(
                'The sampler has no `state_dict()` method, it will fail to '
                'recover to the specific batch.')
        ...
        if should_save_model:
            self.save_model(
                folder.joinpath(FASTNLP_MODEL_FILENAME), only_state_dict,
                **kwargs)
    ...
    def get_optimizer_state(self):
        ...
        for i in range(len(self.optimizers)):
            ...
            optimizers_state_dict[f'optimizer{i}'] = optimizer_state_to_device(
                optimizer.state_dict(), 'cpu')
    ...
    def load_checkpoint(  # type: ignore[override]
        ...
        :param folder: 读取该 folder 下的 ``FASTNLP_CHECKPOINT_FILENAME`` 文件
            与 ``FASTNLP_MODEL_FILENAME`` （如果 ``should_load_model`` 为
            ``True``）。
        ...
        dataloader_args = self.get_dataloader_args(dataloader)
        if isinstance(dataloader_args.batch_sampler, ReproducibleBatchSampler):
            sampler: Any = dataloader_args.batch_sampler
            ...
            logger.debug(
                'Replace paddle RandomSampler into fastNLP RandomSampler.')
        elif self.is_distributed():
            raise RuntimeError(
                'It is not allowed to use checkpoint retraining when you '
                'do not use our or `ReproducibleSampler`.')
        else:
            sampler = ReproduceBatchSampler(
                batch_sampler=dataloader_args.batch_sampler
                if dataloader_args.batch_sampler is not None else
                dataloader_args.sampler,
                batch_size=dataloader_args.batch_size,
                drop_last=dataloader_args.drop_last)
        sampler.load_state_dict(states.pop('sampler_states'))
        states['dataloader'] = self.set_dist_repro_dataloader(
            dataloader, sampler)
        ...
        if not isinstance(sampler, ReproducibleBatchSampler):
            if dataloader_args.drop_last:
                batch_idx_in_epoch = len(
                    sampler
                ) // dataloader_args.batch_size - sampler.num_left_samples // dataloader_args.batch_size
            else:
                batch_idx_in_epoch = (
                    len(sampler) + dataloader_args.batch_size -
                    1) // dataloader_args.batch_size - (
                        sampler.num_left_samples + dataloader_args.batch_size -
                        1) // dataloader_args.batch_size
    ...
    @staticmethod
    def move_model_to_device(model: 'paddle.nn.Layer',
                             device: Union[str, int, 'paddle.CUDAPlace',
                                           'paddle.CPUPlace']):
        r"""
    ...
    def move_data_to_device(self, batch: Any) -> Any:
        ...
        :param batch: 包含 :class:`paddle.Tensor` 的数据集合，可以是 **List**、
            **Dict** 等嵌套类型。
    ...
    def worker_init_function(
            worker_id: int,
            rank: Optional[int] = None) -> None:  # pragma: no cover
        # implementation notes: https://github.com/pytorch/pytorch/issues/5059#issuecomment-817392562
        global_rank = rank if rank is not None else int(
            os.environ.get(FASTNLP_GLOBAL_RANK, 0))
        # TODO gpu
        ...
        # Spawn distinct SeedSequences for the PyTorch PRNG and
    ...
    def set_sampler_epoch(self, dataloader: 'DataLoader', cur_epoch_idx):
        ...
        if callable(getattr(dataloader.batch_sampler, 'set_epoch', None)):
            dataloader.batch_sampler.set_epoch(cur_epoch_idx)
                ...
                getattr(dataloader.batch_sampler.sampler, 'set_epoch', None)):
            dataloader.batch_sampler.sampler.set_epoch(cur_epoch_idx)

    @staticmethod
    def get_dataloader_args(dataloader: 'DataLoader'):
        r"""从 ``dataloader`` 中获取参数 ``dataset``, ``batch_sampler``,
        ``sampler``, ``batch_size``, ``shuffle`` 和 ``drop_last``。"""
        ...
        if dataloader.batch_sampler is not None:
            ...
            if hasattr(dataloader.batch_sampler, 'batch_size'):
                res.batch_size = getattr(dataloader.batch_sampler,
                                         'batch_size')
