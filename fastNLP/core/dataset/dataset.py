...
在 fastNLP 内部每一行是一个 :class:`~fastNLP.core.dataset.Instance` 对象；每一列
是一个 :class:`~fastNLP.core.dataset.FieldArray` 对象。
...
from fastNLP.core.utils.rich_progress import (DummyFRichProgress,
                                              f_rich_progress)
...
__all__ = ['DataSet', 'ApplyResultException']

progress_bars = {'rich': f_rich_progress, 'tqdm': f_tqdm_progress}
...
def _apply_single(ds,
    ...
    :param desc: 进度条的描述字符
    :param progress_bar: 显示 progress_bar 的方式，支持三种取值 ``["rich",
        "tqdm", None]`。
    :return:
    """
    progress_bar = progress_bars.get(
...
class DataSet:
    ...
    * 当 data 为 ``Dict[str, List[Any]]`` 时，则每个 key 的 value 应该为等长的
    ...
    def __init__(self,
                 data: Union[List[Instance], Dict[str, List[Any]],
                             List[Dict[str, Any]], None] = None):
        self.field_arrays: Dict[str, Any] = {}
        ...
        if data is not None:
            if isinstance(data, Dict):
                ...
                assert len(set(length_set.values())) == 1, \
                    'Fields must all be of same length, instead of ' \
                    f'{length_set}.'
                for key, value in data.items():
    ...
    def _inner_iter(self):
        ...
        class Iter_ptr:
            ...
            def __getitem__(self, item):
                assert item in self.dataset.field_arrays, \
                    'no such field:{} in Instance {}'.format(
                        item, self.dataset[self.idx])
                assert self.idx < len(self.dataset.field_arrays[item]), \
                    'index:{} out of range'.format(self.idx)
    ...
    def __getitem__(self, idx: Union[int, slice, str, list]):
        ...
        slice, str, list]``
            ...
            * 当 idx 为 ``str`` 时，该 idx 为 DataSet 的 field_name, 其会返回该
              field_name 的所有内容，为 list 类型。
            * 当 idx 为 ``list`` 时，该 idx 的 list 内全为 int 数字，其会取出所有
              内容组成一个新的 DataSet 返回。
        ...
        if isinstance(idx, int):
            ...
            for field_name, field in self.field_arrays.items():
                dataset.add_field(
                    field_name=field_name, fields=field.content[idx])
    ...
    def __setitem__(self, key, value):
        ...
        if len(ins_keys - ds_keys) != 0:
            raise KeyError('The following keys are not found in the Dataset:'
                           f'{list(ins_keys - ds_keys)}.')
        if len(ds_keys - ins_keys) != 0:
            raise KeyError('The following keys are not found in the Instance:'
                           f'{list(ds_keys - ins_keys)}.')
    ...
    def _from_list_of_dict(self, data):
        for dict_data in data:
            ...
            assert isinstance(dict_data, Dict), \
                'Every item in your data list must be Dict type, ' \
    ...
    def _from_list_of_instance(self, data):
        for ins in data:
            assert isinstance(ins, Instance), \
                'Every item in your data list Must be Instance type, ' \
                f'not {type(ins)}.'
            self.append(ins)
    ...
    def append(self, instance: Instance) -> None:
        ...
        if len(self.field_arrays) == 0:
            ...
            if len(self.field_arrays) != len(instance.fields):
                raise ValueError(
                    'DataSet object has {} fields, but attempt to append an '
                    'Instance object with {} fields.'.format(
                        len(self.field_arrays), len(instance.fields)))
            for name, field in instance.items():
                assert name in self.field_arrays, \
                    'Field:`{}` is not found in {}'.format(
                        name, self.field_arrays.keys())
                try:
    ...
    def apply_field(self,
        ...
        :param new_field_name: 函数执行结果写入的 ``field`` 名称。该函数会将
            ...
            意如果名称与已有的 field 相同则会进行覆盖。如果为 ``None`` 则不会覆盖和
            创建 field ；
        :param num_proc: 使用进程的数量。

            .. note::

                由于 ``python`` 语言的特性，设置该参数后会导致相应倍数的内存增长，这
                可能会对您程序的执行带来一定的影响。另外，使用多进程时，``func`` 函
                数中的打印将不会输出。

        :param progress_desc: 如果不为 ``None``，则会显示当前正在处理的进度条的名
            称；
    ...
    def apply_field_more(self,
                         func: Callable,
                         field_name: str,
                         modify_fields: bool = True,
                         num_proc: int = 0,
                         progress_desc: Optional[str] = None,
                         progress_bar: Optional[str] = 'rich'):
        r"""
        将 ``DataSet`` 中的每个 ``Instance`` 中的名为 `field_name` 的 field 传给
        ...
        if not isinstance(apply_out[0], Mapping):
            raise Exception('The result of func is not a Mapping, '
                            f'but a {type(apply_out[0])}')

        for key, value in apply_out[0].items():
            results[key] = [value]
        # 尝试合并所有dict数据, idx+1 的原因是第一条数据不可能出现错误，
        # 默认第一条数据为准
        try:
            for idx, per_out in enumerate(apply_out[1:]):
                if len(set(results.keys()) - set(per_out.keys())):
                    raise ApplyResultException(
                        'apply results have different fields', idx + 1)
    ...
    def _apply_process(self,
                       ...
                       num_proc: int = 0,
                       progress_bar: Optional[str] = 'rich',
                       _apply_field: Optional[str] = None,
                       progress_desc: Optional[str] = 'Main') -> list:
        """
        ...
        :param progress_bar: 显示 progress_bar 的方式，支持 `["rich", "tqdm",
            None]`。
        :param progress_desc: 进度条的描述字符，默认为'Main'
        """
        if isinstance(
                func,
                LambdaType) and num_proc > 1 and func.__name__ == '<lambda>':
            raise TypeError(
                'Lambda function does not support multiple processes, '
                'please set `num_proc=0`.')
        if num_proc > 1 and sys.platform in ('win32', 'msys', 'cygwin'):
            raise RuntimeError(
                'Your platform does not support multiprocessing with fork, '
                'please set `num_proc=0`')

        if num_proc < 2:
            results = _apply_single(
                ds=self,
                _apply_field=_apply_field,
                func=func,
                desc=progress_desc,
                progress_bar=progress_bar)
        else:
            # TODO 1. desc这个需要修改一下，应该把 subprocess 的 desc 修改一下。
            # 修改成Process 1 / Process 2
            ...
            counter = ctx.Value('i', 0, lock=True)  # type: Any
            pool = []
            queues = []
            results = []
            for i in range(num_proc):
                ...
                proc = ctx.Process(
                    target=_multi_proc,
                    args=(shard_data[i], _apply_field, func, counter, queue))
                proc.start()
                pool.append(proc)
                queues.append(queue)
            progress_bar = progress_bars.get(
                progress_bar,  # type: ignore[arg-type]
                DummyFRichProgress())
            total_len = len(self)
            task_id = progress_bar.add_task(  # type: ignore[union-attr]
            ...
            while counter.value < total_len or last_count == -1:
                ...
                progress_bar.update(  # type: ignore[union-attr]
                    task_id,
                    advance=advance,
                    refresh=True)
            ...
            for idx, proc in enumerate(pool):
                ...
                proc.join()
            progress_bar.destroy_task(task_id)  # type:  ignore
        return results

    def apply_more(self,
                   func: Optional[Callable] = None,
                   modify_fields: Optional[bool] = True,
                   num_proc: int = 0,
                   progress_desc: Optional[str] = '',
                   progress_bar: Optional[str] = 'rich'):
        r"""
        将 ``DataSet`` 中每个 ``Instance`` 传入到 ``func`` 中，并获取它的返回值。
        ...
        .. note::
            ...
            1. ``apply_more`` 可以返回多个 field 的结果，``apply`` 只可以返回一个
               field 的结果；
            2. ``apply_more`` 的返回值是一个字典，每个 key-value 对中的 key 表示
               field 的名字，value 表示计算结果；
        ...
        :param progress_desc: 当 progress_bar 不为 ``None`` 时，可以显示当前正在
            处理的进度条名称
        :param progress_bar: 显示进度条的方式，支持 ``["rich", "tqdm", None]``。
        :return: 一个字典
        """
        ...
        try:
            for idx, per_out in enumerate(apply_out[1:]):
                if len(set(results.keys()) - set(per_out.keys())):
                    raise ApplyResultException(
                        'Apply results have different fields:'
                        f'{set(results.keys())} and {set(per_out.keys())}',
                        idx + 1)
                for key, value in per_out.items():
                    results[key].append(value)
    ...
    def add_seq_len(self, field_name: str, new_field_name='seq_len'):
        r"""
        将使用 :func:`len` 直接对 ``field_name`` 中每个元素作用，将其结果作为
        sequence length, 并放入 ``new_field_name`` 这个 field。
    ...
    def split(self, ratio: float, shuffle=True):
        ...
        :param ratio: 0<ratio<1, 返回的第一个 DataSet 拥有 ``ratio`` 比例的数据，
            第二个 DataSet 拥有 ``1-ratio`` 的数据；
        :param shuffle: 在拆分前是否进行排序。为 False，返回的第一个 dataset 就是
            当前 dataset 中前 ``ratio`` 比例的数据；
    ...
    def concat(self,
        ...
        :param field_mapping: 当传入的 ``dataset`` 中的 field 名称和当前
            dataset 不一致时，需要通过 ``field_mapping`` 把输入的 ``dataset`` 中
        ...
        if fn_not_seen:
            raise RuntimeError(
                'The following fields are not provided in the dataset:'
                f'{fn_not_seen}')
    ...
    def to_pandas(self):
        ...
        dict_ = {
            key: value.content
            for key, value in self.field_arrays.items()
        }
    ...
    def set_pad(self,
        ...
        :param field_name: 需要调整的 field 的名称。如果 :meth:`Dataset.\
            ...
            回的是 Sequence 类型，则可以使用 ``'_0'``, ``'_1'`` 表示序列中第 **0**
            个和第 **1** 个元素。如果该 field 在数据中没有找到，则报错；如果
            :meth:`Dataset.__getitem__` 返回的是整体内容，请使用 "_single" 。
        :param pad_val: 这个 field 的默认 pad 值。如果设置为 ``None``，则表示该
            field 不需要 pad , fastNLP 默认只会对可以 pad 的 field 进行 pad，所以
            如果对应 field 本身就不是可以 pad 的形式，可以不用设置为 ``None``。如
    ...
    def from_datasets(cls, dataset):
        ...
        if not isinstance(dataset, Dataset):
            raise ValueError(
                f'Support huggingface dataset, but is {type(dataset)}!')
