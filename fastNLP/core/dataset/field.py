...
class FieldArray:
    ...
    def __init__(self, name: str, content):
        ...
        try:
            ...
            logger.error(
                f'Cannot convert content(of type:{type(content)}) into list.')
            raise e
        self.name = name
    ...
    def split(self, sep: Optional[str] = None, inplace: bool = True):
        r"""
        依次对自身的元素使用 ``.split()`` 方法，应该只有当本 field 的元素为
        :class:`str` 时，该方法才有用。

        :param sep: 分割符，如果为 ``None`` 则直接调用 ``str.split()``。
        :param inplace: 如果为 ``True``，则将新生成值替换本 field。否则返回
            :class:`list`。
        ...
        for index, cell in enumerate(self.content):
            try:
                ...
                logger.error(
                    f'Exception happens when process value in index {index}.')
                raise e
    ...
    def int(self, inplace: bool = True):
        ...
        将本 field 中的值调用 ``int(cell)``. 支持 field 中内容为以下两种情况:
            ...
            * [['1', '2', ..], ['3', ..], ...](即 field 中每个值为一个
              :class:`list`，:class:`list` 中的值会被依次转换。)

        :param inplace: 如果为 ``True``，则将新生成值替换本 field，并返回当前
            field 。否则返回 :class:`list`。
    ...
    def _after_process(self, new_contents: list,
                       inplace: bool):  # type: ignore[valid-type]
        r"""
