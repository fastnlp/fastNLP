...
__all__ = [
    'is_cur_env_distributed', 'get_global_rank', 'rank_zero_call',
    'all_rank_call_context', 'fastnlp_no_sync_context', 'rank_zero_rm'
]
...
def rank_zero_call(fn: Callable):
    ...
    同时，该函数还会设置环境变量 ``FASTNLP_NO_SYNC`` 为 **2**，在这个环境下，所有
    的 **fastNLP** 内置的 :meth:`barrier` 接口和 ``gather`` / ``broadcast`` 操作
    都没有任何意义。
...
def all_rank_call_context():
    ...
    **"0"**，使得 :func:`rank_zero_call` 函数失效，使得每个进程都会运行该函数。
    ...
    """
    old_fastnlp_global_rank = os.environ[
        FASTNLP_GLOBAL_RANK] if FASTNLP_GLOBAL_RANK in os.environ else None
