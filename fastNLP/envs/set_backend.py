...
from fastNLP.envs.env import (FASTNLP_BACKEND, FASTNLP_GLOBAL_RANK,
                              FASTNLP_GLOBAL_SEED, USER_CUDA_VISIBLE_DEVICES)
...
def _set_backend():
    """根据环境变量或者默认配置文件设置 backend 。
     ...
     backend 为 paddle 时，我们还将设置部分环境变量以使得 paddle 能够在 fastNLP 中
     正确运行。
            ...
            try:
                ...
                if FASTNLP_BACKEND in envs:
                    ...
                    if int(os.environ.get(FASTNLP_GLOBAL_RANK, 0)) == 0:
                        print(f'Set fastNLP backend as {backend} '
                              f'based on {env_path}.')
    ...
    if backend:
        assert backend in SUPPORT_BACKENDS, \
            'Right now fastNLP only support the following backends: ' \
            f'{SUPPORT_BACKENDS}, instead of `{backend}`'

    if backend == 'paddle':
        assert _module_available(
            backend
        ), f'You must have {backend} available to use {backend} backend.'
        assert 'paddle' not in sys.modules, \
            'You have to use `set_backend()` before `import paddle`.'
        user_visible_devices = os.getenv(USER_CUDA_VISIBLE_DEVICES)
        cuda_visible_devices = os.getenv('CUDA_VISIBLE_DEVICES')
        if 'PADDLE_RANK_IN_NODE' in os.environ and \
                'FLAGS_selected_gpus' in os.environ:
            ...
            if user_visible_devices is not None:
                ...
                # 此时经过 set_backend，用户的设置会保存在
                # USER_CUDA_VISIBLE_DEVICES 中
                # 我们需要从中转换为用户找到真正使用的设备编号
                user_visible_devices = user_visible_devices.split(',')
                selected_gpus = [
                    user_visible_devices[int(i)] for i in selected_gpus
                ]
            # 没有找到 USER_CUDA_VISIBLE_DEVICES，说明用户是直接用 launch 启动的
                ...
                selected_gpus = [
                    cuda_visible_devices[int(i)] for i in selected_gpus
                ]
            else:
                # 用户没有设置可见设备，则赋值成所有的设备
                os.environ[USER_CUDA_VISIBLE_DEVICES] = ','.join(
                    map(str, list(range(get_gpu_count()))))
            os.environ['CUDA_VISIBLE_DEVICES'] = ','.join(selected_gpus)
            os.environ['FLAGS_selected_gpus'] = ','.join(
                [str(g) for g in range(len(selected_gpus))])
            os.environ['FLAGS_selected_accelerators'] = ','.join(
                [str(g) for g in range(len(selected_gpus))])
            ...
            os.environ[USER_CUDA_VISIBLE_DEVICES] = CUDA_VISIBLE_DEVICES
            os.environ['CUDA_VISIBLE_DEVICES'] = CUDA_VISIBLE_DEVICES.split(
                ',')[0]
        else:
    ...
    if 'PADDLE_RANK_IN_NODE' in os.environ and \
            ...
            'USER_CUDA_VISIBLE_DEVICES' not in os.environ:
...
def set_env(global_seed=None):
    r"""set_env 用于显式告知 fastNLP 将要使用的相关环境变量是什么，必须在代码最开端
    运行。以下的环境变量设置，优先级分别为：（1）在代码开始的位置显式调用设置；（2）通
    过环境变量注入的；（3）通过读取配置文件（如果有）。

    :param backend: 目前支持的 backend 有 torch, jittor, paddle 。设置特定的
        backend 后，fastNLP 将不再加载其它 backend ，可以提高加载速度。该值对应环境
        变量中的 FASTNLP_BACKEND 。
    :param int global_seed: 对应环境变量为 FASTNLP_GLOBAL_SEED 。设置 fastNLP 的
        全局随机数。
    :param str log_level: 可选 ['INFO','WARNING', 'DEBUG', 'ERROR'] ，对应环境变
        量为 FASTNLP_LOG_LEVEL 。
    :return:
    """
...
def dump_fastnlp_backend(default: bool = False, backend=None):
    ...
    果有）读取环境变量。不过这些变量的优先级低于代码运行时的环境变量注入。

    会保存的环境变量为 FASTNLP_BACKEND 。

    :param default:
    :param backend: 保存使用的 backend 为哪个值，允许的值有 ['torch', 'paddle',
        'jittor']。如果为 None ，则使用环境变量中的值。
    :return:
    """
    if int(os.environ.get(FASTNLP_GLOBAL_RANK, 0)) == 0:
        if default:
            env_path = os.path.join(
                os.path.expanduser('~'), '.fastNLP', 'envs', 'default.json')
        elif 'CONDA_DEFAULT_ENV' in os.environ:
            env_path = os.path.join(
                os.path.expanduser('~'), '.fastNLP', 'envs',
                os.environ['CONDA_DEFAULT_ENV'] + '.json')
        else:
            raise RuntimeError('Did not found `CONDA_DEFAULT_ENV` in your '
        ...
        if backend is None:
            ...
            assert backend in SUPPORT_BACKENDS, \
                f'fastNLP only supports {SUPPORT_BACKENDS} right now.'
        ...
        if len(envs):
            ...
            print(
                f'Writing the default fastNLP backend:{envs[FASTNLP_BACKEND]} '
                f'to {env_path}.')
        else:
