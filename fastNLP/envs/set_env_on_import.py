# 本文件主要用于在分布式启动的情况下，各个backend应该可以提前确定
# FASTNLP_GLOBAL_RANK（例如根据环境变量的中RANK值）。
# 注意！仅有当确定我们的训练是分布式训练时，我们才会将 FASTNLP_GLOBAL_RANK 注入到
# 环境变量中；
import datetime
import os
import sys
...
def remove_local_rank_in_argv():
    r"""通过 torch.distributed.launch 启动的时候，如果没有加入参数 --use_env ，
    pytorch 会默认通过 rank 注入 rank，这就 要求代码中必须有能够 parse rank 的
    parser，这里将 rank 删除掉，防止后续报错。
...
def set_env_on_import_torch():
    if 'WORLD_SIZE' in os.environ and 'LOCAL_RANK' in os.environ and \
            'RANK' in os.environ:
    ...
    if 'WORLD_SIZE' in os.environ \
            and 'LOCAL_RANK' in os.environ and 'RANK' in os.environ \
            and FASTNLP_DISTRIBUTED_CHECK not in os.environ:
...
def set_env_on_import_paddle():
    if 'PADDLE_TRAINERS_NUM' in os.environ \
            and 'PADDLE_TRAINER_ID' in os.environ \
...
def set_env_on_import_oneflow():
    ...
    if 'GLOG_log_dir' in os.environ and \
...
def set_env_on_import():
    ...
    if FASTNLP_LAUNCH_TIME not in os.environ:
        cur_time = datetime.datetime.now().strftime('%Y-%m-%d-%H_%M_%S_%f')
