...
class EmbedLoader:
    ...
    @staticmethod
    def load_with_vocab(embed_filepath: str,
                        vocab,
                        dtype=np.float32,
                        padding: str = '<pad>',
                        unknown: str = '<unk>',
        ...
        embedding。:class:`EmbedLoader` 将自动判断 ``embed_filepath`` 是
        **word2vec** （第一行只有两个元素） 还是 **glove** 格式的数据。

        :param embed_filepath: 预训练的 embedding 的路径。
        :param vocab: 词表 :class:`~fastNLP.core.Vocabulary` 类型，读取出现在
            ``vocab`` 中的词的 embedding。没有出现在 ``vocab`` 中的词的
            embedding 将通过找到的词的 embedding 的 *正态分布* 采样出来，以使得整
            个 Embedding 是同分布的。
        ...
        :param error: 可以为以下值之一： ``['ignore', 'strict']``。如果为
            ``ignore``，错误将自动跳过；如果是 ``strict``，错误将抛出。这里主要可能
            出错的地方在于词表有空行或者词表出现了维度不一致。
        :param init_method: 用于初始化 embedding 的函数。该函数接受一个
            :class:`numpy.ndarray` 类型，返回 :class:`numpy.ndarray`。
        :return: 返回类型为 :class:`numpy.ndarray`，形状为 ``[len(vocab),
            dimension]``，其中 *dimension*由预训练的 embedding 决定。
        """
        ...
        with open(embed_filepath, 'r', encoding='utf-8') as f:
            ...
            for idx, line in enumerate(f, start_idx):
                try:
                    ...
                    if word in vocab:
                        ...
                        matrix[index] = np.fromstring(
                            ' '.join(nums), sep=' ', dtype=dtype, count=dim)
            ...
            if init_method is None:
                ...
                if len(found_vectors) != 0:
                    ...
                    r_vecs = np.random.randn(unfound_vec_num,
                                             dim).astype(dtype) * std + mean
                    matrix[hit_flags == False] = r_vecs  # noqa: E712
    ...
    @staticmethod
    def load_without_vocab(embed_filepath: str,
                           dtype=np.float32,
                           padding: str = '<pad>',
                           unknown: str = '<unk>',
                           normalize: bool = True,
                           error: str = 'ignore'):
        r"""
        从 ``embed_filepath`` 中读取预训练的 word vector。根据预训练的词表读取
        embedding 并生成一个对应的 :class:`~fastNLP.core.Vocabulary`。
        ...
        :param error: 可以为以下值之一： ``['ignore', 'strict']``。如果为
            ``ignore``，错误将自动跳过；如果是 ``strict``，错误将抛出。这里主要可能
            出错的地方在于词表有空行或者词表出现了维度不一致。
        :return: 返回两个结果，第一个返回值为 :class:`numpy.ndarray`，大小为
            ``[词表大小+x, 词表维度]``。``词表大小+x`` 是由于最终的大小还取决于是否
            使用 ``padding``，以及 ``unknown`` 有没有在词表中找到对应的词。第二个返
            回值为 :class:`~fastNLP.core.Vocabulary` 类型的词表，其中词的顺序与
            embedding 的顺序是一一对应的。
        """
        ...
        with open(embed_filepath, 'r', encoding='utf-8') as f:
            ...
            if dim == -1:
                raise RuntimeError(
                    '{} is an empty file.'.format(embed_filepath))
            matrix = np.random.randn(len(vocab), dim).astype(dtype)
            ...
            if ((unknown is not None) and
                (not found_unknown)) or ((padding is not None) and
                                         (not found_pad)):
                start_idx = 0
                ...
                if (unknown is not None) and (not found_unknown):
                    matrix[
                        start_idx -
                        1] = np.random.randn(1, dim).astype(dtype) * std + mean
                if (padding is not None) and (not found_pad):
                    matrix[0] = np.random.randn(1,
                                                dim).astype(dtype) * std + mean
