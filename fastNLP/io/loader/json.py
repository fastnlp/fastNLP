...
class JsonLoader(Loader):
    ...
    :param fields: 需要读入的 json 属性名称，和读入后在 :class:`~fastNLP.core.
        ...
        **属性名**，``fields`` 的 `value` 为读入后在 ``DataSet`` 存储的
        ...
        json 对象对应属性同名。``fields`` 可为 ``None``，这时 json 对象所有属性都
        保存在 ``DataSet`` 中。
    :param dropna: 是否忽略非法数据，若为 ``True`` 则忽略；若为 ``False`` 则在遇到
        非法数据时抛出 :class:`ValueError`。
    """

    def __init__(self, fields: Optional[dict] = None, dropna=False):
    ...
    def _load(self, path):
        ds = DataSet()
        for idx, d in _read_json(
                path, fields=self.fields_list, dropna=self.dropna):
            if self.fields:
