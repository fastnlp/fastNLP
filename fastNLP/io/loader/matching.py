...
class MNLILoader(Loader):
    ...
    **MNLI** 数据集的 **Loader**。
    ...
    读取之后的 :class:`~fastNLP.core.DataSet` 中包含以下的内容： ``raw_words``
    是 ``sentence1``，``raw_words2`` 是 ``sentence2``，``target`` 是
    ``gold_label``。测试集中没有 ``target`` 列。
    ...
    def _load(self, path: str):
        ...
        with open(path, 'r', encoding='utf-8') as f:
            f.readline()  # 跳过header
            if path.endswith('test_matched.tsv') or path.endswith(
                    'test_mismatched.tsv'):
                ...
                for line in f:
                    ...
                    if line:
                        ...
                        if raw_words1 and raw_words2:
                            ds.append(
                                Instance(
                                    raw_words1=raw_words1,
                                    raw_words2=raw_words2,
                                    index=idx))
            else:
                for line in f:
                    line = line.strip()
    ...
    def load(self, paths: Optional[str] = None):  # type: ignore[override]
        r"""
        :param paths: 传入数据所在目录，会在该目录下寻找 ``dev_matched.tsv``,
            ``dev_mismatched.tsv``, ``test_matched.tsv``, ``test_mismatched.
            tsv``, ``train.tsv`` 文件。
        ...
        if paths:
            ...
            paths = self.download()
        if not os.path.isdir(paths):  # type: ignore[arg-type]
            ...
            if not os.path.isfile(filepath):
                if 'test' not in name:
                    raise FileNotFoundError(
                        f'{name} not found in directory {filepath}.')
...
class SNLILoader(JsonLoader):
    ...
    **SNLI** 数据集的 **Loader**。
    ...
    def load(  # type: ignore[override]
        ...
        if paths:
            if os.path.isdir(paths):
                if not os.path.isfile(
                        os.path.join(paths, 'snli_1.0_train.jsonl')):
                    raise FileNotFoundError(
                        f'snli_1.0_train.jsonl is not found in {paths}')
        ...
        datasets = {name: self._load(path) for name, path in _paths.items()}
        data_bundle = DataBundle(datasets=datasets)
        return data_bundle
...
class QNLILoader(JsonLoader):
    ...
    **QNLI** 数据集的 **Loader**。
    ...
    读取数据的格式为：第一行为标题（具体内容会被忽略），之后每一行是一个 sample，由
    **index** 、**问题** 、**句子** 和 **标签** 构成（以制表符分割），数据的结构如
    下::
    ...
    加载的 :class:`~fastNLP.core.DataSet` 将具备以下的内容： ``raw_words1`` 是
    ``question``，``raw_words2`` 是 ``sentence``，``target`` 是 ``label``。测试
    集中没有 ``target`` 列。

    .. csv-table::
        :header: "raw_words1", "raw_words2", "target"

        "What came into force after the new...", "As of that day...",
        "entailment"
        "...","."
...
class RTELoader(Loader):
    ...
    **RTE** 数据集的 **Loader**。
    ...
    读取数据的格式为：第一行为标题（具体内容会被忽略），之后每一行是一个 sample，由
    **index** 、**句子1** 、**句子2** 和 **标签** 构成（以制表符分割），数据结构如
    下::
    ...
    读取的 :class:`~fastNLP.core.DataSet` 将具备以下的内容：``raw_words1`` 是
    ``sentence1``，``raw_words2`` 是 ``sentence2``，``target`` 是 ``label``。
    测试集中没有 ``target`` 列。
    ...
    def _load(self, path: str):
        ...
        for i in ds_all:
            if i['language'] == 'zh':
                ds_zh.append(
                    Instance(
                        raw_chars1=i['sentence1'],
                        raw_chars2=i['sentence2'],
                        target=i['gold_label']))

        return ds_zh

    def _load_train(self, path: str):
        ds = DataSet()
        ...
        with open(path, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                premise = ''.join(raw_instance[0].split(
                ))  # 把已经分好词的premise和hypo强制还原为character segmentation
                hypo = ''.join(raw_instance[1].split())
                label = ''.join(raw_instance[-1].split())
                if premise:
                    ds.append(
                        Instance(premise=premise, hypo=hypo, label=label))

        ds.rename_field('label', 'target')
        ds.rename_field('premise', 'raw_chars1')
        ds.rename_field('hypo', 'raw_chars2')
        ds.apply(
            lambda i: ''.join(i['raw_chars1'].split()),
            new_field_name='raw_chars1')
        ds.apply(
            lambda i: ''.join(i['raw_chars2'].split()),
            new_field_name='raw_chars2')
        return ds

    def load(self,
             paths: Union[str, Dict[str, str], None] = None) -> DataBundle:
        """从指定一个或多个路径中的文件中读取数据，返回 :class:`~fastNLP.io.DataBundle`。读取的 field 根据
        :class:`SNLILoader` 初始化时传入的 ``fields`` 决定。
        ...
        with open(path, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                if raw_chars1:
                    ds.append(
                        Instance(
                            raw_chars1=raw_chars1,
                            raw_chars2=raw_chars2,
                            target=target))
        return ds
