...
__all__ = [
    'ConllLoader', 'Conll2003Loader', 'Conll2003NERLoader',
    'OntoNotesNERLoader', 'CTBLoader', 'CNNERLoader', 'MsraNERLoader',
    'WeiboNERLoader', 'PeopleDailyNERLoader'
]
...
class ConllLoader(Loader):
    ...
    的每一行用空格或者制表符隔开不同的元素。如下例所示::
        ...
        # 如果用以下的参数读取，返回的 DataSet 将包含 raw_words 和 pos 两个 field
        # 这两个 field 的值分别取自于第 0 列与第 1 列
        dataset = ConllLoader(
            headers=['raw_words', 'pos'],
            indexes=[0, 1])._load('/path/to/train.conll')
        # 如果用以下的参数读取，返回的 DataSet 将包含 raw_words 和 ner 两个 field
        # 这两个 field 的值分别取自于第 0 列与第 2 列
        dataset = ConllLoader(
            headers=['raw_words', 'ner'],
            indexes=[0, 3])._load('/path/to/train.conll')
        # 如果用以下的参数读取，返回的 DataSet 将包含 raw_words, pos 和 ner
        # 三个field
        dataset = ConllLoader(
            headers=['raw_words', 'pos', 'ner'],
            indexes=[0, 1, 3])._load('/path/to/train.conll')

    :class:`ConllLoader` 返回的 :class:`~fastNLP.core.DataSet` 的 `field` 由传入
    的 ``headers`` 确定。

    :param headers: 每一列数据的名称，``header`` 与 ``indexes`` 一一对应
    :param sep: 指定分隔符，默认为制表符
    :param indexes: 需要保留的数据列下标，从 **0** 开始。若为 ``None``，则所有列都
        保留。
    :param dropna: 是否忽略非法数据，若为 ``False``，则遇到非法数据时抛出
        :class:`ValueError`。
    ...
    def __init__(self,
        ...
        if not isinstance(headers, (list, tuple)):
            raise TypeError(
                'invalid headers: {}, should be list of strings'.format(
                    headers))
    ...
    def _load(self, path):
        r"""
        传入的一个文件路径，将该文件读入 DataSet 中，field 由 ConllLoader 初始化时
        指定的 headers 决定。
...
class Conll2003NERLoader(ConllLoader):
    r"""
    用于读取 **conll2003** 任务的 NER 数据。每一行有 4 列内容，空行意味着隔开两个句
    子。
    ...
    .. csv-table:: 下面是 Conll2003Loader 加载后数据的结构，target 是 BIO2 编码
...
class OntoNotesNERLoader(ConllLoader):
    ...
    def _load(self, path: str):
        ...
        dataset.apply_field(
            convert_word, field_name='raw_words', new_field_name='raw_words')
        dataset.apply_field(
            convert_to_bio, field_name='target', new_field_name='target')

        return dataset

    def download(self):
        raise RuntimeError(
            'Ontonotes cannot be downloaded automatically, you can refer '
...
class CNNERLoader(Loader):
    ...
    def _load(self, path: str):
        ...
        :param path: 文件路径
        :return: :class:`~fastNLP.core.DataSet`，包含 ``raw_words`` 列和
            ``target`` 列
        """
        ds = DataSet()
...
class MsraNERLoader(CNNERLoader):
    ...
    def download(self,
        ...
        下载完成后在 ``output_dir`` 中有 ``train.conll``，``test.conll``，
        ``dev.conll`` 三个文件。如果 ``dev_ratio`` 为 0，则只有 ``train.conll``
        和 ``test.conll``。
        ...
        if not os.path.exists(os.path.join(data_dir, 'dev.conll')):
            if dev_ratio > 0:
                ...
                try:
                    ...
                    os.renames(
                        os.path.join(data_dir, 'middle_file.conll'),
                        os.path.join(data_dir, 'train.conll'))
                finally:
                    if os.path.exists(
                            os.path.join(data_dir, 'middle_file.conll')):
