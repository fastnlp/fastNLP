...
class CMRC2018Loader(Loader):
    ...
    **CMRC2018** 数据集的 **Loader**，如果您使用了本数据，请引用
    ...
    请直接使用从 **fastNLP** 下载的数据进行处理。该数据集未提供测试集，测试需要通过上
    传到对应的系统进行评测。

    读取之后训练集 :class:`~fastNLP.core.DataSet` 将具备以下的内容，每个问题的答案
    只有一个：

    .. csv-table::
       :header: "title", "context", "question", "answers", "answer_starts",
       "id"
    ...
    其中 ``title`` 是文本的标题，多条记录可能是相同的 ``title``；``id`` 是该问题的
    id，具备唯一性。

    验证集 :class:`~fastNLP.core.DataSet` 将具备以下的内容，每个问题的答案可能有三
    个（有时候只是3个重复的答案）：
    ...
    其中 ``answer_starts`` 是从 0 开始的 index。例如 ``"我来自a复旦大学？"``，其
    中 ``"复"`` 的开始 index 为 **4**。另外 ``"Russell评价说"`` 中的 ``"说"`` 的
    index 为 **9**，因为英文和数字都直接按照 character 计量的。
    """
