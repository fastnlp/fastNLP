...
class DataBundle:
    r"""
    **fastNLP** 提供的用于方便快捷地管理多个数据集的工具，并有诸多接口来进行批量的数
    据处理。

    包括一系列数据集（比如：分开的训练集、验证集和测试集）以及各个 field 对应的
    vocabulary。该对象一般由 fastNLP 中各种 :class:`.Loader` 的 :meth:`.Loader.\
    load` 函数生成，可以通过以下的方法获取里面的内容::

        data_bundle = YelpLoader().load(
            {'train':'/path/to/train', 'dev': '/path/to/dev'})
        train_vocabs = data_bundle.vocabs['train']
        train_data = data_bundle.datasets['train']
        dev_data = data_bundle.datasets['train']

    :param vocabs: 从名称（字符串）到 :class:`.Vocabulary` 类型的字典。
    :param datasets: 从名称（字符串）到 :class:`.DataSet` 类型的字典。建议不要将相
        同的 :class:`.DataSet` 对象重复传入，否则可能会在使用 :class:`.Pipe` 处理
        数据的时候遇到问题，若多个数据集确需一致，请手动 ``deepcopy`` 后传入。
    """
    ...
    def set_vocab(self, vocab: Vocabulary, field_name: str):
        ...
        assert isinstance(vocab,
                          Vocabulary), 'Only fastNLP.core.Vocabulary supports.'
    ...
    def set_dataset(self, dataset: DataSet, name: str):
        ...
        :param dataset: 传递给 :class:`DataBundle` 的 :class:`.DataSet`
    ...
    def copy_field(self,
                   field_name: str,
                   new_field_name: str,
                   ignore_miss_dataset: bool = True):
        r"""
        将所有的 dataset 中名为 ``field_name`` 的 Field 复制一份并命名为
            ``new_field_name``。

        :param field_name:
        :param new_field_name:
        :param ignore_miss_dataset: 如果为 ``True``，则当 ``field_name`` 在某个
            dataset 内不存在时，直接忽略该 dataset，如果为 ``False`` 则会报错。
        :return: self
        """
    ...
    def rename_field(self,
                     ...
                     new_field_name: str,
                     ignore_miss_dataset: bool = True,
                     rename_vocab: bool = True):
        r"""
        将所有的 dataset 中名为 ``field_name`` 的 Field 重命名为
        ``new_field_name``。
        ...
        :param ignore_miss_dataset: 如果为 ``True``，则当 ``field_name`` 在某个
            dataset 内不存在时，直接忽略该 dataset，如果为 ``False`` 则会报错。
        :param rename_vocab: 如果该 ``field_name`` 同时也存在于 vocabs 中，则也会
            进行重命名
        :return: self
        """
        for name, dataset in self.datasets.items():
    ...
    def delete_field(self,
        ...
        :param ignore_miss_dataset: 如果为 ``True``，则当 ``field_name`` 在某个
            dataset 内不存在时，直接忽略该 dataset，如果为 ``False`` 则会报错。
        :param delete_vocab: 如果该 ``field_name`` 也在 vocabs 中存在，则也会删
            除。
        :return: self
        """
        for name, dataset in self.datasets.items():
    ...
    def iter_datasets(self) -> Iterable[Tuple[str, DataSet]]:
        r"""
        迭代 dataset
    ...
    def apply_field_more(self,
        ...
        :param func: 对指定 field 进行处理的函数，注意其输入应为 ``instance`` 中名
            为 ``field_name`` 的 field 的内容；返回值是一个字典，key 是field 的名
            字，value 是对应的结果
        :param field_name: 传入 ``func`` 的 field 名称；
        ...
        :param progress_desc: 如果不为 ``None``，则会显示当前正在处理的进度条的名
            称；
        :param progress_bar: 显示进度条的方式，支持 ``["rich", "tqdm", None]``。
        :return: 一个字典套字典，第一层的 key 是 dataset 的名字，第二层的 key 是
            field 的名字
        """
        res = {}
        _progress_desc = progress_desc
    ...
    def apply_more(self,
        ...
        .. note::
            ...
            中关于 ``apply_more`` 与 ``apply`` 区别的介绍。

        :param func: 参数是 ``DataSet`` 中的 ``Instance``，返回值是一个字典，key
            是 field 的名字，value 是对应的结果
        :param modify_fields: 是否用结果修改 ``DataSet`` 中的 ``Field``，默认为
            ``True``
        :param num_proc: 使用进程的数量。
    ...
    def add_seq_len(self,
                    ...
                    ignore_miss_dataset: bool = True):
        r"""
        将使用 :func:`len` 直接对每个 dataset 的 ``field_name`` 中每个元素作用，将
        其结果作为 sequence length, 并放入 ``new_field_name`` 这个 field。
    ...
    def set_pad(self,
        ...
        :param field_name: 需要调整的 field 的名称。如果 :meth:`Dataset.\
            ...
            回的是 Sequence 类型，则可以使用 ``'_0'``, ``'_1'`` 表示序列中第
        ...
        :param pad_val: 这个 field 的默认 pad 值。如果设置为 ``None``，则表示该
            ...
            如果对应 field 本身就不是可以 pad 的形式，可以不用主动设置为 ``None``。
    ...
    def set_ignore(self, *field_names) -> 'DataBundle':
        r"""如果想要对所有 dataset 绑定的 collator 进行调整可以调用此函数。只有当
