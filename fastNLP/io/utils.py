...
__all__ = ['check_loader_paths']


def check_loader_paths(paths: Union[str, Dict[str, str]]) -> Dict[str, str]:
    r"""
    检查传入 ``dataloader`` 的文件的合法性。如果合法，将返回至少包含 ``'train'`` 这
    个 key 的字典。类似于下面的结果::
    ...
    如果 ``paths`` 为不合法的，将直接进行 raise 相应的错误。如果 ``paths`` 内不包
    含 ``'train'`` 也会报错。

    :param str paths: 路径。可以为：

            - 一个文件路径，此时认为该文件就是 train 的文件；
            - 一个文件目录，将在该目录下寻找包含 ``train`` （文件名中包含 train 这
              个字段），``test``，``dev`` 这三个字段的文件或文件夹；
            - 一个 dict, 则 key 是用户自定义的某个文件的名称，value 是这个文件的路
              径。
    :return:
    """
        ...
        if os.path.isfile(paths):
            ...
            for filename in filenames:
                ...
                if 'dev' in filename:
                    if path_pair:
                        raise Exception(
                            'Directory:{} in {} contains both `{}` and `dev`.'.
                            format(filename, paths, path_pair[0]))
                    path_pair = ('dev', filename)
                if 'test' in filename:
                    if path_pair:
                        raise Exception(
                            'Directory:{} in {} contains both `{}` and `test`.'
                            .format(filename, paths, path_pair[0]))
                    path_pair = ('test', filename)
                if path_pair:
                    if path_pair[0] in files:
                        raise FileExistsError(
                            f'Two files contain `{path_pair[0]}` were found '
                            f'in {paths}, please specify the filepath for `'
                            f'{path_pair[0]}`.')
        ...
        if paths:
            ...
            for key, value in paths.items():
                if isinstance(key, str) and isinstance(value, str):
                    ...
                    raise TypeError(
                        'All keys and values in paths should be str.')
            return paths
        else:
            raise ValueError('Empty paths is not allowed.')
    else:
        raise TypeError(
            f'paths only supports str and dict. not {type(paths)}.')
