...
PRETRAIN_MAP = {
    'elmo': PRETRAINED_ELMO_MODEL_DIR,
...
FASTNLP_EXTEND_EMBEDDING_URL = {
    'elmo': 'fastnlp_elmo_url.txt',
...
def cached_path(url_or_filename: str,
                ...
                name=None) -> Path:
    r"""
    给定一个 url，尝试通过 url 中解析出来的文件名字 filename 到路径
    ``{cache_dir}/{name}/{filename}`` 下寻找这个文件或文件夹：

    1. 如果 ``cache_dir`` 为 ``None``，则默认为 ``~/.fastNLP/``；
    2. 如果 ``name=None``，则没有中间的 {name} 这一层结构。

    如果有该文件，就直接返回路径；如果没有该文件，则尝试用传入的 url 下载，并将文件放
    入 ``cache_dir`` 中。

    :param url_or_filename: 文件的下载 url 或者文件名称。
    :param cache_dir: 文件的缓存文件夹。如果为 ``None``，将使 ``"~/.fastNLP`` 这个
        默认路径
    :param name: 中间一层的名称。如 embedding, dataset
    :return:
    """
    if cache_dir is None:
        data_cache = get_cache_path()
    else:
    ...
    elif parsed.scheme == '' and Path(
            os.path.join(data_cache, url_or_filename)).exists():
        # File, and it exists.
        return Path(os.path.join(data_cache, url_or_filename))
    elif parsed.scheme == '':
        # File, but it doesn't exist.
        raise FileNotFoundError('file {} not found in {}.'.format(
            url_or_filename, data_cache))
    else:
        # Something unknown
        raise ValueError(
...
def get_filepath(filepath):
    r"""
    如果 ``filepath`` 为文件夹且包含多个文件，则直接返回 ``filepath``；反之若只包含
    一个文件，返回该文件的路径 `{filepath}/{filename}``。
    ...
    if os.path.isdir(filepath):
        ...
        raise FileNotFoundError(
            f'{filepath} is not a valid file or directory.')
    ...
    if 'FASTNLP_CACHE_DIR' in os.environ:
        ...
        if os.path.isdir(fastnlp_cache_dir):
            ...
            raise NotADirectoryError(
                f"{os.environ['FASTNLP_CACHE_DIR']} is not a directory.")
...
def _get_embedding_url(embed_type, name):
    ...
    if embed_map:
        ...
        if filename:
            ...
            return url
        raise KeyError('There is no {}. Only supports {}.'.format(
            name, list(embed_map.keys())))
    else:
        raise KeyError(f'There is no {embed_type}. Only supports '
                       f'bert, elmo, static, gpt2, roberta')


def _read_extend_url_file(filename, name) -> Optional[str]:
    r"""
...
def _get_dataset_url(name, dataset_dir: Optional[dict] = None):
    r"""
...
def split_filename_suffix(filepath):
    r"""
    给定 ``filepath`` 返回对应的文件名和后缀。如果后缀是多个点，仅支持 **.tar.gz**
    类型
...
def get_from_cache(url: str, cache_dir: Path) -> Path:
    ...
    # TODO ETag可以用来判断资源是否已经更新了，之后需要加上
    # response = requests.head(url, headers={"User-Agent": "fastNLP"})
    # if response.status_code != 200:
    #     raise IOError(
    #         f"HEAD request failed for url {url} with status code '
        ...
        if req.status_code == 200:
            ...
            try:
                ...
                if suffix in ('.zip', '.tar.gz', '.gz'):
                    ...
                    logger.info(
                        f'Start to uncompress file to {uncompress_temp_dir}')
                    if suffix == '.zip':
                        unzip_file(
                            Path(temp_filename), Path(uncompress_temp_dir))
                    elif suffix == '.gz':
                    ...
                    if len(filenames) == 1:
                        if os.path.isdir(
                                os.path.join(uncompress_temp_dir,
                                             filenames[0])):
                            uncompress_temp_dir = os.path.join(
                                uncompress_temp_dir, filenames[0])
                ...
                if os.path.isdir(uncompress_temp_dir):
                    for filename in os.listdir(uncompress_temp_dir):
                        if os.path.isdir(
                                os.path.join(uncompress_temp_dir, filename)):
                            shutil.copytree(
                                os.path.join(uncompress_temp_dir, filename),
                                cache_path / filename)
                        else:
...
def match_file(dir_name: str, cache_dir: Path) -> str:
    ...
    for file_name in files:
        if re.match(dir_name + '$', file_name) or re.match(
                dir_name + '\\..*', file_name):
    ...
    if len(matched_filenames) == 0:
        ...
        raise RuntimeError(f'Duplicate matched files:{matched_filenames}, '
                           f'this should be caused by a bug.')
...
def _get_bert_dir(model_dir_or_name: str = 'en-base-uncased'):
    if model_dir_or_name.lower() in PRETRAINED_BERT_MODEL_DIR:
        ...
        model_dir = os.path.abspath(  # type: ignore
            os.path.expanduser(model_dir_or_name))
    else:
        logger.info(
            f'Cannot recognize BERT dir or name ``{model_dir_or_name}``.')
        raise ValueError(
            f'Cannot recognize BERT dir or name ``{model_dir_or_name}``.')
...
def _get_gpt2_dir(model_dir_or_name: str = 'en'):
    if model_dir_or_name.lower() in PRETRAINED_GPT2_MODEL_DIR:
        ...
        logger.info(
            f'Cannot recognize GPT2 dir or name ``{model_dir_or_name}``.')
        raise ValueError(
            f'Cannot recognize GPT2 dir or name ``{model_dir_or_name}``.')
    return str(model_dir)
...
def _get_roberta_dir(model_dir_or_name: str = 'en'):
    if model_dir_or_name.lower() in PRETRAINED_ROBERTA_MODEL_DIR:
        ...
        logger.info(
            f'Cannot recognize RoBERTa dir or name ``{model_dir_or_name}``.')
        raise ValueError(
            f'Cannot recognize RoBERTa dir or name ``{model_dir_or_name}``.')
    return str(model_dir)
...
def _get_file_name_base_on_postfix(dir_path, postfix):
    ...
    """
    files = list(
        filter(lambda filename: filename.endswith(postfix),
               os.listdir(os.path.join(dir_path))))
    if len(files) == 0:
        raise FileNotFoundError(
            f'There is no file endswith {postfix} file in {dir_path}')
    elif len(files) > 1:
        raise FileExistsError(
            f'There are multiple *{postfix} files in {dir_path}')
