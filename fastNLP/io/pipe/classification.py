...
from ..loader.classification import (AGsNewsLoader, ChnSentiCorpLoader,
                                     DBPediaLoader, IMDBLoader, MRLoader,
                                     NG20Loader, OhsumedLoader, R8Loader,
                                     R52Loader, SST2Loader, SSTLoader,
                                     THUCNewsLoader, WeiboSenti100kLoader,
                                     YelpFullLoader, YelpPolarityLoader)
from .pipe import Pipe
from .utils import (_add_chars_field, _add_words_field, _granularize,
                    _indexize, get_tokenizer)

__all__ = [
    'CLSBasePipe', 'AGsNewsPipe', 'DBPediaPipe', 'YelpFullPipe',
    'YelpPolarityPipe', 'SSTPipe', 'SST2Pipe', 'IMDBPipe', 'ChnSentiCorpPipe',
    'THUCNewsPipe', 'WeiboSenti100kPipe', 'MRPipe', 'R8Pipe', 'R52Pipe',
    'OhsumedPipe', 'NG20Pipe'
]
...
class CLSBasePipe(Pipe):
    ...
    :param tokenizer: 使用哪种 tokenize 方式将数据切成单词。支持 ``['spacy',
        'raw', 'cn-char']``。``'raw'`` 表示使用空格作为切分，``'cn-char'`` 表示按
        字符切分，``'spacy'`` 则使用 :mod:`spacy` 库进行分词。
    ...
    def process(self, data_bundle: DataBundle):
        ...
        # 进行tokenize
        data_bundle = self._tokenize(
            data_bundle=data_bundle, field_name='words')
        # 建立词表并index
...
class YelpFullPipe(CLSBasePipe):
    ...
    :param lower: 是否对输入进行小写化。
    :param granularity: 支持 ``[2, 3, 5]``。若为 ``2``，则认为是二分类问题，将
        **1、2** 归为一类，**4、5** 归为一类，丢掉 3；若为 ``3``，则认为是三分类问
        题，将 **1、2** 归为一类，**3** 归为一类，**4、5** 归为一类；若为 ``5``，则
        认为是五分类问题。
    :param tokenizer: 使用哪种 tokenize 方式将数据切成单词。支持 ``['spacy',
    ...
    def __init__(self,
                 ...
                 num_proc: int = 0):
        super().__init__(
            lower=lower, tokenizer=tokenizer, lang='en', num_proc=num_proc)
...
class SSTPipe(CLSBasePipe):
    ...
    .. csv-table:: 下面是使用SSTPipe处理后的DataSet所具备的field
        ...
        "It 's a lovely film with lovely perform...", 1, "[187, 6, 5, 132, 120, 70, 132, 188, 25...", 13
            ...
            for ins in dataset:
                ...
                if use_subtree:
                    for t in tree.subtrees():
                        ...
                        instance = Instance(
                            raw_words=raw_words, target=t.label())
                        ds.append(instance)
                else:
                    instance = Instance(
                        raw_words=' '.join(tree.leaves()), target=tree.label())
                    ds.append(instance)
...
class IMDBPipe(CLSBasePipe):
    ...
    其中 ``raw_words`` 为 :class:`str` 类型，是原文；``words`` 是转换为 index 的
    输入；``target`` 是转换为 index 的目标值。``words`` 列被设置为 input，
    ``target`` 列被设置为 target。
    ...
    def process(self, data_bundle: DataBundle):
        r"""
        ``data_bunlde`` 中的 :class:`~fastNLP.core.DataSet` 应该具备如下的结构：
        有两个 field ，``raw_words`` 和 ``target``，且均为 :class:`str` 类型。
        ...
        :param data_bundle: 传入的 DataBundle 中的 DataSet 必须包含 raw_words
            和 target 两个 field，且 raw_words 列应该为 str，target 列应该为
            str。
        :return:  处理后的 ``data_bundle``
        """
...
class ChnSentiCorpPipe(Pipe):
    ...
    :param bigrams: 是否增加一列 ``bigrams``。``bigrams`` 会对原文进行如下转化：
        ``['复', '旦', '大', '学', ...]->["复旦", "旦大", ...]``。如果设置为
        ``True``，返回的 `~fastNLP.core.DataSet` 将有一列名为 ``bigrams``，且已经
        转换为了 index 并设置为 input，对应的词表可以通过 ``data_bundle.get_vocab
        ('bigrams')`` 获取。
    :param trigrams: 是否增加一列 ``trigrams``。``trigrams`` 会对原文进行如下转化
        ``['复', '旦', '大', '学', ...]->["复旦大", "旦大学", ...]``。如果设置为
        ``True``，返回的 `~fastNLP.core.DataSet` 将有一列名为 ``trigrams``，且已
        经转换为了 index 并设置为 input，对应的词表可以通过 ``data_bundle.
        get_vocab('trigrams')`` 获取。
    :param num_proc: 处理数据时使用的进程数目。
    """
    ...
    def _tokenize(self, data_bundle):
        r"""
        将 DataSet 中的"复旦大学"拆分为 ["复", "旦", "大", "学"] . 未来可以通过扩
        展这个函数实现分词。
        ...
        data_bundle.apply_field(
            list, field_name='chars', new_field_name='chars')
        return data_bundle
...
class THUCNewsPipe(CLSBasePipe):
    ...
    def process(self, data_bundle: DataBundle):
        ...
        data_bundle = _indexize(
            data_bundle=data_bundle, input_field_names='chars')
    ...
    def process_from_file(self, paths=None):
        ...
        )  # 此处需要实例化一个data_loader，否则传入load()的参数为None
...
class WeiboSenti100kPipe(CLSBasePipe):
    ...
    def process_from_file(self, paths=None):
        ...
        :return:
        """
        data_loader = WeiboSenti100kLoader(
        )  # 此处需要实例化一个data_loader，否则传入load()的参数为None
        data_bundle = data_loader.load(paths)
        data_bundle = self.process(data_bundle)
        return data_bundle
