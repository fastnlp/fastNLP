...
from ..loader.matching import (BQCorpusLoader, CNXNLILoader, LCQMCLoader,
...
class MatchingBertPipe(Pipe):
    ...
    **Matching** 任务的 **Bert Pipe** 。
    ...
    ``words`` 列是将 ``raw_words1`` （即 ``premise`` ），``raw_words2`` （即
    ``hypothesis`` ）使用 ``[SEP]`` 链接起来转换为 index 的。``words`` 列被设置为
    input，``target`` 列被设置为 target 和 input （设置为 input 以方便在
    :func:`forward` 函数中计算 loss，如果不在也不影响，**fastNLP** 将根据
    :func:`forward` 函数的形参名进行传参）。

    :param lower: 是否对输入进行小写化。
    :param tokenizer: 使用哪种 tokenize 方式将数据切成单词。支持 ``['spacy',
        'raw']``。``'raw'`` 表示使用空格作为切分，``'spacy'`` 则使用
        :mod:`spacy` 库进行分词。
    ...
    def _tokenize(self, data_bundle, field_names, new_field_names):
        ...
        :param list new_field_names: List[str], tokenize之后field的名称，与
            field_names一一对应。
        :return: 输入的DataBundle对象
        """
        for name, dataset in data_bundle.iter_datasets():
            for field_name, new_field_name in zip(field_names,
                                                  new_field_names):
                dataset.apply_field(
                    self.tokenizer,
    ...
    def process(self, data_bundle: DataBundle):
        ...
        for name, dataset in data_bundle.iter_datasets():
            dataset.apply(
                concat, new_field_name='words', num_proc=self.num_proc)
        ...
        word_vocab.from_dataset(
            *[
                dataset for name, dataset in data_bundle.datasets.items()
                if 'train' in name
            ],
            field_name='words',
        ...
        word_vocab.index_dataset(
            *data_bundle.datasets.values(), field_name='words')

        target_vocab = Vocabulary(padding=None, unknown=None)
        target_vocab.from_dataset(
            *[
        ...
        if len(target_vocab._no_create_word) > 0:
            warn_msg = 'There are {} target labels in {} data set ' \
                       'but not in train data set!.'.format(
                           len(target_vocab._no_create_word),
                           [name for name in data_bundle.datasets.keys() if 'train' not in name])
...
class MatchingPipe(Pipe):
    ...
    **Matching** 任务的 Pipe。
    ...
    ``words1`` 是 ``premise``，``words2`` 是 ``hypothesis``。其中 ``words1`` ,
    ``words2`` , ``seq_len1``, ``seq_len2`` 被设置为 input；``target`` 列被设置
    为 target 和 input （设置为 input 以方便在 :func:`forward` 函数中计算 loss，如
    果不在也不影响，**fastNLP** 将根据 :func:`forward` 函数的形参名进行传参）。
    ...
    def __init__(self, lower=False, tokenizer: str = 'raw', num_proc: int = 0):
        ...
        :param bool lower: 是否将所有 raw_words 转为小写。
        :param str tokenizer: 将原始数据 tokenize 的方式。支持spacy，raw；spacy
            是使用 spacy 切分，raw 就是用空格切分。
        """
        super().__init__()
    ...
    def _tokenize(self, data_bundle, field_names, new_field_names):
        ...
        :param field_names: List[str], 需要 tokenize 的 field 名称
        :param new_field_names: List[str], tokenize 之后 field 的名称，与
            field_names 一一对应。
        :return: 输入的 DataBundle 对象
        """
        for name, dataset in data_bundle.iter_datasets():
            for field_name, new_field_name in zip(field_names,
                                                  new_field_names):
    ...
    def process(self, data_bundle: DataBundle):
        r"""
        ``data_bunlde`` 中的 :class:`~fastNLP.core.DataSet` 应该具备以下结构，可
        以没有 ``target`` 列：
        ...
        word_vocab.index_dataset(
            *data_bundle.datasets.values(), field_name=['words1', 'words2'])
        ...
        if (self.task == 'cn-nli'):
            for name, dataset in data_bundle.datasets.items():
                if (dataset.has_field('raw_chars1')):
                    dataset.rename_field('raw_chars1',
                                         'raw_words1')  # RAW_CHARS->RAW_WORDS
                    ...
                    raise RuntimeError(
                        'field name of dataset is not qualified. It should '
                        'have ether RAW_CHARS or WORDS')
            ...
            for name, dataset in data_bundle.datasets.items():
                if (dataset.has_field('raw_chars1')):
                    ...
                    raise RuntimeError(
                        'field name of dataset is not qualified. It should '
                        'have ether RAW_CHARS or RAW_WORDS')
        else:
            raise RuntimeError("Only support task='cn-nli' or 'cn-nli-bert'")

        return data_bundle


class GranularizePipe(Pipe):
    """将数据集中 ``target`` 列中的 tag 按照一定的映射进行重命名，并丢弃不在映射中 的 tag。

    :param task: 任务类型，目前仅支持 ``['XNLI']``。

        - ``'XNLI'`` -- 将  ``neutral``, ``entailment``, ``contradictory``,
          ``contradiction`` 分别映射为 0, 1, 2, 3；
    ...
    def _granularize(self, data_bundle, tag_map):
        ...
        :param data_bundle:
        :param dict tag_map: 将 target 列中的 tag 做以下的映射，比如{"0":0,
            "1":0, "3":1, "4":1}, 则会删除 target 为 "2" 的 instance，且将
            "1" 认为是第 0 类。
        :return: 传入的 data_bundle
        """
    ...
    def process(self, data_bundle: DataBundle):
        ...
        if self.task in task_tag_dict:
            data_bundle = self._granularize(
                data_bundle=data_bundle, tag_map=task_tag_dict[self.task])
        else:
            raise RuntimeError(
                f'Only support {task_tag_dict.keys()} task_tag_map.')
        return data_bundle
...
class TruncateBertPipe(Pipe):
    ...
    **250**，对于英文任务会分别截断至 **215**。
    ...
    :param task: 任务类型，可选 ``['cn', 'en']``，分别表示 **中文任务** 和
        **英文任务**。
    :param num_proc: 处理数据时使用的进程数目。
    """
    ...
    def _truncate(self, sentence_index: list, sep_index_vocab):
        ...
        # 注意此部分包括了[SEP]
        words_after_sep = sentence_index[sep_index_words:]
        if self.task == 'cn':
    ...
    def process(self, data_bundle: DataBundle) -> DataBundle:
        ...
        for name in data_bundle.datasets.keys():
            ...
            dataset.apply_field(
                partial(self._truncate, sep_index_vocab=sep_index_vocab),
                field_name='words',
