...
class ExtCNNDMPipe(Pipe):
    ...
    def process(self, data_bundle: DataBundle):
        ...
        data_bundle.apply_field(
            _lower_text,
            field_name='summary',
            new_field_name='summary',
            num_proc=self.num_proc)
        data_bundle.apply_field(
            _split_list,
            field_name='text',
            new_field_name='text_wd',
            num_proc=self.num_proc)
        # data_bundle.apply(lambda x: _lower_text(
        #     x['text']), new_field_name='text')
        # data_bundle.apply(lambda x: _lower_text(
        #     x['summary']), new_field_name='summary')
        # data_bundle.apply(lambda x: _split_list(
        #     x['text']), new_field_name='text_wd')
        data_bundle.apply(
            lambda x: _convert_label(x['label'], len(x['text'])),
            new_field_name='target')

        data_bundle.apply_field(
            partial(_pad_sent, sent_max_len=self.sent_max_len),
            field_name='text_wd',
            new_field_name='words',
            num_proc=self.num_proc)
        # data_bundle.apply(lambda x: _pad_sent(
        #     x["text_wd"], self.sent_max_len),
        #     new_field_name='words')
        # db.apply(lambda x: _token_mask(
        #     x["text_wd"], self.sent_max_len),
        ...
        data_bundle.apply_field(
            partial(
                _pad_doc,
                sent_max_len=self.sent_max_len,
                doc_max_timesteps=self.doc_max_timesteps),
            field_name='words',
            new_field_name='words',
            num_proc=self.num_proc)
        data_bundle.apply_field(
            partial(_sent_mask, doc_max_timesteps=self.doc_max_timesteps),
            field_name='words',
            new_field_name='seq_len',
            num_proc=self.num_proc)
        data_bundle.apply_field(
            partial(_pad_label, doc_max_timesteps=self.doc_max_timesteps),
            field_name='target',
            new_field_name='target',
            num_proc=self.num_proc)
        # data_bundle.apply(lambda x: _pad_doc(
        #     x['words'], self.sent_max_len, self.doc_max_timesteps),
        #     new_field_name='words')
        # data_bundle.apply(lambda x: _sent_mask(
        #     x['words'], self.doc_max_timesteps), new_field_name='seq_len')
        # data_bundle.apply(lambda x: _pad_label(
        #     x['target'], self.doc_max_timesteps), new_field_name='target')
        ...
        with open(self.vocab_path, 'r', encoding='utf8') as vocab_f:
            ...
            for line in vocab_f:
                ...
                if cnt > self.vocab_size:
                    break
        vocabs = Vocabulary(
            max_size=self.vocab_size, padding=WORD_PAD, unknown=WORD_UNK)
        ...
        if self.domain is True:
            ...
            domaindict.from_dataset(
                data_bundle.get_dataset('train'), field_name='publication')
    ...
    def process_from_file(self, paths=None):
        ...
        for ds in db.datasets.values():
            db.get_vocab('vocab').index_dataset(
                ds, field_name='words', new_field_name='words')
