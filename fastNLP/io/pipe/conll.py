...
from ..loader.conll import (Conll2003NERLoader, ConllLoader, MsraNERLoader,
                            OntoNotesNERLoader, PeopleDailyNERLoader,
                            WeiboNERLoader)
from .pipe import Pipe
from .utils import (_add_chars_field, _add_words_field, _indexize, iob2,
                    iob2bioes)

__all__ = [
    'Conll2003NERPipe', 'Conll2003Pipe', 'OntoNotesNERPipe', 'MsraNERPipe',
    'PeopleDailyPipe', 'WeiboNERPipe'
]


class _NERPipe(Pipe):
    r"""
    NER 任务的处理 Pipe, 该 Pipe 会（1）复制 raw_words 列，并命名为 words; (2）在
    words, target 列建立词表(创建 :class:`fastNLP.Vocabulary` 对象，所以在返回的
    DataBundle 中将有两个 Vocabulary); (3）将 words，target 列根据相应的
    Vocabulary 转换为 index。

    raw_words 列为 List[str], 是未转换的原始数据; words 列为 List[int]，是转换为
    index 的输入数据; target 列是 List[int]，是转换为 index 的 target。返回的
    DataSet 中被设置为 input 有 words, target, seq_len; 设置为 target 有
    target, seq_len。
    """
    ...
    def __init__(self,
        ...
        :param: str encoding_type: target 列使用什么类型的 encoding 方式，支持
            bioes, bio两种。
        :param bool lower: 是否将 words 小写化后再建立词表，绝大多数情况都不需要设
            置为 True。
        """
    ...
    def process(self, data_bundle: DataBundle) -> DataBundle:
        ...
        :param data_bundle: 传入的 DataBundle 中的 DataSet 必须包含 raw_words 和
            ner 两个 field，且两个 field 的内容均为 List[str] 在传入 DataBundle
            基础上原位修改。
...
class Conll2003NERPipe(_NERPipe):
    ...
    **Conll2003** 的 **NER** 任务的处理 **Pipe**，该Pipe会：
    ...
    ``raw_words`` 列为 :class:`List` [ :class:`str` ], 是未转换的原始数据；
    ``words`` 列为 :class:`List` [ :class:`int` ]，是转换为 index 的输入数据；
    ``target`` 列是 :class:`List` [ :class:`int` ] ，是转换为 index 的 target。
    返回的 :class:`~fastNLP.core.DataSet` 中被设置为 input 有 ``words`` ,
    ``target``, ``seq_len``；target 有 ``target``。

    :param encoding_type: ``target`` 列使用什么类型的 encoding 方式，支持
        ``['bioes', 'bio']`` 两种。
    :param lower: 是否将 ``words`` 小写化后再建立词表，绝大多数情况都不需要设置为
        ``True``。
...
class Conll2003Pipe(Pipe):
    ...
    :param chunk_encoding_type: ``chunk`` 列使用什么类型的 encoding 方式，支持
        ``['bioes', 'bio']`` 两种。
    :param ner_encoding_type: ``ner`` 列使用什么类型的 encoding 方式，支持
        ``['bioes', 'bio']`` 两种。
    :param lower: 是否将 ``words`` 小写化后再建立词表，绝大多数情况都不需要设置为
        ``True``。
    :param num_proc: 处理数据时使用的进程数目。
    ...
    def __init__(self,
                 chunk_encoding_type: str = 'bioes',
                 ner_encoding_type: str = 'bioes',
                 lower: bool = False,
                 num_proc: int = 0):
        if chunk_encoding_type == 'bio':
            ...
            raise ValueError(
                'chunk_encoding_type only supports `bio` and `bioes`.')
        if ner_encoding_type == 'bio':
            ...
            raise ValueError(
                'ner_encoding_type only supports `bio` and `bioes`.')
        self.lower = lower
        self.num_proc = num_proc
    ...
    def process(self, data_bundle) -> DataBundle:
        ...
        for name, dataset in data_bundle.datasets.items():
            ...
            dataset.apply_field(
                self.chunk_convert_tag,
                field_name='chunk',
                new_field_name='chunk',
                num_proc=self.num_proc)
            dataset.apply_field(
                self.ner_convert_tag,
                field_name='ner',
                new_field_name='ner',
                num_proc=self.num_proc)
        ...
        _indexize(
            ...
            target_field_names=['pos', 'ner'])
        # chunk中存在一些tag只在dev中出现，没在train中
        tgt_vocab = Vocabulary(unknown=None, padding=None)
        tgt_vocab.from_dataset(
            *data_bundle.datasets.values(), field_name='chunk')
    ...
    def process_from_file(self, paths):
        ...
        :return:
        """
        data_bundle = ConllLoader(
            headers=['raw_words', 'pos', 'chunk', 'ner']).load(paths)
...
class _CNNERPipe(Pipe):
    r"""
    中文NER任务的处理 Pipe, 该 Pipe 会（1）复制 raw_chars 列，并命名为 chars；(2）
    在 chars, target 列建立词表（创建 :class:`fastNLP.Vocabulary` 对象，所以在返回
    的 DataBundle 中将有两个 Vocabulary）； (3）将 chars，target 列根据相应的
    Vocabulary转换为index。

    raw_chars 列为 List[str], 是未转换的原始数据; chars 列为 List[int]，是转换为
    index 的输入数据; target 列是 List[int]，是转换为 index 的 target。返回的
    DataSet 中被设置为 input 有 chars, target, seq_len; 设置为 target 有
    target, seq_len。
    ...
    def __init__(self,
        ...
        :param str encoding_type: target列使用什么类型的encoding方式，支持bioes,
            bio两种。
        :param bool bigrams: 是否增加一列 bigrams. bigrams 的构成是['复', '旦',
            '大', '学', ...]->["复旦", "旦大", ...]。如果设置为 True，返回的
            DataSet 将有一列名为 bigrams, 且已经转换为了 index 并设置为 input，对
            应的 vocab 可以通过 data_bundle.get_vocab('bigrams') 获取.
        :param bool trigrams: 是否增加一列 trigrams. trigrams 的构成是 ['复',
            '旦', '大', '学', ...]->["复旦大", "旦大学", ...]。如果设置为 True，返
            回的 DataSet 将有一列名为 trigrams, 且已经转换为了 index 并设置为
            input，对应的 vocab可以通过 data_bundle.get_vocab('trigrams') 获取.
        """
        if encoding_type == 'bio':
            self.convert_tag = iob2
        elif encoding_type == 'bioes':
    ...
    def process(self, data_bundle: DataBundle) -> DataBundle:
        ...
        raw_chars 列为 List[str], 是未转换的原始数据; chars 列为 List[int]，是转换
        为 index 的输入数据; target 列是 List[int]，是转换为 index 的 target。返回
        的 DataSet 中被设置为 input 有 chars, target, seq_len; 设置为 target 有
        target。

        :param data_bundle: 传入的 DataBundle 中的 DataSet必须包含 raw_words 和
            ner 两个 field，且两个 field 的内容均为 List[str]。在传入 DataBundle
            基础上原位修改。
        :return: 处理后的 ``data_bundle``
        """
...
class MsraNERPipe(_CNNERPipe):
    ...
    ``raw_chars`` 列为 :class:`List` [ :class:`str` ], 是未转换的原始数据；
    ``chars`` 列为 :class:`List` [ :class:`int` ]，是转换为 index 的输入数据；
    ``target`` 列是 :class:`List` [ :class:`int` ] ，是转换为 index 的 target。
    返回的 :class:`~fastNLP.core.DataSet` 中被设置为 input 有 ``chars`` ,
    ``target`` , ``seq_len``；target 有 ``target``。
