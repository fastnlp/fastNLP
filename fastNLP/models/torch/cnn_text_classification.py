...
class CNNText(torch.nn.Module):
    ...
    :param kernel_nums: 输出 channel 的 kernel 数目。
        如果为 :class:`list` 或 :class:`tuple`，则需要与 ``kernel_sizes`` 的大小
        保持一致。
    ...
    def __init__(self,
                 embed,
                 num_classes: int,
                 kernel_nums: Sequence[int] = (30, 40, 50),
                 kernel_sizes: Union[int, Sequence[int]] = (1, 3, 5),
                 dropout: float = 0.5):
    ...
    def evaluate_step(self,
        ...
        :return: 预测结果，仅包含一个键 ``pred``，值为形状为 ``[batch_size,]`` 的
