...
__all__ = ['SeqLabeling', 'AdvSeqLabel', 'BiLSTMCRF']
...
class BiLSTMCRF(nn.Module):
    ...
    :param hidden_size: **BiLSTM** 的 ``hidden_size``，实际 hidden size 为该值
        的 **两倍** （前向、后向）
    :param dropout: dropout 的概率，0 为不 dropout
    :param target_vocab: :class:`~fastNLP.core.Vocabulary` 对象，target 与
        index 的对应关系。如果传入该值，将自动避免非法的解码序列。
    """
    ...
    def __init__(self,
        ...
        if target_vocab is not None:
            assert len(target_vocab) == num_classes, \
                'The number of classes should be same with ' \
                'the length of target vocabulary.'
    ...
    def train_step(self, words: 'torch.LongTensor', target: 'torch.LongTensor',
    ...
    def evaluate_step(self, words: 'torch.LongTensor',
                      seq_len: 'torch.LongTensor'):
        """
...
class AdvSeqLabel(nn.Module):
    ...
    :param dropout: :class:`~fastNLP.modules.torch.LSTM` 中以及 DropOut 层的
        drop 概率
    :param id2words: tag id 转为其 tag word 的表。用于在 CRF 解码时防止解出非法的
        顺序，比如 **'BMES'** 这个标签规范中，``'S'`` 不能出现在 ``'B'`` 之后。这里
        也支持类似于 ``'B-NN'``，即 ``'-'`` 前为标签类型的指示，后面为具体的 tag 的
        情况。这里不但会保证 ``'B-NN'`` 后面不为 ``'S-NN'`` 还会保证 ``'B-NN'`` 后
        面不会出现 ``'M-xx'`` （任何非 ``'M-NN'`` 和 ``'E-NN'`` 的情况）。
    :param encoding_type: 支持 ``["BIO", "BMES", "BEMSO"]``，只有在 ``id2words``
        不为 ``None`` 的情况有用。
    """
