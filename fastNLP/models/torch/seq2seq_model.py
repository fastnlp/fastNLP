r"""
主要包含组成 **Sequence-to-Sequence** 的 model
...
from ...modules.torch.decoder.seq2seq_decoder import (LSTMSeq2SeqDecoder,
                                                      Seq2SeqDecoder,
                                                      TransformerSeq2SeqDecoder
                                                      )
from ...modules.torch.encoder.seq2seq_encoder import (LSTMSeq2SeqEncoder,
                                                      Seq2SeqEncoder,
...
class Seq2SeqModel(nn.Module):
    ...
    **Seq2Seq模型**。正常情况下，继承了该函数之后，只需要实现 classmethod
    ...
    def forward(self,
        ...
        :return: 字典 ``{'pred': torch.Tensor}``, 其中 ``pred`` 的形状为
    ...
    def prepare_state(self,
                      src_tokens: 'torch.LongTensor',
                      src_seq_len: 'torch.LongTensor' = None):
        r"""调用 encoder 获取 state，会把 encoder 的 ``encoder_output``,
        ``encoder_mask`` 直接传入到 :meth:`decoder.init_state` 中初始化一个
        state。
    ...
    def build_model(cls, *args, **kwargs):
        ...
        raise NotImplementedError(
            'A `Seq2SeqModel` must implement its own classmethod '
            '`build_model()`.')
...
class TransformerSeq2SeqModel(Seq2SeqModel):
    ...
    def build_model(cls,
        ...
        :param src_embed: source 的 embedding，支持以下几种输入类型：
            ...
            - ``tuple(num_embedings, embedding_dim)``，即 embedding 的大小和
              每个词的维度，此时将随机初始化一个 :class:`torch.nn.Embedding` 实
              例；
            - :class:`torch.nn.Embedding` 或 **fastNLP** 的 ``Embedding``
              对象，此时就以传入的对象作为 embedding；
            - :class:`numpy.ndarray`，将使用传入的 ndarray 作为 Embedding 初
              始化；
            - :class:`torch.Tensor`，此时将使用传入的值作为 Embedding 初始化；
        :param tgt_embed: target 的 embedding。
            如果 ``bind_encoder_decoder_embed`` 为 ``True``，则不要输入该值。支持
            以下几种输入类型：
        ...
        :param bind_encoder_decoder_embed: 是否对 ``encoder`` 和 ``decoder`` 使
            用相同的 embedding
        :param bind_decoder_input_output_embed: ``decoder`` 的输出 embedding 是
            否与其输入 embedding 是一样的权重
        :return: :class:`TransformerSeq2SeqModel` 模型
        """
        if bind_encoder_decoder_embed and tgt_embed is not None:
            raise RuntimeError('If you set `bind_encoder_decoder_embed=True`, '
                               'please do not provide `tgt_embed`.')
        ...
        if bind_encoder_decoder_embed:
            ...
            assert tgt_embed is not None, \
                'You need to pass `tgt_embed` when ' \
        ...
        if pos_embed == 'sin':
            encoder_pos_embed = nn.Embedding.from_pretrained(
                get_sinusoid_encoding_table(
                    max_position + 1, src_embed.embedding_dim, padding_idx=0),
                freeze=True)  # 这里规定0是padding
            deocder_pos_embed = nn.Embedding.from_pretrained(
                get_sinusoid_encoding_table(
                    max_position + 1, tgt_embed.embedding_dim, padding_idx=0),
                freeze=True)  # 这里规定0是padding
        elif pos_embed == 'learned':
            encoder_pos_embed = get_embeddings(
                (max_position + 1, src_embed.embedding_dim), padding_idx=0)
            deocder_pos_embed = get_embeddings(
                (max_position + 1, src_embed.embedding_dim), padding_idx=1)
        else:
            raise ValueError('pos_embed only supports sin or learned.')

        encoder = TransformerSeq2SeqEncoder(
            embed=src_embed,
...
class LSTMSeq2SeqModel(Seq2SeqModel):
    ...
    def build_model(cls,
        ...
        :param attention: 是否在 ``decoder`` 中使用 attention 来添加 ``encoder``
            在所有时刻的状态
        :param bind_encoder_decoder_embed: 是否对 ``encoder`` 和 ``decoder`` 使
            用相同的 embedding
        :param bind_decoder_input_output_embed: ``decoder`` 的输出 embedding 是
