...
from .file_utils import (CONFIG_NAME, cached_path, hf_bucket_url,
...
class PretrainedConfig:
    ...
    def __init__(self, **kwargs):
        ...
        self.torch_dtype = kwargs.pop('torch_dtype',
                                      None)  # Only used by PyTorch models
    ...
    def from_json_file(
        ...
        """Instantiates a :class:`~transformers.PretrainedConfig` from the path
        to a JSON file of parameters.
    ...
    def update(self, config_dict: Dict[str, Any]):
        """Updates attributes of this class with attributes from
        ``config_dict``.
    ...
    def dict_torch_dtype_to_str(self, d: Dict[str, Any]) -> None:
        """Checks whether the passed dictionary has a `torch_dtype` key and if
        it's not None, converts torch.dtype to a string of just the type.

        For example, :obj:`torch.float32` get converted into `"float32"`
        string, which can then be stored in the json format.
        """
        if d.get('torch_dtype',
                 None) is not None and not isinstance(d['torch_dtype'], str):
