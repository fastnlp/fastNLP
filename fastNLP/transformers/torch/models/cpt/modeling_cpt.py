...
modified from transformers==4.4.1
"""
import math
import random
...
from fastNLP.transformers.torch.modeling_outputs import (
    ...
    Seq2SeqLMOutput, Seq2SeqModelOutput, Seq2SeqQuestionAnsweringModelOutput,
...
if _NEED_IMPORT_TORCH:
    ...
    from torch.nn import CrossEntropyLoss, Embedding, LayerNorm, Module
else:
    from fastNLP.core.utils.dummy_class import DummyClass as Embedding
...
def _expand_mask(mask: 'torch.Tensor',
    ...
    return inverted_mask.masked_fill(inverted_mask.bool(),
...
class CPTDecoder(CPTPretrainedModel):
    ...
    Each layer is a :class:`CPTDecoderLayer`
    Args:
        config: CPTConfig
    ...
    def __init__(self,
        ...
        self.layers = nn.ModuleList(
            [CPTDecoderLayer(config) for _ in range(config.decoder_layers)])
...
class CPTModel(CPTPretrainedModel):
    ...
    def forward(
        ...
        if isinstance(encoder_outputs, (torch.Tensor)):
            ...
            encoder_hidden_states = encoder_outputs[1][-self.num_decoder_layers
                                                       - 1]
        ...
        return Seq2SeqModelOutput(
            ...
            encoder_hidden_states=encoder_outputs.hidden_states if isinstance(
                encoder_outputs, dict) else None,
            encoder_attentions=encoder_outputs.attentions if isinstance(
                encoder_outputs, dict) else None,
        )


@add_start_docstrings(
    'The CPT Model with a language modeling head. Can be used for summarization.',
    CPT_START_DOCSTRING)
