...
class GenerationMixin:
    ...
    def _prepare_input_ids_for_generation(
        ...
        if self.config.is_encoder_decoder and encoder_outputs is not None:
            ...
            return torch.ones(
                shape, dtype=torch.long, device=self.device) * -100
    ...
    def _prepare_attention_mask_for_generation(
        ...
        is_pad_token_in_inputs_ids = (pad_token_id
    ...
    def _prepare_decoder_input_ids_for_generation(
            ...
            bos_token_id: Optional[int] = None) -> 'torch.LongTensor':
    ...
    def _get_decoder_start_token_id(
            ...
            bos_token_id: Optional[int] = None) -> int:
    ...
    def _get_logits_warper(
            ...
            num_beams: Optional[int] = None) -> LogitsProcessorList:
        """
    ...
    def _get_logits_processor(
        ...
        no_repeat_ngram_size = (
            no_repeat_ngram_size if no_repeat_ngram_size is not None else
        ...
        forced_bos_token_id = (
            forced_bos_token_id if forced_bos_token_id is not None else
            self.config.forced_bos_token_id)
        forced_eos_token_id = (
            forced_eos_token_id if forced_eos_token_id is not None else
            self.config.forced_eos_token_id)
        remove_invalid_values = (
            remove_invalid_values if remove_invalid_values is not None else
    ...
    def _get_stopping_criteria(self, max_length: Optional[int],
                               max_time: Optional[float],
                               max_new_tokens: Optional[int],
                               start_length: int) -> StoppingCriteriaList:
    ...
    def generate(
        ...
        **model_kwargs,
    ) -> Union[GreedySearchOutput, SampleOutput, BeamSearchOutput,
        ...
        while True:
            ...
            for beam_group_idx in range(num_beam_groups):
                ...
                next_token_scores = next_token_scores + beam_scores[
                    batch_group_indices].unsqueeze(-1).expand_as(
                        next_token_scores)
