...
class HfDeepSpeedConfig:
    ...
    def is_true(self, ds_key_long):
        ...
        So use this method to ask the very specific question of whether the
        value is set to :obj:`True` (and it's not set to :obj:`False` or isn't
        set).
        """
    ...
    def is_false(self, ds_key_long):
        ...
        So use this method to ask the very specific question of whether the
        value is set to :obj:`False` (and it's not set to :obj:`True` or isn't
        set).
        """
        value = self.get_value(ds_key_long)
...
class HfTrainerDeepSpeedConfig(HfDeepSpeedConfig):
    """The ``HfTrainerDeepSpeedConfig`` object is meant to be created during
    ``TrainingArguments`` object creation and has the same lifespan as the
    latter."""
    ...
    def fill_match(self, ds_key_long, hf_val, hf_key=None, must_match=True):
        """A utility method that massages the config file and can optionally
    ...
    def trainer_config_process(self, args):
        """Adjust the config with ``TrainingArguments`` values.

        This stage is run during ``TrainingArguments`` object creation.
        """
        # DeepSpeed does:
        # train_batch_size = world_size * train_micro_batch_size_per_gpu * gradient_accumulation_steps
        train_batch_size = args.world_size * args.per_device_train_batch_size * args.gradient_accumulation_steps
        self.fill_match('train_micro_batch_size_per_gpu',
                        args.per_device_train_batch_size,
                        'per_device_train_batch_size')
        self.fill_match('gradient_accumulation_steps',
        ...
        self.fill_match('optimizer.params.lr', args.learning_rate,
                        'learning_rate')
        self.fill_match('optimizer.params.betas',
        ...
        self.fill_only('scheduler.params.warmup_min_lr',
                       0)  # not a trainer arg
        ...
        self.fill_match('fp16.enabled', fp16_backend == 'amp',
    ...
    def trainer_config_finalize(self, args, model, num_training_steps):
        """This stage is run after we have the model and know
        num_training_steps.
        ...
        self.fill_match('scheduler.params.warmup_num_steps',
...
def is_deepspeed_zero3_enabled():
    if _hf_deepspeed_config_weak_ref is not None and _hf_deepspeed_config_weak_ref(
    ) is not None:
...
def deepspeed_init(trainer, num_training_steps, resume_from_checkpoint=None):
    """Init DeepSpeed, after updating the DeepSpeed configuration with any
    relevant Trainer's args.
    ...
    hf_deepspeed_config.trainer_config_finalize(args, model,
                                                num_training_steps)
