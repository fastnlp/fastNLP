...
def _get_ngrams(ngram_size: int, prev_input_ids: 'torch.Tensor',
                num_hypos: int):
...
class EncoderNoRepeatNGramLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, encoder_ngram_size: int,
    ...
    def __call__(self, input_ids: 'torch.LongTensor',
        ...
        cur_len = input_ids.shape[-1]
        banned_batch_tokens = [
            _get_generated_ngrams(self.generated_ngrams[hypo_idx // num_beams],
                                  input_ids[hypo_idx], self.ngram_size,
...
class NoBadWordsLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, bad_words_ids: List[List[int]], eos_token_id: int):
        ...
        for banned_token_seq in self.bad_words_id_length_greater_than_1:
            assert len(
                banned_token_seq
            ) > 0, f'Banned words token sequences {bad_words_ids} cannot have an empty list'

    def __call__(self, input_ids: 'torch.LongTensor',
                 scores: 'torch.FloatTensor') -> 'torch.FloatTensor':
        if self.static_bad_words_mask is None and len(
                self.bad_words_id_length_1) > 0:
    ...
    def _calc_static_bad_word_mask(
            self, scores: 'torch.FloatTensor') -> 'torch.BoolTensor':
    ...
    def _calc_banned_bad_words_ids(
            self, prev_input_ids: List[List[int]]) -> Iterable[int]:
        banned_tokens = []
    ...
    def _set_scores_to_inf_for_banned_tokens(
        ...
        """Modifies the scores in place by setting the banned token positions
        to `-inf`. Banned token is expected to be a list of list of banned
        tokens to ban in the format [[batch index, vocabulary position],...
            ...
            if banned_mask_list:
                ...
                banned_mask = (
                    torch.sparse.LongTensor(
                        banned_mask.t(), indices,
                        scores.size()).to(scores.device).to_dense().bool())

                if self.static_bad_words_mask is not None:
                    banned_mask = torch.bitwise_or(banned_mask,
                                                   self.static_bad_words_mask)
            else:
...
class HammingDiversityLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, diversity_penalty: float, num_beams: int,
                 num_beam_groups: int):
        if not isinstance(diversity_penalty,
                          float) or (not diversity_penalty > 0.0):
            raise ValueError(
...
class ForcedBOSTokenLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: 'torch.LongTensor',
        ...
        if cur_len == 1:
            ...
            scores[:, [i for i in range(num_tokens)
                       if i != self.bos_token_id]] = -float('inf')
...
class ForcedEOSTokenLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: 'torch.LongTensor',
        ...
        if cur_len == self.max_length - 1:
            num_tokens = scores.shape[1]
            scores[:, [i for i in range(num_tokens)
                       if i != self.eos_token_id]] = -float('inf')
