...
from .seq2seq_state import LSTMState, State, TransformerState
...
class Seq2SeqDecoder(nn.Module):
    ...
    **Sequence-to-Sequence Decoder** 的基类。一定需要实现 :meth:`forward` 和
    ...
    def forward(self, tokens: 'torch.LongTensor', state: State, **kwargs):
        ...
        :param state: ``state`` 包含了 ``encoder`` 的输出以及 ``decode`` 之前的内
            容
        :return: 返回值可以为 ``[batch_size, max_len, vocab_size]`` 的张量，也可
            以是一个 :class:`list`，但是第一个元素必须是词的预测分布
        """
        raise NotImplementedError

    def reorder_states(self, indices: torch.LongTensor, states):
        r"""根据 ``indices`` 重新排列 ``states`` 中的状态，在 ``beam search`` 进
        行生成时，会用到该函数。
    ...
    def decode(self, tokens: torch.LongTensor, state) -> torch.FloatTensor:
        ...
        if isinstance(outputs, torch.Tensor):
            ...
            raise RuntimeError(
                'Unrecognized output from the `forward()` function. '
                'Please override the `decode()` function.')


class TiedEmbedding(nn.Module):
    """用于将 weight 和原始 weight 绑定。"""
...
def get_bind_decoder_output_embed(embed):
    """给定一个 embedding，输出对应的绑定的 embedding，输出对象为 TiedEmbedding。
    ...
    if isinstance(embed, StaticEmbedding):
        for idx, map2idx in enumerate(embed.words_to_words):
            assert idx == map2idx, \
                'Invalid StaticEmbedding for Decoder, please check:' \
                '(1) whether the vocabulary include `no_create_entry=True` ' \
                'word; (2) StaticEmbedding should not initialize with ' \
                '`lower=True` or `min_freq!=1`.'
    elif not isinstance(embed, nn.Embedding):
        raise TypeError(
            'Only nn.Embedding or StaticEmbedding is allowed for binding.')
...
class LSTMSeq2SeqDecoder(Seq2SeqDecoder):
    ...
    **LSTM** 的 Decoder
    ...
    def __init__(self,
                 embed: Union[nn.Module, Tuple[int, int]],
                 num_layers: int = 3,
                 hidden_size: int = 300,
                 dropout: float = 0.3,
        ...
        if bind_decoder_input_output_embed:
            ...
            self.output_embed = get_embeddings(
                (self.embed.num_embeddings, self.embed.embedding_dim))
            self.output_layer = TiedEmbedding(self.output_embed.weight)

        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.lstm = nn.LSTM(
            input_size=self.embed_dim + hidden_size,
        ...
        self.attention_layer = AttentionLayer(
            hidden_size, hidden_size, hidden_size) if attention else None
    ...
    def forward(  # type: ignore[override]
        ...
        assert tokens.size(1) > state.decode_length, 'The state does not ' \
                                                     'match the tokens.'
        tokens = tokens[:, state.decode_length:]
        x = self.embed(tokens)
        # 保存attention weight, batch,tgt_seq,src_seq
        if self.attention_layer is not None:
            attn_weights: Optional[List] = []
        else:
            attn_weights = None
        ...
        for i in range(tokens.size(1)):
            input = torch.cat((x[:, i:i + 1, :], input_feed[:, None, :]),
                              dim=2)  # batch,1,2*dim
            _, (cur_hidden, cur_cell) = self.lstm(
                input, hx=(cur_hidden, cur_cell))  # hidden/cell保持原来的size
            if self.attention_layer is not None:
                input_feed, attn_weight = self.attention_layer(
                    cur_hidden[-1], src_output, encoder_mask)
                attn_weights.append(attn_weight)  # type: ignore
            else:
        ...
        if attn_weights is not None:
            attn_weights = torch.cat(
                attn_weights, dim=1)  # batch, tgt_len, src_len
    ...
    def init_state(self, encoder_output,
                   encoder_mask: torch.ByteTensor) -> LSTMState:
        """

        :param encoder_output: ``encoder`` 的输出，可以有两种情况：

            - 输入一个 :class:`tuple`。包含三个内容 ``(encoder_output, (hidden,
            ...
            - 只有形状为 ``[batch_size, max_len, hidden_size]`` 的
              ...
              初始化。
        :param encoder_mask: ``[batch_size, max_len]]``，为 **0** 的位置是
        ...
        assert encoder_output.size(-1) == self.hidden_size, \
            'The dimension of encoder outputs should be the same with ' \
            'the hidden_size.'
        ...
        for idx in range(2):
            v = t[idx]
            if v is None:
                v = encoder_output.new_zeros(self.num_layers,
                                             encoder_output.size(0),
                                             self.hidden_size)
            else:
                assert v.dim() == 2
                assert v.size(-1) == self.hidden_size
                # num_layers x bsz x hidden_size
                v = v[None].repeat(self.num_layers, 1, 1)
            t[idx] = v
...
class TransformerSeq2SeqDecoderLayer(nn.Module):
    ...
    **Transformer** 的 Decoder 层
    ...
    def __init__(self,
        ...
        self.encoder_attn = MultiHeadAttention(d_model, n_head, dropout,
                                               layer_idx)
        self.encoder_attn_layer_norm = nn.LayerNorm(d_model)

        self.ffn = nn.Sequential(
            nn.Linear(self.d_model, self.dim_ff), nn.ReLU(),
            nn.Dropout(dropout), nn.Linear(self.dim_ff, self.d_model),
            nn.Dropout(dropout))
    ...
    def forward(self,
        ...
        :param encoder_output: ``encoder`` 的输出，形状为 ``[batch_size,
            src_seq_len, dim]``
        :param encoder_mask: 掩码，形状为 ``[batch_size, src_seq_len]``，为
            **1** 的地方表示需要 attend
        :param self_attn_mask: 下三角的mask矩阵，只在训练时传入。形状为
            ``[seq_len, seq_len]``
        :param state: 只在 inference 阶段传入，记录了 ``encoder`` 和
            ``decoder`` 的状态
        :return:
        """
        ...
        x, _ = self.self_attn(
            query=x, key=x, value=x, attn_mask=self_attn_mask, state=state)
        ...
        x, attn_weight = self.encoder_attn(
            query=x,
            key=encoder_output,
...
class TransformerSeq2SeqDecoder(Seq2SeqDecoder):
    ...
    **Transformer** 的 Decoder
    ...
    def __init__(self,
                 embed: Union[nn.Module, StaticEmbedding, Tuple[int, int]],
        ...
        self.layer_stacks = nn.ModuleList([
            TransformerSeq2SeqDecoderLayer(d_model, n_head, dim_ff, dropout,
                                           layer_idx)
    ...
    def forward(  # type: ignore[override]
        ...
        :param state: 用于记录 ``encoder`` 的输出以及 ``decode`` 状态的对象，可以
            通过 :meth:`init_state` 获取
        ...
        assert state.decode_length < tokens.size(
            1), 'The decoded tokens in State should be less than tokens.'
        tokens = tokens[:, state.decode_length:]
        device = tokens.device

        x = self.embed_scale * self.embed(tokens)
        if self.pos_embed is not None:
            position = torch.arange(state.decode_length, state.decode_length +
                                    tokens.size(1)).long().to(device)[None]
    ...
    def init_state(self, encoder_output: torch.FloatTensor,
                   encoder_mask: torch.ByteTensor) -> TransformerState:
        """初始化一个 :class:`~fastNLP.modules.torch.decoder.TransformerState`` 用于
        :meth:`forward`
        ...
        if isinstance(encoder_output, torch.Tensor):
            ...
            raise TypeError(
                'Unsupported `encoder_output` for TransformerSeq2SeqDecoder')
        state = TransformerState(
            encoder_output, encoder_mask, num_decoder_layer=self.num_layers)
        return state
