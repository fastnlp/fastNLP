...
from ....core.metrics.span_f1_pre_rec_metric import (
    _check_tag_vocab_and_encoding_type, _get_encoding_type_from_tag_vocab)
from ....core.vocabulary import Vocabulary

__all__ = ['ConditionalRandomField', 'allowed_transitions']


def allowed_transitions(
        tag_vocab: Union[Vocabulary, dict],
        encoding_type: Optional[str] = None,
        include_start_end: bool = False) -> List[Tuple[int, int]]:
    r"""
    ...
    :param tag_vocab: 支持类型为 tag 或 tag-label。只有 tag 的,比如 ``"B"`` 、
        ...
        即 **index 在前，tag 在后**。
    :param encoding_type: 支持 ``["bio", "bmes", "bmeso", "bioes", None]``。默
        认为 ``None``，通过 ``tag_vocab`` 自动推断
    ...
    for from_id, from_label in id_label_lst:
        ...
        for to_id, to_label in id_label_lst:
            ...
            if _is_transition_allowed(encoding_type, from_tag, from_label,
                                      to_tag, to_label):
                allowed_trans.append((from_id, to_id))
    return allowed_trans


def _is_transition_allowed(encoding_type, from_tag, from_label, to_tag,
                           to_label):
    r"""
    ...
    if encoding_type == 'bio':
        r"""
        第一行是to_tag, 第一列是from_tag. y任意条件下可转，-只有在label相同时可转，
        n不可转
        +-------+---+---+---+-------+-----+
        |       | B | I | O | start | end |
        +-------+---+---+---+-------+-----+
        ...
        if from_tag == 'start':
            ...
            raise ValueError(
                "Unexpected tag {}. Expect only 'B', 'I', 'O'.".format(
                    from_tag))
        ...
        if from_tag == 'start':
            ...
            raise ValueError(
                "Unexpected tag type {}. Expect only 'B', 'M', 'E', 'S'.".
                format(from_tag))
        ...
        if from_tag == 'start':
            ...
            raise ValueError(
                "Unexpected tag type {}. Expect only 'B', 'M', 'E', 'S', 'O'.".
                format(from_tag))
        ...
        if from_tag == 'start':
            ...
            raise ValueError(
                "Unexpected tag type {}. Expect only 'B', 'I', 'E', 'S', 'O'.".
                format(from_tag))
    else:
        raise ValueError(
            'Only support BIO, BMES, BMESO, BIOES encoding type, got {}.'.
