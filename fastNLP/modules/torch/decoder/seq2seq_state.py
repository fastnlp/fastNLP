...
__all__ = ['State', 'LSTMState', 'TransformerState']
...
class State:
    ...
    def num_samples(self):
        r"""返回的 State 中包含的是多少个 sample 的 encoder 状态，主要用于
        Generate 的时候确定 batch_size 的大小。"""
    ...
    def decode_length(self):
        r"""当前 Decode 到哪个 token 了，decoder 只会从 decode_length 之后的
        token 开始 decode，为 **0** 说明还没开始 decode。"""
    ...
    def reorder_state(self, indices: torch.LongTensor):
        ...
        if self.encoder_output is not None:
            self.encoder_output = self._reorder_state(self.encoder_output,
                                                      indices)
...
class LSTMState(State):
    ...
    :param encoder_mask: 掩码，形状为 ``[batch_size, src_seq_len]``，为 **1** 的
        地方表示需要 attend
    :param hidden: 上个时刻的 ``hidden`` 状态，形状为 ``[num_layers,
        batch_size, hidden_size]``
    :param cell: 上个时刻的 ``cell`` 状态，形状为 ``[num_layers, batch_size,
        hidden_size]``
    """

    def __init__(self, encoder_output: torch.FloatTensor,
                 encoder_mask: torch.BoolTensor, hidden: torch.FloatTensor,
    ...
    def reorder_state(self, indices: torch.LongTensor):
        ...
        if self.input_feed is not None:
            self.input_feed = self._reorder_state(
                self.input_feed, indices, dim=0)
...
class TransformerState(State):
    ...
    :param encoder_mask: 掩码，形状为 ``[batch_size, encode_max_len]``，为
        **1** 的地方表示需要 attend
    :param num_decoder_layer: decoder 层数
    """

    def __init__(self, encoder_output: torch.FloatTensor,
                 encoder_mask: torch.FloatTensor, num_decoder_layer: int):
        super().__init__(encoder_output, encoder_mask)
        # 每一个元素 bsz x encoder_max_len x key_dim
        self.encoder_key = [None] * num_decoder_layer
        # 每一个元素 bsz x encoder_max_len x value_dim
        self.encoder_value = [None] * num_decoder_layer
        # 每一个元素 bsz x decode_length x key_dim
    ...
    def reorder_state(self, indices: torch.LongTensor):
        ...
        self.decoder_prev_key = self._reorder_state(self.decoder_prev_key,
                                                    indices)
        self.decoder_prev_value = self._reorder_state(self.decoder_prev_value,
                                                      indices)
