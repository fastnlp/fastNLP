...
class MLP(nn.Module):
    ...
    :param size_layer: 一个 int 的列表，用来定义 :class:`MLP` 的层数，列表中的数字
    ...
    def __init__(self,
                 size_layer: List[int],
                 activation: Union[str, Callable, List[str]] = 'relu',
        ...
        for i in range(1, len(size_layer)):
            if i + 1 == len(size_layer):
                ...
                self.hiddens.append(
                    nn.Linear(size_layer[i - 1], size_layer[i]))
        ...
        if not isinstance(activation, list):
            activation = [activation] * (len(size_layer) - 2)  # type: ignore
        elif len(activation) == len(size_layer) - 2:
            pass
        else:
            raise ValueError(
                f'the length of activation function list except '
                f'{len(size_layer) - 2} but got {len(activation)}!')
        self.hidden_active = []
