...
class ConvMaxpool(nn.Module):
    ...
    :param activation: **卷积** 后的结果将通过该 ``activation`` 后再经过
        ``max-pooling``。支持 ``['relu', 'sigmoid', 'tanh']``。
    """
    ...
    def __init__(self,
        ...
        if isinstance(kernel_sizes, (list, tuple, int)):
            if isinstance(kernel_sizes, int) and isinstance(out_channels, int):
                assert kernel_sizes % 2 == 1, 'kernel size has to be ' \
                                              'odd numbers.'
                out_channels = [out_channels]
                kernel_sizes = [kernel_sizes]
            elif isinstance(kernel_sizes, (tuple, list)) and isinstance(
                    out_channels, (tuple, list)):
                for kernel_size in kernel_sizes:
                    assert kernel_size % 2 == 1, 'kernel size has to be ' \
                                                 'odd numbers.'
                assert len(out_channels) == len(kernel_sizes), \
                    'The number of out_channels should be equal to the ' \
                    'number of kernel_sizes.'
            else:
                raise ValueError(
                    'The type of out_channels and kernel_sizes should '
                    'be the same.')
            ...
            self.convs = nn.ModuleList([
                nn.Conv1d(
                    ...
                    dilation=1,
                    groups=1,
                    bias=False) for oc, ks in zip(out_channels, kernel_sizes)
            ])
    ...
    def forward(self, x: torch.FloatTensor, mask=None):
        ...
        :param x: ``[batch_size, max_len, input_size]``，一般是经过
            ``embedding`` 后的值
        :param mask: ``[batch_size, max_len]``，**0** 的位置表示 padding，不影响
            卷积运算，``max-pooling`` 一定不会 pool 到 padding 为 0 的位置
        :return:
        """
        # [N,L,C] -> [N,C,L]
        x = torch.transpose(x, 1, 2)
        # convolution
        xs = [self.activation(conv(x))
              for conv in self.convs]  # [[N,C,L], ...]
        if mask is not None:
            mask = mask.unsqueeze(1)  # B x 1 x L
            xs = [x.masked_fill(mask.eq(False), float('-inf')) for x in xs]
        # max-pooling
        xs = [
            F.max_pool1d(input=i, kernel_size=i.size(2)).squeeze(2) for i in xs
        ]  # [[N, C], ...]
