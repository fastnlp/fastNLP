...
class Seq2SeqEncoder(nn.Module):
    r"""所有 **Sequence2Sequence Encoder** 的基类。需要实现 :meth:`forward`
    函数。"""
...
class TransformerSeq2SeqEncoderLayer(nn.Module):
    ...
    **Self-Attention** 的 Layer，
    ...
    def forward(self, x, mask):
        ...
        x, _ = self.self_attn(query=x, key=x, value=x, key_mask=mask)
...
class TransformerSeq2SeqEncoder(Seq2SeqEncoder):
    ...
    def __init__(self,
        ...
        self.layer_stacks = nn.ModuleList([
            TransformerSeq2SeqEncoderLayer(d_model, n_head, dim_ff, dropout)
    ...
    def forward(self, tokens, seq_len):
        ...
        :param seq_len: 序列长度，形状为 ``[batch_size, ]``，若为 ``None``，表示
            所有输入看做一样长
        :return: 一个元组，第一个元素形状为 ``[batch_size, max_len, d_model]`` 表
            示前向传播的结果，第二个元素形状为 ``[batch_size, max_len]``，表示产生
            的掩码 ``encoder_mask``，为 **0** 的地方为 padding
        """
        x = self.embed(tokens) * self.embed_scale  # batch, seq, dim
        batch_size, max_src_len, _ = x.size()
        device = x.device
        if self.pos_embed is not None:
            position = torch.arange(1, max_src_len +
    ...
    **LSTM** 的 Encoder
    ...
    def forward(self, tokens: torch.LongTensor, seq_len: torch.LongTensor):
        ...
        :return: 返回 ``((output, (ht, ct)), encoder_mask)`` 格式的结果。
            ...
            - ``output`` 形状为 ``[batch_size, seq_len, hidden_size *
              num_direction]``，表示输出序列；
            - ``ht`` 和 ``ct`` 形状为 ``[num_layers*num_direction, batch_size,
              hidden_size]``，表示最后时刻隐状态；
            - ``encoder_mask`` 形状为 ``[batch_size, max_len]``，表示产生的掩码
              ``encoder_mask``，为 **0** 的地方为 padding
        ...
        if self.bidirectional:
            # 将双向的hidden state拼接起来，用于接下来的decoder的input
            final_hidden = self.concat_bidir(final_hidden)
            final_cell = self.concat_bidir(final_cell)
        # 为了配合Seq2SeqBaseModel的forward，这边需要分为两个return
        return (x, (final_hidden[-1], final_cell[-1])), encoder_mask

    def concat_bidir(self, input):
        output = input.view(self.num_layers, 2, input.size(1),
                            -1).transpose(1, 2)
