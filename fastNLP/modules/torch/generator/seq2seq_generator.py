...
class SequenceGenerator:
    ...
    :param pad_token_id: 当某句话生成结束之后，之后生成的内容用 ``pad_token_id``
        补充
    """
...
def greedy_generate(decoder,
    ...
    :param float max_len_a: 每句话的 decode 长度为max_length + max_len_a *
        src_len。如果不为 0，需要保证 State 中包含 encoder_mask
    :param int num_beams: 使用多大的 beam 进行解码。
    :param int bos_token_id: 如果 tokens 传入为 None，则使用 bos_token_id 开始往
        后解码。
    :param int eos_token_id: 结束的 token，如果为 None，则一定会解码到 max_length
        这么长。
    :param int pad_token_id: pad的token id
    :param float repetition_penalty: 对重复出现的 token 多大的惩罚。
...
def sample_generate(decoder,
    ...
    :param int num_beam: 使用多大的 beam 进行解码。
    :param float temperature: 采样时的退火大小
    :param int top_k: 只在 top_k 的 sample 里面采样
    :param float top_p: 介于 0,1 的值。
    :param int bos_token_id: 如果 tokens 传入为 None，则使用 bos_token_id 开始往
        后解码。
    :param int eos_token_id: 结束的 token，如果为 None，则一定会解码到 max_length
        这么长。
    :param int pad_token_id: pad的token id
    :param float repetition_penalty: 对重复出现的 token 多大的惩罚。
    :param float length_penalty: 对每个 token（除了eos）按照长度进行一定的惩罚。
    :return:
    """
    # 每个位置在生成的时候会sample生成
    if num_beams == 1:
...
def _no_beam_search_generate(decoder: Seq2SeqDecoder,
    ...
    if state.num_samples:
        assert state.num_samples == batch_size, \
    ...
    while cur_len < real_max_length:
        ...
        if _eos_token_id != -1:
            next_tokens = next_tokens.masked_fill(
                max_lengths.eq(cur_len + 1), _eos_token_id)
        next_tokens = next_tokens.masked_fill(
...
def _beam_search_generate(decoder: Seq2SeqDecoder,
    ...
    assert vocab_size >= num_beams, 'num_beams should be smaller than ' \
                                    'the number of vocabulary size.'
    ...
    if do_sample:
        ...
        next_scores, next_tokens = torch.topk(
            scores, num_beams, dim=1, largest=True, sorted=True)
    ...
    hypos = [
        BeamHypotheses(
            num_beams, real_max_length, length_penalty, early_stopping=False)
    ...
    while cur_len < real_max_length:
        ...
        if _eos_token_id != -1:
            ...
            scores[:,
                   _eos_token_id] = torch.where(max_len_eos_mask,
                                                eos_scores + 1e32, eos_scores)
        ...
        if do_sample:
            ...
            # 多召回一个防止eos
            scores = top_k_top_p_filtering(
                scores, top_k, top_p, min_tokens_to_keep=num_beams + 1)
            # 加上1e-12是为了避免https://github.com/pytorch/pytorch/pull/27523
            probs = F.softmax(scores, dim=-1) + 1e-12
        ...
        if cur_len + 1 == real_max_length:
            ...
            if effective_eos_mask.sum().gt(0):
                eos_batch_idx, eos_beam_ind = effective_eos_mask.nonzero(
                    as_tuple=True)
                # 是由于from_which_beam是 (batch_size, 2*num_beams)的
                # 所以需要2*num_beams
        ...
        for batch_idx in range(batch_size):
            dones[batch_idx] = dones[batch_idx] or \
                hypos[batch_idx].is_done(next_scores[batch_idx, 0].item()) or \
