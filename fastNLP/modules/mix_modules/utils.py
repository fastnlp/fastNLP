...
from fastNLP.core.utils import apply_to_collection, paddle_to
from fastNLP.envs.imports import (_NEED_IMPORT_JITTOR, _NEED_IMPORT_PADDLE,
                                  _NEED_IMPORT_TORCH)
...
def _paddle2torch(paddle_tensor: 'paddle.Tensor',
                  device: Optional[Union[str, int]] = None,
                  no_gradient: Optional[bool] = None) -> 'torch.Tensor':
    r"""将 :class:`paddle.Tensor` 转换为 :class:`torch.Tensor`，并且能够保留梯度
    进行反向传播。
    ...
    """
    if no_gradient is None:
        no_gradient = paddle.tensor.stop_gradient
    ...
    if not no_gradient:
        ...
        torch_tensor = torch.tensor(
            paddle_numpy, requires_grad=True, device=device)
        torch_tensor.register_hook(lambda grad: paddle.autograd.backward(
            paddle_tensor, paddle.to_tensor(grad.cpu().numpy())))
    else:
        # 不保留梯度
        torch_tensor = torch.tensor(
            paddle_numpy, requires_grad=False, device=device)

    return torch_tensor


def _torch2paddle(torch_tensor: 'torch.Tensor',
                  device: Optional[str] = None,
                  no_gradient: Optional[bool] = None) -> 'paddle.Tensor':
    r"""将 :class:`torch.Tensor` 转换为 :class:`paddle.Tensor`，并且能够保留梯度
    进行反向传播。
    ...
    """
    if no_gradient is None:
        no_gradient = not torch_tensor.requires_grad
    if device is None:
    ...
    if not no_gradient:
        ...
        paddle_tensor = paddle.to_tensor(
            torch_tensor.detach().cpu().numpy(), stop_gradient=False)
        paddle_tensor.register_hook(lambda grad: torch.autograd.backward(
            torch_tensor, torch.tensor(grad.numpy())))
    else:
        paddle_tensor = paddle.to_tensor(
            torch_tensor.detach().cpu().numpy(), stop_gradient=True)
...
def _jittor2torch(jittor_var: 'jittor.Var',
                  device: Optional[Union[str, int]] = None,
                  no_gradient: Optional[bool] = None) -> 'torch.Tensor':
    ...
    if no_gradient is None:
        no_gradient = not jittor_var.requires_grad
    if not no_gradient:
        logger.warning('The result tensor will not keep gradients due to '
                       'differences between jittor and pytorch.')
    ...
    torch_tensor = torch.tensor(
        jittor_numpy, requires_grad=not no_gradient, device=device)

    return torch_tensor


def _torch2jittor(torch_tensor: 'torch.Tensor',
                  no_gradient: Optional[bool] = None) -> 'jittor.Var':
    ...
    if not no_gradient:
        ...
        jittor_var.register_hook(lambda grad: torch.autograd.backward(
            torch_tensor, torch.tensor(grad.numpy())))
    else:
        jittor_var = jittor.Var(torch_tensor.detach().numpy())
...
def torch2paddle(batch: Any,
                 device: Optional[str] = None,
                 no_gradient: Optional[bool] = None) -> Any:
    ...
    :param device: 是否将转换后的张量迁移到特定设备上。为 ``None`` 时，和输入保持一
        致；
    :param no_gradient: 是否保留原张量的梯度。为 ``None`` 时，新的张量与输入张量保
        持一致；为 ``True`` 时，不保留梯度；为 ``False`` 时，保留梯度；
    :return: 转换后的数据；
    """
...
def jittor2torch(batch: Any,
    ...
    .. note::
        ...
        注意，由于 **pytorch** 和 **jittor** 之间的差异，从 :class:`jittor.Var`
...
def torch2jittor(batch: Any, no_gradient: Optional[bool] = None) -> Any:
