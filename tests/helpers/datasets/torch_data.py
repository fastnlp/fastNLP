...
class TorchNormalDataset_Classification(Dataset):
    ...
    def __init__(self,
        ...
        self.x_center = torch.randint(
            low=-100, high=100, size=[num_labels, feature_dimension])
        random_shuffle = torch.randn(
            [num_labels, each_label_data, feature_dimension]) / 10
        self.x = self.x_center.unsqueeze(1).expand(
            num_labels, each_label_data, feature_dimension) + random_shuffle
        self.x = self.x.view(num_labels * each_label_data, feature_dimension)
        self.y = reduce(lambda x, y: x + y,
                        [[i] * each_label_data for i in range(num_labels)])
