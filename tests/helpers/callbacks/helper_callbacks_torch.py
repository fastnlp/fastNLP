...
class RecordAccumulationStepsCallback_Torch(Callback):
    ...
    def on_train_batch_end(self, trainer):
        ...
        if trainer.global_forward_batches % trainer.accumulation_steps == 0:
            ...
            cur_batch_params = deepcopy(
                next(
                    trainer.driver.unwrap_model().parameters()).cpu().detach())
            ...
            for optimizer in optimizers:
                ...
                for group in param_groups:
                    for p in group['params']:
                        assert p.grad is None or p.grad.equal(
            ...
            for optimizer in optimizers:
                ...
                for group in param_groups:
                    for p in group['params']:
                        assert p.grad is not None and not p.grad.equal(
                            torch.zeros_like(p.grad))
