"""这个文件主要用于提供测试 callback 时的 Trainer
的参数，可以直接使用进行对Trainer进行初始化。只需要再额外传入相应的callback就可以运行."""
    ...
    class Model(nn.Module):
        ...
        def __init__(self, num_features=5):
            ...
            self.mlps = nn.Sequential(
                nn.Linear(num_features, 20), nn.ReLU(), nn.Linear(20, 20),
                nn.Dropout(p=0.3), nn.ReLU(), nn.Linear(20, num_features))
...
def get_trainer_args(num_features=5,
                     num_samples=20,
                     bsz=4,
                     lr=0.1,
                     n_epochs=5,
