...
class TestLoad:
    ...
    def test_norm1(self):
        ...
        embed = StaticEmbedding(
            vocab,
            model_dir_or_name=tests_folder +
            '/helpers/data/embedding/small_static_embedding/'
    ...
    def test_only_use_pretrain_word(self):
        ...
        def check_word_unk(words, vocab, embed):
            for word in words:
                assert embed(torch.LongTensor([
                    vocab.to_index(word)
                ])).tolist()[0] == embed(torch.LongTensor([1])).tolist()[0]
        ...
        embed_dict = read_static_embed(
            tests_folder + '/helpers/data/embedding/small_static_embedding/'
            'glove.6B.50d_test.txt')
    ...
    def test_save_load_static_embed(self):
        ...
        try:
            ...
            vocab.add_word_lst(['notinfile2', 'notinfile2'],
...
class TestRandomSameEntry:
    ...
    def test_same_vector(self):
        ...
        embed = StaticEmbedding(
            vocab, model_dir_or_name=None, embedding_dim=5, lower=True)
        words = torch.LongTensor([[
            vocab.to_index(word) for word in ['The', 'the', 'THE', 'a', 'A']
        ]])
        words = embed(words)
        embed_0 = words[0, 0]
        for i in range(1, 3):
