...
if _NEED_IMPORT_TORCH:
    ...
    from fastNLP.models.torch.seq2seq_model import (LSTMSeq2SeqModel,
...
class TestTransformerSeq2SeqModel:
    ...
    @pytest.mark.parametrize('pos_embed', ['learned', 'sin'])
    def test_run_v1(self, pos_embed):
        # 测试能否跑通
        embed, src_words_idx, tgt_words_idx, src_seq_len, tgt_seq_len = \
            prepare_env()
        model = TransformerSeq2SeqModel.build_model(
            src_embed=embed,
            tgt_embed=None,
            pos_embed=pos_embed,
            max_position=20,
    ...
    @pytest.mark.parametrize('bind_encoder_decoder_embed', [True, False])
    @pytest.mark.parametrize('bind_decoder_input_output_embed', [True, False])
    def test_run_v2(self, bind_encoder_decoder_embed,
                    bind_decoder_input_output_embed):
        # 测试能否跑通
        embed, src_words_idx, tgt_words_idx, src_seq_len, tgt_seq_len = \
            prepare_env()

        tgt_embed = None
        if bind_encoder_decoder_embed is False:
            tgt_embed = embed
...
class TestLSTMSeq2SeqModel:
    ...
    @pytest.mark.parametrize('bind_encoder_decoder_embed', [True, False])
    @pytest.mark.parametrize('bind_decoder_input_output_embed', [True, False])
    def test_run(self, bind_encoder_decoder_embed,
                 bind_decoder_input_output_embed):
        # 测试能否跑通
        embed, src_words_idx, tgt_words_idx, src_seq_len, tgt_seq_len = \
            prepare_env()
        ...
        if bind_encoder_decoder_embed is False:
            tgt_embed = embed
        model = LSTMSeq2SeqModel.build_model(
            src_embed=embed,
            tgt_embed=tgt_embed,
            num_layers=2,
            hidden_size=20,
