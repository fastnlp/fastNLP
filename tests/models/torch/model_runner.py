"""此模块可以非常方便的测试模型。若你的模型属于：文本分类，序列标注，自然语言推理（NLI），可以直接使用此模块测试
...
from fastNLP import (Accuracy, Callback, DataSet, Evaluator, TorchDataLoader,
                     Trainer)
...
class ModelRunner():
    ...
    def prepare_text_classification_data(self):
        ...
        ds.apply_field(
            lambda x: self.gen_var_seq(MAX_LEN, VOCAB_SIZE),
            field_name=index,
            new_field_name='words')
        ds.apply_field(
            lambda x: randrange(NUM_CLS),
            field_name=index,
    ...
    def prepare_pos_tagging_data(self):
        ...
        ds.apply_field(
            ...
            new_field_name='words')
        ds.apply_field(
            lambda x: self.gen_seq(len(x), NUM_CLS),
            field_name='words',
            new_field_name='target')
        ds.apply_field(len, 'words', 'seq_len')
        dl = TorchDataLoader(ds, batch_size=BATCH_SIZE)
    ...
    def run_model(self, model, data, metrics):
        ...
        tester = Evaluator(
            model, data, metrics={'metric': metrics}, driver='torch')
