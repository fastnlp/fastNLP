...
if _NEED_IMPORT_TORCH:
    ...
    from fastNLP import Vocabulary, seq_len_to_mask
    from fastNLP.embeddings.torch import StaticEmbedding
    from fastNLP.modules.torch import (LSTMSeq2SeqDecoder, Seq2SeqDecoder,
                                       State, TransformerSeq2SeqDecoder)
...
class DummyState(State):
    ...
    def reorder_state(self, indices: 'torch.LongTensor'):
        self.decoder.decoder_output = self._reorder_state(
            self.decoder.decoder_output, indices, dim=0)
...
class TestSequenceGenerator:
        ...
        for do_sample in [True, False]:
            for num_beams in [1, 3, 5]:
                ...
                decoder = TransformerSeq2SeqDecoder(
                    embed=embed,
                    pos_embed=nn.Embedding(10, embed.embedding_dim),
                    d_model=encoder_output.size(-1),
                    num_layers=2,
                    n_head=2,
                    dim_ff=10,
                    dropout=0.1,
    ...
    def test_greedy_decode(self):
        ...
        for beam_search in [1, 3]:
            ...
            generator = SequenceGenerator(
                decoder=decoder,
                max_length=decoder_output.size(1),
                ...
                length_penalty=1,
                pad_token_id=0)
            decode_path = generator.generate(
                DummyState(decoder),
