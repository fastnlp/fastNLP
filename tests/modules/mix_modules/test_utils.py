...
from fastNLP.envs.imports import (_NEED_IMPORT_JITTOR, _NEED_IMPORT_PADDLE,
                                  _NEED_IMPORT_TORCH)
from fastNLP.modules.mix_modules.utils import (jittor2torch, paddle2torch,
                                               torch2jittor, torch2paddle)
    ...
    def check_paddle_tensor(self, tensor, device, stop_gradient):
        ...
        if device == 'cpu':
            ...
            if hasattr(tensor.place, 'gpu_device_id'):
                ...
                assert tensor.place.gpu_device_id(
                ) == paddle_device.get_device_id()
            else:
                # 通过_convert_to_place得到的是paddle.CUDAPlace
                # 通过get_device_id获取设备
                assert tensor.place.get_device_id(
                ) == paddle_device.get_device_id()
        else:
