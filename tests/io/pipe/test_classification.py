...
from fastNLP.io.pipe.classification import (AGsNewsPipe, ChnSentiCorpPipe,
                                            DBPediaPipe, IMDBPipe, SST2Pipe,
                                            SSTPipe, THUCNewsPipe,
                                            WeiboSenti100kPipe, YelpFullPipe,
                                            YelpPolarityPipe)
...
class TestClassificationPipe:
    ...
    def test_process_from_file(self):
        for pipe in [
                YelpPolarityPipe, SST2Pipe, IMDBPipe, YelpFullPipe, SSTPipe
        ]:
            print(pipe)
    ...
    def test_process_from_file_proc(self, num_proc=2):
        for pipe in [
            ...
            print(pipe)
            data_bundle = pipe(
                tokenizer='raw', num_proc=num_proc).process_from_file()
            print(data_bundle)
...
class TestRunPipe:
    ...
    def test_load(self):
        for pipe in [IMDBPipe]:
            data_bundle = pipe(
                tokenizer='raw',
                num_proc=0).process_from_file('data_for_tests/io/imdb')
            print(data_bundle)
    ...
    def test_load_proc(self):
        for pipe in [IMDBPipe]:
            data_bundle = pipe(
                tokenizer='raw',
                num_proc=2).process_from_file('data_for_tests/io/imdb')
            print(data_bundle)
...
class TestRunClassificationPipe:
    ...
    def test_process_from_file(self):
        data_set_dict = {
            'yelp.p':
            ('data_for_tests/io/yelp_review_polarity', YelpPolarityPipe, {
                'train': 6,
                'dev': 6,
                'test': 6
            }, {
            ...
            'ChnSentiCorp':
            ('data_for_tests/io/ChnSentiCorp', ChnSentiCorpPipe, {
                'train': 6,
                'dev': 6,
                'test': 6
            }, {
            ...
            'Chn-WeiboSenti100k':
            ('data_for_tests/io/WeiboSenti100k', WeiboSenti100kPipe, {
                'train': 6,
                'dev': 6,
                'test': 7
            }, {
        ...
        for k, v in data_set_dict.items():
            ...
            if 'Chn' not in k:
                if warns:
                    data_bundle = pipe(
                        tokenizer='raw', num_proc=0).process_from_file(path)
                else:
                    data_bundle = pipe(
                        tokenizer='raw', num_proc=0).process_from_file(path)
            else:
                data_bundle = pipe(
                    bigrams=True, trigrams=True).process_from_file(path)
        ...
        for k, v in data_set_dict.items():
            ...
            if 'Chn' not in k:
                ...
                data_bundle = pipe(
                    ...
                    num_proc=2).process_from_file(path)
