...
from fastNLP.io.pipe.matching import (BQCorpusBertPipe, BQCorpusPipe,
                                      CNXNLIBertPipe, CNXNLIPipe,
                                      LCQMCBertPipe, LCQMCPipe, MNLIBertPipe,
                                      MNLIPipe, QNLIBertPipe, QNLIPipe,
                                      QuoraBertPipe, QuoraPipe, RTEBertPipe,
                                      RTEPipe, SNLIBertPipe, SNLIPipe)
...
class TestRunMatchingPipe:
    ...
    def test_load(self):
        data_set_dict = {
            'RTE': ('data_for_tests/io/RTE', RTEPipe, RTEBertPipe, (5, 5, 5),
                    (449, 2), True),
            'SNLI': ('data_for_tests/io/SNLI', SNLIPipe, SNLIBertPipe,
                     (5, 5, 5), (110, 3), False),
            'QNLI': ('data_for_tests/io/QNLI', QNLIPipe, QNLIBertPipe,
                     (5, 5, 5), (372, 2), True),
            'MNLI': ('data_for_tests/io/MNLI', MNLIPipe, MNLIBertPipe,
                     (5, 5, 5, 5, 6), (459, 3), True),
            'BQCorpus': ('data_for_tests/io/BQCorpus', BQCorpusPipe,
                         BQCorpusBertPipe, (5, 5, 5), (32, 2), False),
            'XNLI': ('data_for_tests/io/XNLI', CNXNLIPipe, CNXNLIBertPipe,
                     (6, 6, 8), (39, 3), False),
            'LCQMC': ('data_for_tests/io/LCQMC', LCQMCPipe, LCQMCBertPipe,
                      (6, 5, 6), (36, 2), False),
        }
    ...
    def test_load_proc(self):
        ...
        for k, v in data_set_dict.items():
            ...
            if warns:
                data_bundle1 = pipe1(
                    tokenizer='raw', num_proc=2).process_from_file(path)
                data_bundle2 = pipe2(
                    tokenizer='raw', num_proc=2).process_from_file(path)
            else:
                data_bundle1 = pipe1(
    ...
    def test_spacy(self):
        data_set_dict = {
            'Quora': ('data_for_tests/io/Quora', QuoraPipe, QuoraBertPipe,
                      (2, 2, 2), (93, 2)),
        }
        for k, v in data_set_dict.items():
