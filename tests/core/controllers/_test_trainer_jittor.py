...
class PrintWhileTrainingCallBack(Callback):
    ...
    def on_train_begin(self, trainer):
        """在训练开始前输出信息."""
        print('Start training. Total {} epochs and {} batches in each epoch.'.
              format(trainer.n_epochs, trainer.num_batches_per_epoch))
    ...
    def on_train_epoch_end(self, trainer):
        ...
        if trainer.cur_epoch_idx % self.print_every_epoch == 0 \
            ...
            print('Epoch: {} Loss: {} Current epoch training time: {}s'.format(
                ...
                self.loss / trainer.num_batches_per_epoch,
    ...
    def on_train_batch_end(self, trainer):
        ...
        if trainer.batch_idx_in_epoch % self.print_every_batch == 0 \
            ...
            print('\tBatch: {} Loss: {}'.format(
                trainer.batch_idx_in_epoch,
                self.loss / trainer.batch_idx_in_epoch))

    def on_train_end(self, trainer):
        print('Total training time: {}s'.format(time.time() - self.start))


def process_data(ds: DatasetDict,
                 vocabulary: Vocabulary,
                 max_len=256) -> DatasetDict:
    # 分词
    ds = ds.map(
        lambda x: {'input_ids': text_to_id(vocabulary, x['text'], max_len)})
...
def get_dataset(name, max_len, train_format='', test_format=''):
    ...
    train_dataset = load_dataset(
        name, split='train' + train_format).shuffle(seed=123)
    test_dataset = load_dataset(
        name, split='test' + test_format).shuffle(seed=321)
    ...
    return TextClassificationDataset(train_dataset), \
        TextClassificationDataset(val_dataset), \
...
if __name__ == '__main__':
    ...
    # 获取数据集
    # imdb.py SetFit/sst2
    train_data, val_data, test_data, vocab = get_dataset(
        'SetFit/sst2', max_len, '', '')
    # 使用dataloader
