    ...
    for i in range(number_labels):
        ...
        for tag in encoding_type:
            if tag == 'O':
                ...
                continue
            vocab['{}-{}'.format(tag,
                                 label)] = len(vocab) + 1  # 其实表达的是这个的count
    return vocab
...
def _test(local_rank: int, world_size: int, device: 'torch.device',
          dataset: DataSet, metric_class, metric_kwargs,
          sklearn_metric) -> None:
    ...
    for i in range(local_rank, len(dataset), world_size):
        pred, tg, seq_len = dataset[i]['pred'].to(device), dataset[i]['tg'].to(
            device), dataset[i]['seq_len']
        print(tg, seq_len)
...
class TestSpanFPreRecMetric:
    ...
    def test_case1(self):
        from fastNLP.core.metrics.span_f1_pre_rec_metric import (
            _bio_tag_to_spans, _bmes_tag_to_spans)

        bmes_lst = [
            'M-8', 'S-2', 'S-0', 'B-9', 'B-6', 'E-5', 'B-7', 'S-2', 'E-7',
            'S-8'
        ]
        bio_lst = [
            'O-8', 'O-2', 'B-0', 'O-9', 'I-6', 'I-5', 'I-7', 'I-2', 'I-7',
            'O-8'
        ]
        expect_bmes_res = set()
        expect_bmes_res.update([('8', (0, 1)), ('2', (1, 2)), ('0', (2, 3)),
                                ('9', (3, 4)), ('6', (4, 5)), ('5', (5, 6)),
                                ('7', (6, 7)), ('2', (7, 8)), ('7', (8, 9)),
                                ('8', (9, 10))])
        expect_bio_res = set()
        expect_bio_res.update([('7', (8, 9)), ('0', (2, 3)), ('2', (7, 8)),
                               ('5', (5, 6)), ('6', (4, 5)), ('7', (6, 7))])
    ...
    def test_case2(self):
        ...
        expect_bmes_res = set()
        expect_bmes_res.update([('', (0, 2)), ('', (2, 3)), ('', (3, 4)),
                                ('', (4, 7)), ('', (7, 8)), ('', (8, 10))])
        expect_bio_res = set()
        expect_bio_res.update([('', (7, 8)), ('', (6, 7)), ('', (4, 5)),
                               ('', (0, 1)), ('', (1, 2))])
        assert expect_bmes_res == set(_bmes_tag_to_spans(bmes_lst))
        assert expect_bio_res == set(_bio_tag_to_spans(bio_lst))
    ...
    def test_case3(self):
        ...
        fastnlp_bio_vocab.word_count = Counter(
            _generate_tags('BIO', number_labels))
        fastnlp_bio_metric = SpanFPreRecMetric(
    ...
    def test_encoding_type(self):
            ...
            for i in range(random.randint(10, 100)):
                ...
                for tag in encoding_type:
                    if tag != 'o':
                        ...
                        vocab.add_word('o')
            vocabs[encoding_type] = vocab
        for e1, e2 in product(['bio', 'bioes', 'bmeso'],
                              ['bio', 'bioes', 'bmeso']):
            if e1 == e2:
                SpanFPreRecMetric(tag_vocab=vocabs[e1], encoding_type=e2)
            else:
                s2 = set(e2)
                s2.update(set(e1))
                if s2 == set(e2):
        ...
        for encoding_type in ['bio', 'bioes', 'bmeso']:
            with pytest.raises(AssertionError):
                SpanFPreRecMetric(
                    tag_vocab=vocabs[encoding_type], encoding_type='bmes')

        vocab = Vocabulary(
            unknown=None, padding=None).add_word_lst(list('bmes'))
        SpanFPreRecMetric(tag_vocab=vocab, encoding_type='bmeso')
    ...
    def test_case5(self):
        ...
        dataset = DataSet({
            'pred': [
                torch.FloatTensor(
                    [[[
                        -0.4424, -0.4579, -0.7376, 1.8129, 0.1316, 1.6566,
                        -1.2169, -0.3782, 0.8240
                    ],
                      [
                          -1.2348, -0.1876, -0.1462, -0.4834, -0.6692, -0.9735,
                          1.1563, -0.3562, -1.4116
                      ],
                      [
                          1.6550, -0.9555, 0.3782, -1.3160, -1.5835, -0.3443,
                          -1.7858, 2.0023, 0.7075
                      ],
                      [
                          -0.3772, -0.5447, -1.5631, 1.1614, 1.4598, -1.2764,
                          0.5186, 0.3832, -0.1540
                      ],
                      [
                          -0.1011, 0.0600, 1.1090, -0.3545, 0.1284, 1.1484,
                          -1.0120, -1.3508, -0.9513
                      ],
                      [
                          1.8948, 0.8627, -2.1359, 1.3740, -0.7499, 1.5019,
                          0.6919, -0.0842, -0.4294
                      ]]]),
                torch.FloatTensor([[[
                    -0.2802, 0.6941, -0.4788, -0.3845, 1.7752, 1.2950, -1.9490,
                    -1.4138, -0.8853
                                    ...
                                    [
                                        -1.3752, -0.5457, -0.5305, 0.4018,
                                        0.2934, 0.7931, 2.3845, -1.0726, 0.0364
                                    ],
                                    [
                                        0.3621, 0.2609, 0.1269, -0.5950,
                                        0.7212, 0.5959, 1.6264, -0.8836,
                                        -0.9320
                                    ],
                                    [
                                        0.2003, -1.0758, -1.1560, -0.6472,
                                        -1.7549, 0.1264, 0.6044, -1.6857,
                                        1.1571
                                    ],
                                    [
                                        1.4277, -0.4915, 0.4496, 2.2027,
                                        0.0730, -3.1792, -0.5125, -0.5837,
                                        1.0184
                                    ],
                                    [
                                        1.9495, 1.7145, -0.2143, -0.1230,
                                        -0.2205, 0.8250, 0.4943, -0.9025,
                                        0.0864
                                    ]]])
            ],
            'tg': [
