...
class TestClassfiyFPreRecMetric:
    ...
    def test_case_1(self):
        ...
        target = torch.tensor([
            0, 2, 4, 1, 4, 0, 1, 3, 3, 3, 1, 3, 4, 4, 3, 4, 0, 2, 4, 4, 3, 4,
            4, 3, 0, 3, 0, 0, 0, 1, 3, 1
        ])
        ...
        for keys in ['f', 'pre', 'rec']:
            assert np.allclose(
                result_dict[keys], ground_truth[keys], atol=0.000001)
        ...
        for keys in result_dict.keys():
            ...
            gk = tmp_d[keys[0]]
            assert np.allclose(
                result_dict[keys], ground_truth[gl][gk], atol=0.000001)

    def test_seq_len(self):
        pred = torch.tensor([[[0.3, 0.7, 0.1], [0.4, 0.1, 0.1],
                              [0.3, 0.1, 0.7]],
                             [[0.7, 0.1, 0.1], [0.5, 0.9, 0.1],
                              [0.3, 0.1, 0.7]]])
    ...
    def test_case_2(self, f_type, f1_score, recall, pre):
        ...
        dataset = DataSet({
            'pred': [
                torch.tensor([[-0.4375, -0.1779, -1.0985, -1.1592, 0.4910],
                              [1.3410, 0.2889, -0.8667, -1.8580, 0.3029],
                              [0.7459, -1.1957, 0.3231, 0.0308, -0.1847],
                              [1.1439, -0.0057, 0.8203, 0.0312, -1.0051],
                              [-0.4870, 0.3215, -0.8290, 0.9221, 0.4683],
                              ...
                              [0.4165, 0.4084, -0.1280, 1.4489, -2.3058],
                              [-0.5826, -0.5469, 1.5898, -0.2786, -0.9882]]),
                torch.tensor([[-1.5548, -2.2891, 0.2983, -1.2145, -0.1947],
                              [-0.7222, 2.3543, -0.5801, -0.0640, -1.5614],
                              [-1.4978, 1.9297, -1.3652, -0.2358, 2.5566],
                              [0.1561, -0.0316, 0.9331, 1.0363, 2.3949],
                              [0.2650, -0.8459, 1.3221, 0.1321, -1.1900],
            ...
            'tg': [
                torch.LongTensor(
                    [0, 2, 4, 1, 4, 0, 1, 3, 3, 3, 1, 3, 4, 4, 3, 4]),
                torch.LongTensor(
                    [0, 2, 4, 4, 3, 4, 4, 3, 0, 3, 0, 0, 0, 1, 3, 1])
            ]
        })
        metric_kwargs = {
            'f_type': f_type,
        ...
        pool.starmap(
            ...
            [(rank, NUM_PROCESSES, torch.device(f'cuda:{rank}'))
             for rank in range(NUM_PROCESSES)])
        pool.close()
        pool.join()
