...
from .utils import _assert_allclose, find_free_network_port, setup_ddp
...
def _test(local_rank: int,
    ...
    for i in range(local_rank, len(dataset), world_size):
        pred, tg = torch.tensor(dataset[i]['pred']).to(device), torch.tensor(
            dataset[i]['target']).to(device)
    ...
    using_target, using_predict = np.array(using_target), np.array(
        using_predict)
    sklearn_result = sklearn_metric(using_predict, using_target)
    _assert_allclose(my_result, sklearn_result, atol=atol)
...
@pytest.mark.parametrize('dataset', [
    DataSet({
        'pred': np.random.randint(low=0, high=1, size=(36, 32)),
        'target': np.random.randint(low=0, high=1, size=(36, 32))
    }),
    DataSet({
        'pred': np.random.randint(low=0, high=1, size=(360, 32)),
