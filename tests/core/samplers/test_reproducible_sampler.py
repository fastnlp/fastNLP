...
class TestRandomSamplerYh:
    ...
    @pytest.mark.parametrize('num_consumed_samples',
                             [0] + np.random.randint(1, 100, size=3).tolist())
    def test_state_dict(self, pad, pre_shuffle, post_shuffle,
    ...
    @pytest.mark.parametrize('pre_shuffle', [True, False])
    @pytest.mark.parametrize('post_shuffle', [True, False])
    @pytest.mark.parametrize('num_consumed_samples', [0] +
                             np.random.randint(1, 100 // 2, size=3).tolist())
    def test_state_dict_2(self, pad, pre_shuffle, post_shuffle,
                          num_consumed_samples):
    ...
    def test_num_consumed_samples_array(self, shuffle, pad, num_samples,
                                        num_replicas):
        ...
        num_consumed_samples_array = list(
            range(0,
                  len(dataset) + num_replicas, num_replicas))
...
class TestRandomSampler:
    ...
    def test_seed_work_when_shuffle_is_true(self):
        ...
        for shuffle in [True, False]:
            iterable = RandomSampler(
                dataset=torch_normal_data, shuffle=shuffle)
            # 迭代一些数据，但是不迭代完；
    ...
    def test_2(self):
        data_length = 100
        torch_normal_data = TorchNormalDataset(num_of_data=data_length)
        random_sampler_1 = RandomSampler(
            dataset=torch_normal_data, shuffle=True)
        ...
        random_sampler_2 = RandomSampler(
            dataset=torch_normal_data, shuffle=True)
    ...
    def test_3(self):
        ...
        torch_normal_data = TorchNormalDataset(num_of_data=data_length)
        random_sampler_1 = partial(
            RandomSampler, dataset=torch_normal_data, shuffle=False)
        random_sampler_2 = partial(
            RandomSampler, dataset=torch_normal_data, shuffle=True)
        ...
        for pad in {True, False}:
            for iterable in iterable_items:
                for rank in range(world_size):
                    ...
                    each_rank_iterable.set_distributed(
                        num_replicas=world_size, rank=rank, pad=pad)
                    # 迭代一些数据，但是不迭代完；
    ...
    def test_4(self):
        ...
        for pad in {True, False}:
            ...
            for rank in range(world_size_1):
                ...
                each_rank_iterable.set_distributed(
                    num_replicas=world_size_1, rank=rank, pad=pad)
                iterator = iter(each_rank_iterable)
                # 第一轮
                each_rank_iterable.set_epoch(0)
                first_epoch = []
    ...
    def test_multi(self, pad, num_replicas, num_of_data):
        ...
        for sampler in samplers:
            ...
            for index in sampler:
                seen_in_other_rank += int(index
                                          in already_seen_index)  # 不同的卡不交叉
                cur_set.add(index)
