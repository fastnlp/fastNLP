...
from fastNLP.envs import FASTNLP_DISTRIBUTED_CHECK, FASTNLP_LAUNCH_TIME
            ...
            try:
                ...
                if version == 0:
                    ...
                    if not isinstance(device, list):
                        ...
                        epoch_save_path = all_saved_model_paths[
                            'model-epoch_10']
                    ...
                    if not isinstance(device, list):
                        ...
                        for each_folder_name in all_saved_model_paths:
                            each_folder_name = pattern.findall(
                                each_folder_name)
                        ...
                        topk_save_path = all_saved_model_paths[
                            aLL_topk_folders[0]]
...
@magic_argv_env_context(timeout=100)
def test_model_checkpoint_callback_2(model_and_optimizers: TrainerParameters,
                                     driver, device, only_state_dict):
    skip_no_cuda()
    try:
        path = Path.cwd().joinpath('test_model_checkpoint')
        path.mkdir(exist_ok=True, parents=True)
        ...
        def raise_exception(trainer):
            if trainer.driver.get_local_rank(
            ) == 0 and trainer.cur_epoch_idx == 4:
        ...
        callbacks = [
            CheckpointCallback(
                ...
                every_n_batches=None,
                last=False,
                on_exceptions=NotImplementedError,
                topk=None,
                monitor=None,
                only_state_dict=only_state_dict,
                save_object='model'),
        ...
        if not isinstance(device, list):
            ...
            exception_model_path = all_saved_model_paths[
                'model-epoch_4-batch_100-exception_NotImplementedError']
        # ddp 下的文件名不同，因为同样的数据，ddp 用了更少的步数跑完；
        else:
            assert 'model-epoch_4-batch_52-exception_NotImplementedError' in all_saved_model_paths
            exception_model_path = all_saved_model_paths[
                'model-epoch_4-batch_52-exception_NotImplementedError']
        ...
        for only_state_dict in [True, False]:
            try:
                ...
                if version == 0:
                    ...
                    pattern = re.compile(
                        'trainer-epoch_[0-9]+-batch_[0-9]+-[a-zA-Z#]+_[0-9]*.?[0-9]*'
                    )
...
def test_load_state(model_and_optimizers):
    try:
        path = Path.cwd().joinpath('test_model_checkpoint')
        path.mkdir(exist_ok=True, parents=True)
        from fastNLP import Callback, Event
        ...
        callbacks = [
            StateCallback('old_callback1'),
            StateCallback('old_callback2'),
            CheckpointCallback(
                folder=path, every_n_epochs=1, save_object='trainer')
        ]
        ...
        callbacks = [
            StateCallback('new_callback1'),
            StateCallback('new_callback2')
        ]
        trainer = Trainer(
            model=model_and_optimizers.model,
            driver='torch',
...
@pytest.mark.parametrize('driver,device', [('torch', [6, 7]), ('torch', 7)]
                         )  # ("torch", "cpu"), ("torch", [0, 1]), ("torch", 1)
@pytest.mark.parametrize('version', [0, 1])
@magic_argv_env_context
@pytest.mark.skip('Skip transformers test for now.')
def test_trainer_checkpoint_callback_2(driver, device, version):
    skip_no_cuda(device)
    path = Path.cwd().joinpath('test_model_checkpoint')
    path.mkdir(exist_ok=True, parents=True)
    ...
    class TestDistilBertDataset(Dataset):
        ...
        def __getitem__(self, item):
            ...
            return _data['input_ids'], _data['attention_mask'], [
                _data['label']
            ]  # , _data["sentence1"], _data["sentence2"]
    ...
    def test_bert_collate_fn(batch):
        ...
        return {
            'input_ids':
            torch.cat([torch.tensor([item]) for item in input_ids], dim=0),
            'attention_mask':
            torch.cat([torch.tensor([item]) for item in atten_mask], dim=0),
            'labels':
            torch.cat([torch.tensor(item) for item in labels], dim=0)
        }

    test_bert_dataset_train = TestDistilBertDataset(encoded_dataset['train'])
    test_bert_dataloader_train = DataLoader(
        dataset=test_bert_dataset_train,
        batch_size=32,
        shuffle=True,
        collate_fn=test_bert_collate_fn)
    test_bert_dataset_validate = TestDistilBertDataset(encoded_dataset['test'])
    test_bert_dataloader_validate = DataLoader(
        dataset=test_bert_dataset_validate,
        batch_size=32,
        shuffle=False,
        collate_fn=test_bert_collate_fn)
