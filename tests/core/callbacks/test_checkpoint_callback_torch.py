...
@pytest.mark.torch
# 测试保存断点时不产生时间戳文件夹
@pytest.mark.parametrize("driver,device,use_timestamp_folder", [("torch", "cpu", False), ("torch", [1, 2], False)])
@magic_argv_env_context(timeout=100)
def test_trainer_checkpoint_callback_without_timestamp_folder(
    ...
    try:
        callbacks = [
            CheckpointCallback(folder=path, every_n_epochs=1, every_n_batches=None, save_object='trainer'
