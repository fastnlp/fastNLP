...
def test_get_monitor_value():
    ...
    with Capturing():
        monitor, value = _get_monitor_value(
            monitor='f1', real_monitor=None, res=res)
    ...
    with Capturing():
        monitor, value = _get_monitor_value(
            monitor='acc', real_monitor='acc#rec', res=res)
    ...
    with Capturing():
        monitor, value = _get_monitor_value(
            monitor='acc#f', real_monitor='acc#r', res=res)
    assert monitor == 'acc#f1' and value == 0.2
    ...
    with Capturing():
        monitor, value = _get_monitor_value(
            monitor='-loss', real_monitor=None, res=res)
