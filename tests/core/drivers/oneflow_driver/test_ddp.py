...
from fastNLP import logger, prepare_oneflow_dataloader
...
def generate_driver(labels,
    ...
    oneflow_opt = oneflow.optim.Adam(
        params=oneflow_model.parameters(), lr=0.01)
...
def test_shuffle_dataloader(shuffle, batch_size, drop_last, reproducible=True):
    try:
        ...
        res = fastnlp_oneflow_all_gather(data)
        if drop_last:
            assert len(
                set(res[0] +
