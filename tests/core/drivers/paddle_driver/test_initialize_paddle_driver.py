...
from fastNLP.core.drivers import PaddleFleetDriver, PaddleSingleDriver
...
def test_incorrect_driver():
    ...
    with pytest.raises(ValueError):
        initialize_paddle_driver('torch', 0, model)


@pytest.mark.paddle
@pytest.mark.parametrize('device', ['cpu', 'gpu:0', 0])
...
@pytest.mark.paddle
@pytest.mark.parametrize('device', [[0, 2, 3], -1])
...
@pytest.mark.parametrize(
    'device', [-2, [0, get_gpu_count() + 1, 3], [-2],
               get_gpu_count() + 1])
@magic_argv_env_context
def test_device_out_of_range(device):
    ...
    model = PaddleNormalModel_Classification_1(20, 10)
    with pytest.raises(ValueError):
        initialize_paddle_driver('paddle', device, model)
