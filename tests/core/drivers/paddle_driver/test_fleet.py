...
from fastNLP import logger, prepare_paddle_dataloader
...
def generate_driver(labels,
    ...
    paddle_opt = paddle.optimizer.Adam(
        parameters=paddle_model.parameters(), learning_rate=0.01)
...
class TestFleetDriverFunction:
    ...
    def test_multi_drivers(self):
        ...
        driver2 = generate_driver(20, 10)  # noqa: F841

        with pytest.raises(RuntimeError):
            # 设备设置不同，应该报错
            generate_driver(20, 3, device=[0, 2])

        dist.barrier()

    @magic_argv_env_context
    def test_move_data_to_device(self):
        """这个函数仅调用了paddle_move_data_to_device，测试例在tests/core/utils/test_paddle_
        utils.py中 就不重复测试了."""
    ...
    def test_all_gather(self):
        """测试 all_gather 函数 详细的测试在 test_dist_utils.py 中完成."""
    ...
    def test_broadcast_object(self, src_rank):
        """测试 broadcast_object 函数 详细的函数在 test_dist_utils.py 中完成."""
...
class TestSetDistReproDataloader:
    ...
    def test_with_dist_none_reproducible_true(self):
        ...
        with pytest.raises(RuntimeError):
            # 应当抛出 RuntimeError
            self.driver.set_dist_repro_dataloader(dataloader, None, True)
    ...
    def test_with_dist_none_reproducible_false_dataloader_reproducible_sampler(
        ...
        dataloader = DataLoader(self.dataset, batch_sampler=batch_sampler)
        replaced_loader = self.driver.set_dist_repro_dataloader(
            dataloader, None, False)
    ...
    def test_with_dist_dist_dataloader_reproducible_sampler(self, shuffle):
        ...
        batch_sampler = BatchSampler(
            dataset=self.dataset, batch_size=4, shuffle=shuffle)
        batch_sampler.sampler = RandomSampler(self.dataset, shuffle)
        dataloader = DataLoader(self.dataset, batch_sampler=batch_sampler)
        replaced_loader = self.driver.set_dist_repro_dataloader(
            dataloader, 'dist', False)
    ...
    def check_set_dist_repro_dataloader(self, dataloader, replaced_loader,
        ...
        if isinstance(replaced_loader.batch_sampler, BucketedBatchSampler):
            ...
            sampler_states = replaced_loader.batch_sampler.sampler.set_epoch(
                10)
        ...
        if isinstance(replaced_loader.batch_sampler, BucketedBatchSampler):
            ...
            batch_sampler.sampler = RandomSampler(
                replaced_loader.dataset, shuffle=shuffle)
            batch_sampler.sampler.set_distributed(
                num_replicas=self.driver.world_size,
                rank=self.driver.global_rank)
            new_loader = DataLoader(
                replaced_loader.dataset, batch_sampler=batch_sampler)
    ...
    def test_shuffle_dataloader(self,
        ...
        try:
            ...
            res = fastnlp_paddle_all_gather(data)
            if drop_last:
                assert len(
                    set(res[0] +
    ...
    @pytest.mark.parametrize('batch_size', ([1, 3, 16, 17]))
    @pytest.mark.parametrize('drop_last', ([True, False]))
    def test_batch_sampler_dataloader(self,
                                      shuffle,
                                      batch_size,
                                      drop_last,
                                      reproducible=True):
    ...
    def test_save_and_load_model(self, only_state_dict):
        ...
        try:
            ...
            if only_state_dict:
                ...
                self.driver1.save_model(
                    path, only_state_dict, input_spec=[paddle.ones((4, 1))])
    ...
    def test_save_and_load_with_bucketedbatchsampler(self, device,
        ...
        try:
            ...
            self.driver1, self.driver2 = \
                generate_driver(40, 1, device=device, fp16=fp16), \
            ...
            dataloader.batch_sampler.set_distributed(
                ...
                pad=True)
            dist.barrier()
            load_states = self.driver2.load_checkpoint(
                Path(path),
                dataloader,
                only_state_dict,
