...
from fastNLP import BucketedBatchSampler, prepare_paddle_dataloader
...
class TestPaddleDriverFunctions:
    ...
    def test_check_optimizers_legality(self):
        ...
        optimizer = paddle.optimizer.Adam(
            parameters=self.driver.model.parameters(), learning_rate=0.01)
...
class TestSingleDeviceFunction:
    ...
    def test_move_data_to_device(self):
        """这个函数仅调用了 paddle_move_data_to_device ，测试例在
        tests/core/utils/test_paddle_utils.py 中 就不重复测试了."""
...
class TestSetDistReproDataloader:
    ...
    def test_with_dist_batch_sampler(self, shuffle):
        ...
        dist = ReproduceBatchSampler(
            BatchSampler(self.dataset, batch_size=4, shuffle=shuffle), 4,
            False)
        replaced_loader = self.driver.set_dist_repro_dataloader(
            dataloader, dist=dist, reproducible=False)
    ...
    def test_with_dataloader_reproducible_sampler(self, shuffle):
        ...
        dataloader，且其余各项设置和原来相同."""
        batch_sampler = BatchSampler(
            dataset=self.dataset, batch_size=2, shuffle=shuffle)
    ...
    def test_shuffle_dataloader(self, shuffle, batch_size, drop_last,
                                reproducible):
    ...
    @pytest.mark.parametrize('drop_last', ([True, False]))
    @pytest.mark.parametrize('reproducible', ([True, False]))
    def test_batch_sampler_dataloader(self, shuffle, batch_size, drop_last,
                                      reproducible):
        # 需要检验一下 set_dist_repro_dataloader 没有修改参数
        num_samples = 200
        dataset = PaddleNormalXYDataset(num_samples)
...
def test_save_and_load_model(only_state_dict):
    ...
    try:
        ...
        if only_state_dict:
            ...
            driver1.save_model(
                path, only_state_dict, input_spec=[paddle.ones((32, 10))])
...
def test_save_and_load_with_randombatchsampler(only_state_dict, fp16):
    ...
    try:
        ...
        dataloader = DataLoader(
            dataset=dataset,
            batch_sampler=ReproduceBatchSampler(
                BatchSampler(dataset, batch_size=4), 4, False))
        driver1, driver2 = generate_random_driver(
            40, 1, fp16, 'gpu'), generate_random_driver(40, 1, False, 'gpu')
        ...
        dataloader = DataLoader(
            dataset=dataset,
            batch_sampler=ReproduceBatchSampler(
                BatchSampler(dataset, batch_size=2, shuffle=True), 2, False))
