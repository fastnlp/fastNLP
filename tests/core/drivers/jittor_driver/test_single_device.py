...
from tests.helpers.datasets.jittor_data import (JittorNormalDataset,
                                                JittorNormalXYDataset)
from tests.helpers.datasets.torch_data import TorchNormalDataset
from tests.helpers.models.jittor_model import \
    JittorNormalModel_Classification_1
...
class TestJittorDriverFunctions:
    ...
    def test_check_optimizers_legality(self):
        ...
        # optimizer 列表
        optimizers = [
            jt.optim.Adam(params=self.driver.model.parameters(), lr=0.01)
            for i in range(10)
        ]
    ...
    def test_invalid_optimizers(self):
        ...
        optimizer = torch.optim.Adam(
            TorchNormalModel_Classification_1(10, 32).parameters(), 0.01)
...
class TestSetDistReproDataloader:
    ...
    def test_with_dist_batch_sampler(self, shuffle, use_dataloader):
        ...
        对应的 Sampler jittor 暂时不支持这种情况，会报错."""
        dataloader = get_dataloader(
            self.dataset,
            use_dataloader,
            sampler=None,
        ...
        dist = ReproduceBatchSampler(
            JittorBatchSampler(JittorRandomSampler(self.dataset), 4, False), 4,
            False)

        with pytest.raises(RuntimeError):
            self.driver.set_dist_repro_dataloader(
                dataloader, dist=dist, reproducible=False)
    ...
    @pytest.mark.parametrize('shuffle', ([True, False]))
    @pytest.mark.parametrize('use_dataloader', [True, False])
    def test_with_dataloader_reproducible_batch_sampler(
            self, shuffle, use_dataloader):
        ...
        dataloader = get_dataloader(
            ...
            sampler=ReproduceBatchSampler(
                JittorBatchSampler(
                    JittorRandomSampler(self.dataset), 4, False),
                batch_size=4,
                drop_last=False,
            ),
            batch_size=4,
    ...
    def check_set_dist_repro_dataloader(self, dataloader, replaced_loader,
        ...
        assert len(left_idxes) + len(
            already_seen_idx) == self.dataset.total_len
...
def generate_random_driver(labels,
                           features,
                           fp16=False,
                           device='cpu',
                           lr=0.01):
...
def test_save_and_load_model(only_state_dict, use_dataloader):
    ...
    try:
        ...
        driver1, driver2 = generate_random_driver(
            20, 1, device='gpu'), generate_random_driver(
                20, 1, device='gpu')
...
def test_save_and_load_with_randomsampler(only_state_dict, use_dataloader):
    ...
    try:
        ...
        driver1, driver2 = \
            generate_random_driver(20, 1, device='gpu', lr=0.01), \
        ...
        assert replaced_loader.sampler.dataset.total_len == sampler_states[
            'length']
