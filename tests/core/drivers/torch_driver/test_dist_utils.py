...
from fastNLP.core.drivers.torch_driver.dist_utils import (
    fastnlp_torch_all_gather, fastnlp_torch_broadcast_object)
from fastNLP.envs.imports import _NEED_IMPORT_TORCH
from tests.helpers.utils import (magic_argv_env_context,
                                 re_run_current_cmd_for_torch, skip_no_cuda)
...
def test_fastnlp_torch_all_gather():
    ...
    try:
        ...
        obj = {
            ...
            'numpy':
            np.full(shape=(2, ), fill_value=local_rank),
            'bool':
            local_rank % 2 == 0,
            'float':
            local_rank + 0.1,
            ...
            f'{local_rank}',
            'tensors': [
                torch.full(size=(2, ), fill_value=local_rank,
                           dtype=int).cuda(),
                torch.full(size=(2, ), fill_value=local_rank,
                           dtype=int).cuda()
            ]
...
def test_fastnlp_torch_broadcast_object():
    ...
    try:
        ...
        if os.environ['LOCAL_RANK'] == '0':
            obj = {
                ...
                torch.full(size=(2, ), fill_value=local_rank,
                           dtype=int).cuda(),
                'numpy':
                np.full(shape=(2, ), fill_value=local_rank, dtype=int),
                'bool':
                local_rank % 2 == 0,
                'float':
                local_rank + 0.1,
            ...
            obj = None
        data = fastnlp_torch_broadcast_object(
            obj, src=0, device=torch.cuda.current_device())
        i = 0
        ...
        for obj in [
                ...
                bool(os.environ['LOCAL_RANK'] == '1'), os.environ['LOCAL_RANK']
        ]:
            data = fastnlp_torch_broadcast_object(
                obj, src=0, device=torch.cuda.current_device())
            assert int(data) == 0
