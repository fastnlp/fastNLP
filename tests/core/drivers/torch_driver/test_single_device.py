...
from fastNLP import BucketedBatchSampler, prepare_torch_dataloader
...
class TestTorchDriverFunctions:
    ...
    def test_check_optimizers_legality(self):
        ...
        # 列表
        optimizers = [
            torch.optim.Adam(params=self.driver.model.parameters(), lr=0.01)
            for i in range(10)
        ]
    ...
    def test_invalid_optimizers(self):
        ...
        optimizers = [
            paddle.optimizer.Adam(
                parameters=PaddleNormalModel_Classification_1(10,
                                                              32).parameters(),
                learning_rate=0.01,
            )
        ]
    ...
    def test_check_dataloader_legality_invalid(self):
        ...
        dataloader = paddle.io.DataLoader(
            PaddleNormalDataset(), batch_size=32, shuffle=True)
        with pytest.raises(TypeError):
            self.driver.check_dataloader_legality(dataloader)
    ...
    def test_worker_init_function(self):
        ...
        if parse_version(torch.__version__) < parse_version('1.7'):
            pytest.skip(
                'Skip if torch version smaller than 1.6 since torch.manual_seed my cause bug:'
...
class TestSingleDeviceFunction:
    ...
    def test_move_data_to_device(self):
        """这个函数仅调用了 torch_move_data_to_device ，测试例在
        tests/core/utils/test_torch_utils.py 中 就不重复测试了."""
