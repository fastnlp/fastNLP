...
def dataloader_with_bucketedbatchsampler(dataset, length, batch_size, shuffle,
                                         drop_last):
...
class TestSetDistReproDataloader:
    ...
    def test_with_dist_batch_sampler(self, shuffle):
        ...
        dataloader = DataLoader(
            self.dataset, batch_size=4, shuffle=not shuffle)
        batch_sampler = BucketedBatchSampler(
            self.dataset, self.dataset._data, batch_size=4, shuffle=shuffle)
        replaced_loader = driver.set_dist_repro_dataloader(
            dataloader, batch_sampler, False)
        ...
        self.check_set_dist_repro_dataloader(driver, dataloader,
                                             replaced_loader, shuffle)
    ...
    def test_with_dist_sampler(self, shuffle):
        """测试 set_dist_repro_dataloader 中 dist 为 RandomSampler 时的表现 此时应该将
        batch_sampler.sampler 替换为 dist 对应的 RandomSampler."""
        driver = generate_driver(10, 10, device=self.device)
        dataloader = DataLoader(
            self.dataset, batch_size=4, shuffle=not shuffle)
        sampler = RandomSampler(self.dataset, shuffle=shuffle)
        replaced_loader = driver.set_dist_repro_dataloader(
            dataloader, sampler, False)
    ...
    def test_with_dist_none_reproducible_true(self):
        ...
        with pytest.raises(RuntimeError):
            # 应当抛出 RuntimeError
            driver.set_dist_repro_dataloader(dataloader, None, True)
    ...
    @magic_argv_env_context
    @pytest.mark.parametrize('shuffle', ([True, False]))
    def test_with_dist_none_reproducible_false_dataloader_reproducible_batch_sampler(
            self, shuffle):
        """测试 set_dist_repro_dataloader 中 dist 为 None、reproducible 为 False.

        、dataloader 有 BucketedBatchSampler 时的表现 此时传入的 dataloader 的
        batch_sampler 应该已经执行了 set_distributed，产生一个新的 dataloader，其 batch_sampler
        和原 dataloader 相同.
        """
        driver = generate_driver(10, 10, device=self.device)
        dataloader = dataloader_with_bucketedbatchsampler(
            self.dataset, self.dataset._data, 4, shuffle, False)
        dataloader.batch_sampler.set_distributed(
            num_replicas=driver.world_size, rank=driver.global_rank, pad=True)
        replaced_loader = driver.set_dist_repro_dataloader(
            dataloader, None, False)
    ...
    @magic_argv_env_context
    @pytest.mark.parametrize('shuffle', ([True, False]))
    def test_with_dist_none_reproducible_false_dataloader_reproducible_sampler(
            self, shuffle):
        """测试 set_dist_repro_dataloader 中 dist 为 None、reproducible 为 False.

        、dataloader 有 RandomSampler 时的表现 此时传入的 dataloader 的
        batch_sampler.sampler 应该已经执行了 set_distributed，产生一个新的 dataloader，其
        batch_sampler.sampler 和原 dataloader 相同.
        """
        driver = generate_driver(10, 10, device=self.device)
        dataloader = dataloader_with_randomsampler(
            self.dataset, 4, shuffle, False, unrepeated=False)
        ...
        assert replaced_loader.batch_sampler.batch_size == 4
        assert replaced_loader.batch_sampler.drop_last is False
        self.check_distributed_sampler(replaced_loader.batch_sampler.sampler)
        self.check_set_dist_repro_dataloader(driver, dataloader,
                                             replaced_loader, shuffle)
    ...
    @magic_argv_env_context
    @pytest.mark.parametrize('shuffle', ([True, False]))
    def test_with_dist_none_reproducible_false_dataloader_normal(
            self, shuffle):
        """测试 set_dist_repro_dataloader 中 dist 为 None、reproducible 为 False.

        、dataloader 为一般情况时的表现 此时直接返回原来的 dataloader，不做任何处理。
        """
        driver = generate_driver(10, 10, device=self.device)
    ...
    @magic_argv_env_context
    @pytest.mark.parametrize('shuffle', ([True, False]))
    def test_with_dist_dist_dataloader_reproducible_batch_sampler(
            self, shuffle):
        """测试 set_dist_repro_dataloader 中 dist 为
        'dist'、dataloader.batch_sampler 为 ReproducibleBatchSampler 的表现
        此时应该返回一个新的 dataloader，其batch_sampler 和原 dataloader
        相同，且应该正确地设置了分布式相关的属性."""
        driver = generate_driver(10, 10, device=self.device)
        dataloader = DataLoader(
    ...
    def test_with_dist_dist_dataloader_reproducible_sampler(self, shuffle):
        """测试 set_dist_repro_dataloader 中 dist 为
        'dist'、dataloader.batch_sampler.sampler 为 ReproducibleSampler 的表现
        此时应该返回一个新的 dataloader，其 batch_sampler.sampler 和原 dataloader
        相同，且应该正确地设置了分布式相关 的属性."""
        driver = generate_driver(10, 10, device=self.device)
        dataloader = dataloader_with_randomsampler(
            self.dataset, 4, shuffle, False, unrepeated=False)
        replaced_loader = driver.set_dist_repro_dataloader(
    ...
    def test_with_dist_dist_dataloader_normal(self, shuffle):
        ...
        此时应该返回一个新的 dataloader，并替换其 batch_sampler.sampler 为
        RandomSampler，且应该正确设置了分布式相关 的属性."""
        driver = generate_driver(10, 10, device=self.device)
        dataloader = DataLoader(self.dataset, batch_size=4, shuffle=shuffle)
        replaced_loader = driver.set_dist_repro_dataloader(
            dataloader, 'dist', False)
    ...
    def test_with_dist_unrepeat_dataloader_reproducible_sampler(self, shuffle):
        """测试 set_dist_repro_dataloader 中 dist 为
        'unrepeatdist'、dataloader.batch_sampler.sampler 为 ReproducibleSampler
        的表现 此时应该返回一个新的 dataloader，且将原来的 Sampler 替换为
        UnrepeatedRandomSampler，且正确地设置了分布式相关 的属性."""
        driver = generate_driver(10, 10, device=self.device)
        dataloader = dataloader_with_randomsampler(
            self.dataset, 4, shuffle, False, unrepeated=False)
        replaced_loader = driver.set_dist_repro_dataloader(
            dataloader, 'unrepeatdist', False)
    ...
    def test_with_dist_unrepeat_dataloader_unrepreated_sampler(self, shuffle):
        """测试 set_dist_repro_dataloader 中 dist 为
        'unrepeatdist'、dataloader.batch_sampler.sampler 为 UnrepeatedSampler 的表现
        此时应该返回一个新的 dataloader，且重新实例化了原来的 Sampler."""
        driver = generate_driver(10, 10, device=self.device)
        dataloader = dataloader_with_randomsampler(
            self.dataset, 4, shuffle, False, unrepeated=True)
        replaced_loader = driver.set_dist_repro_dataloader(
            dataloader, 'unrepeatdist', False)
    ...
    def test_with_dist_unrepeat_dataloader_normal(self, shuffle):
        """测试 set_dist_repro_dataloader 中 dist 为 'unrepeatdist'、dataloader
        为一般情况的表现 此时应该返回一个新的 dataloader，且将 sampler 替换为
        UnrepeatedSequentialSampler，并正确地设置了分布式相关 的属性."""
        driver = generate_driver(10, 10, device=self.device)
        dataloader = DataLoader(self.dataset, batch_size=4, shuffle=shuffle)
        replaced_loader = driver.set_dist_repro_dataloader(
            dataloader, 'unrepeatdist', False)
    ...
    def check_distributed_sampler(self, sampler):
        ...
        if not isinstance(sampler, UnrepeatedSampler):
            assert sampler.pad is True

    def check_set_dist_repro_dataloader(self, driver, dataloader,
                                        replaced_loader, shuffle):
        ...
        if isinstance(replaced_loader.batch_sampler, BucketedBatchSampler):
            ...
            sampler_states[
                'num_consumed_samples'] = num_consumed_batches * batch_size * num_replicas
            # 重新改造 dataloader
            ...
            sampler_states[
                'num_consumed_samples'] = num_consumed_batches * batch_size * num_replicas
            # 重新构造 dataloader
            new_loader = dataloader_with_randomsampler(
                replaced_loader.dataset, batch_size, shuffle, drop_last=False)
        ...
        assert len(left_idxes) + len(already_seen_idx) == len(
            self.dataset) / num_replicas
        assert len(left_idxes
                   | already_seen_idx) == len(self.dataset) / num_replicas
...
class TestSaveLoad:
    ...
    def test_save_and_load_with_bucketedbatchsampler(self, device,
                                                     only_state_dict, fp16):
        ...
        try:
            ...
            if os.environ['FASTNLP_GLOBAL_RANK'] == '0':
                assert replaced_loader.batch_sampler.seed == sampler_states[
                    'seed']
            ...
            assert len(left_x_batches) + len(already_seen_x_set) == len(
                self.dataset) / num_replicas
            assert len(left_x_batches | already_seen_x_set) == len(
                self.dataset) / num_replicas
            assert len(left_y_batches) + len(already_seen_y_set) == len(
                self.dataset) / num_replicas
            assert len(left_y_batches | already_seen_y_set) == len(
                self.dataset) / num_replicas
            dist.barrier()
        finally:
            rank_zero_rm(path)
    ...
    @pytest.mark.parametrize('fp16', ([True, False]))
    @pytest.mark.parametrize('device', ([[0, 1]]))
    def test_save_and_load_with_randomsampler(self, device, only_state_dict,
                                              fp16):
        ...
        try:
            ...
            driver1 = generate_driver(100, 1, device=device, fp16=fp16)
            driver2 = generate_driver(100, 1, device=device, fp16=False)
            ...
            # 更改 batch_size
            dataloader = dataloader_with_randomsampler(
                self.dataset, 2, True, False, unrepeated=False)
            dataloader.batch_sampler.sampler.set_distributed(
                num_replicas=driver2.world_size,
                rank=driver2.global_rank,
                pad=True)
...
def test_shuffle_dataloader(shuffle, batch_size, drop_last, reproducible=True):
    ...
    try:
        ...
        dl = driver.set_dist_repro_dataloader(
            dataloader=dl, dist='dist', reproducible=reproducible)
        ...
        res = fastnlp_torch_all_gather(data)
        if drop_last:
            assert len(
                set(res[0] +
                    res[1])) == num_samples - _num_samples % batch_size * 2
        else:
            assert len(set(res[0] + res[1])) == num_samples
    finally:
        if dist.is_initialized():
            dist.barrier()
            dist.destroy_process_group()
...
@pytest.mark.parametrize('batch_size', ([1, 3, 16, 17]))
@pytest.mark.parametrize('drop_last', ([True, False]))
def test_batch_sampler_dataloader(shuffle,
                                  batch_size,
                                  drop_last,
                                  reproducible=True):
    skip_no_cuda()
...
def test_customized_batch_sampler_dataloader(inherit):
    ...
    try:
        ...
        dl = prepare_torch_dataloader(
            dataset, batch_sampler=BatchSampler(dataset, batch_size=4))
        model = TorchNormalModel_Classification_1(10, 32)
        device = [torch.device(i) for i in [0, 1]]
        driver = TorchDDPDriver(model, parallel_device=device)
        driver.setup()
        ...
        with pytest.raises(TypeError):
            dl = driver.set_dist_repro_dataloader(
                dataloader=dl, dist='dist', reproducible=False)
    finally:
        if dist.is_initialized():
            dist.barrier()
            dist.destroy_process_group()
...
def test_customized_sampler_dataloader(inherit):
    ...
    try:
        ...
        dl = prepare_torch_dataloader(
            dataset, sampler=Sampler(dataset, batch_size=4))
        model = TorchNormalModel_Classification_1(10, 32)
        device = [torch.device(i) for i in [0, 1]]
        driver = TorchDDPDriver(model, parallel_device=device)
        driver.setup()
...
def test_sync_batchnorm(device):
    ...
    from fastNLP import DataSet, Event, Instance, Trainer
    ...
    try:
        ...
        def check_sync(trainer):
            if not isinstance(device, list) or len(device) == 0:
                # 单卡或 CPU 的情况下不需要检查
                return
            running_mean_list = fastnlp_torch_all_gather(
                trainer.model.norm.running_mean)
            running_var_list = fastnlp_torch_all_gather(
                trainer.model.norm.running_var)
        ...
        for i in np.arange(30):
            dataset.append(
                Instance(x=np.random.rand(10, 1), y=np.random.rand(10, 1)))
