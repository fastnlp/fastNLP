...
from fastNLP.core.collators.padders.paddle_padder import (PaddleNumberPadder,
...
class TestPaddleSequencePadder:
    ...
    def test_dtype_check(self):
        padder = PaddleSequencePadder(
            ele_dtype=np.zeros(3, dtype=np.int32).dtype, dtype=int, pad_val=-1)
        ...
        padder = PaddleSequencePadder(
            ele_dtype=np.int32, dtype=None, pad_val=-1)
        padder([[1], [2, 322]])
        # assert (a>67).sum()==0  # 因为int8的范围为-67 - 66
        padder = PaddleSequencePadder(
            ele_dtype=np.zeros(2).dtype, dtype=None, pad_val=-1)
...
class TestPaddleTensorPadder:
    ...
    def test_run(self):
        padder = PaddleTensorPadder(
            ele_dtype=paddle.zeros((3, )).dtype,
            dtype=paddle.zeros((3, )).dtype,
            pad_val=-1)
        ...
        padder = PaddleTensorPadder(
            ele_dtype=paddle.zeros((3, 2)).dtype, dtype=None, pad_val=-1)
        a = [
    ...
    def test_dtype_check(self):
        PaddleTensorPadder(
            ele_dtype=np.zeros(3, dtype=np.int8).dtype, dtype=int, pad_val=-1)
        with pytest.raises(DtypeError):
            PaddleTensorPadder(ele_dtype=str, dtype=int, pad_val=-1)
