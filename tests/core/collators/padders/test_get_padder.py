...
from fastNLP.core.collators.padders.get_padder import (
    DtypeError, InconsistencyError, _get_element_shape_dtype, get_padder)
...
def test_get_element_shape_dtype():
    _get_element_shape_dtype([[1], [2, 3], [3], 2])
    _get_element_shape_dtype([['1'], [2, 3]])
    _get_element_shape_dtype([['1'], [2, 3]])
    _get_element_shape_dtype([['1'], ['2', '3']])
    _get_element_shape_dtype([np.zeros(3), np.zeros((2, 1))])


@pytest.mark.parametrize(
    'backend', ['raw', None, 'numpy', 'torch', 'paddle', 'jittor', 'oneflow'])
@pytest.mark.torch
@pytest.mark.paddle
@pytest.mark.jittor
@pytest.mark.oneflow
def test_get_padder_run(backend):
    ...
    batch_field = [1, 2, 3]
    get_padder(
        batch_field, pad_val=0, backend=backend, dtype=int, field_name='test')
...
def test_torch_padder():
    ...
    assert isinstance(pad_batch, target_type)
    assert (
        pad_batch == torch.LongTensor(batch_field)).sum() == len(batch_field)
    ...
    batch_field = [torch.ones((3, 3)), torch.ones((2, 3)), torch.ones((1, 0))]
    padder = get_padder(
        batch_field, pad_val=0, backend='numpy', dtype=int, field_name='test')
    pad_batch = padder(batch_field)
...
def test_oneflow_padder():
    ...
    assert isinstance(pad_batch, target_type)
    assert (
        pad_batch == oneflow.LongTensor(batch_field)).sum() == len(batch_field)
