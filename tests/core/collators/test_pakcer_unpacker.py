from fastNLP.core.collators.packer_unpacker import (
    MappingPackerUnpacker, NestedMappingPackerUnpacker, SequencePackerUnpacker)


def test_unpack_batch_mapping():
    batch = [{'a': [1, 2], 'b': 1}, {'a': [3], 'b': 2}]
    assert MappingPackerUnpacker.unpack_batch(batch, {}, {}) == {
        'a': [[1, 2], [3]],
        'b': [1, 2]
    }
...
def test_unpack_batch_nested_mapping():
    ...
    assert NestedMappingPackerUnpacker.unpack_batch(batch, {}, {}) == {
        'a': [[1, 2], [3]],
        'b': [1, 2],
        ('c', 'c'): [1, 2]
    }
