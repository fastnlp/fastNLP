...
class TestCollator:
    ...
    def test_run_traw(self):
        ...
        raw_pad_batch = {
            ...
            'int': [1, 2],
            'lst_int': [[1, 0], [1, 2]],
            'nest_lst_int': [[[1, 0], [0, 0]], [[1, 0], [1, 2]]],
            'float': [1.1, 2.1],
            'lst_float': [[1.1], [2.1]],
            'bool': [True, False],
            'numpy': [np.array([1.]), np.array([0.])],
            'dict': {
                '1': ['1', '2']
            },
            'set': [{'1'}, {'2'}],
        ...
        collator = Collator(backend='raw')
        raw_pad_lst = [['1', '2'], [['1'], ['2', '2']], [1, 2], [[1, 0],
                                                                 [2, 2]],
                       [[[1, 0], [0, 0]], [[1, 0], [1, 2]]], [1.1, 2.1],
                       [[1.1], [2.1]], [True, False], [[1, 0], [1, 1]],
                       [{
                           '1': '1'
                       }, {
                           '2': '2'
    ...
    def test_run_numpy(self):
        ...
        numpy_pad_batch = {
            ...
            'lst_str': [['1'], ['2', '2']],
            'int': np.array([1, 2]),
            'lst_int': np.array([[1, 0], [1, 2]]),
            'nest_lst_int': np.array([[[1, 0], [0, 0]], [[1, 0], [1, 2]]]),
            'float': np.array([1.1, 2.1]),
            'lst_float': np.array([[1.1], [2.1]]),
    ...
    def test_run_torch(self):
        ...
        numpy_pad_batch = {
            'str': ['1', '2'],
            'lst_str': [['1'], ['2', '2']],
            'int': torch.LongTensor([1, 2]),
            'lst_int': torch.LongTensor([[1, 0], [1, 2]]),
            'nest_lst_int': torch.LongTensor([[[1, 0], [0, 0]], [[1, 0],
                                                                 [1, 2]]]),
            'float': torch.FloatTensor([1.1, 2.1]),
            'lst_float': torch.FloatTensor([[1.1], [2.1]]),
            'bool': torch.BoolTensor([True, False]),
            ...
            'nested_dict': {
                ...
                'b': torch.LongTensor([[1, 2], [1, 2]])
            }
        }

        findDictDiff(numpy_pad_batch, collator(self.dict_batch))
        collator = Collator(backend='torch')
        torch_pad_lst = [['1', '2'], [['1'], ['2', '2']],
                         torch.LongTensor([1, 2]),
                         torch.LongTensor([[1, 0], [2, 2]]),
                         torch.LongTensor([[[1, 0], [0, 0]], [[1, 0], [1,
                                                                       2]]]),
                         torch.FloatTensor([1.1, 2.1]),
                         torch.FloatTensor([[1.1], [2.1]]),
                         torch.BoolTensor([True, False]),
                         torch.LongTensor([[1, 0], [1, 1]]),
                         [{
                             '1': '1'
                         }, {
                             '2': '2'
    ...
    def test_run_oneflow(self):
        ...
        numpy_pad_batch = {
            ...
            'lst_str': [['1'], ['2', '2']],
            'int':
            oneflow.LongTensor([1, 2]),
            'lst_int':
            oneflow.LongTensor([[1, 0], [1, 2]]),
            'nest_lst_int':
            oneflow.LongTensor([[[1, 0], [0, 0]], [[1, 0], [1, 2]]]),
            'float':
            oneflow.FloatTensor([1.1, 2.1]),
            'lst_float':
            oneflow.FloatTensor([[1.1], [2.1]]),
            'bool':
            oneflow.BoolTensor([True, False]),
            'numpy':
            oneflow.FloatTensor([[1], [0]]),
            'dict': {
                '1': ['1', '2']
            },
            'set': [{'1'}, {'2'}],
            'nested_dict': {
                'a': oneflow.LongTensor([1, 2]),
                'b': oneflow.LongTensor([[1, 2], [1, 2]])
            }
        }

        findDictDiff(numpy_pad_batch, collator(self.dict_batch))
        collator = Collator(backend='oneflow')
        oneflow_pad_lst = [['1', '2'], [['1'], ['2', '2']],
                           oneflow.LongTensor([1, 2]),
                           oneflow.LongTensor([[1, 0], [2, 2]]),
                           oneflow.LongTensor([[[1, 0], [0, 0]],
                                               [[1, 0], [1, 2]]]),
                           oneflow.FloatTensor([1.1, 2.1]),
                           oneflow.FloatTensor([[1.1], [2.1]]),
                           oneflow.BoolTensor([True, False]),
                           oneflow.LongTensor([[1, 0], [1, 1]]),
                           [{
                               '1': '1'
                           }, {
                               '2': '2'
    ...
    def test_pad(self):
        ...
        raw_pad_batch = {
            'lst_str': [['1'], ['2', '2']],
            'nest_lst_int': [[[1, 100], [100, 100]], [[1, 100], [1, 2]]],
            'float': [1.1, 2.1],
            'lst_float': [[1.1], [2.1]],
            'bool': [True, False],
            'numpy': [np.array([1.]), np.array([0.])],
        ...
        raw_pad_lst = [[1, 2], [[[1]], [[1], [1, 2]]], [1.1, 2.1],
                       [[1.1], [2.1]], [True, False], [[1, 0], [1, 1]],
                       [{
                           '1': '1'
                       }, {
        ...
        raw_pad_lst = [
            np.array([1, 2]),
            np.array([[[1, 100], [100, 100]], [[1, 100], [1, 2]]]), [1.1, 2.1],
            [[1.1], [2.1]], [True, False], [[1, 0], [1, 1]],
            [{
                '1': '1'
            }, {
    ...
    def test_nest_ignore(self):
        ...
        collator = Collator(backend='raw')
        collator.set_ignore('str', 'int', 'lst_int')
        collator.set_pad(('nested_dict', 'c'),
                         pad_fn=lambda x: [d['int'] for d in x])
        pad_batch = collator(dict_batch)
        raw_pad_batch = {
            'lst_str': [['1'], ['2', '2']],
            'nest_lst_int': [[[1, 0], [0, 0]], [[1, 0], [1, 2]]],
...
def test_oneflow_dl():
    ...
    from fastNLP import DataSet, OneflowDataLoader
...
def test_compare_tuple():
    ...
    for t1, t2, t in zip([(1, ), (1, 2, 3), (1, ), (1, 2)], [(1, 2, 3), (1, ),
                                                             (2, ), (1, 3)],
                         [-2, 2, None, None]):
