...
class TestAutoParamCall:
    ...
    def test_basic(self):
        ...
        assert 'The following key present in several inputs' in exc_info.value.args[
            0]
        assert 'chongfu1' in exc_info.value.args[
            0] and 'chongfu2' in exc_info.value.args[0]
        ...
        assert 'meiti1' in exc_info.value.args[
            0] and 'meiti2' in exc_info.value.args[0]
    ...
    def test_msg(self):
        ...
        with pytest.raises(BaseException) as exc_info:
            auto_param_call(
                call_this_for_auto_param_call, {'x': 1},
                signature_fn=self.call_this)
        assert 'TestAutoParamCall.call_this' in exc_info.value.args[
            0]  # 应该是signature的信息
...
class TestCheckNumberOfParameters:
    ...
    def test_validate_every(self):
        ...
        _check_valid_parameters_number(
            validate_every, expected_params=['trainer'])
        ...
        with pytest.raises(TypeError) as exc_info:
            _check_valid_parameters_number(
                validate_every, expected_params=['trainer', 'other'])
        print(exc_info.value.args[0])
        ...
        _check_valid_parameters_number(
            partial(validate_every, other=1), expected_params=['trainer'])
        with pytest.raises(TypeError):
            _check_valid_parameters_number(
                partial(validate_every, other=1),
                expected_params=['trainer', 'other'])
        with pytest.raises(TypeError) as exc_info:
            _check_valid_parameters_number(
                partial(validate_every, other=1),
                expected_params=['trainer', 'other', 'more'])
        print(exc_info.value.args[0])
        ...
        _check_valid_parameters_number(
            validate_every, expected_params=['trainer', 'other', 'more'])
        ...
        with pytest.raises(TypeError):
            _check_valid_parameters_number(
                partial(validate_every, trainer=1),
                expected_params=['trainer', 'other', 'more'])
        ...
        with pytest.raises(TypeError) as exc_info:
            _check_valid_parameters_number(
                inner.demo, expected_params=['trainer', 'other', 'more'])
