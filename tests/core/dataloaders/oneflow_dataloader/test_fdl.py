...
from fastNLP.core.dataloaders.oneflow_dataloader import (
    OneflowDataLoader, prepare_oneflow_dataloader)
...
class TestFdl:
    ...
    def test_init_v1(self):
        ...
        for batch in fdl:
            print(batch)
        fdl1 = OneflowDataLoader(
            ds, batch_size=3, shuffle=True, drop_last=True)
        for batch in fdl1:
            print(batch)
    ...
    def test_default_collate_fn(self):
        ...
        fdl = OneflowDataLoader(
            _DataSet(), batch_size=3, collate_fn=None, drop_last=True)
        for batch in fdl:
    ...
    def test_my_collate_fn(self):
        ...
        fdl = OneflowDataLoader(
            ds, collate_fn=collate_fn, batch_size=3, drop_last=True)
        for batch in fdl:
    ...
    def test_prepare_oneflow_dataloader(self):
        ...
        ds2 = _DataSet()
        dl1 = prepare_oneflow_dataloader(
        ...
        for batch in dl:
            ...
            assert (batch[1] == oneflow.LongTensor([[1, 0], [2,
                                                             2]])).sum() == 4
