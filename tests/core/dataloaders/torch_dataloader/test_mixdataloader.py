...
d2 = DataSet({
    'x': [[101, 201], [201, 301, 401], [100]] * 10,
    'y': [20, 10, 10] * 10
})

d3 = DataSet({
    'x': [[1000, 2000], [0], [2000, 3000, 4000, 5000]] * 100,
    'y': [100, 100, 200] * 100
})
...
class TestMixDataLoader:
    ...
    def test_sequential_init(self):
        ...
        collate_fns = {
            'd1': Collator(backend='auto').set_pad('x', -1),
        ...
        for idx, batch in enumerate(dl3):
            if idx == 0:
                # d1
                assert batch['x'][:3].tolist() == [[1, 2, 0, 0], [2, 3, 4, 0],
                                                   [4, 5, 6, 7]]
                assert batch['x'].shape == torch.Size([16, 4])
            if idx == 1:
                # d2
                assert batch['x'][:3].tolist() == [[101, 201, 0],
                                                   [201, 301, 401],
                                                   [100, 0, 0]]
                assert batch['x'].shape == torch.Size([16, 3])
            if idx == 2:
                # d3
                assert batch['x'][:3].tolist() == [[1000, 2000, 0, 0],
                                                   [0, 0, 0, 0],
                                                   [2000, 3000, 4000, 5000]]
            if idx > 1:
                # d3
        ...
        for idx, batch in enumerate(dl6):
            ...
            assert _test_pad_val(batch['x'], val=0)
            if idx > 2:
                raise ValueError("ds_ratio: 'truncate_to_least' error")
        ...
        for idx, batch in enumerate(dl7):
            ...
            assert _test_pad_val(batch['x'], val=0)
            if idx >= 54:
                raise ValueError("ds_ratio: 'pad_to_most' error")
    ...
    def test_mix(self):
        datasets = {'d1': d1, 'd2': d2, 'd3': d3}
        dl = MixDataLoader(
            datasets=datasets, mode='mix', collate_fn='auto', drop_last=True)
        for idx, batch in enumerate(dl):
            assert _test_pad_val(batch['x'], val=0)
            if idx >= 22:
        ...
        with pytest.raises(ValueError):
            MixDataLoader(
                datasets=datasets, mode='mix', collate_fn=collate_fns)

        # sampler 为 str
        dl3 = MixDataLoader(
            datasets=datasets, mode='mix', sampler='seq', drop_last=True)
        for idx, batch in enumerate(dl3):
            assert _test_pad_val(batch['x'], val=0)
            if idx >= 22:
                raise ValueError('out of range')
        dl4 = MixDataLoader(
            datasets=datasets, mode='mix', sampler='rand', drop_last=True)
        for idx, batch in enumerate(dl4):
            assert _test_pad_val(batch['x'], val=0)
            if idx >= 22:
                raise ValueError('out of range')
        ...
        dl5 = MixDataLoader(
            datasets=datasets, mode='mix', sampler=samplers, drop_last=True)
        for idx, batch in enumerate(dl5):
            assert _test_pad_val(batch['x'], val=0)
            if idx >= 22:
                raise ValueError('out of range')
        # ds_ratio 为 'truncate_to_least'
        dl6 = MixDataLoader(
            datasets=datasets, mode='mix', ds_ratio='truncate_to_least')
        ...
        for idx, batch in enumerate(dl6):
            for item in batch['y'].tolist():
                if item in [1, 0, 1]:
                    ...
                    d3_len += 1
            if idx >= 6:
                raise ValueError("ds_ratio 为 'truncate_to_least'出错了")
        assert d1_len == d2_len == d3_len == 30

        # ds_ratio 为 'pad_to_most'
        dl7 = MixDataLoader(
            datasets=datasets, mode='mix', ds_ratio='pad_to_most')
        d1_len, d2_len, d3_len = 0, 0, 0
        for idx, batch in enumerate(dl7):
            for item in batch['y'].tolist():
                if item in [1, 0, 1]:
            ...
            if idx >= 57:
                raise ValueError("ds_ratio 为 'pad_to_most'出错了")
        ...
        for idx, batch in enumerate(dl8):
            for item in batch['y'].tolist():
                if item in [1, 0, 1]:
                    ...
                    d3_len += 1
            if idx >= 44:
                raise ValueError("ds_ratio 为 'Dict'出错了")
        assert d1_len == 30
        assert d2_len == 60
        assert d3_len == 600
    ...
    def test_polling(self):
        ...
        for idx, batch in enumerate(dl1):
            if idx == 0 or idx == 3:
                assert _test_pad_val(batch['x'], val=-1)
                assert batch['x'][:3].tolist() == [[1, 2, -1, -1],
                                                   [2, 3, 4, -1], [4, 5, 6, 7]]
                assert batch['x'].shape[1] == 4
            elif idx == 1 or idx == 4:
                # d2
                assert _test_pad_val(batch['x'], val=-2)
                assert batch['x'][:3].tolist() == [[101, 201, -2],
                                                   [201, 301, 401],
                                                   [100, -2, -2]]
                assert batch['x'].shape[1] == 3
            elif idx == 2 or 4 < idx <= 20:
                assert _test_pad_val(batch['x'], val=-3)
                assert batch['x'][:3].tolist() == [[1000, 2000, -3, -3],
                                                   [0, -3, -3, -3],
                                                   [2000, 3000, 4000, 5000]]
                assert batch['x'].shape[1] == 4
            if idx > 20:
        ...
        # sampler 为 str
        dl2 = MixDataLoader(
            datasets=datasets, mode='polling', sampler='seq', batch_size=18)
        dl3 = MixDataLoader(
            datasets=datasets, mode='polling', sampler='rand', batch_size=18)
        for idx, batch in enumerate(dl2):
            if idx == 0 or idx == 3:
                assert batch['x'][:3].tolist() == [[1, 2, 0, 0], [2, 3, 4, 0],
                                                   [4, 5, 6, 7]]
        ...
        dl4 = MixDataLoader(
            datasets=datasets, mode='polling', sampler=samplers, batch_size=18)
        for idx, batch in enumerate(dl4):
            if idx == 0 or idx == 3:
                assert batch['x'][:3].tolist() == [[1, 2, 0, 0], [2, 3, 4, 0],
                                                   [4, 5, 6, 7]]
